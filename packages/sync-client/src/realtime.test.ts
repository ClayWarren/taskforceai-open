...
const waitForCondition = async (isReady: () => boolean): Promise<void> => {
...
const isTokenUrl = (url: string): boolean => url.endsWith('/api/v1/sync/realtime/token');
const isPollUrl = (url: string): boolean =>
  url.includes('/api/v1/sync/realtime') && !url.endsWith('/api/v1/sync/realtime/token');

const toIntervalTick = (handler: TimerHandler): (() => void) => {
...
const invokeIntervalTick = (tick: unknown): void => {
...
const makeParams = (overrides?: Partial<Parameters<typeof createRealtimeConnection>[0]>) => ({
