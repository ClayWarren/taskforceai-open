  ...
  it('converts AbortError into timeout error', async () => {
    ...
    await expect(
      makeRequest<{ ok: boolean }>(
        '/slow-task',
        { method: 'GET' },
        { ...TRANSPORT_CONFIG, timeout: 20 }
      )
    ).rejects.toThrow(/timeout/i);
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });

  it('retries internal request timeouts for retryable requests', async () => {
    ...
    await expect(
      makeRequest<{ ok: boolean }>('/slow-task', { method: 'GET' }, TRANSPORT_CONFIG, true, 1)
    ).resolves.toEqual({ ok: true });
    expect(fetchMock).toHaveBeenCalledTimes(2);
  });

  it('normalizes native TimeoutError failures as request timeouts', async () => {
    const timeoutError = new Error('The operation timed out');
    timeoutError.name = 'TimeoutError';
    const fetchMock = installFetchMock(vi.fn().mockRejectedValue(timeoutError));

    await expect(
      makeRequest<{ ok: boolean }>('/slow-task', { method: 'GET' }, TRANSPORT_CONFIG)
