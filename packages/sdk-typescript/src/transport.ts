...
const parseSuccessPayload = async <T>(response: Response): Promise<T> => {
    ...
    try {
      ...
      if (retryable && attempt < maxRetries) {
        try {
          ...
          const retryAbortError = normalizeAbortError(retryError, externalSignal);
          if (retryAbortError) throw retryAbortError;
          throw retryError;
        }
        continue;
      }
