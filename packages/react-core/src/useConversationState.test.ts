...
describe('useConversationState', () => {
  ...
  it('ignores non-callable latency markers while restoring a saved conversation', async () => {
    ...
    const conversationStore = createConversationStore({
      ...
      getConversationMessages: vi
        .fn()
        .mockResolvedValue([createMessageRecord('m-1', conversationId, 'assistant', 'hello')]),
    });
