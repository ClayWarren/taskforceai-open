...
import { type StoredModelSelection, deriveSelectionFromOptions } from '@taskforceai/shared';
...
export function useModelSelector({
  ...
  }, [data?.options, preloadedSelection, selectedModelId, selectedModelLabel]);
  ...
  // Initial persistence if none exists
  useEffect(() => {
    if (!storedSelection && preloadedSelection) {
      void onPersistRef.current(preloadedSelection);
    }
  }, [preloadedSelection, storedSelection]);
