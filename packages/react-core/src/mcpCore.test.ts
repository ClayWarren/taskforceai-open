...
describe('SharedMcpManager', () => {
  ...
  it('reuses matching sessions and reconnects when the endpoint changes', async () => {
    ...
    const connect = vi
      .fn()
      .mockResolvedValueOnce({ client: firstClient, transport: { close: vi.fn() } })
      .mockResolvedValueOnce({ client: secondClient, transport: { close: vi.fn() } });
    const manager = createManager(connect);
  ...
  it('closes named sessions and all active sessions', async () => {
    ...
    const connect = vi
      .fn()
      .mockResolvedValueOnce({ client: clientA, transport: { close: vi.fn() } })
      .mockResolvedValueOnce({ client: clientB, transport: { close: vi.fn() } });
    const manager = createManager(connect);
...
describe('MCP inventory and store helpers', () => {
  ...
  it('returns safe MCP store fallbacks and reports adapter failures', async () => {
    ...
    expect(onWriteError).toHaveBeenCalledWith(expect.any(Error), 1);
  });
