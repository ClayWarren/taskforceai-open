...
pub(crate) async fn read_http_events(
    ...
    loop {
        let outcome = read_http_events_once(
            base_url.clone(),
            session_token.clone(),
            client.clone(),
            event_tx.clone(),
        ...
        .await;
        let (delay, next_delay) = http_event_reconnect_schedule(reconnect_delay, outcome);
        sleep(delay).await;
        reconnect_delay = next_delay;
    }
}

pub(crate) fn http_event_reconnect_schedule(
    current_delay: Duration,
    outcome: HttpEventStreamOutcome,
) -> (Duration, Duration) {
    match outcome {
        HttpEventStreamOutcome::Closed => (
            HTTP_EVENT_RECONNECT_INITIAL_DELAY,
            HTTP_EVENT_RECONNECT_INITIAL_DELAY,
        ),
        HttpEventStreamOutcome::Retry => (
            current_delay,
            (current_delay * 2).min(HTTP_EVENT_RECONNECT_MAX_DELAY),
        ),
    }
}
