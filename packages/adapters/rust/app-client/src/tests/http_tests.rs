...
use crate::transport::{
    handle_http_event_line, http_event_reconnect_schedule, read_http_events, read_http_events_once,
    send_event_nonblocking, AppServerEventMessage, AppServerTransport, HttpEventStreamOutcome,
    MAX_HTTP_EVENT_LINE_BYTES,
};
use crate::{AppClientError, AppServerClient};

#[test]
fn healthy_http_event_stream_resets_reconnect_backoff() {
    let initial = Duration::from_millis(250);
    let (first_sleep, next_delay) =
        http_event_reconnect_schedule(initial, HttpEventStreamOutcome::Retry);
    assert_eq!(first_sleep, initial);
    assert_eq!(next_delay, Duration::from_millis(500));

    let (retry_sleep, backed_off_delay) =
        http_event_reconnect_schedule(next_delay, HttpEventStreamOutcome::Retry);
    assert_eq!(retry_sleep, Duration::from_millis(500));
    assert_eq!(backed_off_delay, Duration::from_secs(1));

    let (closed_sleep, reset_delay) =
        http_event_reconnect_schedule(Duration::from_secs(5), HttpEventStreamOutcome::Closed);
    assert_eq!(closed_sleep, initial);
    assert_eq!(reset_delay, initial);
}
