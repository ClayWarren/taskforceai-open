...
func TestUsageExtensionQueries(t *testing.T) {
	t.Run("usage events success empty rows and exec error", func(t *testing.T) {
		mockPool := newRegexpTestMockPool(t)
		q := New(mockPool)
		require.NoError(t, q.CreateUsageEvents(context.Background(), nil))

		mockPool.ExpectExec("INSERT INTO usage_events").
			WithArgs(pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), "voice", "voice", "speech", pgxmock.AnyArg(), float64(12), "characters", int64(180), pgxmock.AnyArg()).
			WillReturnResult(pgxmock.NewResult("INSERT", 1))
		require.NoError(t, q.CreateUsageEvents(context.Background(), []coreusage.EventRow{{
			Source: "voice", Modality: "voice", Operation: "speech", Quantity: 12, Unit: "characters", CostMicros: 180,
		}}))
		...
		err := q.CreateUsageEvents(context.Background(), []coreusage.EventRow{{Source: "task"}})
