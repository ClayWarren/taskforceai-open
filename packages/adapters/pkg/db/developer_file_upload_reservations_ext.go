...
WITH expired AS (
	...
	SELECT user_id, COALESCE(SUM(reserved_bytes), 0)::BIGINT AS reserved_bytes
	FROM expired
	GROUP BY user_id
), released AS (
	UPDATE user_storage_quotas AS quota
	SET
		used_bytes = GREATEST(0, quota.used_bytes - totals.reserved_bytes),
		updated_at = CURRENT_TIMESTAMP
	FROM totals
	WHERE quota.user_id = totals.user_id
)
SELECT reserved_bytes FROM expired
`
