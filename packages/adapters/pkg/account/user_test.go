...
func TestStoreLookupsPropagateErrors(t *testing.T) {
	expectedErr := errors.New("lookup failed")
	store := NewStore(&fakeUserQueries{err: expectedErr})

	_, err := store.GetByCustomerID(context.Background(), "cus_missing")
	require.ErrorIs(t, err, expectedErr)

	_, err = store.GetByEmail(context.Background(), "missing@example.com")
	require.ErrorIs(t, err, expectedErr)

	_, err = store.GetByRevenueCatAppUserID(context.Background(), "rc_missing")
	require.ErrorIs(t, err, expectedErr)
}
