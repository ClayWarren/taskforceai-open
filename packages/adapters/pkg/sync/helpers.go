...
func applyPulledDeletions(s SyncStorage, deletions []DeletionRecord) error {
	for _, d := range deletions {
		switch d.Type {
		case "conversation":
			if err := s.DeleteConversation(normalizeConversationStorageID(d.ID)); err != nil {
				return err
			}
		case "message":
...
func normalizeConversationStorageID(value string) string {
	normalized := strings.TrimSpace(value)
	if normalized == "" || strings.HasPrefix(normalized, "remote-") || strings.HasPrefix(normalized, "local-") {
		return normalized
	}
