...
func toSyncID(v any) (int, bool) {
	switch value := v.(type) {
	case int:
		return positiveSyncID(int64(value))
	case int8:
		return positiveSyncID(int64(value))
	case int16:
		return positiveSyncID(int64(value))
	case int32:
		return positiveSyncID(int64(value))
	case int64:
		return positiveSyncID(value)
	case float32:
		return floatSyncID(float64(value))
	case float64:
		return floatSyncID(value)
	case string:
...
func positiveSyncID(value int64) (int, bool) {
	if value <= 0 {
		return 0, false
	}
...
func floatSyncID(value float64) (int, bool) {
...
func ClearAcceptedPendingChanges(s SyncStorage, p []PendingChange, accepted []string) error {
	...
	for _, id := range accepted {
		...
		for _, ch := range p {
			match := false
			if isTypedID {
				if ch.Type == acceptedType && ch.EntityID == entityID {
					match = true
				} else if acceptedType == "deletion" && isDeletionPendingChange(ch) && ch.EntityID == entityID {
					match = true
				}
			} else if ch.EntityID == entityID {
				match = true
			}
