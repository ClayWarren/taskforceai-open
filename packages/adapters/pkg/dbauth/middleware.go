...
func WithOptionalDBAuth(q *db.Queries, next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		user, buildErr := handler.BuildAuthenticatedUser(claims)
		if buildErr != nil || strings.TrimSpace(user.Email) == "" {
			next(w, r)
			return
		}
		...
		scopedCtx, err := withOrganizationScope(ctx, q, dbUser.ID, requestedOrgID)
		if err != nil {
			slog.Warn("WithOptionalDBAuth: Organization scope access denied", "userId", dbUser.ID, "orgId", requestedOrgID, "error", err)
			handler.JSONError(w, http.StatusForbidden, "You are not a member of this organization")
			return
		}

		next(w, r.WithContext(scopedCtx))
	}
}
...
func WithFlexibleAuth(q *db.Queries, next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		token := handler.ExtractToken(r)
		if token == "" {
			// Proceed without user, let AuthContext handle 401 if needed
			next(w, r)
			return
		}
		...
		// Fetch database user for full details
		dbUser, err := q.GetUserByEmail(r.Context(), user.Email)
		if err != nil || dbUser.Disabled || dbUser.ID < 0 {
			next(w, r)
			return
		...
		// Initial promotion via env var
		if promoteAdminFromEnv(r.Context(), q, user.Email, user.IsAdmin) {
			user.IsAdmin = true
		}
		...
		scopedCtx, err := withOrganizationScope(ctx, q, dbUser.ID, requestedOrgID)
		if err != nil {
			slog.Warn("WithFlexibleAuth: Organization scope access denied", "userId", dbUser.ID, "orgId", requestedOrgID, "error", err)
			handler.JSONError(w, http.StatusForbidden, "You are not a member of this organization")
			return
		}
		ctx = scopedCtx
