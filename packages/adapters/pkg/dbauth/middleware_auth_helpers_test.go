...
func runReturnsForbidden(t *testing.T, kind authMiddlewareKind, q *Queries, configureReq func(*http.Request)) {
	t.Helper()
...
func TestAuthMiddlewareSharedUnauthenticatedPaths(t *testing.T) {
	...
	for _, kind := range kinds {
		t.Run(string(kind)+"/invalid org header", func(t *testing.T) {
			...
			runReturnsForbidden(t, kind, New(fakeDB), func(req *http.Request) {
				req.Header.Set("Authorization", "Bearer "+token)
