...
func (r middlewareAPIKeyRow) Scan(dest ...any) error {
	if len(dest) < 13 {
		return fmt.Errorf("expected 13 destinations for API key scan, got %d", len(dest))
	}
	...
	if err := assignScanDest(dest, 8, r.record.UserPlan); err != nil {
		return err
	}
	if err := assignScanDest(dest, 9, r.record.UserApiTier); err != nil {
		return err
	}
	if err := assignScanDest(dest, 10, r.record.UserApiRequestsLimit); err != nil {
		return err
	}
	if err := assignScanDest(dest, 11, r.record.UserApiRequestsUsed); err != nil {
		return err
	}
	if err := assignScanDest(dest, 12, r.record.UserApiCurrentPeriodEnd); err != nil {
		return err
	}
