...
func (s *SQLStore) CreateArtifactWithInitialVersion(
	ctx context.Context,
	artifactInput coreartifacts.CreateArtifactStoreInput,
	versionInput coreartifacts.CreateArtifactVersionStoreInput,
) (coreartifacts.ArtifactRecord, coreartifacts.ArtifactVersionRecord, error) {
	beginner, ok := s.q.GetDB().(interface {
		Begin(context.Context) (pgx.Tx, error)
	})
	if !ok {
		return coreartifacts.ArtifactRecord{}, coreartifacts.ArtifactVersionRecord{}, errors.New("artifact store does not support transactions")
	}

	tx, err := beginner.Begin(ctx)
	if err != nil {
		return coreartifacts.ArtifactRecord{}, coreartifacts.ArtifactVersionRecord{}, err
	}
	defer func() { _ = tx.Rollback(ctx) }()

	txStore := NewSQLStore(s.q.WithTx(tx))
	artifact, err := txStore.CreateArtifact(ctx, artifactInput)
	if err != nil {
		return coreartifacts.ArtifactRecord{}, coreartifacts.ArtifactVersionRecord{}, err
	}
	versionInput.ArtifactID = artifact.ID
	version, err := txStore.CreateArtifactVersion(ctx, versionInput)
	if err != nil {
		return coreartifacts.ArtifactRecord{}, coreartifacts.ArtifactVersionRecord{}, err
	}
	artifact, err = txStore.SetArtifactCurrentVersion(ctx, coreartifacts.SetArtifactCurrentVersionInput{
		ID:               artifact.ID,
