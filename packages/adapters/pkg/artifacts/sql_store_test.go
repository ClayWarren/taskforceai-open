...
var artifactColumns = []string{
	"id", "organization_id", "owner_user_id", "conversation_id", "message_id", "task_id",
	"type", "title", "status", "visibility", "current_version_id", "metadata",
	"created_at", "updated_at", "deleted_at",
}

var artifactVersionColumns = []string{
	"id", "artifact_id", "version", "file_id", "mime_type", "filename", "bytes",
	"render_metadata", "source_tool_name", "source_prompt", "created_by_user_id", "created_at",
}
...
func expectInitialArtifactTransaction(mock pgxmock.PgxPoolIface, failureStage string) {
	mock.ExpectBegin()
	artifactQuery := mock.ExpectQuery(`INSERT INTO artifacts \(`).WithArgs(
		"artifact-1", pgxmock.AnyArg(), int32(7), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(),
		db.ArtifactTypeDOCUMENT, "Report", db.ArtifactStatusREADY,
		db.ArtifactVisibilityPRIVATE, []byte(`{"kind":"document"}`),
	)
	if failureStage == "artifact" {
		artifactQuery.WillReturnError(errors.New("artifact failed"))
		mock.ExpectRollback()
		return
	}
	artifactQuery.WillReturnRows(pgxmock.NewRows(artifactColumns).
		AddRow(artifactDBRow("artifact-1", nil, db.ArtifactVisibilityPRIVATE)...))

	versionQuery := mock.ExpectQuery("INSERT INTO artifact_versions").WithArgs(
		"version-1", "artifact-1", int32(1), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(),
		pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(),
	)
	if failureStage == "version" {
		versionQuery.WillReturnError(errors.New("version failed"))
		mock.ExpectRollback()
		return
	}
	versionQuery.WillReturnRows(pgxmock.NewRows(artifactVersionColumns).
		AddRow(artifactVersionDBRow("version-1", 1)...))

	currentRow := artifactDBRow("artifact-1", nil, db.ArtifactVisibilityPRIVATE)
	currentVersionID := "version-1"
	currentRow[10] = &currentVersionID
	currentQuery := mock.ExpectQuery("UPDATE artifacts").WithArgs(&currentVersionID, "artifact-1", int32(7), pgxmock.AnyArg())
	if failureStage == "current" {
		currentQuery.WillReturnError(errors.New("current failed"))
		mock.ExpectRollback()
		return
	}
	currentQuery.WillReturnRows(pgxmock.NewRows(artifactColumns).AddRow(currentRow...))
...
func TestSQLStoreCreateArtifactWithInitialVersionTransaction(t *testing.T) {
	...
	versionInput := CreateArtifactVersionStoreInput{
		ID:         "version-1",
		ArtifactID: "ignored-before-transaction",
		Version:    1,
	}

	t.Run("requires transaction support", func(t *testing.T) {
		store := NewSQLStore(db.New(fakeArtifactStoreDB{t: t}))
		artifact, version, err := store.CreateArtifactWithInitialVersion(ctx, artifactInput, versionInput)
	...
	t.Run("begin failure", func(t *testing.T) {
		...
		_, _, err = NewSQLStore(db.New(mock)).CreateArtifactWithInitialVersion(ctx, artifactInput, versionInput)
		require.ErrorContains(t, err, "begin failed")
		assert.NoError(t, mock.ExpectationsWereMet())
	})

	for _, failureStage := range []string{"artifact", "version", "current", "commit", "none"} {
		t.Run(failureStage, func(t *testing.T) {
			mock, err := pgxmock.NewPool(pgxmock.QueryMatcherOption(pgxmock.QueryMatcherRegexp))
			require.NoError(t, err)
			t.Cleanup(mock.Close)
			expectInitialArtifactTransaction(mock, failureStage)

			artifact, version, callErr := NewSQLStore(db.New(mock)).CreateArtifactWithInitialVersion(ctx, artifactInput, versionInput)
