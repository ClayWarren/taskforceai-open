...
func ParseJSONSchema[T any](raw string) shared.Result[T] {
	var target T
	if raw == "" {
		return shared.Err[T](ErrEmptyInput)
	}

	err := json.Unmarshal([]byte(raw), &target)
	if err != nil {
		if _, ok := errors.AsType[*json.SyntaxError](err); ok {
			return shared.Err[T](ErrInvalidJSON)
		}
		// In Go, Unmarshal also returns errors for type mismatches, which we map to INVALID_SCHEMA.
		return shared.Err[T](ErrInvalidSchema)
	}

	return shared.Ok[T](target)
}
...
func ParseJSONValueSchema[T any](raw any) shared.Result[T] {
	if raw == nil {
		return shared.Err[T](ErrEmptyInput)
	}
