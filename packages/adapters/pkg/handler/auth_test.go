...
func TestContextKeyConstants(t *testing.T) {
	assert.NotEmpty(t, UserContextKey)
	assert.NotEmpty(t, UserIDContextKey)
	assert.NotEmpty(t, EmailContextKey)
	assert.NotEmpty(t, OrgIDContextKey)
	assert.NotEmpty(t, TokenIssuedAtContextKey)
}

func TestAuthExportedFunctions(t *testing.T) {
...
func TestBuildAuthenticatedUser(t *testing.T) {
	...
	// Internal version
	user2, _ := buildAuthenticatedUser(claims)
