...
func WithCSRF(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		path := r.URL.Path
		if isWebhookCSRFExemptPath(path) || isLocalTestLoginCSRFExemptPath(path) || isNativeOAuthTokenExchangePath(path) {
			next(w, r)
			return
		}
		...
		authMetadata := map[string]any{
			"path": r.URL.Path, "method": r.Method, "ua": ua,
			"hasSessionCookie": hasSessionCookie, "hasBearerToken": hasBearerToken,
			"hasAPIKey": hasAPIKey, "hasHeaderOnlyAuth": hasHeaderOnlyAuth,
		}
		csrfToken := r.Header.Get("X-CSRF-Token")
		if csrfToken == "" {
			GetLogger().Warn("CSRF token missing", authMetadata)
		...
		if err != nil {
			GetLogger().Warn("CSRF cookie missing", authMetadata)
