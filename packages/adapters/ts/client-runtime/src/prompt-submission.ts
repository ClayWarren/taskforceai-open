...
export interface SubmitStreamingPromptParams {
  ...
  /** Client-selected task mode; omitted when the surface has no mode selector. */
...
const buildSubmissionPayload = (
  ...
  options: {
    ...
    ...(params.taskMode ? { mode: params.taskMode } : {}),
...
const buildPromptSubmissionContext = async (
  ...
  return {
    ...
    payload: { ...payload, conversation_id: streamMetadata.conversationId },
    streamMetadata,
  };
};
