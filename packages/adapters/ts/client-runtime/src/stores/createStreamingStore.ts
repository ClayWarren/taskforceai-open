...
} from '@taskforceai/client-core/streaming/engine';
import { definedProps } from '@taskforceai/client-core/utils/object';
import {
  initialStreamingState,
  type StreamingState,
  type StreamSettlement,
} from '@taskforceai/client-core/streaming/state';
import { parseStreamingPayload } from '@taskforceai/client-core/streaming/schema';
import type {
  PendingApproval,
  SourceReference,
  ToolUsageEvent,
} from '@taskforceai/client-core/streaming/types';

import { streamingFailureDisplayMessage, toStreamingConnectionError } from '../streaming-errors';
...
export const createStreamingStore = (adapter: StreamingStoreAdapter) => {
  ...
  return create<StreamingStoreState>((set, get) => ({
    ...
    startStreaming: async (options: StartStreamingOptions): Promise<void> => {
      ...
      let startupErrorFromCallback: unknown = null;
      ...
      try {
        const disconnect = await adapter.connect(
          ...
          (payloadStr: string) => {
            ...
            const ctx: StreamingEngineContext = {
              ...
              now: Date.now,
              reportLatencyMark: adapter.reportLatencyMark,
            };
          ...
          (error: unknown) => {
            ...
            startupErrorFromCallback = error;
            adapter.logger.error('[StreamingStore] Streaming runtime error', error);
            closeStream('error', set, get);
            const state = get();
            if (!state.errorMessage) {
              set({ isStreaming: false, errorMessage: streamingFailureDisplayMessage(error) });
            } else {
        ...
        if (currentStreamId !== internals.streamId) {
          if (startupErrorFromCallback) {
            throw toStreamingConnectionError(startupErrorFromCallback);
          }
          return;
        }

        adapter.logger.error('Streaming connection failed', { error });
        const streamingError = toStreamingConnectionError(error);
        closeStream('error', set, get);
        set({
          isStreaming: false,
          errorMessage: streamingFailureDisplayMessage(streamingError),
        });
