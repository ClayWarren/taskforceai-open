...
describe('client-runtime prompt submission', () => {
  ...
  it('forwards the selected task mode in run payload options', async () => {
    const params = createSubmitStreamingParams({ taskMode: 'work' });

    await submitStreamingPrompt(params);

    expect(params.runTask).toHaveBeenCalledWith(
      expect.objectContaining({
        options: expect.objectContaining({ mode: 'work' }),
      })
    );
  });

  it('omits mode from run payload options when no task mode is selected', async () => {
    const params = createSubmitStreamingParams({});
    ...
    expect(params.runTask).toHaveBeenCalledWith(
      expect.objectContaining({
        options: expect.not.objectContaining({ mode: expect.anything() }),
      })
    );
  });
