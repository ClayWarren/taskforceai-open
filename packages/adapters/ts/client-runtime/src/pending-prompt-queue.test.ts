...
describe('PendingPromptQueueProcessor', () => {
  ...
  it('skips failed records while continuing queued prompts', async () => {
    const adapter = createAdapter({
      listPendingPrompts: vi
        .fn()
        .mockResolvedValue([
          createPrompt({ id: 1, status: 'failed' }),
          createPrompt({ id: 2, prompt: 'send me', createdAt: 456 }),
        ]),
    });
    const processor = new PendingPromptQueueProcessor({ adapter, logger: createLogger() });
    processor.setEnvironment({ isOnline: true, isStreaming: false });
    ...
    expect(adapter.updatePromptStatus).not.toHaveBeenCalledWith(1, expect.any(String));
    expect(adapter.updatePromptStatus).toHaveBeenCalledWith(2, 'pending');
    expect(adapter.runTask).toHaveBeenCalledWith('send me', {
      idempotencyKey: 'queue-2-456',
    });
