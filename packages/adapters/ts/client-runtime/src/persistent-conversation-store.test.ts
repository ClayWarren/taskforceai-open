...
describe('PersistentConversationStore', () => {
  let repository: {
    ...
    setConversationProjectId: ReturnType<typeof vi.fn>;
    ...
    truncateConversation: ReturnType<typeof vi.fn>;
  ...
  const createRepository = () => ({
    ...
    setConversationProjectId: vi.fn(async () => {}),
    ...
    truncateConversation: vi.fn(async () => {}),
  ...
  it('delegates repository methods and emits conversation events', async () => {
    ...
    await store.setConversationProjectId('c1', 17);
    ...
    expect(repository.setConversationProjectId).toHaveBeenCalledWith('c1', 17);
    ...
    repository.listPendingPrompts.mockResolvedValue([
      {
        ...
        createdAt: 10,
        status: 'queued',
        runPayload: { prompt: 'retry me', demo: false, modelId: 'openai/gpt-5.6-sol' },
      },
    ]);
    const store = new PersistentConversationStore({
      adapter: {} as never,
  ...
  it('clears or truncates conversations and emits the matching refresh events', async () => {
    const store = new PersistentConversationStore({
      adapter: {} as never,
      logger,
    });
    ...
    await store.truncateConversation('c1', 'm2');

    expect(repository.clearConversation).toHaveBeenCalledWith('c1');
    expect(repository.truncateConversation).toHaveBeenCalledWith('c1', 'm2');
    expect(events).toEqual([
      { type: 'conversations-changed', conversationId: 'c1' },
