...
const looksLikeEmail = (value: string): boolean => /^[^\s@]+@[^\s@]+$/.test(value);

export const createSidebarIdentity = (user: {
  email?: string | null;
  ...
  const isEmailIdentity =
    !!fullName &&
    (looksLikeEmail(fullName) || (!!email && fullName.toLowerCase() === email.toLowerCase()));
  const displayName = fullName && !isEmailIdentity ? fullName : 'Account';
  const initials =
    displayName === 'Account'
      ? 'TF'
      : displayName
