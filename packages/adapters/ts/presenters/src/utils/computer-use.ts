import type { ToolUsageEvent } from '@taskforceai/client-core/types';
...
const SCREEN_CHROME_CROP_TOP_PX = 24;
...
interface ComputerTheaterActionLog {
  timestamp: string;
  toolName: string;
...
export const createComputerTheaterViewModel = (
  ...
  const logLimit = Math.max(0, options.logLimit ?? 5);
  const latestEvents = logLimit === 0 ? [] : computerEvents.slice(-logLimit);
  const actionLogs = Array.from({ length: latestEvents.length }, (_, index) => {
    const event = latestEvents[latestEvents.length - index - 1];
    if (!event) {
      return null; // coverage-ignore-line -- Array.from length mirrors latestEvents, so this is defensive only.
    }
    return {
