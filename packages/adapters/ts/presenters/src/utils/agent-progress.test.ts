...
describe('streaming progress smoothing', () => {
  ...
  it('returns terminal or non-streaming agent records without smoothing', () => {
    const completed = createAgentVisualization({ status: 'COMPLETED', agent_id: 0 }, 0);
    const failed = createAgentVisualization({ status: 'FAILED', agent_id: 1 }, 1);
    const queued = createAgentVisualization({ status: 'QUEUED', agent_id: 2 }, 2);

    expect(smoothStreamingAgentProgress(completed, 12, true)).toBe(completed);
    expect(smoothStreamingAgentProgress(failed, 12, true)).toBe(failed);
    expect(smoothStreamingAgentProgress(queued, 12, false)).toBe(queued);
  });
});
...
describe('agent visualizations', () => {
  ...
  it('preserves reasoning text and resolves streaming fallbacks', () => {
    const visualization = createAgentVisualization(
      { status: 'RUNNING', reasoning: 'checking inputs' },
      0
    );

    expect(visualization.reasoning).toBe('checking inputs');
    expect(
      resolveExecutionAgentVisualizations({
        isStreaming: true,
        streamingStatuses: [{ agent_id: 1, status: 'RUNNING' }],
      })
    ).toEqual([
