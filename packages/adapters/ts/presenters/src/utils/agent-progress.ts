import { getPublicModelLabel } from '@taskforceai/client-core/chat/model-catalog';
...
const progressState = (upper: string): AgentProgressState => {
  if (upper.includes('FAIL') || upper === 'ERROR') return 'failed';
  if (['COMPLETED', 'COMPLETE', 'DONE', 'SUCCESS'].includes(upper)) return 'completed';
  if (upper.includes('QUEUE') || upper.includes('WAIT')) return 'queued';
  return 'running';
};

const numericProgress = (progress: number, state: AgentProgressState): AgentProgress => {
...
export const parseAgentProgress = (status: string, explicit?: number): AgentProgress => {
  ...
  const state = progressState(upper);

  if (typeof explicit === 'number' && Number.isFinite(explicit)) {
    return numericProgress(explicit, state);
  }

  if (normalized.toUpperCase().startsWith('PROGRESS:')) {
    const raw = parseFloat(normalized.split(':')[1] ?? '0');
    return numericProgress(Number.isFinite(raw) ? raw : 0, state);
  }
