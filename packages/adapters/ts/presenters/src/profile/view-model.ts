...
const PLAN_THROUGHPUT_LABELS: Record<ProfilePlan, string> = {
  free: PLAN_TASK_ALLOWANCES.free.label,
  pro: PLAN_TASK_ALLOWANCES.pro.label,
  super: PLAN_TASK_ALLOWANCES.super.label,
};
...
export const buildProfileUsageLimitViewModel = ({
  ...
  if (normalized === 'free') {
    ...
    return {
      ...
      description: PLAN_THROUGHPUT_LABELS.free,
  ...
  const messageLabel = usedMessages === 1 ? 'task credit' : 'task credits';
