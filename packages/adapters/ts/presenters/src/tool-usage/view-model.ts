...
import { formatDuration, formatStatus, formatToolName } from '../utils/formatters';
import {
...
interface ToolUsageSearchPreviewView {
  domains: string[];
...
const buildToolUsageEventKey = (event: ToolUsageEvent, index: number): string =>
  `${event.timestamp ?? index}-${event.toolName}-${index}`;

const buildToolUsageSearchPreviewView = (
  preview?: string | null,
