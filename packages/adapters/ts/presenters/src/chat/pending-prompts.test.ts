...
describe('pending prompt summary', () => {
  it('summarizes queued, pending, and failed prompts', () => {
    ...
    expect(summary).toEqual({
      ...
      failedRetryLabel: '1 message failed to send. Remove or resubmit when ready.',
    });
  });
  ...
  it('omits processing suffix when no prompts are pending', () => {
    const summary = summarizePendingPrompts([{ status: 'queued' }]);

    expect(summary.webQueueLabel).toBe('1 message queued');
  });
});
