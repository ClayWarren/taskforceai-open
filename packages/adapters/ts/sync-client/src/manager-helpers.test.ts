...
describe('sync-client/manager-helpers', () => {
  it('drops invalid message payloads when building push payload', async () => {
    ...
    const pending: PendingChange[] = [
      ...
      },
      {
        ...
        data: {
          ...
          syncVersion: 12,
          vectorClock: 'eyJkZXZpY2UtNDIiOjEyfQ==',
        },
        createdAt: now,
      },
      {
    ...
    expect(payload.messages[0]).toEqual(
      expect.objectContaining({
        ...
        device_id: 'device-42',
        sync_version: 12,
        vector_clock: 'eyJkZXZpY2UtNDIiOjEyfQ==',
        is_deleted: false,
      })
    );
  ...
  it('encodes remote conversation entity IDs as numeric server IDs in push payload', async () => {
    ...
    const pending: PendingChange[] = [
      {
        ...
        operation: 'create',
        data: {
          prompt: 'Update existing remote conversation',
          projectId: 9,
          syncVersion: 17,
          vectorClock: 'eyJkZXZpY2UtNDIiOjE3fQ==',
        },
        createdAt: now,
      },
      {
    ...
    expect(remotePayload).toMatchObject({
      ...
      project_id: 9,
      sync_version: 17,
      vector_clock: 'eyJkZXZpY2UtNDIiOjE3fQ==',
    });
