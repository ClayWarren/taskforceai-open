...
describe('sync-client/realtime polling', () => {
  ...
  it('releases a timed-out request so later polls can continue', async () => {
    const originalSetInterval = globalThis.setInterval;
    const originalClearInterval = globalThis.clearInterval;
    let intervalTick: unknown = null;
    const logger = { warn: mock(), debug: mock() };
    ...
    try {
      ...
      const disconnect = createRealtimeConnection(makeParams({ logger, requestTimeoutMs: 5 }));
      await waitForCondition(() =>
        logger.warn.mock.calls.some(
          (call) =>
            call[0] === 'Failed to fetch sync realtime token' &&
            (call[1] as { error?: Error } | undefined)?.error?.name === 'AbortError'
        )
      );
  ...
  it('aborts an in-flight poll when the connection closes', async () => {
    ...
    await waitForCondition(() => mockFetch.mock.calls.length >= 2);
    disconnect();
    await waitForCondition(() => pollAborted);
