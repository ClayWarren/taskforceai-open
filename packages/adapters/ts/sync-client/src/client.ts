...
export function createHttpSyncClient(
  ...
  const retryAfterDelay = async (
    ...
    await sleep(backoff + jitter, context.externalSignal);
  ...
  const handleHttpFailure = async <T>(
    ...
    const shouldRetry =
      (response.status >= 500 || response.status === 429) && context.attempt < retryAttempts;
    if (shouldRetry) {
    ...
    if (fallback && !isUnauthorizedStatus(response.status)) {
