...
const addOptionalMessagePayloadFields = (
  ...
  if (fields.vector_clock !== undefined) payload.vector_clock = fields.vector_clock;
  return payload;
};
...
const toMessagePayload = (change: PendingChange, deviceId: string): MessagePayloadResult => {
  ...
  const vectorClock = change.data['vectorClock'] ?? change.data['vector_clock'];
  const vector_clock = typeof vectorClock === 'string' ? vectorClock : undefined;
...
export async function applyPullResponse(s: SyncStorage, r: SyncPullResponse) {
  ...
  await processInBatches(rawConversations, STORAGE_WRITE_BATCH_SIZE, async (c) => {
    ...
    const projectId = readAliasedField<number | null>(c, 'project_id', 'projectId');
...
const toConversationPayload = (
  change: PendingChange,
  deviceId: string,
  syncTimestamp: string
): ConversationSyncPayload => {
  const remoteConversationID = parseRemoteConversationID(change.entityId);
  const conversationData = isRecord(change.data) ? change.data : {};
  const prompt = typeof conversationData['prompt'] === 'string' ? conversationData['prompt'] : '';
  const projectId = conversationData['projectId'] ?? conversationData['project_id'];
  const syncVersionValue = conversationData['syncVersion'] ?? conversationData['sync_version'];
  const syncVersion = typeof syncVersionValue === 'number' ? syncVersionValue : 0;
  const vectorClockValue = conversationData['vectorClock'] ?? conversationData['vector_clock'];

  return {
    timestamp: new Date(change.createdAt).toISOString(),
    user_input: prompt,
    sync_version: syncVersion,
    last_synced_at: syncTimestamp,
    device_id: deviceId,
    is_deleted: false,
    ...(projectId === null || typeof projectId === 'number' ? { project_id: projectId } : {}),
    ...(typeof vectorClockValue === 'string' ? { vector_clock: vectorClockValue } : {}),
...
export async function buildPushPayload(p: PendingChange[], devId: string) {
  ...
  for (const ch of p) {
    ...
    if (ch.type === 'conversation') {
      c.push(toConversationPayload(ch, devId, syncTimestamp));
      continue;
    }
    if (ch.type === 'message') {
