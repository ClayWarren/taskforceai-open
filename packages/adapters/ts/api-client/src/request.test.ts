...
import { ApiClientError, configureApiTraceContextInjector, createRequestContext } from './request';
import {
...
describe('contracts-ts/request', () => {
  beforeEach(() => {
    configureApiTraceContextInjector();
  });
  ...
  describe('createRequestContext', () => {
    ...
    it('applies the configured trace context injector', async () => {
      const injector = vi.fn((headers: Headers) => {
        headers.set('traceparent', 'test-trace');
      });
      configureApiTraceContextInjector(injector);
      ...
      expect(headers.get('traceparent')).toBe('test-trace');
    });
    ...
    it('cancels retryable HTTP failure bodies before retrying', async () => {
      let cancelCount = 0;
    ...
    it('aborts an in-flight request when the external signal fires', async () => {
      ...
      const controller = new AbortController();
      const request = ctx.request('/test', { signal: controller.signal });
    ...
    it('cancels retry backoff when the request signal aborts', async () => {
      const incrementCounter = vi.fn();
      const startTimer = vi.fn(() => () => {});
      const fetchMock = buildFetchMock()
        .mockResolvedValueOnce(
      ...
      const ctx = createRequestContext({
        ...
        resiliencePolicy: {
          apiClient: {
            timeoutMs: 50_000,
            circuitBreaker: { failureThreshold: 5, recoveryTimeMs: 30000 },
            retry: { attempts: 2, baseDelayMs: 5_000, jitterMs: 0 },
          },
        },
      });
      const controller = new AbortController();
...
describe('contracts-ts/request.utils', () => {
  ...
  describe('parseSuccessPayload', () => {
    ...
    it('returns undefined for 205 status and empty JSON bodies', async () => {
      const resetResponse = new Response(null, { status: 205 });
      await expect(parseSuccessPayload(resetResponse, true)).resolves.toBeUndefined();

      const emptyJsonResponse = new Response('', {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      });
      await expect(parseSuccessPayload(emptyJsonResponse, true)).resolves.toBeUndefined();
    });
    ...
    it('rejects malformed JSON responses with a stable error', async () => {
      const response = new Response('not-json', {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      });

      await expect(parseSuccessPayload(response, true)).rejects.toThrow('Invalid JSON response');
    });
