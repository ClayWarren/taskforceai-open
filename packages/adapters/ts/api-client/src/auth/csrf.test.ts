...
describe('shared/auth/csrf', () => {
  ...
  it('ignores malformed csrf response payloads when no cookie was established', async () => {
    setCookie('');
    const fetchMock = createFetchMock(async () => jsonResponse('csrf-token'));
    globalThis.fetch = fetchMock;
  ...
  it('returns empty token when refresh response is not ok', async () => {
    setCookie('');
  ...
  it('refreshes the csrf token from the production origin in Tauri', async () => {
    ...
    setCookie('');
    const fetchMock = createFetchMock(async () => {
      setCookie('csrf_token=tauri-cookie-token');
      return jsonResponse({ csrfToken: 'network-token' });
    });
    globalThis.fetch = fetchMock;
    ...
    expect(token).toBe('tauri-cookie-token');
    expect(fetchMock).toHaveBeenCalledWith('https://taskforceai.chat/api/auth/csrf', {
      credentials: 'include',
    });
  });

  it('does not resolve a csrf token when document is unavailable', async () => {
    // @ts-expect-error - deleting test-injected document to simulate SSR
    delete globalThis.document;
    const fetchMock = createFetchMock(async () => jsonResponse({ csrfToken: 'network-token' }));
    globalThis.fetch = fetchMock;
