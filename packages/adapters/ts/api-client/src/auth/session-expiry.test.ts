...
const createToken = (payload: unknown): string => {
  ...
  const binaryPayload = Array.from(jsonBytes, (byte) => String.fromCharCode(byte)).join('');
...
describe('session expiry helpers', () => {
  ...
  it('decodes UTF-8 jwt payloads without corrupting string claims', () => {
    expect(getJwtExpiryMs(createToken({ exp: 10, name: 'Jos\u00e9 \ud83d\ude80' }))).toBe(10_000);
  });
