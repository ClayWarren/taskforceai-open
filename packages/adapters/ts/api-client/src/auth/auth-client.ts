...
export { getAuthBaseUrl, getAuthPath, joinBaseAndPath } from './auth-url';
...
const getSignInBaseUrl = () => {
  if (typeof window === 'undefined') {
    return trimTrailingSlashes(getAuthEnvironmentVariable('NEXT_PUBLIC_AUTH_URL') ?? '');
  }
...
export const authClient = {
  ...
  async getToken(): Promise<string | null> {
    ...
    try {
      const base = this._config.baseUrl || getAuthTokenBaseUrl();
      const fetchImpl = this._config.fetchImpl ?? fetch.bind(globalThis);
