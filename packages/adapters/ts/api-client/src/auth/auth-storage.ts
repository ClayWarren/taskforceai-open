...
import { setCookieSafely } from '@taskforceai/browser-runtime/cookies';
...
export const loadStoredUser = (): Result<Partial<AuthenticatedUser>, UserStorageError> => {
  ...
  const normalized: Record<string, unknown> = { ...parsed.value };
  if (typeof normalized['disabled'] === 'boolean') {
    normalized['disabled'] = String(normalized['disabled']);
  }
  if (normalized['is_admin'] === 'true' || normalized['is_admin'] === 'false') {
    normalized['is_admin'] = normalized['is_admin'] === 'true';
  } else if ('is_admin' in normalized && typeof normalized['is_admin'] !== 'boolean') {
    delete normalized['is_admin'];
  }

  return ok(normalized as Partial<AuthenticatedUser>);
};
