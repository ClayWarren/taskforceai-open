...
describe('shared/auth/auth-storage', () => {
  ...
  it('normalizes legacy stored boolean fields at the storage boundary', () => {
    mockLocalStorage['authUser'] = JSON.stringify({
  ...
  it('drops invalid legacy admin flags at the storage boundary', () => {
    mockLocalStorage['authUser'] = JSON.stringify({
      email: 'legacy@example.com',
      is_admin: 'invalid',
    });
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value).not.toHaveProperty('is_admin');
    }
  });
