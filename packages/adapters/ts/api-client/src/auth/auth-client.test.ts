...
describe('shared/auth/auth-client', () => {
  ...
  describe('getSignInUrl', () => {
    ...
    it('uses the injected auth URL for server-side sign-in URLs', () => {
      delete (globalThis as { window?: unknown }).window;
      authClient.configure({ authUrl: ' https://auth.taskforceai.chat/// ' });
    ...
    it('does not require process globals when auth configuration is injected', () => {
      delete (globalThis as { window?: unknown }).window;
  ...
  describe('getSession', () => {
    ...
    it('uses the injected server API URL when no base URL is configured', async () => {
      delete (globalThis as { window?: unknown }).window;
      authClient.configure({ apiUrl: ' https://api.taskforceai.chat/// ' });
      const fetchMock = installFetchResponses(
        jsonResponse({
          user: { email: 'test@example.com' },
          expires: '2099-01-01T00:00:00.000Z',
  ...
  describe('signOut', () => {
    it('supports React Native globals where window exists without location', async () => {
      globalThis.window = {} as Window & typeof globalThis;
      const fetchMock = installFetchResponses(
        jsonResponse({ csrfToken: 'csrf-token-native' }),
        jsonResponse({})
      );
      authClient.configure({ baseUrl: 'https://auth.taskforceai.chat' });

      await expect(authClient.signOut({ redirect: false })).resolves.toBeUndefined();

      const [, signOutInit] = fetchCalls(fetchMock)[1]!;
      const params = (signOutInit as RequestInit).body as URLSearchParams;
