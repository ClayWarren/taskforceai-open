...
describe('persistence/chat-repository', () => {
  ...
  it('truncates messages from a selected turn and refreshes the conversation preview', async () => {
    const messages = [
      storageMessage({
        messageId: 'before',
      ...
      storageMessage({
        messageId: 'restore-from',
        conversationId: 'conv-restore',
        role: 'user',
        content: 'Change the files',
      }),
      storageMessage({
        messageId: 'after',
        conversationId: 'conv-restore',
    ...
    storage.getMessages.mockResolvedValueOnce(messages);
    storage.getConversation.mockResolvedValueOnce(
      ok(
        storageConversation({
          conversationId: 'conv-restore',
          lastMessagePreview: 'Changed them',
        })
      )
    );

    await repository.truncateConversation('conv-restore', 'restore-from');

    expect(storage.deleteMessage).toHaveBeenCalledTimes(2);
    expect(storage.deleteMessage).toHaveBeenNthCalledWith(1, 'restore-from');
    expect(storage.deleteMessage).toHaveBeenNthCalledWith(2, 'after');
    expect(storage.upsertConversation).toHaveBeenCalledWith(
      expect.objectContaining({ lastMessagePreview: 'Earlier answer' })
    );
    expect(searchIndex.removeItem).toHaveBeenCalledWith('restore-from');
    expect(searchIndex.removeItem).toHaveBeenCalledWith('after');
  });
