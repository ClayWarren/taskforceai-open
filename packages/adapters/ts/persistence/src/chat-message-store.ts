...
export class MessageStore {
  ...
  async truncateConversation(
    ...
    return await traceOperation('MessageStore.truncateConversation', async (span) => {
      span.setAttribute('conversation.id', conversationId);
      span.setAttribute('message.id', fromMessageId);
      const messages = await this.adapter.getMessages(conversationId);
      const targetIndex = messages.findIndex((message) => message.messageId === fromMessageId);
      if (targetIndex < 0) {
        throw new Error('The selected message no longer exists.');
      }
      const deleted = messages.slice(targetIndex);
      await Promise.all(deleted.map((message) => this.adapter.deleteMessage(message.messageId)));
      const lastVisibleMessage = messages
        ...
        .find(
          (message) =>
            message.role !== 'system' &&
            message.isAgentStatus !== true &&
            message.content.trim().length > 0
        );
      await this.conversationStore.setLastMessagePreview(
        conversationId,
