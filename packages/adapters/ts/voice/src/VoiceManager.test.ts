...
import type { VoiceLogger } from './logger';
import type { VoiceAdapter } from './types';

void vi.mock('./logger', () => ({
  getVoiceLogger: vi.fn(() => ({
    debug: vi.fn(),
    info: vi.fn(),
    warn: vi.fn(),
    error: vi.fn(),
...
describe('voice/VoiceManager', () => {
  ...
  describe('setAdapter', () => {
    ...
    it('cancels the previous adapter when setting a new adapter', async () => {
      const previousAdapter: VoiceAdapter = {
        init: vi.fn().mockResolvedValue(undefined),
        speak: vi.fn(),
        listen: vi.fn(),
        record: vi.fn(),
      ...
      const nextAdapter: VoiceAdapter = {
        init: vi.fn().mockResolvedValue(undefined),
        speak: vi.fn(),
        listen: vi.fn(),
        record: vi.fn(),
      ...
      const manager = new VoiceManager(previousAdapter, mockLogger);

      manager.setAdapter(nextAdapter);
      await Promise.resolve();

      expect(previousAdapter.cancel).toHaveBeenCalledTimes(1);
      expect(manager.getStatus()).toBe('idle');
      expect(mockLogger.warn).not.toHaveBeenCalled();
    });

    it('logs previous adapter cleanup failures without entering error state', async () => {
      const previousAdapter: VoiceAdapter = {
        init: vi.fn().mockResolvedValue(undefined),
        speak: vi.fn(),
        listen: vi.fn(),
  ...
  describe('cancel', () => {
    ...
    it('ignores stale cancel errors after adapter swap', async () => {
      const rejectCancelCalls: Array<(error: unknown) => void> = [];
      const staleAdapter: VoiceAdapter = {
        init: vi.fn().mockResolvedValue(undefined),
        speak: vi.fn(),
        listen: vi.fn(),
      ...
      manager.setAdapter(replacementAdapter);
      const rejectStaleCancel = rejectCancelCalls[0];
      expect(rejectStaleCancel).toBeDefined();
      rejectStaleCancel?.(new Error('Stale cancel failed'));
