...
import { getVoiceLogger, type VoiceLogger } from './logger';
import type { VoiceAdapter, VoiceAdapterFactory, VoiceRecording, VoiceStatus } from './types';
...
export class VoiceManager {
  ...
  constructor(
    ...
    logger: VoiceLogger = getVoiceLogger(),
  ...
  setAdapter(adapter: VoiceAdapter): void {
    ...
    try {
      void Promise.resolve(previousAdapter?.cancel()).catch((error) => {
        this.logger.warn('Voice adapter cleanup failed', { error: normalizeError(error) });
      });
    } catch (error) {
      this.logger.warn('Voice adapter cleanup failed', { error: normalizeError(error) });
    }
  ...
  async finishListening(): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Voice finish failed', {
        error: {
  ...
  private ensureReady(): VoiceAdapter | Promise<VoiceAdapter> {
    const adapter = this.getReadyAdapter();
    return adapter ?? this.waitForReady();
  }

  private getReadyAdapter(): VoiceAdapter | null {
    return this.state.status === 'ready' ? this.adapter : null;
  }

  private async waitForReady(): Promise<VoiceAdapter> {
    await this.init();
    const adapter = this.getReadyAdapter();
    if (!adapter) {
