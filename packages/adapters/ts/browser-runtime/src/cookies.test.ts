...
const COOKIE_KEY = 'client-core-cookie-key';
const COOKIE_KEY_SECONDARY = 'client-core-cookie-secondary';
...
describe('client-core/utils/cookies', () => {
  beforeEach(() => {
  ...
  it('round-trips encoded fallback cookie values', () => {
    writeCookie(COOKIE_KEY, 'value;with=reserved and unicode \u2665');

    expect(readCookie(COOKIE_KEY)).toBe('value;with=reserved and unicode \u2665');
  });

  it('returns raw cookie values when percent decoding fails', () => {
    const result = setCookieSafely(`${COOKIE_KEY}=%E0%A4%A; Path=/`);

    expect(result).toEqual({ ok: true, value: true });
    expect(readCookie(COOKIE_KEY)).toBe('%E0%A4%A');
  });
  ...
  it('writes browser cookies with path, SameSite, and optional expiration', () => {
    ...
    expect(writes[0]).toMatch(
      /^client-core-cookie-key=from-browser; expires=.+ GMT; path=\/; SameSite=Lax$/
    );
    expect(writes[1]).toBe(`${COOKIE_KEY_SECONDARY}=; path=/; SameSite=Lax`);
  });

  it('encodes browser cookie values and decodes reads', () => {
    const writes: string[] = [];
    globalScope['document'] = {};
    Object.defineProperty(globalScope['document'], 'cookie', {
      get: () => `${COOKIE_KEY}=value%3Bwith%3Dreserved%20and%20unicode%20%E2%99%A5`,
      set: (value: string) => {
        writes.push(value);
      },
      configurable: true,
    ...
    expect(writes[0]).toBe(
      `${COOKIE_KEY}=value%3Bwith%3Dreserved%20and%20unicode%20%E2%99%A5; path=/; SameSite=Lax`
    );
    expect(readCookie(COOKIE_KEY)).toBe('value;with=reserved and unicode \u2665');
  });
