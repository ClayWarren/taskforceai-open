...
describe('client-core/utils/browser-storage', () => {
  ...
  it('returns unavailable when window or localStorage is not available', () => {
    delete globalScope['window'];
    expect(readStorageItem('missing')).toEqual({
      ok: false,
  ...
  it('writes a storage value when localStorage is available', () => {
    ...
    const readResult = readStorageItem('session');
    const removeResult = removeStorageItem('session');
    ...
    expect(readResult).toEqual({ ok: true, value: 'abc123' });
