...
func TestSearchTelemetryAttributesExcludeQueryContent(t *testing.T) {
	const secretQuery = "private acquisition target user@example.com"
	attributes := searchTelemetryAttributes(SearchParams{
		OriginalQuery: secretQuery,
		MaxResults:    7,
	}, "rewritten private query")

	for _, attr := range attributes {
		assert.NotEqual(t, "search.query", string(attr.Key))
		assert.NotEqual(t, "search.effective_query", string(attr.Key))
		assert.NotContains(t, attr.Value.String(), secretQuery)
		assert.NotContains(t, attr.Value.String(), "rewritten private query")
	}
}
