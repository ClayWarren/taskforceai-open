...
func (c *Client) Expire(ctx context.Context, key string, ttl time.Duration) (bool, error) {
	...
	if err != nil {
		slog.Error("Redis Expire failed", "key", key, "error", err)
		return false, err
	}
	return res, nil
}
...
func (c *Client) TTL(ctx context.Context, key string) (time.Duration, error) {
	...
	if err != nil {
		slog.Error("Redis TTL failed", "key", key, "error", err)
	}
	return res, err
}
...
func (c *Client) Incr(ctx context.Context, key string) (int, error) {
	...
	if err != nil {
		slog.Error("Redis Incr failed", "key", key, "error", err)
	}
	return int(res), err
}
...
func (c *Client) IncrWithExpire(ctx context.Context, key string, ttl time.Duration) (int, error) {
	...
	if err != nil {
		slog.Error("Redis IncrWithExpire failed", "key", key, "error", err)
	}
	return res, err
}
...
func (c *Client) CheckRateLimit(ctx context.Context, key string, limit int, window time.Duration) (bool, int, time.Time, error) {
	...
	if err != nil {
		slog.Error("Redis CheckRateLimit failed", "key", key, "error", err)
...
func (c *Client) XAdd(ctx context.Context, stream string, values map[string]any) (string, error) {
	...
	if err != nil {
		slog.Error("Redis XAdd failed", "stream", stream, "error", err)
	}
	return res, err
}
...
func (c *Client) xRead(ctx context.Context, stream string, lastID string, count int64, block time.Duration) ([]goredis.XMessage, error) {
	...
	if err != nil {
		...
		slog.Error("Redis XRead failed", "stream", stream, "error", err)
		return nil, err
	}
