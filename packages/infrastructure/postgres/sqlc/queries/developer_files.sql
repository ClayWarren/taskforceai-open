...
WITH deleted AS (
    ...
    WHERE
        developer_files.id = $1
        AND developer_files.user_id = $2
        AND developer_files.deleted_at IS NULL
    RETURNING developer_files.*
),

released AS (
    UPDATE user_storage_quotas AS quota
    SET
        used_bytes = GREATEST(0, quota.used_bytes - deleted.bytes),
        updated_at = CURRENT_TIMESTAMP
    FROM deleted
    WHERE quota.user_id = deleted.user_id
)
...
WITH restored AS (
    ...
    WHERE
        developer_files.id = $1
        AND developer_files.user_id = $2
        AND developer_files.deleted_at IS NOT NULL
    RETURNING developer_files.user_id, developer_files.bytes
)

UPDATE user_storage_quotas AS quota
SET
    used_bytes = quota.used_bytes + restored.bytes,
    updated_at = CURRENT_TIMESTAMP
FROM restored
WHERE quota.user_id = restored.user_id;
