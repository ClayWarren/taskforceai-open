...
func TestCheckUserAndOrgValidationAndEvalBranches(t *testing.T) {
	ctx := context.Background()
	_, err := NewRedisLimiter(nil, "test").CheckUserAndOrg(ctx, "user", 1, time.Second, 1, 1, time.Second)
	require.ErrorContains(t, err, "client is nil")
	_, err = NewRedisLimiter(&redisLimiterTestClient{}, "test").CheckUserAndOrg(ctx, "user", 0, time.Second, 1, 1, time.Second)
	require.ErrorContains(t, err, "positive")

	client := &redisLimiterTestClient{evalFunc: func(ctx context.Context, _ string, _ []string, _ ...any) *goredis.Cmd {
		cmd := goredis.NewCmd(ctx)
		cmd.SetVal([]any{int64(1), int64(1), int64(0), int64(1000), int64(1), int64(0), int64(1000)})
		return cmd
	}}
	result, err := NewRedisLimiter(client, "test").CheckUserAndOrg(ctx, "user", 1, 0, 1, 1, 0)
	require.NoError(t, err)
	assert.True(t, result.Allowed)
	assert.True(t, result.User.Allowed)
	assert.True(t, result.Org.Allowed)

	client.evalFunc = func(ctx context.Context, _ string, _ []string, _ ...any) *goredis.Cmd {
		cmd := goredis.NewCmd(ctx)
		cmd.SetErr(errors.New("eval failed"))
		return cmd
	}
	_, err = NewRedisLimiter(client, "test").CheckUserAndOrg(ctx, "user", 1, time.Second, 1, 1, time.Second)
	require.ErrorContains(t, err, "eval failed")

	_, err = NewRedisLimiter(&combinedErrorClient{}, "test").CheckUserAndOrg(ctx, "user", 1, time.Second, 1, 1, time.Second)
	require.ErrorContains(t, err, "combined failed")

	scriptClient := &redisLimiterScriptClient{redisLimiterTestClient: redisLimiterTestClient{evalFunc: func(ctx context.Context, _ string, _ []string, _ ...any) *goredis.Cmd {
		cmd := goredis.NewCmd(ctx)
		cmd.SetVal([]any{int64(1), int64(1), int64(0), int64(1000), int64(1), int64(0), int64(1000)})
		return cmd
	}}}
	_, err = NewRedisLimiter(scriptClient, "test").CheckUserAndOrg(ctx, "user", 1, time.Second, 1, 1, time.Second)
	require.NoError(t, err)
	assert.True(t, scriptClient.ranScript)
}

func TestParseCombinedSlidingWindowResult(t *testing.T) {
	_, err := parseCombinedSlidingWindowResult(nil, errors.New("redis failed"))
	require.ErrorContains(t, err, "redis failed")
	_, err = parseCombinedSlidingWindowResult("wrong", nil)
	require.ErrorContains(t, err, "unexpected")
	_, err = parseCombinedSlidingWindowResult([]any{int64(1)}, nil)
	require.ErrorContains(t, err, "unexpected")
	_, err = parseCombinedSlidingWindowResult([]any{int64(1), int64(1), "bad", int64(0), int64(1), int64(0), int64(0)}, nil)
	require.ErrorContains(t, err, "parse combined result 2")
	parsed, err := parseCombinedSlidingWindowResult([]any{int64(1), int64(1), int64(2), int64(3), int64(1), int64(4), int64(5)}, nil)
	require.NoError(t, err)
	assert.Equal(t, [7]int64{1, 1, 2, 3, 1, 4, 5}, parsed)
}
...
type combinedErrorClient struct{ redisLimiterTestClient }

func (*combinedErrorClient) SupportsEval() bool { return false }
func (*combinedErrorClient) CheckRateLimits(context.Context, string, int, time.Duration, string, int, time.Duration) (bool, int, time.Time, bool, int, time.Time, error) {
	return false, 0, time.Time{}, false, 0, time.Time{}, errors.New("combined failed")
}
