...
func TestGatewayVideoPromptNoUserMessage(t *testing.T) {
	assert.Empty(t, gatewayVideoPromptInputFromParams(agent.ChatCompletionCreateParams{
...
func TestGatewayVideoPromptStartFrame(t *testing.T) {
	...
	assert.Equal(t, "Animate this product photo", input.Text)
	assert.Equal(t, "data:image/png;base64,QUFB", input.ImageURL)
}
