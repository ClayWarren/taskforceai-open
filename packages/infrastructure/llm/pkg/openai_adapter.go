...
func (a *OpenAIAdapter) resolveBaseURL(modelID string) string {
	for _, opt := range a.cfg.Models.Options {
		if opt.ID == modelID && opt.BaseURL != "" {
			return normalizeBaseURL(opt.BaseURL)
		}
	}
	if a.cfg.Gateway.BaseURL != "" {
		return normalizeBaseURL(a.cfg.Gateway.BaseURL)
	}
