...
func TestOpenAIAdapter(t *testing.T) {
	...
	t.Run("resolveBaseURL variations", func(t *testing.T) {
		cfg := config.Config{
		...
		assert.Equal(t, "http://m1/", a.resolveBaseURL("m1"))
		assert.Equal(t, "http://g/", a.resolveBaseURL("other"))
		assert.Equal(t, defaultOpenAIBaseURL, NewOpenAIAdapter(config.Config{}).resolveBaseURL("other"))
