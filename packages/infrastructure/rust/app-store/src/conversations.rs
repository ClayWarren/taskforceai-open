...
fn conversation_record_from_row(row: &Row<'_>) -> rusqlite::Result<ConversationRecord> {
    Ok(ConversationRecord {
        ...
        device_id: row.get(9)?,
        is_deleted: sqlite_bool(row.get::<_, i64>(10)?),
        is_archived: sqlite_bool(row.get::<_, i64>(11)?),
    })
}
...
impl SqliteRunStore {
    pub fn list_conversations(&self, limit: usize) -> StoreResult<Vec<ConversationRecord>> {
        ...
        let mut statement = connection.prepare(
                "select rowid, conversation_id, title, created_at, updated_at, last_message_preview, project_id, \
    ...
    pub fn upsert_conversation(&self, conversation: &ConversationRecord) -> StoreResult<()> {
        ...
        connection.execute(
            "insert into conversations (
                    conversation_id, title, created_at, updated_at, last_message_preview, project_id,
                    sync_version, last_synced_at, device_id, is_deleted, is_archived
