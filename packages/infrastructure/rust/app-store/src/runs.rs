...
use super::{
    ...
    storage_error, SqliteRunStore, StoreResult,
};
...
fn run_record_from_row(row: &Row<'_>) -> StoreResult<Option<RunRecord>> {
    ...
    let status: String = row.get(4).map_err(storage_error)?;
    ...
    Ok(Some(RunRecord {
        id,
        prompt: row.get(1).map_err(storage_error)?,
        model_id: row.get(2).map_err(storage_error)?,
        project_id: row.get(3).map_err(storage_error)?,
        status,
        output: row.get(5).map_err(storage_error)?,
        error: row.get(6).map_err(storage_error)?,
        created_at: row.get(7).map_err(storage_error)?,
        updated_at: row.get(8).map_err(storage_error)?,
        tool_events: parse_json_array_column(
        ...
        .map_err(storage_error)?,
        sources: parse_json_array_column(
        ...
        .map_err(storage_error)?,
        agent_statuses: parse_json_array_column(
...
fn pending_prompt_from_row(row: &Row<'_>) -> StoreResult<Option<PendingPromptRecord>> {
    let id: String = row.get(0).map_err(storage_error)?;
    let status: String = row.get(5).map_err(storage_error)?;
    ...
    Ok(Some(PendingPromptRecord {
        ...
        prompt: row.get(1).map_err(storage_error)?,
        model_id: row.get(2).map_err(storage_error)?,
        reasoning_effort: row.get(3).map_err(storage_error)?,
        project_id: row.get(4).map_err(storage_error)?,
        status,
        retry_count: row.get(6).map_err(storage_error)?,
        last_error: row.get(7).map_err(storage_error)?,
        created_at: row.get(8).map_err(storage_error)?,
        updated_at: row.get(9).map_err(storage_error)?,
    }))
}

fn upsert_run_on_connection(connection: &rusqlite::Connection, run: &RunRecord) -> StoreResult<()> {
    let tool_events = serde_json::to_string(&run.tool_events).map_err(storage_error)?;
    let sources = serde_json::to_string(&run.sources).map_err(storage_error)?;
    let agent_statuses = serde_json::to_string(&run.agent_statuses).map_err(storage_error)?;
