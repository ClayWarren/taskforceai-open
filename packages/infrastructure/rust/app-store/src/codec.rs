...
fn json_column_error(column: usize, err: serde_json::Error) -> rusqlite::Error {
...
pub(super) fn parse_json_array_column(
    ...
    serde_json::from_str(raw).map_err(|err| json_column_error(column, err))
}
...
pub(super) fn parse_optional_json_column(
    ...
    serde_json::from_str(raw)
        .map(Some)
        .map_err(|err| json_column_error(column, err))
}
