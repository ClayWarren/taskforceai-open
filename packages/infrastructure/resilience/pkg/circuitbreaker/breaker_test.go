...
func TestCircuitBreaker_AdmitHandlesClosedStateAfterFastPath(t *testing.T) {
	cb := NewWithDefaults()
	cb.mu.Lock()
	cb.state = StateOpen
	cb.mu.Unlock()
	...
	if _, ok := cb.admit(context.Background()); !ok {
		t.Fatal("expected admit to allow state that closes after fast path check")
	}
}
