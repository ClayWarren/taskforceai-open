...
func TestIsTransientError(t *testing.T) {
	...
	assert.True(t, IsTransientError(errors.New("request failed: status code: 503")))
	...
	assert.False(t, IsTransientError(errors.New("request failed: statuscode=503")))
	assert.False(t, IsTransientError(errors.New("request failed: error:: 503")))
...
func TestIsTransientErrorRejectsRepeatedStatusFragmentsWithoutQuadraticWork(t *testing.T) {
	err := errors.New("brave search failed for query '" + strings.Repeat("x500 ", 8_000) + "'")
	started := time.Now()
