...
const scrub = (value: unknown, state: ScrubState = createScrubState()): unknown => {
  ...
  if (isRecord(value)) {
    ...
    for (const [key, val] of Object.entries(value)) {
      sanitized[key] = isSensitiveSentryKey(key) ? '[Filtered]' : scrub(val, state);
    }
    state.active.delete(value);
    return sanitized;
  }

  return typeof value === 'string' ? sanitizeSentryString(value) : value;
};
...
const sanitizeBreadcrumbs = (breadcrumbs: unknown, scrubState: ScrubState): void => {
  if (!Array.isArray(breadcrumbs)) return;
  for (const breadcrumb of breadcrumbs) {
    if (!isRecord(breadcrumb)) continue;
    const data = breadcrumb['data'];
    breadcrumb['data'] = scrub(isRecord(data) ? data : (data ?? {}), scrubState);
...
export function sanitizeEvent(event: BeforeSendEvent): BeforeSendEvent {
  ...
  const scrubState = createScrubState();
  if (isRecord(request)) {
    request['data'] = scrub(request['data'], scrubState);
  }
  if (event['extra']) {
    const scrubbedExtra = scrub(event['extra'], scrubState);
    if (scrubbedExtra !== undefined) {
      event['extra'] = scrubbedExtra as NonNullable<(typeof event)['extra']>;
    }
  }
  if (event['contexts']) {
    const scrubbedContexts = scrub(event['contexts'], scrubState);
    if (scrubbedContexts !== undefined) {
      event['contexts'] = scrubbedContexts as NonNullable<(typeof event)['contexts']>;
    }
  }

  sanitizeBreadcrumbs(event['breadcrumbs'], scrubState);
