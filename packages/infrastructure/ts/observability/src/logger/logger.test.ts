...
describe('client-core/logger/logger', () => {
  ...
  it('reports rejected transport writes and flushes without rejecting callers', async () => {
    const stderrSpy = vi.spyOn(process.stderr, 'write').mockImplementation(() => true);
    const logger = new Logger({
      transports: [
        {
          name: 'rejecting-transport',
          log: () => Promise.reject(new Error('write failed')),
