...
export const createConsoleTransport = (options: ConsoleTransportOptions = {}): LogTransport => {
  ...
  const enabledLevels = new Set(options.levels ?? ['debug', 'info', 'warn', 'error']); // coverage-ignore-line -- Bun misses this executed initializer.
  const includeTimestamp = options.includeTimestamp ?? false; // coverage-ignore-line -- Bun misses this executed initializer.

  // coverage-ignore-next-line -- Bun attributes execution to the object body.
  return {
    name: 'console',
    log(entry: LogEntry) {
