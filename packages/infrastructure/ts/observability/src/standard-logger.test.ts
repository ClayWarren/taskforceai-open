...
describe('createStandardAppLogger', () => {
  ...
  it('defaults to development console policy when no environment is configured', () => {
    const originalNodeEnv = process.env['NODE_ENV'];
    const originalBunTest = process.env['BUN_TEST'];
    setEnvValue('NODE_ENV', undefined);
    setEnvValue('BUN_TEST', undefined);
    setWindow(undefined);
    ...
    try {
      ...
      expect(String(consoleSpies.debug.mock.calls[0]?.[0])).toContain('environment=development');
    } finally {
      setEnvValue('NODE_ENV', originalNodeEnv);
      setEnvValue('BUN_TEST', originalBunTest);
    }
  });

  it('treats Bun test runs as test mode unless explicitly overridden', () => {
    const originalNodeEnv = process.env['NODE_ENV'];
    const originalBunTest = process.env['BUN_TEST'];
    setEnvValue('NODE_ENV', 'production');
    setEnvValue('BUN_TEST', '1');
    setWindow(undefined);
    const consoleSpies = installConsoleSpies();
    ...
    try {
      ...
      logger.error('hidden-during-bun-test');
