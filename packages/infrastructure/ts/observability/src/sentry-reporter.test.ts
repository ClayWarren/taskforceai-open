...
describe('observability/sentry-reporter', () => {
  ...
  it('reuses normalized array references in Sentry extras', () => {
    const mockSentry = createMockSentry();
    const reporter = createSentryErrorReporter(mockSentry);
    const sharedArray = [{ apiKey: 'secret' }];

    reporter({
      message: 'Repeated metadata',
      environment: 'test',
    ...
    const meta = mockSentry.mockScope.setExtra.mock.calls.find((call) => call[0] === 'meta')?.[1];
    expect(meta.first).toBe(meta.second);
    expect(meta.first).toEqual([{ apiKey: '[REDACTED_API_KEY]' }]);
  });

  it('uses the shared sensitive key predicate for Sentry extras', () => {
    const mockSentry = createMockSentry();
    const reporter = createSentryErrorReporter(mockSentry);
  ...
  it('preserves repeated object references while normalizing extras', () => {
    const mockSentry = createMockSentry();
    const reporter = createSentryErrorReporter(mockSentry);
    const shared = { authorization: 'Bearer token-value', label: 'shared' };

    reporter({
      message: 'Shared metadata',
      environment: 'test',
    ...
    const metaCall = mockSentry.mockScope.setExtra.mock.calls.find(
      (call: unknown[]) => call[0] === 'meta'
    );
    expect(metaCall?.[1].first).toBe(metaCall?.[1].second);
    expect(metaCall?.[1].first).toEqual({ authorization: '[REDACTED]', label: 'shared' });
  });
