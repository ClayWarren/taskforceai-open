...
import { DEFAULT_CONSOLE_LEVELS, resolveConsoleLevels } from './console-levels';
...
const addOptionalLoggerTransports = (
  logger: Logger,
  options: Pick<AppLoggerOptions, 'isTest' | 'tauri' | 'sentry' | 'tags'>
): void => {
  if (!options.isTest && options.tauri?.enabled) {
    logger.addTransport(
      createTauriTransport({
        levels: options.tauri.levels ?? DEFAULT_CONSOLE_LEVELS,
        ...(options.tauri.onError ? { onError: options.tauri.onError } : {}),
      })
    );
  }
...
export const createAppLogger = (options: AppLoggerOptions): AppLoggerResult => {
  ...
  addOptionalLoggerTransports(logger, { isTest, tauri, sentry, tags });
