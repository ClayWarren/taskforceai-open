...
const fakeStripeKey = (kind: 'pk' | 'sk', environment: 'live' | 'test') =>
  [kind, environment, '1234567890abcdefghijklmnop'].join('_');
...
describe('logging/sanitize', () => {
  describe('sanitizeValue', () => {
    ...
    it('redacts Stripe keys', () => {
      expect(sanitizeValue(fakeStripeKey('sk', 'test'))).toBe('[REDACTED_STRIPE_KEY]');
      expect(sanitizeValue(fakeStripeKey('pk', 'live'))).toBe('[REDACTED_STRIPE_KEY]');
    });
