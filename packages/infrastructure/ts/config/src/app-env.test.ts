...
describe('config/app-env', () => {
  ...
  it('creates Next public env readers with extra schemas', () => {
  ...
  it('can skip validation while normalizing empty runtime values', () => {
    const nextEnv = createNextPublicAppEnv({
      client: { NEXT_PUBLIC_EXTRA_FLAG: z.string() },
      runtimeEnv: {
        NEXT_PUBLIC_EXTRA_FLAG: 'enabled',
    ...
    expect(Object.isFrozen(nextEnv)).toBe(true);
    expect(nextEnv.NEXT_PUBLIC_EXTRA_FLAG).toBe('enabled');
    expect(nextEnv.NEXT_PUBLIC_SITE_URL).toBeUndefined();
  });
});
