...
const parseLineParts = (
  trimmed: string
): { key: string | undefined; isListItem: boolean; rawValue: unknown } => {
  ...
  const rawValue: unknown = colonMatch ? colonMatch[2]?.trim() : '';
  return { key, isListItem, rawValue };
};

// Handle multi-line strings with |
const readMultilineBlock = (
  lines: string[],
  startIndex: number
...
// Clean up inline comments and coerce common scalar types
const coerceScalarValue = (value: string, trimmed: string): unknown => {
  ...
  if (v === "''" || v === '""') return '';
  if (v && !isNaN(Number(v)) && trimmed.includes(':')) return Number(v);
...
export const basicYamlParse = (content: string): unknown => {
  try {
    ...
    const stack: YamlStackFrame[] = [{ indent: -1, obj: result }];
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const { key, isListItem, rawValue } = parseLineParts(trimmed);
      let value = rawValue;

      if (value === '|') {
        const block = readMultilineBlock(lines, i + 1);
        value = block.value;
        i = block.endIndex;
      } else if (typeof value === 'string') {
        value = coerceScalarValue(value, trimmed);
      }

      if (isListItem) {
        appendListItem(stack, current, trimmed, indent, key, value);
      } else if (key) {
        assignScalarOrNest(stack, current, trimmed, indent, key, value);
      }
    }
