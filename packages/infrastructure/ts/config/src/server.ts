...
export const webEnvSchema = z.object({
  ...
  AUTH_SECRET: z.union([z.string().min(32), z.literal('')]).default(DEFAULT_AUTH_SECRET),
...
const validationErrors = (result: ReturnType<typeof webEnvSchema.safeParse>): string[] => {
  const errors = result.success
    ? []
    : result.error.issues.map((issue) => `${issue.path.join('.')}: ${issue.message}`);
  if (
    result.success &&
    hasInvalidAuthSecretInProduction(result.data.NODE_ENV, result.data.AUTH_SECRET)
  ) {
...
const validateWebEnv = (source: EnvSource) => {
  const errors = validationErrors(webEnvSchema.safeParse(source));
  return errors.length === 0 ? { success: true as const } : { success: false as const, errors };
};
...
export const loadWebEnv = (opts: LoadWebEnvOptions = {}) => {
  ...
  const errors = validationErrors(res);
  ...
  return {
    env,
    validateEnv: (t: EnvSource = src) => validateWebEnv(t),
  };
};
