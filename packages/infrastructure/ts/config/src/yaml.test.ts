...
items:
  ...
  it('should keep root-level list fallback behavior stable', () => {
    expect(basicYamlParse('- first')).toEqual({ undefined: ['first'] });
  });
...
agent:
  max_iterations: 5
    ...
    expect(result.gateway.base_url).toBe('https://api.vercel.ai/v1');
    expect(result.models.options).toHaveLength(2);
    expect(result.models.options[0].id).toBe('xai/grok-4.5');
