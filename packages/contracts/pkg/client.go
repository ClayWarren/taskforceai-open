...
func (c *httpApiClient) DeleteConversation(ctx context.Context, id int) error {
	return c.ctx.Do(ctx, "DELETE", fmt.Sprintf("/api/v1/conversations/%d", id), nil, nil)
}
...
func (c *httpApiClient) UpdateTheme(ctx context.Context, theme Theme) (*MessageResponse, error) {
	...
	err = c.ctx.Do(ctx, "PUT", fmt.Sprintf("/api/v1/auth/theme?theme=%s", url.QueryEscape(string(theme))), nil, &resp)
	if err != nil {
		return nil, err
	}
	return &resp, nil
