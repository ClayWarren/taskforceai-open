...
var benchmarkRequestMessage string
var benchmarkStringResponse string
...
func BenchmarkRequestContextDoGETStructuredTarget(b *testing.B) {
	ctx := &RequestContext{
		...
		GetToken: func() string { return "test-token" },
	}
	...
	for b.Loop() {
		...
		if err := ctx.Do(context.Background(), http.MethodGet, "/api/v1/auth/me", nil, &resp); err != nil {
			b.Fatal(err)
		}
		benchmarkRequestMessage = resp.Message
	}
}

func BenchmarkRequestContextDoPOSTJSONBody(b *testing.B) {
	ctx := &RequestContext{
		BaseURL: "https://api.taskforce.ai",
		HTTPClient: &http.Client{
			Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
	...
	body := map[string]string{"theme_preference": "dark"}
	...
	for b.Loop() {
		...
		if err := ctx.Do(context.Background(), http.MethodPut, "/api/v1/auth/settings", body, &resp); err != nil {
			b.Fatal(err)
		}
		benchmarkRequestMessage = resp.Message
	}
...
func BenchmarkDecodeStringResponseJSONString(b *testing.B) {
	const body = `"exported-data-json"`

	b.ReportAllocs()
	for b.Loop() {
		var target string
		if err := decodeStringResponse(strings.NewReader(body), &target); err != nil {
			b.Fatal(err)
		}
		benchmarkStringResponse = target
	}
}

func BenchmarkDecodeStringResponseJSONObject(b *testing.B) {
	const body = `{"user":{"id":1},"conversations":[]}`
...
func BenchmarkDeriveAPIErrorMessageJSONDetail(b *testing.B) {
	body := []byte(`{"detail":[{"message":"nested message"}]}`)

	b.ReportAllocs()
	for b.Loop() {
		benchmarkRequestMessage = deriveAPIErrorMessage("422 Unprocessable Entity", body)
	}
}
