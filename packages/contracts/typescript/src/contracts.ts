...
// pgtype.Timestamp implements json.Marshaler and emits its database timestamp
// representation as a timezone-less ISO string. Nullable database columns emit null.
const databaseTimestampSchema = z.string();

export const agentSchema = z.object({
  active_days: z.array(z.number().int()).nullable(),
  active_end: z.string(),
  active_start: z.string(),
  autonomy_enabled: z.boolean(),
  avatar: z.string().nullable(),
  check_interval: z.number().int(),
  created_at: databaseTimestampSchema,
  description: z.string().nullable(),
  id: z.string(),
  last_run_at: databaseTimestampSchema.nullable(),
  model_id: z.string().nullable(),
  name: z.string(),
  next_run_at: databaseTimestampSchema.nullable(),
  status: z.string(),
  timezone: z.string(),
...
export const agentInputSchema = z.object({
  activeDays: z.array(z.number().int()).nullable(),
  activeEnd: z.string().default('17:00'),
  activeStart: z.string().default('09:00'),
  autonomyEnabled: z.boolean(),
  avatar: z.string().optional(),
  check_interval: z.number().int().default(600),
...
export const pendingTaskApprovalSchema = z.object({
...
const readAliased = (value: Record<string, unknown>, key: string, alias = key): unknown =>
  Object.hasOwn(value, key) ? value[key] : Object.hasOwn(value, alias) ? value[alias] : undefined;

export const balanceResponseSchema = z
  .record(z.string(), z.unknown())
  .transform((value) => ({
    creditBalance: readAliased(value, 'creditBalance', 'credit_balance'),
    autoRechargeEnabled: readAliased(value, 'autoRechargeEnabled', 'auto_recharge_enabled'),
    autoRechargeAmount: readAliased(value, 'autoRechargeAmount', 'auto_recharge_amount'),
    autoRechargeThreshold: readAliased(value, 'autoRechargeThreshold', 'auto_recharge_threshold'),
    subscriptionStatus: readAliased(value, 'subscriptionStatus', 'subscription_status'),
    subscriptionId: readAliased(value, 'subscriptionId', 'subscription_id'),
    cancelAtPeriodEnd: readAliased(value, 'cancelAtPeriodEnd', 'cancel_at_period_end'),
    currentPeriodEnd: readAliased(value, 'currentPeriodEnd', 'current_period_end'),
    currentPeriodStart: readAliased(value, 'currentPeriodStart', 'current_period_start'),
  }))
  .pipe(
    z
      .object({
...
export const paymentMethodResponseSchema = z
  .record(z.string(), z.unknown())
  .transform((value) => ({
    id: readAliased(value, 'id'),
    brand: readAliased(value, 'brand'),
    last4: readAliased(value, 'last4'),
    expMonth: readAliased(value, 'expMonth', 'exp_month'),
    expYear: readAliased(value, 'expYear', 'exp_year'),
    isDefault: readAliased(value, 'isDefault', 'is_default'),
  }))
  .pipe(
    z.object({
      id: z.string(),
...
export const invoiceResponseSchema = z
  ...
  .transform((value) => ({
    id: readAliased(value, 'id'),
    number: readAliased(value, 'number'),
    amountPaid: readAliased(value, 'amountPaid', 'amount_paid'),
    currency: readAliased(value, 'currency'),
    status: readAliased(value, 'status'),
    createdAt: readAliased(value, 'createdAt', 'created_at'),
    invoicePdf: readAliased(value, 'invoicePdf', 'invoice_pdf'),
    hostedUrl: readAliased(value, 'hostedUrl', 'hosted_url'),
  }))
  .pipe(
    z.object({
      id: z.string(),
