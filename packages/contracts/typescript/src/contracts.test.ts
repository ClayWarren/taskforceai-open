...
describe('contracts-ts/contracts project requests', () => {
  it('accepts the complete project creation contract', () => {
    expect(
      createProjectRequestSchema.parse({
        name: 'Research',
        description: 'Frontier model research',
        custom_instructions: 'Prefer primary sources',
        icon: 'microscope',
        color: '#0A84FF',
  ...
  it('enforces project field constraints', () => {
    expect(createProjectRequestSchema.safeParse({ name: 'x'.repeat(101) }).success).toBe(false);
    expect(createProjectRequestSchema.safeParse({ name: 'Research', color: 'blue' }).success).toBe(
      false
    );
    expect(
