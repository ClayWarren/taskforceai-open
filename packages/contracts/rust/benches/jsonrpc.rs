...
use taskforceai_app_protocol::{
    JsonRpcRequest, JsonRpcResponse, OutgoingMessage, SubmitRunParams, JSONRPC_VERSION,
};
    ...
    let payload = json!({
        ...
        "params": {
            ...
            "clientMcpTools": [
                ...
                { "serverName": "github", "toolName": "create_pull_request" }
            ]
        }
    });
    serde_json::from_value::<SubmitRunParams>(payload["params"].clone())
        .expect("bench request params should match SubmitRunParams");
