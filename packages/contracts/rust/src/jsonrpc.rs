...
impl OutgoingMessageParts {
        ...
        E: de::Error,
    {
        ...
        let has_response_field = self.id_present || self.result_present || self.error_present;
        if has_method && has_response_field {
            return Err(E::custom(
                "JSON-RPC outgoing message must not mix method with id, result, or error",
            ));
        }
