...
impl OutgoingMessageParts {
        ...
        E: de::Error,
    {
        ...
        if self.result_present == self.error_present {
            return Err(E::custom(
                "JSON-RPC response must contain exactly one of result or error",
            ));
        }
        if self.error_present && self.error.is_none() {
            return Err(E::custom("JSON-RPC response error must be an object"));
        }
...
impl<'de> Deserialize<'de> for JsonRpcResponse {
    ...
    where
        D: de::Deserializer<'de>,
    {
        match deserializer.deserialize_map(OutgoingMessageVisitor)? {
            OutgoingMessage::Response(response) => Ok(response),
            OutgoingMessage::Request(_) | OutgoingMessage::Notification(_) => {
                Err(de::Error::custom("expected JSON-RPC response object"))
            }
        }
    }
}
