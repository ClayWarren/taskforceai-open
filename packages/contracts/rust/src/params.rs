...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ProjectWorkspaceSetParams {
    pub project_id: i64,
    #[serde(default)]
    pub workspace_roots: Vec<String>,
}
...
pub struct McpServerStatusListParams {
    ...
    /// Reserved for clients that scope MCP sessions to an agent thread. The
    /// local app-server inventory is currently workspace-global.
    #[serde(default)]
