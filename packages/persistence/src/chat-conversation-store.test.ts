...
describe('persistence/chat-conversation-store', () => {
  ...
  it('ensureConversation keeps existing non-default title', async () => {
  ...
  it('renameConversation treats Error not found messages as missing', async () => {
    storage.getConversation.mockResolvedValueOnce(conversationResultError(new Error('not found')));

    await store.renameConversation('conv-1', 'Ignored');
  ...
  it('getConversation maps not found and storage errors', async () => {
    ...
    expect(fromMessage).toEqual(
      err({ kind: 'not_found', message: 'Conversation Not Found in adapter' })
    );
