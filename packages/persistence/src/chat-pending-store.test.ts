...
describe('persistence/chat-pending-store', () => {
  ...
  it('falls back to a status-only update when the pending prompt is missing', async () => {
    storage.getPendingChanges.mockResolvedValueOnce([]);

    await store.updatePromptStatus(12, 'failed');

    expect(storage.updatePendingChangeData).toHaveBeenCalledWith(12, { status: 'failed' });
  });
