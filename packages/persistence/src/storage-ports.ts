...
import type { PendingChange, StorageConversation, StorageMessage } from './storage-adapter';

export interface ConversationStorage {
  getConversations(limit?: number, offset?: number): Promise<StorageConversation[]>;
  getConversation(conversationId: string): Promise<Result<StorageConversation>>;
...
export interface MessageStorage {
  ...
  getMessage(messageId: string): Promise<Result<StorageMessage>>;
