...
describe('persistence/chat-message-store', () => {
  ...
  it('creates a new message when adapter returns an Error with not found message', async () => {
    vi.spyOn(Date, 'now').mockReturnValue(1710001000250);
    storage.getMessage.mockResolvedValueOnce(messageResultError(new Error('message not found')));
