import { LocalSearch, type SearchableItem } from '../search/local-search';
...
export const filterSidebarConversations = <TConversation extends SidebarConversationLike>(
  conversations: TConversation[],
  query: string,
  search: Pick<LocalSearch, 'initialize' | 'search'> = new LocalSearch()
): TConversation[] => {
  const trimmed = query.trim();
  if (!trimmed) {
    return conversations;
  }
  const searchItems: SearchableItem[] = conversations.map((conversation) => ({
