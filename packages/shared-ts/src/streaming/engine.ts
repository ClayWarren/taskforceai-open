...
const insertNumberAscending = (items: number[], value: number) => {
  const insertionIndex = items.findIndex((item) => value < item);
  if (insertionIndex === -1) {
    items.push(value);
    return;
  }
  items.splice(insertionIndex, 0, value);
};

const sortedNumbersAscending = (items: number[]): number[] => {
  const sorted: number[] = [];
  for (const item of items) {
    insertNumberAscending(sorted, item);
  }
  return sorted;
};

const insertEntryByKey = (items: [string, unknown][], entry: [string, unknown]) => {
  const insertionIndex = items.findIndex(([key]) => entry[0].localeCompare(key) < 0);
  if (insertionIndex === -1) {
    items.push(entry);
    return;
  }
  items.splice(insertionIndex, 0, entry);
};

const sortedEntriesByKey = (entries: [string, unknown][]): [string, unknown][] => {
  const sorted: [string, unknown][] = [];
  for (const entry of entries) {
    insertEntryByKey(sorted, entry);
  }
  return sorted;
};
...
const mergeToolEvents = (
  ...
  for (const event of incoming) {
    ...
    const incomingKey = toolEventMergeKey(event);
    const duplicate = merged.some((existing) => toolEventMergeKey(existing) === incomingKey);
...
const toolEventMergeKey = (event: ToolUsageEvent): string =>
  [
    toolEventInvocationKey(event),
    event.success ? '1' : '0',
    event.durationMs,
    event.resultPreview ?? '',
    event.error ?? '',
    stableStringify(event.sources ?? []),
  ].join('\u001f');
...
function handleProgressPayload(payload: StreamingPayload, ctx: StreamingEngineContext): void {
  ...
  if (!ctx.refs.ttftReported && (payload.chunk || payload.reasoning) && ctx.refs.streamStartTime) {
    const ttft = Date.now() - ctx.refs.streamStartTime;
  ...
  const progressSources = [...agentSources, ...extractSourcesFromText(payload.chunk)];
  ...
  if (Array.isArray(payload.tool_events)) {
    nextToolEvents = mergeToolEvents(
      ctx.refs.toolEvents,
      normalizeToolUsageEvents(payload.tool_events)
    );
  } else if (Array.isArray(payload.tool_usage)) {
    nextToolEvents = mergeToolEvents(
      ctx.refs.toolEvents,
      normalizeToolUsageEvents(payload.tool_usage)
    );
  } else if (payload.tool_event) {
    nextToolEvents = mergeToolEvents(ctx.refs.toolEvents, [
      normalizeToolUsageEvent(payload.tool_event),
    ]);
  }
...
function handleCompletePayload(payload: StreamingPayload, ctx: StreamingEngineContext): void {
  ...
  const completionToolEvents = Array.isArray(payload.tool_usage)
    ? mergeToolEvents(ctx.refs.toolEvents, normalizeToolUsageEvents(payload.tool_usage))
    : ctx.refs.toolEvents;
  ...
  if (ctx.refs.streamStartTime) {
    const elapsed = Math.floor((Date.now() - ctx.refs.streamStartTime) / 1000);
