...
import { reportOptionalLatencyMark } from '../utils/latency-mark';
...
function reportStreamingTTFTMark(detail: {
  ttftMs: number;
  hasChunk: boolean;
  hasReasoning: boolean;
}): void {
  reportOptionalLatencyMark('streaming.ttft', detail);
}
...
function handleProgressPayload(payload: StreamingPayload, ctx: StreamingEngineContext): void {
  ...
  if (!ctx.refs.ttftReported && (payload.chunk || payload.reasoning) && ctx.refs.streamStartTime) {
    ...
    reportStreamingTTFTMark(detail);
  }
