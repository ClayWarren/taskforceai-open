...
describe('handleStreamingPayload', () => {
  ...
  describe('progress payload', () => {
    ...
    it('emits optional browser TTFT latency mark on first chunk', () => {
      const globalWithMark = globalThis as typeof globalThis & {
        __TASKFORCEAI_LATENCY_MARK__?: (name: string, detail: unknown) => void;
      };
      const previousMark = globalWithMark.__TASKFORCEAI_LATENCY_MARK__;
      const mark = vi.fn();
      ...
      try {
        ...
        if (previousMark) {
          globalWithMark.__TASKFORCEAI_LATENCY_MARK__ = previousMark;
        } else {
