...
describe('Branded Types', () => {
  ...
  describe('ID Generation', () => {
    it('creates unique ConversationIds with prefix', () => {
      const id1 = createConversationId();
      const id2 = createConversationId();

      expect(id1).not.toBe(id2);
      expect(id1.startsWith('conv-')).toBe(true);
      expect(id2.startsWith('conv-')).toBe(true);
      expect(unwrapId(id1)).toMatch(
        /^conv-[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i
      );
    });

    it('creates MessageIds with role prefix', () => {
      const userId = createMessageId('user');
      const assistantId = createMessageId('assistant');
      const systemId = createMessageId('system');
      const defaultId = createMessageId();

      expect(userId.startsWith('user-')).toBe(true);
      expect(assistantId.startsWith('assistant-')).toBe(true);
      expect(systemId.startsWith('system-')).toBe(true);
      expect(defaultId.startsWith('msg-')).toBe(true);
    });

    it('creates unique DeviceIds', () => {
      const id1 = createDeviceId();
      const id2 = createDeviceId();

      expect(id1).not.toBe(id2);
      expect(id1.startsWith('device-')).toBe(true);
    });
