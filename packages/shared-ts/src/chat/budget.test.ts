...
import { calculateBudgetStats, getBudgetColor } from './budget';
...
describe('chat/budget', () => {
  ...
  describe('getBudgetColor', () => {
    it('returns blue before warning threshold', () => {
      expect(getBudgetColor(69.99)).toBe('#3b82f6');
    });

    it('returns yellow at the warning threshold', () => {
      expect(getBudgetColor(70)).toBe('#eab308');
    });

    it('returns red at the exhausted threshold', () => {
