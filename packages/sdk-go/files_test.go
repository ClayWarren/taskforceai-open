...
func TestClient_UploadFile(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"id":"file-123","filename":"test.txt","purpose":"testing","bytes":7,"created_at":1767225600}`))
	}))
...
func TestClient_ListFiles(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"files":[{"id":"file-1","filename":"test.txt","purpose":"assistants","bytes":5,"created_at":1767225600}],"total":1}`))
	}))
	client := clientForServer(t, server)
...
func TestClient_GetFile(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"id":"file-1","filename":"test.txt","purpose":"assistants","bytes":5,"created_at":1767225600}`))
	}))
	client := clientForServer(t, server)
...
func TestClient_FileIDPathSegmentsAreEscaped(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodGet && !strings.HasSuffix(r.URL.EscapedPath(), "/content") {
			_, _ = w.Write([]byte(`{"id":"file/team one?","filename":"test.txt","purpose":"assistants","bytes":5,"created_at":1767225600}`))
		}
	}))
	client := clientForServer(t, server)
...
func TestClient_UploadFileMimeTypeAndResponseHook(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"id":"file-2","filename":"test.txt","purpose":"testing","bytes":7,"created_at":1767225600}`))
	}))
	defer server.Close()
...
func TestClient_UploadAttachmentSuccessAndResponseHook(t *testing.T) {
	var hookStatus int
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != "POST" {
			t.Errorf("expected POST, got %s", r.Method)
		...
		if r.FormValue("mime_type") != "image/png" {
			t.Errorf("expected mime_type image/png, got %s", r.FormValue("mime_type"))
		}
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"id":"att-123"}`))
	}))
	defer server.Close()
	...
	id, err := client.UploadAttachment(context.Background(), "img.png", strings.NewReader("data"), "image/png")
	if err != nil {
		t.Fatalf("UploadAttachment failed: %v", err)
	}
	if id != "att-123" || hookStatus != http.StatusOK {
		t.Fatalf("unexpected result id=%s hook=%d", id, hookStatus)
	}
}

func TestClient_UploadAttachmentInvalidURL(t *testing.T) {
	client, _ := NewClient(TaskForceAIOptions{APIKey: "key", BaseURL: "://bad-url"})
	_, err := client.UploadAttachment(context.Background(), "img.png", strings.NewReader("data"), "image/png")
	if err == nil {
		t.Fatal("expected attachment url error")
	}
}

func TestClient_UploadAttachmentRequestError(t *testing.T) {
	errServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusInternalServerError)
	}))
	defer errServer.Close()
...
func TestClient_UploadAttachmentRequestTransportError(t *testing.T) {
	client, _ := NewClient(TaskForceAIOptions{APIKey: "key"})
	client.uploadRequestHook = func(req *http.Request) (*http.Response, error) {
		_, _ = io.Copy(io.Discard, req.Body)
...
func TestClient_UploadAttachmentNilResponse(t *testing.T) {
	client, _ := NewClient(TaskForceAIOptions{APIKey: "key"})
	client.uploadRequestHook = func(req *http.Request) (*http.Response, error) {
		_, _ = io.Copy(io.Discard, req.Body)
		return nil, nil
...
func TestClient_UploadAttachmentInvalidJSON(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{`))
	}))
	...
	_, err := client.UploadAttachment(context.Background(), "img.png", strings.NewReader("data"), "image/png")
	if err == nil {
		t.Fatal("expected attachment decode error")
	}
}

func TestClient_UploadAttachmentMissingID(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{}`))
	}))
