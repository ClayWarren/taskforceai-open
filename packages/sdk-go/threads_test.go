...
const threadJSON = `{"id":1,"timestamp":"2026-01-01T00:00:00Z","user_input":"My Thread","result":"","execution_time":0,"model":"","agent_count":0,"sources":[],"agentStatuses":[],"toolEvents":[]}`
...
func TestClient_ListThreads(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"conversations":[{"id":1,"timestamp":"2026-01-01T00:00:00Z","user_input":"Thread 1","result":"","execution_time":0,"model":"","agent_count":0,"sources":[],"agentStatuses":[],"toolEvents":[]}],"total":1,"limit":20,"offset":0,"has_more":false}`))
	}))
	client := clientForServer(t, server)
...
func TestClient_GetThread(t *testing.T) {
	...
	if thread.UserInput != "My Thread" {
		t.Errorf("unexpected thread input: %s", thread.UserInput)
	}
}
...
func TestClient_GetThreadMessages(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"messages":[{"id":100,"thread_id":1,"role":"assistant","content":"hello","created_at":"2026-01-01T00:00:00Z"}],"truncated":false}`))
	}))
	client := clientForServer(t, server)
...
func TestClient_CreateThreadOmitsUnsupportedFieldsAndHandlesErrors(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		var body map[string]any
		_ = json.NewDecoder(r.Body).Decode(&body)
		if body["metadata"] != nil || body["messages"] != nil {
			t.Fatalf("unsupported fields must be omitted: %+v", body)
		}
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"id":2,"timestamp":"2026-01-01T00:00:00Z","user_input":"T","result":"","execution_time":0,"model":"","agent_count":0,"sources":[],"agentStatuses":[],"toolEvents":[]}`))
	}))
	client := clientForServer(t, server)
...
func TestClient_ThreadRequestHookErrorsAndNilResponses(t *testing.T) {
	...
	assertRequestFailureModes(t, cases)
}
...
func TestClient_CreateAndGetThreadValidateResponses(t *testing.T) {
	...
	client.requestHook = func(context.Context, string, string, any) (*http.Response, error) {
		return sdkTestResponse(http.StatusOK, `{"id":0,"timestamp":"now","user_input":"Invalid","result":"","execution_time":0,"model":"","agent_count":0,"sources":[],"agentStatuses":[],"toolEvents":[]}`), nil
	}
...
func TestClient_ThreadResponsesValidateRequiredFields(t *testing.T) {
	listServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"conversations":[{"id":1}],"total":1,"limit":20,"offset":0,"has_more":false}`))
	}))
	defer listServer.Close()

	client, _ := NewClient(TaskForceAIOptions{APIKey: "key", BaseURL: listServer.URL})
	_, err := client.ListThreads(context.Background(), 1, 0)
	if err == nil || !strings.Contains(err.Error(), "timestamp is required") {
	...
	messagesServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"messages":[{"id":1,"thread_id":1,"role":"system"}]}`))
	}))
...
func TestClient_RunInThreadWithModelAndStream(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		var body map[string]any
		_ = json.NewDecoder(r.Body).Decode(&body)
		if body["modelId"] != "gpt-test" || body["stream"] != true || body["options"] != nil {
			t.Fatalf("unexpected body: %+v", body)
		}
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"taskId":"task-2","status":"processing"}`))
	}))
	client := clientForServer(t, server)
