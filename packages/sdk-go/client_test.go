...
func TestClient_TaskRequestHookNilResponses(t *testing.T) {
	client, _ := NewClient(TaskForceAIOptions{APIKey: "key"})
	...
	if err == nil || !strings.Contains(err.Error(), "response unavailable") {
		t.Fatalf("expected submit nil response error, got %v", err)
	}

	_, err = client.GetTaskStatus(context.Background(), "task-1")
	if err == nil || !strings.Contains(err.Error(), "response unavailable") {
		t.Fatalf("expected status nil response error, got %v", err)
	}
}

func TestClient_TaskRequestHookErrors(t *testing.T) {
	...
	if !errors.Is(err, boom) {
		t.Fatalf("expected submit request error, got %v", err)
	}

	_, err = client.GetTaskStatus(context.Background(), "task-1")
	if !errors.Is(err, boom) {
		t.Fatalf("expected status request error, got %v", err)
	}
}
