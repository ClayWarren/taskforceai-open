...
func validateTaskStatus(status TaskStatus, label string) error {
	...
	case "processing", "completed", "failed", "canceled", "awaiting_approval":
		return nil
	default:
...
func validateThread(thread Thread, label string) error {
	...
	if strings.TrimSpace(thread.Timestamp) == "" {
		return fmt.Errorf("invalid %s response: timestamp is required", label)
	}
	if thread.ExecutionTime < 0 {
		return fmt.Errorf("invalid %s response: execution_time must be non-negative", label)
	}
	if thread.AgentCount < 0 {
		return fmt.Errorf("invalid %s response: agent_count must be non-negative", label)
	}
	return nil
}
...
func validateThreadList(result ThreadListResponse, label string) error {
	...
	if result.Limit < 0 || result.Offset < 0 {
		return fmt.Errorf("invalid %s response: pagination values must be non-negative", label)
	}
	for index, thread := range result.Conversations {
...
func validateThreadRun(result ThreadRunResponse, label string) error {
	...
	if strings.TrimSpace(result.Status) == "" {
		return fmt.Errorf("invalid %s response: status is required", label)
	}
	return nil
}
...
func validateFile(file File, label string) error {
	...
	if file.CreatedAt < 0 {
		return fmt.Errorf("invalid %s response: created_at must be non-negative", label)
	}
	return nil
}
