...
describe('handleStreamingPayload', () => {
  ...
  describe('progress payload', () => {
    ...
    it('emits an optional TTFT latency event through the output port', () => {
      const mark = vi.fn();
      const ctx = createMockContext({
        refs: { ...createMockContext().refs, streamStartTime: Date.now() - 100 },
        reportLatencyMark: mark,
      });
      handleStreamingPayload(ctx, { type: 'progress', chunk: 'Hello' });

      expect(mark).toHaveBeenCalledWith('streaming.ttft', {
    ...
    it('preserves launch labels when progress arrives without model fields', () => {
      const ctx = createMockContext({
        refs: {
          ...
          agentCount: 2,
          agentLabels: ['gpt-5.6-sol', 'grok-4.5'],
        },
      });
      ...
      expect(ctx.setters.setAgentStatuses).toHaveBeenCalledWith([
        { agent_id: 0, status: 'PROCESSING...', progress: 0.4, model: 'gpt-5.6-sol' },
        { agent_id: 1, status: 'PROCESSING...', progress: 0.3, model: 'grok-4.5' },
      ]);
    });

    it('preserves previous reasoning and fills missing queued agent statuses', () => {
      const ctx = createMockContext({
        state: {
          ...createMockContext().state,
          agentStatuses: [{ agent_id: 0, status: 'RUNNING', progress: 0.4, reasoning: 'thinking' }],
        } as StreamingState,
      ...
      handleStreamingPayload(ctx, {
        type: 'progress',
        agent_statuses: [{ agent_id: 0, status: 'RUNNING', progress: 0.5 }],
      });

      expect(ctx.setters.setAgentStatuses).toHaveBeenCalledWith([
        { agent_id: 0, status: 'RUNNING', progress: 0.5, reasoning: 'thinking' },
        { agent_id: 1, status: 'QUEUED', progress: 0.05 },
      ]);
    });

    it('leaves statuses unchanged when launch labels do not include a matching agent', () => {
      const ctx = createMockContext({
        refs: { ...createMockContext().refs, agentCount: 2, agentLabels: ['model-a'] },
      });

      handleStreamingPayload(ctx, {
        type: 'progress',
        agent_statuses: [{ agent_id: 1, status: 'RUNNING', progress: 0.5 }],
      });

      expect(ctx.setters.setAgentStatuses).toHaveBeenCalledWith([
        { agent_id: 0, status: 'QUEUED', progress: 0.05, model: 'model-a' },
        { agent_id: 1, status: 'RUNNING', progress: 0.5 },
      ]);
    });
    ...
    it('extracts sources from the changed tail of cumulative chunks', () => {
      const ctx = createMockContext({
        state: {
          ...createMockContext().state,
          streamContent: 'Read https://exa',
        } as StreamingState,
      });

      handleStreamingPayload(ctx, {
        type: 'progress',
        chunk: 'Read https://example.com/report',
      });
      ...
      expect(ctx.refs.sources).toEqual([
        {
          ...
          title: 'example.com',
          snippet: 'https://example.com/report',
        },
      ]);
    });

    it('does not rescan unchanged cumulative chunks for sources', () => {
      const source = { url: 'https://example.com/report', title: 'example.com' };
      const ctx = createMockContext({
        state: {
          ...createMockContext().state,
          streamContent: 'Read https://example.com/report',
        } as StreamingState,
        refs: {
          ...createMockContext().refs,
          sources: [source],
      ...
      expect(ctx.refs.sources).toEqual([source]);
      expect(ctx.setters.setSources).not.toHaveBeenCalled();
    });
    ...
    it('deduplicates tool events with array and cyclic arguments', () => {
      const ctx = createMockContext();
      ...
      handleStreamingPayload(ctx, {
        ...
        tool_events: [
          {
            tool_name: 'array_args',
            arguments: ['one', { two: 2 }],
          },
          {
            tool_name: 'cyclic_args',
            arguments: cyclic,
          },
        ],
      });
      ...
      expect(ctx.refs.toolEvents.map((event) => event.status)).toEqual(['completed', 'completed']);
    });
  ...
  describe('complete payload', () => {
    it('reports TTFT when the terminal payload contains the first assistant content', () => {
      const mark = vi.fn();
      const ctx = createMockContext({
        refs: { ...createMockContext().refs, streamStartTime: Date.now() - 100 },
        reportLatencyMark: mark,
      ...
      handleStreamingPayload(ctx, { type: 'complete', message: 'Final answer' });
    ...
    it('persists launch labels in final agent statuses when backend omits model names', () => {
      ...
      expect(ctx.setters.setAgentStatuses).toHaveBeenCalledWith([
        { agent_id: 0, status: 'COMPLETED', progress: 1, model: 'gpt-5.6-sol' },
        { agent_id: 1, status: 'COMPLETED', progress: 1, model: 'grok-4.5' },
      ]);
    });
