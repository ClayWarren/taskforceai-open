...
function handleCompletePayload(payload: StreamingPayload, ctx: StreamingEngineContext): void {
  // Some runtimes, including desktop's app-server bridge, deliver the first
  // assistant content in the terminal payload rather than a progress chunk.
  if (!ctx.refs.ttftReported && payload.message && ctx.refs.streamStartTime) {
    const ttft = ctx.now() - ctx.refs.streamStartTime;
    ctx.refs.ttftReported = true;
    const detail = {
      ttftMs: ttft,
    ...
    ctx.logger.info('Streaming Time-to-First-Token (TTFT)', detail);
    reportStreamingTTFTMark(ctx, detail);
  }
