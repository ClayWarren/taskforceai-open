...
describe('client-core/streaming/schema', () => {
  ...
  it('preserves reasoning and model fields on agent statuses', () => {
    const result = parseStreamingPayload(
      JSON.stringify({
        type: 'progress',
        agent_statuses: [
          { status: 'RUNNING', agent_id: 1, reasoning: 'Checking files', model: 'xai/grok-4.5' },
        ],
      })
    );
    ...
    if (result.ok) {
      ...
      expect(result.value.agent_statuses?.[0]?.model).toBe('xai/grok-4.5');
    }
  });
  ...
  it('validates and normalizes completion metadata', () => {
    const result = parseStreamingPayload(
      JSON.stringify({
        type: 'complete',
        conversation_id: 123,
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.conversation_id).toBe(123);
      expect(result.value.trace_id).toBe('trace-123');
      expect(result.value.pending_approval?.patterns).toEqual([]);
      expect(result.value.pending_approval?.metadata).toEqual({});
    }
  });

  it('rejects malformed completion metadata fields', () => {
    const invalidPayloads = [
      { type: 'complete', conversation_id: '123' },
      { type: 'complete', conversation_id: 1.5 },
      { type: 'complete', trace_id: 123 },
      { type: 'complete', pending_approval: 'approval-1' },
      {
        type: 'complete',
        pending_approval: {
          permission: 'fs.read',
    ...
    for (const payload of invalidPayloads) {
      expect(parseStreamingPayload(JSON.stringify(payload))).toEqual({
        ok: false,
  ...
  it('rejects invalid legacy tool event timestamps', () => {
    for (const timestamp of ['not-a-date', 1e100]) {
      const result = parseStreamingPayload(
        JSON.stringify({
          type: 'progress',
          tool_events: [{ timestamp, custom_field: 'legacy' }],
        })
      );
      expect(result).toEqual({ ok: false, error: 'INVALID_PAYLOAD' });
    }
