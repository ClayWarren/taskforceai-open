...
describe('client-core/streaming/lifecycle-effects', () => {
  ...
  it('does not apply local streaming errors without an active conversation', async () => {
    const applyLocalError = vi.fn();
    ...
    expect(result).toBe(false);
    expect(applyLocalError).not.toHaveBeenCalled();
