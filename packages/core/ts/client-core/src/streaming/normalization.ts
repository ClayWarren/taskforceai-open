...
const resolveGeneratedFile = (
  ...
  const artifactId = stringValue(raw['artifactId'] ?? raw['artifact_id']);
  const filepath = stringValue(raw['filepath']);
  const mimeType = stringValue(raw['mimeType'] ?? raw['mime_type']);
  const bytes = numberValue(raw['bytes']);
  const fileId = stringValue(raw['fileId'] ?? raw['file_id']);
  const downloadUrl = stringValue(raw['downloadUrl'] ?? raw['download_url']);
  return {
    filename,
    ...definedProps({
      artifactId,
...
export const normalizeToolUsageEvent = (
  ...
  const normalized: ToolUsageEvent = {
    timestamp: resolveTimestamp(event, fallbackTimestamp),
...
export const normalizeToolUsageEvents = (
  ...
  fallbackTimestamp = '1970-01-01T00:00:00.000Z'
): ToolUsageEvent[] => events.map((event) => normalizeToolUsageEvent(event, fallbackTimestamp));
