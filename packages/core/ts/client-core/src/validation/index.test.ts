...
import {
  ...
  attachmentMetadataCollectionSchema,
...
describe('client-core/validation', () => {
  ...
  describe('attachmentMetadataSchema', () => {
    ...
    it('rejects empty attachments and files above their tier-specific limits', () => {
      ...
      expect(image.error?.issues[0]?.message).toBe('Image exceeds maximum size of 20MB');
    ...
    it('rejects known unsupported MIME types and normalizes office containers', () => {
      expect(
        attachmentMetadataSchema.safeParse({
          name: 'payload.xml',
          size: 1024,
    ...
    it('rejects collections above the aggregate attachment budget', () => {
      const result = attachmentMetadataCollectionSchema.safeParse([
        { name: 'first.mp4', size: 60 * 1024 * 1024, mimeType: 'video/mp4' },
        { name: 'second.mp4', size: 60 * 1024 * 1024, mimeType: 'video/mp4' },
      ]);
      expect(result.success).toBe(false);
