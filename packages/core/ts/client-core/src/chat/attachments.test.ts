...
describe('chat/attachments', () => {
  it('detects model families that support richer attachment types', () => {
    ...
    expect(isOpenAIModelId('openai/o1-preview')).toBe(true);
    expect(isOpenAIModelId(undefined)).toBe(false);
    expect(isOpenAIModelId('anthropic/claude-sonnet-4.5')).toBe(false);
    expect(isOpenAIModelId('vendor-o10-preview')).toBe(false);
    expect(isOpenAIModelId('xai/gpt-compatible')).toBe(false);
  ...
  it('builds model-specific attachment accept strings', () => {
    ...
    expect(buildPromptAttachmentAccept(VIDEO_GENERATION_MODEL_ID)).toContain('image/png');
    expect(buildPromptAttachmentAccept(VIDEO_GENERATION_MODEL_ID)).not.toContain('video/mp4');
    ...
    expect(buildPromptAttachmentAccept('gpt-5')).not.toContain('application/xml');
