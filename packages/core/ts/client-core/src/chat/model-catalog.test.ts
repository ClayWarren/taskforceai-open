...
describe('chat/model-catalog', () => {
  ...
  it('keeps hosted text-model multipliers aligned with the pricing snapshot', () => {
    ...
    for (const [modelId, multiplier] of expectedMultipliers) {
      expect(optionsById.get(modelId)?.usageMultiple).toBe(multiplier);
    }
  });
