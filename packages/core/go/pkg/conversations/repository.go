...
func (r *PgConversationRepository) fetchMetadataForConversations(ctx context.Context, convIDs []int32) map[int32]*MessageData {
	...
	for _, id := range convIDs {
		...
		if err == nil {
			if message := latestAssistantMessage(msgs); message != nil {
				messages[id] = message
			}
		}
	}
...
func (r *PgConversationRepository) GetConversation(ctx context.Context, userID string, orgID *int, conversationID int) (*ConversationRecord, error) {
	...
	record := mapDbConversation(&conv, latestAssistantMessage(msgs))
	return &record, nil
}
...
func (r *PgConversationRepository) CreateConversation(ctx context.Context, input ConversationCreateInput) (*ConversationRecord, error) {
	...
	orgID32, err := checkedOptionalInt32(input.OrganizationID, "organization_id")
	if err != nil {
		return nil, err
	}
...
func (r *PgConversationRepository) UpdateConversation(ctx context.Context, userID string, orgID *int, conversationID int, update ConversationUpdatePayload) (bool, error) {
	...
	orgID32, err := checkedOptionalInt32(orgID, "organization_id")
	if err != nil {
		return false, err
	}
...
func latestAssistantMessage(messages []MessageData) *MessageData {
	for i := len(messages) - 1; i >= 0; i-- {
		if messages[i].Role == "assistant" && messageHasConversationMetadata(&messages[i]) {
			return &messages[i]
		}
	}
	return nil
...
func checkedOptionalInt32(value *int, name string) (*int32, error) {
	if value == nil {
		//nolint:nilnil // A missing optional value is a successful conversion to nil.
		return nil, nil
	}
	converted, err := checkedInt32(*value, name)
