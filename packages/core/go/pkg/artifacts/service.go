...
func (s *Service) ListArtifacts(ctx context.Context, ownerUserID int32, organizationID *int32, limit, offset int32) ([]Artifact, error) {
	...
	for _, row := range rows {
		artifacts = append(artifacts, Artifact(row))
	}
...
func (s *Service) GetArtifactVersions(ctx context.Context, artifactID string, ownerUserID int32, organizationID *int32) ([]ArtifactVersion, error) {
	...
	for _, row := range rows {
		versions = append(versions, ArtifactVersion(row))
	}
	return versions, nil
}
...
func (s *Service) UpdateArtifactVisibility(ctx context.Context, id string, ownerUserID int32, organizationID *int32, visibility ArtifactVisibility) (*Artifact, error) {
	if strings.TrimSpace(id) == "" || ownerUserID <= 0 || !validVisibility(visibility) {
		return nil, ErrInvalidArtifactInput
	}
	if visibility == ArtifactVisibilityOrganization && organizationID == nil {
		return nil, ErrInvalidArtifactInput
	}
	row, err := s.store.UpdateArtifactVisibilityForOwner(ctx, UpdateArtifactVisibilityForOwnerInput{
		ID:             id,
		OwnerUserID:    ownerUserID,
		OrganizationID: organizationID,
		Visibility:     visibility,
...
func (s *Service) CreatePublicLink(ctx context.Context, artifactID string, ownerUserID int32, organizationID *int32) (*PublicLink, error) {
	...
	artifact, err := s.store.UpdateArtifactVisibilityForOwner(ctx, UpdateArtifactVisibilityForOwnerInput{
		ID:             artifactID,
		OwnerUserID:    ownerUserID,
		OrganizationID: organizationID,
		Visibility:     ArtifactVisibilityPublicLink,
	})
	if err != nil {
		return nil, err
	}
...
func (s *Service) RevokePublicLinks(ctx context.Context, artifactID string, ownerUserID int32, organizationID *int32) error {
	...
	_, err := s.store.UpdateArtifactVisibilityForOwner(ctx, UpdateArtifactVisibilityForOwnerInput{
		ID:             artifactID,
		OwnerUserID:    ownerUserID,
		OrganizationID: organizationID,
		Visibility:     ArtifactVisibilityPrivate,
...
func validVisibility(visibility ArtifactVisibility) bool {
	switch visibility {
	case ArtifactVisibilityPrivate, ArtifactVisibilityOrganization:
		return true
	case ArtifactVisibilityPublicLink:
		// Public-link visibility is only entered atomically by CreatePublicLink,
		// which also creates the corresponding token-backed share record.
		return false
	default:
		return false
	}
