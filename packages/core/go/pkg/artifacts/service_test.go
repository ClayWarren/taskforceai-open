...
func TestUpdateArtifactVisibilityRejectsOrganizationVisibilityWithoutOrg(t *testing.T) {
	svc := NewService(stubArtifactStore{})

	artifact, err := svc.UpdateArtifactVisibility(context.Background(), "artifact-1", 7, nil, ArtifactVisibilityOrganization)

	require.ErrorIs(t, err, ErrInvalidArtifactInput)
	assert.Nil(t, artifact)
}

func TestUpdateArtifactVisibilityRejectsPublicLinkUntilTokenSharingExists(t *testing.T) {
	svc := NewService(stubArtifactStore{})

	artifact, err := svc.UpdateArtifactVisibility(context.Background(), "artifact-1", 7, nil, ArtifactVisibilityPublicLink)

	require.ErrorIs(t, err, ErrInvalidArtifactInput)
	assert.Nil(t, artifact)
}

func TestUpdateArtifactVisibilityRejectsUnknownVisibility(t *testing.T) {
	svc := NewService(stubArtifactStore{})

	artifact, err := svc.UpdateArtifactVisibility(context.Background(), "artifact-1", 7, nil, ArtifactVisibility("unknown"))
