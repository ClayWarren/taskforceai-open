...
func ExecuteRead(ctx protocol.ToolContext, args map[string]any) protocol.ToolResult {
	state := toolutil.NewResult(args)
	parsed, missing := parseReadArgs(args)
	if len(missing) > 0 {
		return toolutil.InvalidArgs("read", args, missing...)
	}
	fp := parsed.filePath
...
func fileNotFoundResult(state protocol.ToolResult, full, displayPath string) protocol.ToolResult {
...
func mediaReadResult(ctx protocol.ToolContext, state protocol.ToolResult, fp, full, mimeType string, data []byte, isPDF bool) protocol.ToolResult {
