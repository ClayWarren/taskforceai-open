...
func parseApplyPatchArgs(args map[string]any) (applyPatchArgs, []string) {
	out := applyPatchArgs{patch: toolutil.GetString(args, "patch")}
	missing := []string{}
	if out.patch == "" {
		missing = append(missing, "missing patch")
	}
	return out, missing
}
