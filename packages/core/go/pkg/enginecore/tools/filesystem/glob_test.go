...
func TestToolGlob_MatchesPathPatterns(t *testing.T) {
	...
	result := ExecuteGlob(protocol.ToolContext{
...
func TestToolGlob_Errors(t *testing.T) {
	...
	// 1. Missing pattern
	res := ExecuteGlob(protocol.ToolContext{Ctx: context.Background(), Cwd: root}, map[string]any{})
	if res.Status != "error" {
		t.Error("expected error for missing pattern")
	}

	// 2. No such directory
	res = ExecuteGlob(protocol.ToolContext{Ctx: context.Background(), Cwd: root}, map[string]any{"pattern": "*", "path": "missing"})
	if res.Status != "error" {
		t.Error("expected error for missing path")
	}

	// 3. No files found
	res = ExecuteGlob(protocol.ToolContext{Ctx: context.Background(), Cwd: root}, map[string]any{"pattern": "*.missing"})
...
func TestToolGlob_TruncatesToNewestMatches(t *testing.T) {
	...
	result := ExecuteGlob(protocol.ToolContext{Ctx: context.Background(), Cwd: root}, map[string]any{
		"pattern": "*.go",
	})
