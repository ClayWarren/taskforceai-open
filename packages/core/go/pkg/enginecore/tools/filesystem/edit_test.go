...
func TestToolEdit(t *testing.T) {
	...
	t.Run("edit replaces all occurrences", func(t *testing.T) {
		...
		res := ExecuteEdit(ctx, map[string]any{
	...
	t.Run("edit fails when old string is not present", func(t *testing.T) {
		...
		assert.Equal(t, "error", res.Status)
		assert.Contains(t, res.Error, "oldString was not found")
		data := mustReadTestFile(t, testFile)
		assert.Equal(t, "stable line", string(data))
	})

	t.Run("edit rejects empty old string for an existing file", func(t *testing.T) {
		...
		assert.Equal(t, "error", res.Status)
		assert.Contains(t, res.Error, "oldString must not be empty")
		assert.Equal(t, "stable line", string(mustReadTestFile(t, testFile)))
	})
	...
	t.Run("edit invalid args are rejected", func(t *testing.T) {
		res := ExecuteEdit(ctx, map[string]any{"filePath": "dir/"})
		assert.Equal(t, "error", res.Status)
