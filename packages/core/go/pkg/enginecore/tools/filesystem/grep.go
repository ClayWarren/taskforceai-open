...
func ExecuteGrep(ctx protocol.ToolContext, args map[string]any) protocol.ToolResult {
	state := toolutil.NewResult(args)
	parsed, missing := parseGrepArgs(args)
	if len(missing) > 0 {
		return toolutil.InvalidArgs("grep", args, missing...)
	}
...
func grepNoMatchesResult(state protocol.ToolResult, pattern string) protocol.ToolResult {
	state.Output = "No files found"
	state.Title = pattern
	state.TitleSet = true
	state.Metadata = map[string]any{
