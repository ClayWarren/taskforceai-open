...
func ExecuteChart(ctx protocol.ToolContext, args map[string]any) protocol.ToolResult {
	state := toolutil.NewResult(args)
	filePath := toolutil.GetString(args, "filePath")
	if filePath == "" {
		return toolutil.InvalidArgs("create_chart", args, "missing filePath")
	}

	chartType := toolutil.GetString(args, "type") // "bar" or "pie"
	title := toolutil.GetString(args, "title")
	data, ok := args["data"].([]any)
	if !ok || len(data) == 0 {
		return toolutil.InvalidArgs("create_chart", args, "missing or empty data")
	}
	...
	for _, item := range data {
		...
		label := toolutil.GetString(m, "label")
		val := toFloat(m["value"])
