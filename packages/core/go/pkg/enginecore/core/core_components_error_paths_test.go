...
func TestCoreSessionOrchestratorAndSystemErrorPaths(t *testing.T) {
	...
	orch = &Orchestrator{
		...
		Store:      store,
		Summary:    &HeuristicSummaryGenerator{MaxLines: 4},
	}
...
func TestCoreSummaryAndRetryFallbacks(t *testing.T) {
	generated := HeuristicSummaryGenerator{}.Generate([]Message{{Info: MessageInfo{Role: RoleUser}, Parts: []Part{{Type: PartText, Text: "hello"}}}})
...
func TestCoreCompactionAndChannelFallbacks(t *testing.T) {
	...
	changed, next := (&Compactor{Options: CompactionOptions{Auto: true}}).Compact(messages, HeuristicSummaryGenerator{})
