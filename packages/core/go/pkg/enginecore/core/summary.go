...
// SummaryGenerator produces a compaction summary from the messages being
// dropped from context. Implementations may be deterministic (see
// HeuristicSummaryGenerator) or LLM-backed.
type SummaryGenerator interface {
	Generate(messages []Message) string
}

// HeuristicSummaryGenerator produces a deterministic compaction summary.
...
func (s HeuristicSummaryGenerator) Generate(messages []Message) string {
	maxLines := s.MaxLines
	if maxLines <= 0 {
		maxLines = 24
	}
