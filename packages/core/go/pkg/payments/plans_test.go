...
func TestAgentLimitForPlan_ClampsInvalidCoreLimit(t *testing.T) {
	original := planAgentLimits[PlanPro]
	planAgentLimits[PlanPro] = 0
	t.Cleanup(func() { planAgentLimits[PlanPro] = original })

	if got := AgentLimitForPlan("pro"); got != 1 {
		t.Fatalf("expected invalid core limit to clamp to 1, got %d", got)
	}
}
