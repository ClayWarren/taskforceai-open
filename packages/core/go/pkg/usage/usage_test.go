...
func TestComputeModelCostUSD(t *testing.T) {
	...
	t.Run("Case insensitive model lookup", func(t *testing.T) {
		...
		assert.InDelta(t, 0.0058, cost, 1e-12)
	})
	...
	t.Run("Partial token usage", func(t *testing.T) {
		...
		assert.InDelta(t, 0.0025, cost, 1e-12)
	})
...
func TestTokenUsageRecorder_RecordTokenUsage_AccountsForCachedPromptTokens(t *testing.T) {
	...
	err := recorder.RecordTokenUsage(ctx, RecordTokenUsageParams{
		TaskID:        "task-1",
		OverrideCosts: `{"cache-test-model":{"prompt":10,"completion":0,"cacheRead":1}}`,
		Records: []TokenUsageRecord{
			{
...
type textEventRepository struct {
	tokens   []TokenUsageRow
	events   []EventRow
	eventErr error
}

func (r *textEventRepository) CreateTokenUsage(_ context.Context, rows []TokenUsageRow) error {
	r.tokens = append(r.tokens, rows...)
	return nil
}

func (*textEventRepository) CreateToolUsage(context.Context, []ToolUsageRow) error { return nil }

func (r *textEventRepository) CreateUsageEvents(_ context.Context, rows []EventRow) error {
	r.events = append(r.events, rows...)
	return r.eventErr
}

func TestTokenUsageRecorder_WritesUnifiedTextEvent(t *testing.T) {
	repo := &textEventRepository{}
	recorder := NewTokenUsageRecorder(repo, slog.Default())
	require.NoError(t, recorder.RecordTokenUsage(context.Background(), RecordTokenUsageParams{
		TaskID: "task-api", Source: "developer", Records: []TokenUsageRecord{{
			Model: "zai/glm-5.2", Stage: "main", PromptTokens: 100, CompletionTokens: 20, TotalTokens: 120,
		}},
	}))
	require.Len(t, repo.tokens, 1)
	require.Len(t, repo.events, 1)
	assert.Equal(t, "text", repo.events[0].Modality)
	assert.Equal(t, "developer", repo.events[0].Source)
	assert.Equal(t, float64(120), repo.events[0].Quantity)
	assert.Positive(t, repo.events[0].CostMicros)
}

func TestTokenUsageRecorder_DefaultSourceAndEventError(t *testing.T) {
	repo := &textEventRepository{eventErr: assert.AnError}
	recorder := NewTokenUsageRecorder(repo, slog.Default())
	err := recorder.RecordTokenUsage(context.Background(), RecordTokenUsageParams{
		TaskID: "task-api", Records: []TokenUsageRecord{{Model: "zai/glm-5.2", Stage: "main", TotalTokens: 1}},
	})
	require.ErrorIs(t, err, assert.AnError)
	require.Len(t, repo.events, 1)
	assert.Equal(t, "task", repo.events[0].Source)
}
