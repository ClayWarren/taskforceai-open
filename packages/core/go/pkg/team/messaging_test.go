...
func TestTeamServiceMessagingAndTasksGapCoverage(t *testing.T) {
	...
	t.Run("mark read sends singular and plural receipts", func(t *testing.T) {
		...
		if err := svc.AddMember(ctx, "receipt-team", Member{Name: "worker", SessionID: "worker-ses", Status: MemberStatusReady}); err != nil {
			t.Fatalf("add worker: %v", err)
		}
		if err := svc.AddMember(ctx, "receipt-team", Member{Name: "helper", SessionID: "helper-ses", Status: MemberStatusReady}); err != nil {
			t.Fatalf("add helper: %v", err)
		}

		_ = inbox.Write("receipt-team", "worker", InboxMessage{ID: "m1", From: "lead", Text: "one"})
		_ = inbox.Write("receipt-team", "worker", InboxMessage{ID: "m2", From: "helper", Text: "two"})
		_ = inbox.Write("receipt-team", "worker", InboxMessage{ID: "m3", From: "helper", Text: "three"})
	...
	t.Run("mark read does not receipt receipts", func(t *testing.T) {
		...
		if err := svc.AddMember(ctx, "receipt-loop", Member{Name: "worker", SessionID: "worker-ses", Status: MemberStatusReady}); err != nil {
			t.Fatalf("add worker: %v", err)
		}

		_ = inbox.Write("receipt-loop", "worker", InboxMessage{ID: "r1", From: "lead", Text: "[receipt] lead has read your message"})
	...
	t.Run("mark read returns zero when no messages were read", func(t *testing.T) {
		svc := NewService(nil, newTestTeamInbox(t.TempDir()), nil, nil, nil)
	...
	t.Run("broadcast tolerates inbox and session failures", func(t *testing.T) {
		...
		_ = svc.AddMember(ctx, "broadcast-warn", Member{Name: "helper", SessionID: "helper-ses", Status: MemberStatusReady})
	...
	t.Run("task store failures and find current member miss", func(t *testing.T) {
		...
		if err := svc.AddTasks(ctx, "task-errors", []Task{{ID: "1", Content: "task"}}); err == nil || !strings.Contains(err.Error(), "get tasks failed") {
...
func (m *mockStore) GetTeam(ctx context.Context, name string) (*Team, error) {
	t, ok := m.teams[name]
	if !ok {
...
func (m *mockStore) SaveTeam(ctx context.Context, team *Team) error {
	m.teams[team.Name] = team
	return nil
}
func (m *mockStore) ListTeams(ctx context.Context) ([]Team, error) {
	list := make([]Team, 0, len(m.teams))
	for _, t := range m.teams {
		list = append(list, *t)
	}
	return list, nil
}
func (m *mockStore) GetTasks(ctx context.Context, teamName string) ([]Task, error) {
	return m.tasks[teamName], nil
}
func (m *mockStore) SaveTasks(ctx context.Context, teamName string, tasks []Task) error {
...
func (m *mockStore) FindBySession(ctx context.Context, sessionID string) (*Team, string, string, error) {
	for _, t := range m.teams {
...
func (s *addMemberBlockingStore) GetTeam(ctx context.Context, name string) (*Team, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *addMemberBlockingStore) SaveTeam(ctx context.Context, team *Team) error {
	s.mu.Lock()
	s.saveCalls++
...
func (s *addMemberBlockingStore) ListTeams(ctx context.Context) ([]Team, error) { return nil, nil }
func (s *addMemberBlockingStore) GetTasks(ctx context.Context, teamName string) ([]Task, error) {
	return nil, nil
}
func (s *addMemberBlockingStore) SaveTasks(ctx context.Context, teamName string, tasks []Task) error {
	return nil
}
func (s *addMemberBlockingStore) DeleteTeam(ctx context.Context, name string) error { return nil }
func (s *addMemberBlockingStore) FindBySession(ctx context.Context, sessionID string) (*Team, string, string, error) {
	return nil, "", "", nil
}
...
func TestTeamServiceMoreCoverageGapPaths(t *testing.T) {
	...
	t.Run("get reports missing team when store returns nil", func(t *testing.T) {
		svc := NewService(&nilTeamStore{mockStore: &mockStore{teams: map[string]*Team{}, tasks: map[string][]Task{}}}, nil, &mockSessions{}, &mockModels{}, &mockBus{})
	...
	t.Run("send write failure and mark read inbox failure", func(t *testing.T) {
		...
		_ = svc.AddMember(ctx, "send-team", Member{Name: "worker", SessionID: "worker-ses", Status: MemberStatusReady})
	...
	t.Run("recover inbox unreadable path and complete task list failure", func(t *testing.T) {
		inbox := newTestTeamInbox(t.TempDir())
		svc := NewService(&erroringTeamStore{
	...
	t.Run("resolve dependencies marks blocked tasks and unblocks when deps complete", func(t *testing.T) {
		...
		blocked := svc.resolveDependencies([]Task{
		...
		unblocked := svc.resolveDependencies([]Task{
...
func TestTeamServiceRemainingCoverageGapPaths(t *testing.T) {
	...
	t.Run("transition execution status member not found", func(t *testing.T) {
		store := &mockStore{teams: map[string]*Team{
			"t": {Name: "t", LeadSessionID: "lead", Members: []Member{}},
		}, tasks: map[string][]Task{}}
		svc := NewService(store, nil, &mockSessions{}, &mockModels{}, &mockBus{})
	...
	t.Run("approve plan save failure and send plan feedback warning", func(t *testing.T) {
		store := &failingSaveStore{
			mockStore: &mockStore{teams: map[string]*Team{
				"plan": {
					Name:          "plan",
					LeadSessionID: "lead",
					Members:       []Member{{Name: "worker", SessionID: "worker", PlanApproval: PlanApprovalPending}},
				},
			}, tasks: map[string][]Task{}},
		}
		svc := NewService(store, newTestTeamInbox(t.TempDir()), &mockSessions{}, &mockModels{}, &mockBus{})
		...
		inbox := newTestTeamInbox(t.TempDir())
		svc.inbox = inbox
		if err := inbox.Write("plan", "worker", InboxMessage{ID: "seed", From: "lead", Text: "seed"}); err != nil {
	...
	t.Run("broadcast validates text and tolerates autowake failures", func(t *testing.T) {
		...
		svc := NewService(store, newTestTeamInbox(t.TempDir()), &failingAutoWakeSessions{wakeErr: errors.New("wake failed")}, &mockModels{}, &mockBus{})
		if err := svc.Broadcast(ctx, "bcast", "lead", strings.Repeat("x", MaxMessageTextBytes+1)); err == nil {
...
func TestTeamServiceSpawnMemberGapCoverage(t *testing.T) {
	...
	t.Run("inject message failure", func(t *testing.T) {
		...
		team, err := svc.Get(ctx, "spawn-inject-fail")
		if err != nil {
			t.Fatalf("get team after rollback: %v", err)
		}
		if len(team.Members) != 0 {
			t.Fatalf("expected failed member to be rolled back, got %#v", team.Members)
		}
		if sessions.removeCalls != 1 {
			t.Fatalf("expected session cleanup after inject failure, got %d remove calls", sessions.removeCalls)
		}
	})

	t.Run("inject message failure reports rollback failure", func(t *testing.T) {
		store := &mockStore{teams: make(map[string]*Team), tasks: make(map[string][]Task)}
		sessions := &spawnSessions{
		...
		svc := NewService(store, nil, sessions, &mockModels{}, &mockBus{})
		if _, err := svc.Create(ctx, "spawn-rollback-fail", "lead", false); err != nil {
			t.Fatalf("create team: %v", err)
		}
		input := SpawnInput{TeamName: "spawn-rollback-fail", Name: "worker", ParentSessionID: "lead", Prompt: "work"}
		input.Agent.Name = "agent"
		input.Model.ProviderID = "p"
		input.Model.ModelID = "m"
		_, _, err := svc.SpawnMember(ctx, input)
		if err == nil || !strings.Contains(err.Error(), "inject failed") || !strings.Contains(err.Error(), "remove failed") {
			t.Fatalf("expected injection and rollback errors, got %v", err)
		}
	})
