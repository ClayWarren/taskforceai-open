...
func TestInMemoryTeamInboxRejectsInvalidNames(t *testing.T) {
	inbox := NewInMemoryInbox()
	err := inbox.Write("../team", "worker", InboxMessage{ID: "1", From: "lead", Text: "hello"})
...
func TestInMemoryTeamInboxEdges(t *testing.T) {
	inbox := &InMemoryInbox{}
	if err := inbox.Write("team", "worker", InboxMessage{ID: "1", From: "lead", Text: "hello"}); err != nil {
	...
	inbox.messages[key] = make([]InboxMessage, MaxInboxMessages)
	if err := inbox.Write("team", "full", InboxMessage{ID: "overflow"}); err == nil || !strings.Contains(err.Error(), "maximum") {
		t.Fatalf("expected full inbox error, got %v", err)
	}

	inbox.messages[key][0] = InboxMessage{ID: "read", Read: true}
	inbox.messages[key][1] = InboxMessage{ID: "unread"}
