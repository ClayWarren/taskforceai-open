...
func TestTeamService_InitAndMemberStatusHandlerBranches(t *testing.T) {
	t.Run("init subscribes all handlers even when subscribe errors occur", func(t *testing.T) {
		...
		svc := NewService(&mockStore{teams: make(map[string]*Team), tasks: make(map[string][]Task)}, newTestTeamInbox(t.TempDir()), &mockSessions{}, &mockModels{}, bus)
	...
	t.Run("handleMemberStatusChanged validates payload and cleans only fully-shutdown teams", func(t *testing.T) {
		...
		if err := svc.AddMember(ctx, "mixed-team", Member{Name: "w1", SessionID: "s1", Status: MemberStatusShutdown}); err != nil {
			t.Fatalf("add member: %v", err)
		}
		if err := svc.AddMember(ctx, "mixed-team", Member{Name: "w2", SessionID: "s2", Status: MemberStatusReady}); err != nil {
			t.Fatalf("add member: %v", err)
		}
		if err := svc.handleMemberStatusChanged(ctx, map[string]any{
			"status":   string(MemberStatusShutdown),
		...
		if err := svc.AddMember(ctx, "done-team", Member{Name: "a", SessionID: "sa", Status: MemberStatusShutdown}); err != nil {
			t.Fatalf("add member: %v", err)
		}
		if err := svc.AddMember(ctx, "done-team", Member{Name: "b", SessionID: "sb", Status: MemberStatusShutdown}); err != nil {
			t.Fatalf("add member: %v", err)
		}
		if err := svc.handleMemberStatusChanged(ctx, map[string]any{
			"status":   string(MemberStatusShutdown),
...
func TestTeamService_HandleTeamCleanedBranches(t *testing.T) {
	...
	svc := NewService(&mockStore{teams: make(map[string]*Team), tasks: make(map[string][]Task)}, newTestTeamInbox(t.TempDir()), sessions, &mockModels{}, &mockBus{})
	...
	if len(sessions.lastRestoreTools) != len(writeTools) {
		t.Fatalf("expected %d write tools, got %d", len(writeTools), len(sessions.lastRestoreTools))
	}
}
...
func TestTeamService_AddAndTransitionEdgeBranches(t *testing.T) {
	...
	if err := svc.AddMember(ctx, "edge-team", Member{Name: "worker", SessionID: "s1", Status: MemberStatusReady}); err != nil {
		t.Fatalf("add member: %v", err)
	}

	if err := svc.AddMember(ctx, "edge-team", Member{Name: "lead", SessionID: "s2"}); err == nil {
		t.Fatal("expected reserved-name add member error")
	}
	if err := svc.AddMember(ctx, "edge-team", Member{Name: "Lead", SessionID: "s2"}); err == nil {
		t.Fatal("expected case-insensitive reserved-name add member error")
	}
	if err := svc.AddMember(ctx, "edge-team", Member{Name: "worker", SessionID: "s3"}); err == nil {
		t.Fatal("expected duplicate-name add member error")
	}
	if err := svc.AddMember(ctx, "edge-team", Member{Name: "worker-2", SessionID: "s1"}); err == nil {
		t.Fatal("expected duplicate-session add member error")
	}

	maxMembers := make([]Member, MaxTeamMembers)
	for i := range maxMembers {
		maxMembers[i] = Member{Name: "m" + string(rune('a'+(i%26))) + string(rune('A'+(i/26))), SessionID: "x" + string(rune('a'+(i%26))) + string(rune('A'+(i/26)))}
	}
	store.teams["max-team"] = &Team{Name: "max-team", LeadSessionID: "lead", Members: maxMembers}
	if err := svc.AddMember(ctx, "max-team", Member{Name: "overflow", SessionID: "overflow"}); err == nil {
	...
	store.teams["exec-busy"] = &Team{
		Name:          "exec-busy",
		LeadSessionID: "lead",
		Members:       []Member{{Name: "w", SessionID: "sw", Status: MemberStatusBusy}},
	}
	...
	store.teams["exec-ready"] = &Team{
		Name:          "exec-ready",
		LeadSessionID: "lead",
		Members:       []Member{{Name: "w", SessionID: "sw2", Status: MemberStatusReady}},
	}
...
func TestTeamService_CancelAndResolveModelBranches(t *testing.T) {
	t.Run("cancel member guard branches", func(t *testing.T) {
		...
		_ = svc.AddMember(ctx, "cancel-team", Member{Name: "idle", SessionID: "idle-session", Status: MemberStatusReady, ExecutionStatus: ExecutionStatusIdle})
		_ = svc.AddMember(ctx, "cancel-team", Member{Name: "terminal", SessionID: "terminal-session", Status: MemberStatusBusy, ExecutionStatus: ExecutionStatusCompleted})
		_ = svc.AddMember(ctx, "cancel-team", Member{Name: "err", SessionID: "err-session", Status: MemberStatusBusy, ExecutionStatus: ExecutionStatusRunning})
		_ = svc.AddMember(ctx, "cancel-team", Member{Name: "ok", SessionID: "ok-session", Status: MemberStatusBusy, ExecutionStatus: ExecutionStatusRunning})
...
func TestTeamService_SendBroadcastAndRecoverInboxBranches(t *testing.T) {
	t.Run("send surfaces inject and autowake failures", func(t *testing.T) {
		...
		if err := svc.AddMember(ctx, "msg-team", Member{Name: "worker", SessionID: "worker-ses", Status: MemberStatusReady}); err != nil {
			t.Fatalf("add worker: %v", err)
		}
		if err := svc.AddMember(ctx, "msg-team", Member{Name: "retired", SessionID: "retired-ses", Status: MemberStatusShutdown}); err != nil {
			t.Fatalf("add retired: %v", err)
		}

		big := strings.Repeat("x", MaxMessageTextBytes+1)
	...
	t.Run("broadcast from member includes lead and skips shutdown members", func(t *testing.T) {
		...
		store := &mockStore{teams: make(map[string]*Team), tasks: make(map[string][]Task)}
		inbox := newTestTeamInbox(t.TempDir())
		svc := NewService(store, inbox, &additionalSessions{}, &mockModels{}, &mockBus{})
		if _, err := svc.Create(ctx, "broadcast-team", "lead-ses", false); err != nil {
			t.Fatalf("create team: %v", err)
		}
		_ = svc.AddMember(ctx, "broadcast-team", Member{Name: "sender", SessionID: "sender-ses", Status: MemberStatusReady})
		_ = svc.AddMember(ctx, "broadcast-team", Member{Name: "active", SessionID: "active-ses", Status: MemberStatusReady})
		_ = svc.AddMember(ctx, "broadcast-team", Member{Name: "off", SessionID: "off-ses", Status: MemberStatusShutdown})
	...
	t.Run("recover inbox counts only successful reinjections", func(t *testing.T) {
		...
		inbox := newTestTeamInbox(t.TempDir())
		svc := NewService(nil, inbox, sessions, &mockModels{}, &mockBus{})
		...
		_ = inbox.Write("t", "agent", InboxMessage{ID: "m1", From: "lead", Text: "one"})
		_ = inbox.Write("t", "agent", InboxMessage{ID: "m2", From: "lead", Text: "two"})
