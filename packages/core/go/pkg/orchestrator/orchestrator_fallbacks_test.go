...
func TestOrchestratorExecutionFallbacks(t *testing.T) {
	...
	t.Run("exec agents logs ready transition failures", func(t *testing.T) {
		store := &erroringTeamStore{
			mockStore: &mockStore{
				teams: map[string]*TeamInfo{"ready-team": {Name: "ready-team", LeadSessionID: "lead"}},
		...
		mockClient.On("CreateChatCompletionStream", mock.Anything, mock.Anything, mock.Anything).Return(nil).Run(func(args mock.Arguments) {
			store.saveTeamErr = errors.New("ready transition failed")
			if cb, ok := args.Get(2).(func(agent.ChatCompletionChunk)); ok {
				cb(agent.ChatCompletionChunk{Choices: []agent.ChatCompletionChunkChoice{{Delta: agent.ChatCompletionChunkDelta{Content: "done"}}}})
			}
		}).Once()

		results := orch.execAgentsWithCheckpoint(ctx, "ready-team", []string{"task"}, nil, "task-ready", nil, nil)
