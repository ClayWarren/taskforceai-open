...
func TestService_Tasks_Extra(t *testing.T) {
	...
	// UpdateTasks
	tasks := []TeamTask{{ID: "1", Content: "T1", Status: TaskStatusPending}}
...
func TestTeamServiceApprovePlanAndCleanupGapCoverage(t *testing.T) {
	...
	t.Run("approve plan rejected with feedback and permission failure", func(t *testing.T) {
		...
		if err := svc.AddMember(ctx, "plan-team", TeamMember{Name: "worker", SessionID: "ses_worker", PlanApproval: PlanApprovalPending}); err != nil {
	...
	t.Run("cleanup delete failure and inbox remove warnings", func(t *testing.T) {
		...
		inbox := newTestTeamInbox(t.TempDir())
		svc := NewTeamService(store, inbox, &mockSessions{}, &mockModels{}, &failingBus{publishErr: errors.New("publish failed")})
		if _, err := svc.Create(ctx, "cleanup-team", "lead", false); err != nil {
			t.Fatalf("create team: %v", err)
		}
		if err := svc.AddMember(ctx, "cleanup-team", TeamMember{Name: "worker", SessionID: "ses_worker", Status: MemberStatusShutdown}); err != nil {
			t.Fatalf("add member: %v", err)
		}
...
func TestTeamServiceCoverageGapPaths(t *testing.T) {
	...
	t.Run("get and transition member status propagate store errors", func(t *testing.T) {
		...
		if err := svc.AddMember(ctx, "transition-team", TeamMember{Name: "worker", SessionID: "ses_worker"}); err != nil {
			t.Fatalf("add member: %v", err)
		}
	...
	t.Run("approve plan sends approved feedback and tolerates publish failures", func(t *testing.T) {
		...
		if err := svc.AddMember(ctx, "plan-feedback", TeamMember{Name: "worker", SessionID: "worker-ses", PlanApproval: PlanApprovalPending}); err != nil {
			t.Fatalf("add member: %v", err)
		}
	...
	t.Run("broadcast tolerates inbox write failures", func(t *testing.T) {
		store := &mockStore{teams: make(map[string]*TeamInfo), tasks: make(map[string][]TeamTask)}
		inbox := newTestTeamInbox(t.TempDir())
		if err := inbox.Write("broadcast-block", "lead", agent.InboxMessage{ID: "seed", From: "lead", Text: "seed"}); err != nil {
		...
		_ = svc.AddMember(ctx, "broadcast-block", TeamMember{Name: "worker", SessionID: "worker-ses", Status: MemberStatusReady})
	...
	t.Run("mark read uses singular receipt text", func(t *testing.T) {
		...
		_ = svc.AddMember(ctx, "receipt-singular", TeamMember{Name: "worker", SessionID: "worker-ses", Status: MemberStatusReady})
		_ = inbox.Write("receipt-singular", "worker", agent.InboxMessage{ID: "m1", From: "lead", Text: "one"})
	...
	t.Run("claim and complete task error paths", func(t *testing.T) {
		...
		if err := svc.AddTasks(ctx, "task-team", []TeamTask{{ID: "t1", Content: "work", Status: TaskStatusPending}}); err != nil {
	...
	t.Run("resolve dependencies unblocks tasks with no dependencies", func(t *testing.T) {
		...
		resolved := svc.resolveDependencies([]TeamTask{
...
func TestTeamServiceCreateAndLookupGapCoverage(t *testing.T) {
	...
	t.Run("get missing team and add member failures", func(t *testing.T) {
		...
		if err := svc.AddMember(ctx, "missing", TeamMember{Name: "w", SessionID: "s"}); err == nil {
