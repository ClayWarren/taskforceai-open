...
func (s *TeamService) Create(ctx context.Context, name string, leadSessionID string, delegate bool) (*TeamInfo, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if err := s.store.SaveTasks(ctx, name, []TeamTask{}); err != nil {
...
func (s *TeamService) Get(ctx context.Context, name string) (*TeamInfo, error) {
...
func (s *TeamService) ListTeams(ctx context.Context) ([]TeamInfo, error) {
	return s.store.ListTeams(ctx)
}

func (s *TeamService) AddMember(ctx context.Context, teamName string, member TeamMember) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *TeamService) SetDelegate(ctx context.Context, teamName string, delegate bool) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *TeamService) FindBySession(ctx context.Context, sessionID string) (*TeamInfo, string, string, error) {
	return s.store.FindBySession(ctx, sessionID)
}

func (s *TeamService) Cleanup(ctx context.Context, teamName string) error {
	team, err := s.Get(ctx, teamName)
	if err != nil {
		return err
	}
