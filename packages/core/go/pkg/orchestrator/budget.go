...
// CheckSpawnAvailable reports whether the current budget permits another team
// member to start. It satisfies the team package's narrow SpawnBudget port.
func (b *BudgetManager) CheckSpawnAvailable() error {
	usage := b.GetUsage().Value
	if usage.RemainingUSD != nil && *usage.RemainingUSD <= 0 {
		return fmt.Errorf("cannot spawn member: organization USD budget exhausted")
	}
	if usage.Initial > 0 && usage.Remaining <= 0 {
