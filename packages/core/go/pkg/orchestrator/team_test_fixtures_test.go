...
// mockStore mirrors the production InMemoryStore's locking and clone-on-
// read/write semantics (pkg/team/memory_store.go). Without them, a Team's
// Members slice is shared (not copied) across every caller holding a
// *TeamInfo obtained from this store, which races the moment more than one
// goroutine touches it concurrently - exactly what a real, running
// TeamSessionManager does (a spawned teammate's background prompt loop
// alongside the test's own calls).
...
func cloneMockTeamInfo(team *TeamInfo) *TeamInfo {
	if team == nil {
		return nil
	}
	cloned := *team
...
func (m *mockStore) GetTeam(_ context.Context, name string) (*TeamInfo, error) {
	...
	return cloneMockTeamInfo(value), nil
}

func (m *mockStore) SaveTeam(_ context.Context, team *TeamInfo) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.teams[team.Name] = cloneMockTeamInfo(team)
	return nil
}
...
func (m *mockStore) ListTeams(context.Context) ([]TeamInfo, error) {
	...
	for _, value := range m.teams {
		result = append(result, *cloneMockTeamInfo(value))
	}
	return result, nil
}
...
func (m *mockStore) GetTasks(_ context.Context, teamName string) ([]TeamTask, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	return append([]TeamTask{}, m.tasks[teamName]...), nil
}

func (m *mockStore) SaveTasks(_ context.Context, teamName string, tasks []TeamTask) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.tasks[teamName] = append([]TeamTask{}, tasks...)
	return nil
}
...
func (m *mockStore) FindBySession(_ context.Context, sessionID string) (*TeamInfo, string, string, error) {
	...
	for _, value := range m.teams {
		if value.LeadSessionID == sessionID {
			return cloneMockTeamInfo(value), "lead", "", nil
		}
		for _, member := range value.Members {
			if member.SessionID == sessionID {
				return cloneMockTeamInfo(value), "member", member.Name, nil
			}
		}
	}
	return nil, "", "", nil
...
func (*mockModels) ParseModel(model string) (ModelInfo, error) {
	...
	if found {
		return ModelInfo{ProviderID: providerID, ModelID: modelID}, nil
	}
