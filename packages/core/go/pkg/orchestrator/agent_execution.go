...
func (o *TaskOrchestrator) execAgentsWithCheckpoint(ctx context.Context, teamName string, qs []string, images []agent.ContentPart, taskID string, userID *int32, completed []AgentResult) []AgentResult {
	...
	for i, q := range qs {
		...
		if err := o.TeamService.AddMember(ctx, teamName, TeamMember{
			Name:            roleName,
			SessionID:       sessionID,
		...
		go withBackgroundRecovery(fmt.Sprintf("agent_%s_%d", teamName, agentIndex+1), o.panicReporter, func() {
			...
			if _, err := o.TeamService.TransitionMemberStatus(agentCtx, teamName, memberRoleName, MemberStatusBusy, true); err != nil {
			...
			if _, err := o.TeamService.TransitionMemberStatus(agentCtx, teamName, memberRoleName, MemberStatusReady, true); err != nil {
