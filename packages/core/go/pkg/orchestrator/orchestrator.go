...
type TaskOrchestrator struct {
	...
	responseCompactor ResponseCompactor
...
type SteeringProvider func(context.Context) ([]string, error)
...
type OrchestratorDeps struct {
	...
	ErrorReporter      IErrorReporter
	TeamService        *team.Service
	TeamSessionManager *TeamSessionManager // optional: refreshed with this request's client/config/registry even when TeamService is a shared singleton
	TeamInbox          team.InboxStore
	TraceRepo          IExecutionTraceRepository
	ReportGenerator    IReportGenerator
	ApprovalReg        IApprovalRegistry
...
type OrchestratorOptions struct {
	// Mode is the client-selected task mode (chat|work|code), or "" when the
	// client didn't send one. Carried for future capability gating - no
	// tool-availability policy is applied to it yet.
	Mode                 string
	AgentCount           int
	Silent               bool
	Mock                 bool
...
func New(cfg config.Config, d OrchestratorDeps, opts OrchestratorOptions) *TaskOrchestrator {
	...
	orch := &TaskOrchestrator{
		...
		promptProvider:       promptProvider,
		steeringProvider:     d.SteeringProvider,
		TeamInbox:            inbox,
	...
	orch.responseCompactor = &LLMResponseCompactor{Orchestrator: orch, Fallback: HeuristicResponseCompactor{}}
	...
	if orch.TeamService == nil {
		sessionManager := &TeamSessionManager{orch: orch}
		orch.TeamService = team.NewService(
		...
		sessionManager.SetTeamService(orch.TeamService)
		sessionManager.SetRunnerDeps(runnerDeps)
	} else if d.TeamSessionManager != nil {
		// Keep the compatibility fallback current for non-tool callers. The
		// request's TeamTools instance below carries its own immutable snapshot
		// through team_spawn, so shared managers never infer tenant identity from
		// this process-global fallback.
		d.TeamSessionManager.SetRunnerDeps(runnerDeps)
	}

	// Register collaborative tools
	teamTools := NewTeamToolsWithRunnerDeps(orch.TeamService, runnerDeps)
...
// Mode returns the client-selected task mode (chat|work|code), or "" when
// none was sent. Available for capability gating once the Chat/Work split
// policy is decided.
func (o *TaskOrchestrator) Mode() string {
	return o.mode
}
