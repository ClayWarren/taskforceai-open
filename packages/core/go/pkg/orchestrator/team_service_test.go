...
func (s *nilTeamStore) GetTeam(ctx context.Context, name string) (*TeamInfo, error) {
	if name == "nil-team" {
		return nil, nil
	}
	return s.mockStore.GetTeam(ctx, name)
...
func agentInboxMessage(from, text string) agent.InboxMessage {
	return agent.InboxMessage{ID: "m1", From: from, Text: text}
}
...
func TestService_AddMember_DoesNotReadStaleStateWhenConcurrent(t *testing.T) {
	...
	go func() {
		errCh <- svc.AddMember(ctx, "race-team", TeamMember{Name: "a", SessionID: "sa"})
	}()

	<-store.firstSaveEntered

	go func() {
		errCh <- svc.AddMember(ctx, "race-team", TeamMember{Name: "b", SessionID: "sb"})
	}()
...
func TestService_ApprovePlan(t *testing.T) {
	...
	tmpDir, _ := os.MkdirTemp("", "inbox-plan-test-*")
	...
	_ = svc.AddMember(ctx, teamName, TeamMember{Name: "worker", SessionID: "ses_w", PlanApproval: PlanApprovalPending})
...
func TestService_CancelAll(t *testing.T) {
	...
	_, _ = svc.Create(ctx, teamName, "lead", false)
	_ = svc.AddMember(ctx, teamName, TeamMember{Name: "w1", SessionID: "s1", Status: MemberStatusBusy})
	_ = svc.AddMember(ctx, teamName, TeamMember{Name: "w2", SessionID: "s2", Status: MemberStatusBusy})
...
func TestService_Cleanup(t *testing.T) {
	store := &mockStore{teams: make(map[string]*TeamInfo), tasks: make(map[string][]TeamTask)}
	tmpDir, _ := os.MkdirTemp("", "inbox-clean-test-*")
	defer func() { _ = os.RemoveAll(tmpDir) }()
	inbox := newTestTeamInbox(tmpDir)
	svc := NewTeamService(store, inbox, &mockSessions{}, &mockModels{}, &mockBus{})
	ctx := context.Background()
	teamName := "clean-team"
	_, _ = svc.Create(ctx, teamName, "lead", false)
	_ = svc.AddMember(ctx, teamName, TeamMember{Name: "worker", SessionID: "ses_w", Status: MemberStatusReady})
...
func TestService_EventHandlers(t *testing.T) {
	store := &mockStore{teams: make(map[string]*TeamInfo), tasks: make(map[string][]TeamTask)}
	tmpDir, _ := os.MkdirTemp("", "inbox-evt-test-*")
	defer func() { _ = os.RemoveAll(tmpDir) }()
	inbox := newTestTeamInbox(tmpDir)
	bus := &mockBus{}
	sessions := &mockSessions{}
	svc := NewTeamService(store, inbox, sessions, &mockModels{}, bus)
	ctx := context.Background()
	...
	teamName := "evt-team"
	_, _ = svc.Create(ctx, teamName, "lead", false)
	_ = svc.AddMember(ctx, teamName, TeamMember{Name: "worker", Status: MemberStatusShutdown})
...
func TestService_FindBySession(t *testing.T) {
	store := &mockStore{teams: make(map[string]*TeamInfo), tasks: make(map[string][]TeamTask)}
	svc := NewTeamService(store, nil, nil, &mockModels{}, &mockBus{})
	ctx := context.Background()
	_, _ = svc.Create(ctx, "t1", "lead-ses", false)
	_ = svc.AddMember(ctx, "t1", TeamMember{Name: "w1", SessionID: "mem-ses"})
...
func TestService_MarkRead(t *testing.T) {
	store := &mockStore{teams: make(map[string]*TeamInfo), tasks: make(map[string][]TeamTask)}
	tmpDir, _ := os.MkdirTemp("", "inbox-read-test-*")
	defer func() { _ = os.RemoveAll(tmpDir) }()
	inbox := newTestTeamInbox(tmpDir)
	svc := NewTeamService(store, inbox, &mockSessions{}, &mockModels{}, &mockBus{})
	ctx := context.Background()
	teamName := "read-team"
	_, _ = svc.Create(ctx, teamName, "lead", false)
	_ = svc.AddMember(ctx, teamName, TeamMember{Name: "worker", SessionID: "ses_w"})
...
func TestService_Messaging(t *testing.T) {
	store := &mockStore{teams: make(map[string]*TeamInfo), tasks: make(map[string][]TeamTask)}
	tmpDir, _ := os.MkdirTemp("", "inbox-msg-test-*")
	defer func() { _ = os.RemoveAll(tmpDir) }()
	inbox := newTestTeamInbox(tmpDir)
	svc := NewTeamService(store, inbox, &mockSessions{}, &mockModels{}, &mockBus{})
	ctx := context.Background()
...
func TestService_Messaging_Extra(t *testing.T) {
	...
	bigText := strings.Repeat("A", MAX_TEXT+1)
	...
	svc2 := NewTeamService(&mockStore{teams: make(map[string]*TeamInfo), tasks: make(map[string][]TeamTask)}, nil, nil, &mockModels{}, &mockBus{})
...
func TestService_Recover(t *testing.T) {
	store := &mockStore{teams: make(map[string]*TeamInfo), tasks: make(map[string][]TeamTask)}
	tmpDir, _ := os.MkdirTemp("", "inbox-recover-test-*")
	defer func() { _ = os.RemoveAll(tmpDir) }()
	inbox := newTestTeamInbox(tmpDir)
	svc := NewTeamService(store, inbox, &mockSessions{}, &mockModels{}, &mockBus{})
	ctx := context.Background()
...
func TestService_RecoverInbox_Extra(t *testing.T) {
	tmpDir, _ := os.MkdirTemp("", "inbox-rec-extra-*")
	defer func() { _ = os.RemoveAll(tmpDir) }()
	inbox := newTestTeamInbox(tmpDir)
	svc := NewTeamService(nil, inbox, &mockSessions{}, &mockModels{}, &mockBus{})
	ctx := context.Background()

	_ = inbox.Write("t", "a", agent.InboxMessage{ID: "1", From: "f", Text: "hi"})
...
func TestService_ResolveModel_Error(t *testing.T) {
	models := &mockModels{}
	svc := NewTeamService(nil, nil, nil, models, &mockBus{})
	ctx := context.Background()
...
func TestService_SpawnMember(t *testing.T) {
	store := &mockStore{teams: make(map[string]*TeamInfo), tasks: make(map[string][]TeamTask)}
	tmpDir, _ := os.MkdirTemp("", "inbox-spawn-test-*")
	defer func() { _ = os.RemoveAll(tmpDir) }()
	inbox := newTestTeamInbox(tmpDir)
	svc := NewTeamService(store, inbox, &mockSessions{}, &mockModels{}, &mockBus{})
	ctx := context.Background()
...
func TestService_SpawnMemberPlanApprovalSkillsAndClaim(t *testing.T) {
	...
	_, _ = svc.Create(ctx, teamName, "lead", false)
	if err := svc.AddTasks(ctx, teamName, []TeamTask{{ID: "task-1", Content: "work", Status: TaskStatusPending}}); err != nil {
