...
func TestSanitizeHelperEdges(t *testing.T) {
	...
	_, key, circular := trackSanitizeValue(reflect.ValueOf(nilMap), nil)
	assert.False(t, circular)
	assert.Zero(t, key)

	_, arrayKey, arrayCircular := trackSanitizeValue(reflect.ValueOf([1]string{"x"}), nil)
	assert.False(t, arrayCircular)
	assert.Zero(t, arrayKey)

	_, defaultKey, defaultCircular := trackSanitizeValue(reflect.ValueOf(1), nil)
