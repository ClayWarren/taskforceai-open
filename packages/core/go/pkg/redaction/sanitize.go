...
func sanitizeAnyMap(m map[string]any, state *sanitizeState) any {
	...
	state, key, circular := trackSanitizeValue(rv, state)
	if circular {
		return "[Circular]"
	}
...
func trackSanitizeValue(rv reflect.Value, state *sanitizeState) (*sanitizeState, sanitizeVisitKey, bool) {
	if state == nil {
		state = &sanitizeState{}
	}
	if rv.Kind() != reflect.Map && rv.Kind() != reflect.Slice && rv.Kind() != reflect.Pointer {
		return state, sanitizeVisitKey{}, false
	}
	if rv.IsNil() {
		return state, sanitizeVisitKey{}, false
	}
