...
func (p *SandboxPool) AcquireWithProfile(ctx context.Context, kind string, profile string) (sandboxSession, bool, error) {
	...
	if sbx, remaining, ok := p.popReusableSandboxLocked(bucket); ok {
		p.mu.Unlock()
...
func (p *SandboxPool) popReusableSandboxLocked(bucket string) (sandboxSession, int, bool) {
	...
	sbx := pool[last]
	pool[last] = nil
	p.popReleasedAtLocked(bucket)
	if last == 0 {
		delete(p.pools, bucket)
		delete(p.releasedAt, bucket)
		return sbx, 0, true
	}
	p.pools[bucket] = pool[:last]
	return sbx, last, true
}

func (p *SandboxPool) popReleasedAtLocked(bucket string) {
	...
	p.releasedAt[bucket] = times[:last]
}
...
func (p *SandboxPool) pruneExpiredLocked(ctx context.Context, now time.Time) {
	...
	for bucket, pool := range p.pools {
		...
		times := p.releasedAt[bucket]
		kept := make([]sandboxSession, 0, len(pool))
		keptTimes := make([]time.Time, 0, len(times))
...
// User OAuth credentials are intentionally never delegated to model-generated code.
func CreateCodeExecutionTool(pool *SandboxPool, _ ...string) ITool {
