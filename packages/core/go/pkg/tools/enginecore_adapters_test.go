...
func TestEngineCoreToolAugmentsGeneratedFileFromMetadataPath(t *testing.T) {
	...
	adapter := &EngineCoreTool{
		name:    "create_spreadsheet",
		toolCtx: &protocol.ToolContext{Cwd: dir},
		exec: func(ctx protocol.ToolContext, args map[string]any) ectools.ToolResult {
...
func TestEngineCoreToolRuntimeAndGeneratedFileDefaultEdges(t *testing.T) {
	...
	previousRuntime := engineCoreToolRuntime
	engineCoreToolRuntime = nil
	engineCoreToolRuntimeMu.Unlock()
	t.Cleanup(func() {
		engineCoreToolRuntimeMu.Lock()
		engineCoreToolRuntime = previousRuntime
		engineCoreToolRuntimeMu.Unlock()
	})
	assert.IsType(t, emptyEngineCoreToolRuntime{}, currentEngineCoreToolRuntime())
