...
func (f *fakeSandbox) DeleteFile(_ context.Context, path string) error {
...
func (f *fakeSandbox) MoveFile(_ context.Context, oldPath, newPath string) error {
	if f.moveFileErr != nil {
		return f.moveFileErr
	}
	f.mu.Lock()
	defer f.mu.Unlock()
	f.movedFrom = append(f.movedFrom, oldPath)
	f.movedTo = append(f.movedTo, newPath)
	return nil
}
...
func TestCodeExecutionSandboxPoolAcquireReleaseBuckets(t *testing.T) {
	...
	assert.NotContains(t, pool.pools, "code")
	assert.NotContains(t, pool.pools, "code:session-1")
	assert.NotContains(t, pool.releasedAt, "code")
	assert.NotContains(t, pool.releasedAt, "code:session-1")
}

func TestCodeExecutionSandboxPoolAcquireRetainsNonEmptyBucket(t *testing.T) {
	pool := &SandboxPool{
		maxPoolSize: 2,
		pools:       make(map[string][]sandboxSession),
	}
	first := &fakeSandbox{id: "first"}
	second := &fakeSandbox{id: "second"}
	pool.Release(context.Background(), first, "code", true)
	pool.Release(context.Background(), second, "code", true)

	acquired, reusable, err := pool.Acquire(context.Background(), "code")
	require.NoError(t, err)
	assert.True(t, reusable)
	assert.Same(t, second, acquired)
	require.Len(t, pool.pools["code"], 1)
	assert.Same(t, first, pool.pools["code"][0])
	assert.Len(t, pool.releasedAt["code"], 1)

	delete(pool.releasedAt, "code")
	acquired, reusable, err = pool.Acquire(context.Background(), "code")
	require.NoError(t, err)
	assert.True(t, reusable)
	assert.Same(t, first, acquired)
	assert.NotContains(t, pool.pools, "code")

	orphan := &fakeSandbox{id: "orphan"}
	pool.pools["orphan"] = []sandboxSession{orphan}
	acquired, remaining, ok := pool.popReusableSandboxLocked("orphan")
	assert.True(t, ok)
	assert.Zero(t, remaining)
	assert.Same(t, orphan, acquired)
}
...
func TestSandboxPoolPruneAndStateEdges(t *testing.T) {
	...
	pool.pools["empty"] = nil
	pool.releasedAt["empty"] = []time.Time{now}
	pool.pruneExpiredLocked(context.Background(), now)
	assert.NotContains(t, pool.pools, "empty")
	assert.NotContains(t, pool.releasedAt, "empty")
...
func TestCodeExecutionToolDoesNotInjectGitHubTokenIntoSandbox(t *testing.T) {
	sbx := &fakeSandbox{
		id:         "no-github-credentials",
		codeResult: sandboxCodeResult{Stdout: "ok\n"},
	}
	ctx := WithComputerUseExecutionContext(context.Background(), ComputerUseExecutionContext{
		ProfileKey: "user:1",
	...
	assert.Equal(t, true, result["success"])
	assert.Equal(t, "ok\n", result["output"])
	assert.Empty(t, sbx.commandCalls)
	assert.Empty(t, sbx.commandEnvs)
	require.Len(t, pool.pools["code:profile:user:1"], 1)
}
