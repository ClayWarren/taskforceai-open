...
var (
	archiveWriterMu sync.RWMutex
	archiveWriter   ArchiveWriter = emptyArchiveWriter{}
)
...
func SetArchiveWriter(writer ArchiveWriter) func() {
	if writer == nil {
		writer = emptyArchiveWriter{}
	}

	archiveWriterMu.Lock()
	previous := archiveWriter
	archiveWriter = writer
	archiveWriterMu.Unlock()

	return func() {
		archiveWriterMu.Lock()
		archiveWriter = previous
		archiveWriterMu.Unlock()
	}
}

func currentArchiveWriter() ArchiveWriter {
	archiveWriterMu.RLock()
	writer := archiveWriter
	archiveWriterMu.RUnlock()
	if writer == nil {
		return emptyArchiveWriter{}
	}
	return writer
}
...
func toolCreateArchive(ctx protocol.ToolContext, args map[string]any) ToolResult {
	...
	if filePath == "" {
		return invalidArgs("create_archive", args, "missing filePath")
	}

	files, ok := args["files"].([]any)
	if !ok || len(files) == 0 {
		return invalidArgs("create_archive", args, "missing or empty files list")
	}
	...
	fullZipPath, ok := prepareExternalFile(ctx, filePath, &state)
	if !ok {
		return state
	}
	...
	for _, f := range files {
		...
		if err := ask(ctx, "read", map[string]any{"filePath": fileName}); err != nil {
			continue // Skip files without read permission.
		}

		fullFilePath, err := externalFilePath(ctx, fileName, kindFile)
		if err != nil {
		...
		entries = append(entries, ArchiveEntry{
			...
			Name:       externalFileBaseName(fileName),
		})
	}
