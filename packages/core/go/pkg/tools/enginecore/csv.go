...
var (
	csvWriterMu sync.RWMutex
	csvWriter   CSVWriter = emptyCSVWriter{}
)
...
func currentCSVWriter() CSVWriter {
	...
	csvWriterMu.RUnlock()
	if writer == nil {
		return emptyCSVWriter{}
	}
	return writer
}

func toolCreateCSV(ctx protocol.ToolContext, args map[string]any) (state ToolResult) {
	state = NewToolResult(args)
	filePath := getString(args, "filePath")
	if filePath == "" {
		return invalidArgs("create_csv", args, "missing filePath")
	}

	rows, ok := args["rows"].([]any)
	if !ok || len(rows) == 0 {
		return invalidArgs("create_csv", args, "missing or empty rows")
	}
