...
var (
	webFetchSourceMu sync.RWMutex
	webFetchSource   WebFetchSource = emptyWebFetchSource{}
)
...
func SetWebFetchSource(source WebFetchSource) func() {
	if source == nil {
		source = emptyWebFetchSource{}
	}

	webFetchSourceMu.Lock()
	previous := webFetchSource
	webFetchSource = source
	webFetchSourceMu.Unlock()

	return func() {
		webFetchSourceMu.Lock()
		webFetchSource = previous
		webFetchSourceMu.Unlock()
	}
}

func currentWebFetchSource() WebFetchSource {
	webFetchSourceMu.RLock()
	source := webFetchSource
	webFetchSourceMu.RUnlock()
	if source == nil {
		return emptyWebFetchSource{}
	}
	return source
}
