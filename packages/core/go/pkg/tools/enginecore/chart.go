...
var (
	chartWriterMu sync.RWMutex
	chartWriter   ChartWriter = emptyChartWriter{}
)
...
func SetChartWriter(writer ChartWriter) func() {
	if writer == nil {
		writer = emptyChartWriter{}
	}

	chartWriterMu.Lock()
	previous := chartWriter
	chartWriter = writer
	chartWriterMu.Unlock()

	return func() {
		chartWriterMu.Lock()
		chartWriter = previous
		chartWriterMu.Unlock()
	}
}
...
func currentChartWriter() ChartWriter {
	chartWriterMu.RLock()
	writer := chartWriter
	chartWriterMu.RUnlock()
	if writer == nil {
		return emptyChartWriter{}
	}
	return writer
}
