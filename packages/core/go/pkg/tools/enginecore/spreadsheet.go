...
var (
	spreadsheetWriterMu sync.RWMutex
	spreadsheetWriter   SpreadsheetWriter = emptySpreadsheetWriter{}
)
...
func SetSpreadsheetWriter(writer SpreadsheetWriter) func() {
	if writer == nil {
		writer = emptySpreadsheetWriter{}
	}

	spreadsheetWriterMu.Lock()
	previous := spreadsheetWriter
	spreadsheetWriter = writer
	spreadsheetWriterMu.Unlock()

	return func() {
		spreadsheetWriterMu.Lock()
		spreadsheetWriter = previous
		spreadsheetWriterMu.Unlock()
	}
}

func currentSpreadsheetWriter() SpreadsheetWriter {
	spreadsheetWriterMu.RLock()
	writer := spreadsheetWriter
	spreadsheetWriterMu.RUnlock()
	if writer == nil {
		return emptySpreadsheetWriter{}
	}
	return writer
}
