...
func TestToolCreateArchive(t *testing.T) {
	...
	t.Run("skips files denied by read permission", func(t *testing.T) {
		...
		res := toolCreateArchive(protocol.ToolContext{
			Ctx:        context.Background(),
			Cwd:        tmpDir,
	...
	t.Run("invalid args and file limit", func(t *testing.T) {
		assert.Equal(t, "error", toolCreateArchive(ctx, map[string]any{}).Status)
		assert.Equal(t, "error", toolCreateArchive(ctx, map[string]any{"filePath": "missing.zip"}).Status)
		files := make([]any, MaxArchiveFiles+1)
		for i := range files {
			files[i] = "f1.txt"
		}
		res := toolCreateArchive(ctx, map[string]any{"filePath": "many.zip", "files": files})
		assert.Equal(t, "error", res.Status)
