...
func TestReadEdgeBranches(t *testing.T) {
	...
	res := toolRead(ctx, map[string]any{"filePath": outsideRel})
	assert.Equal(t, "error", res.Status)
	assert.Contains(t, res.Error, "external directory")

	res = toolRead(ctx, map[string]any{"filePath": "/"})
	assert.Equal(t, "error", res.Status)
...
func TestGlobEdgeBranches(t *testing.T) {
	...
	require.NoError(t, err)
	res := toolGlob(ctx, map[string]any{"path": outsideRel, "pattern": "*"})
	assert.Equal(t, "error", res.Status)
	assert.Contains(t, res.Error, "external directory")
	...
	res = toolGlob(ctx, map[string]any{"pattern": "*"})
	assert.Equal(t, "error", res.Status)
	...
	res = toolGlob(ctx, map[string]any{"path": "sub", "pattern": "*.txt"})
	assert.Equal(t, "completed", res.Status)
...
func TestGrepEdgeBranches(t *testing.T) {
	...
	res := toolGrep(ctx, map[string]any{"pattern": "x"})
	assert.Equal(t, "error", res.Status)
	...
	res = toolGrep(protocol.ToolContext{Ctx: cancelCtx, Cwd: tmpDir}, map[string]any{"pattern": "x"})
	assert.Equal(t, "error", res.Status)
...
func TestWebFetchEdgeBranches(t *testing.T) {
	...
	res := toolWebFetch(ctx, map[string]any{"url": "http://example.com"})
	assert.Equal(t, "error", res.Status)
