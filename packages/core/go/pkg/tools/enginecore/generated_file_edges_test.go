...
func TestGeneratedDocumentEdges(t *testing.T) {
	...
	res := toolCreateDocument(ctx, args)
...
func TestGeneratedPresentationEdges(t *testing.T) {
	...
	res := toolCreatePresentation(ctx, args)
	require.Equal(t, "completed", res.Status)
...
func TestGeneratedCSVEdges(t *testing.T) {
	...
	res = toolCreateCSV(ctx, map[string]any{
...
func TestGeneratedArchiveEdges(t *testing.T) {
	...
	res := toolCreateArchive(ctx, map[string]any{"filePath": "missing/out.zip", "files": []any{"f1.txt"}})
	assert.Equal(t, "error", res.Status)
	...
	res = toolCreateArchive(ctx, args)
	assert.Equal(t, "error", res.Status)
	...
	res = toolCreateArchive(ctx, map[string]any{"filePath": "../out.zip", "files": []any{"f1.txt"}})
	assert.Equal(t, "error", res.Status)
	assert.Contains(t, res.Error, "external directory")
...
func TestGeneratedChartEdges(t *testing.T) {
	...
	res := toolCreateChart(ctx, map[string]any{"filePath": "../chart.png", "data": args["data"]})
	assert.Equal(t, "error", res.Status)
	assert.Contains(t, res.Error, "external directory")
	...
	res = toolCreateChart(ctx, map[string]any{"filePath": "empty.png", "data": []any{"skip"}})
	assert.Equal(t, "error", res.Status)
...
func TestGeneratedPDFEdges(t *testing.T) {
	...
	res := toolCreatePDF(ctx, map[string]any{"filePath": "../report.pdf", "sections": args["sections"]})
	assert.Equal(t, "error", res.Status)
	assert.Contains(t, res.Error, "external directory")

	usePDFWriter(t, &fakePDFWriter{})

	res = toolCreatePDF(ctx, args)
	require.Equal(t, "completed", res.Status)
	assert.Equal(t, 2, res.Metadata["sections"])
