...
func TestConfigLoaderNilGlobalFallbacks(t *testing.T) {
	...
	promptOverrideProvider = nil
	promptOverrideProviderMu.Unlock()
	if currentPromptOverrideProvider().SystemPromptOverride() != "" {
		t.Fatal("nil stored prompt provider should resolve to empty provider")
	}
	...
	configLoaderSource = nil
	configLoaderSourceMu.Unlock()
	if _, err := currentConfigLoaderSource().ReadConfigDocument(ConfigDocument{}); !errors.Is(err, ErrConfigLoaderSourceUnavailable) {
		t.Fatalf("nil stored config source should resolve to empty source, got %v", err)
	}
}
