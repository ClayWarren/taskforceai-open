...
type ownerSafeMembershipStoreStub struct {
	*stubMembershipStore
	updatePreservingOwnersFunc func(ctx context.Context, input UpdateMembershipRoleInput) (bool, error)
	deletePreservingOwnersFunc func(ctx context.Context, input DeleteMembershipInput) (bool, error)
}

func (s ownerSafeMembershipStoreStub) UpdateMembershipRolePreservingOwners(ctx context.Context, input UpdateMembershipRoleInput) (bool, error) {
	return s.updatePreservingOwnersFunc(ctx, input)
}

func (s ownerSafeMembershipStoreStub) DeleteMembershipPreservingOwners(ctx context.Context, input DeleteMembershipInput) (bool, error) {
	return s.deletePreservingOwnersFunc(ctx, input)
}
...
func (s stubMembershipStore) UpdateMembershipRolePreservingOwners(ctx context.Context, input UpdateMembershipRoleInput) (bool, error) {
	if input.Role != "OWNER" {
		allowed, err := s.ownerMutationAllowed(ctx, input.OrganizationID, input.UserID)
		if err != nil || !allowed {
			return allowed, err
		}
	}
	return true, s.UpdateMembershipRole(ctx, input)
}

func (s stubMembershipStore) DeleteMembershipPreservingOwners(ctx context.Context, input DeleteMembershipInput) (bool, error) {
	allowed, err := s.ownerMutationAllowed(ctx, input.OrganizationID, input.UserID)
	if err != nil || !allowed {
		return allowed, err
	}
	return true, s.DeleteMembership(ctx, input)
}

func (s stubMembershipStore) ownerMutationAllowed(ctx context.Context, orgID, targetUserID int32) (bool, error) {
	members, err := s.GetOrganizationMembers(ctx, orgID)
	if err != nil {
		return false, err
	}
	ownerCount := 0
	...
	for _, member := range members {
		if OrganizationRole(normalizeRole(member.Role)) != RoleOwner {
			continue
		}
		ownerCount++
		targetIsOwner = targetIsOwner || member.UserID == targetUserID
	}
	return !targetIsOwner || ownerCount > 1, nil
}
...
func TestMembershipService_UsesAtomicOwnerSafeMutations(t *testing.T) {
	newService := func(updateResult bool, updateErr error, deleteResult bool, deleteErr error) *MembershipService {
		_, base := newMembershipServiceTest()
		base.getMembershipFunc = func(ctx context.Context, input GetMembershipInput) (MembershipRecord, error) {
			return MembershipRecord{OrganizationID: input.OrganizationID, UserID: input.UserID, Role: "OWNER"}, nil
		}
		store := ownerSafeMembershipStoreStub{
			stubMembershipStore: base,
			updatePreservingOwnersFunc: func(ctx context.Context, input UpdateMembershipRoleInput) (bool, error) {
				assert.Equal(t, UpdateMembershipRoleInput{OrganizationID: 42, UserID: 200, Role: "ADMIN"}, input)
				return updateResult, updateErr
			},
			deletePreservingOwnersFunc: func(ctx context.Context, input DeleteMembershipInput) (bool, error) {
				assert.Equal(t, DeleteMembershipInput{OrganizationID: 42, UserID: 200}, input)
	...
	t.Run("demotion protects last owner", func(t *testing.T) {
		err := newService(false, nil, true, nil).UpdateMemberRole(context.Background(), 42, 200, 100, RoleAdmin)
		require.ErrorIs(t, err, ErrCannotRemoveLastOwner)
	})

	t.Run("demotion propagates store error", func(t *testing.T) {
		expected := errors.New("update failed")
		err := newService(false, expected, true, nil).UpdateMemberRole(context.Background(), 42, 200, 100, RoleAdmin)
		require.ErrorIs(t, err, expected)
	})

	t.Run("demotion succeeds", func(t *testing.T) {
		err := newService(true, nil, true, nil).UpdateMemberRole(context.Background(), 42, 200, 100, RoleAdmin)
		require.NoError(t, err)
	})

	t.Run("removal protects last owner", func(t *testing.T) {
		err := newService(true, nil, false, nil).RemoveMember(context.Background(), 42, 200, 100)
		require.ErrorIs(t, err, ErrCannotRemoveLastOwner)
	})

	t.Run("removal propagates store error", func(t *testing.T) {
		expected := errors.New("delete failed")
		err := newService(true, nil, false, expected).RemoveMember(context.Background(), 42, 200, 100)
		require.ErrorIs(t, err, expected)
	})

	t.Run("removal succeeds", func(t *testing.T) {
		err := newService(true, nil, true, nil).RemoveMember(context.Background(), 42, 200, 100)
		require.NoError(t, err)
	})
}
