...
type MembershipStore interface {
	...
	UpdateMembershipRole(ctx context.Context, input UpdateMembershipRoleInput) error
...
func (s *MembershipService) UpdateMemberRole(ctx context.Context, orgID, targetUserID, actorUserID int32, role OrganizationRole) error {
	...
	if OrganizationRole(normalizeRole(targetMembership.Role)) == RoleOwner && normalizedRole != RoleOwner {
		if OrganizationRole(normalizeRole(actorMembership.Role)) != RoleOwner {
			return ErrOwnerRoleProtected
		}
		if lastOwner, err := s.isLastOwner(ctx, orgID, targetUserID); err != nil {
			return err
		} else if lastOwner {
	...
	err = s.store.UpdateMembershipRole(ctx, UpdateMembershipRoleInput{
		OrganizationID: orgID,
		UserID:         targetUserID,
...
func (s *MembershipService) RemoveMember(ctx context.Context, orgID, targetUserID, actorUserID int32) error {
	...
	if OrganizationRole(normalizeRole(targetMembership.Role)) == RoleOwner {
		if OrganizationRole(normalizeRole(actorMembership.Role)) != RoleOwner {
			return ErrOwnerRoleProtected
		}
		if lastOwner, err := s.isLastOwner(ctx, orgID, targetUserID); err != nil {
	...
	if err := s.store.DeleteMembership(ctx, DeleteMembershipInput{
		OrganizationID: orgID,
		UserID:         targetUserID,
	}); err != nil {
...
func (s *MembershipService) isLastOwner(ctx context.Context, orgID, targetUserID int32) (bool, error) {
	members, err := s.store.GetOrganizationMembers(ctx, orgID)
	if err != nil {
		return false, err
	}
	ownerCount := 0
