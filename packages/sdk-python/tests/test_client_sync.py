...
def test_wait_for_completion_does_not_retry_permanent_http_errors() -> None:
    attempts = 0
    ...
    with pytest.raises(TaskForceAIError) as exc:
        client.wait_for_completion("task_denied", poll_interval=0.0, max_attempts=5)
...
def test_wait_for_completion_canceled() -> None:
    transport = build_transport(
        [
            {
                "status": 200,
                ...
                "path": "/api/v1/developer/status/task_canceled",
            },
        ]
    )
    ...
    with pytest.raises(TaskForceAIError, match="Run canceled"):
        client.wait_for_completion("task_canceled", poll_interval=0.0, max_attempts=1)
...
def test_upload_attachment_mock_mode_returns_mock_id() -> None:
    client = TaskForceAIClient(mock_mode=True)

    attachment_id = client.upload_attachment("mock.txt", b"content", "text/plain")

    assert attachment_id == "mock-attachment-id"
    client.close()


def test_upload_attachment_sends_file_and_invokes_response_hook() -> None:
    captured_requests: list[httpx.Request] = []
    captured_statuses: list[int] = []

    def handler(request: httpx.Request) -> httpx.Response:
        captured_requests.append(request)
        return httpx.Response(200, json={"id": "attachment-sync"}, request=request)
    ...
    assert attachment_id == "attachment-sync"
...
def test_upload_attachment_error_paths() -> None:
    ...
    with pytest.raises(TaskForceAIError, match="Invalid attachment upload response from API"):
        client.upload_attachment("bad.txt", b"bad")
    ...
    client = TaskForceAIClient("key", transport=httpx.MockTransport(http_error_handler))

    with pytest.raises(TaskForceAIError) as exc:
        client.upload_attachment("large.bin", b"x")
    ...
    client = TaskForceAIClient("key", transport=httpx.MockTransport(network_error_handler))

    with pytest.raises(TaskForceAIError, match="Network error: upload down"):
        client.upload_attachment("offline.bin", b"x")
...
def test_submit_task_malformed_success_json_raises_sdk_error() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
