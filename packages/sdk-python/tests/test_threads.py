...
def thread_payload(thread_id: int = 1, user_input: str = "My Thread") -> Dict[str, Any]:
    return {
        "id": thread_id,
...
def test_create_thread() -> None:
    ...
    assert thread.id == 1
    assert thread.user_input == "My Thread"
    client.close()
        ...
        [
            {
                "status": 200,
                "json": {
                    "conversations": [thread_payload(user_input="t1")],
                    "total": 1,
                    "limit": 20,
                    "offset": 0,
                    "has_more": False,
...
def test_get_thread() -> None:
    transport = build_transport(
        [
            {
                "status": 200,
                "json": thread_payload(user_input="t1"),
...
def test_get_thread_messages_rejects_malformed_success_payload() -> None:
    transport = build_transport(
        [
            {
                "status": 200,
    ...
    with pytest.raises(TaskForceAIError, match="Invalid thread messages response from API"):
        client.get_thread_messages(1)
