...
def test_upload_file_malformed_success_json_raises_sdk_error() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
...
def test_list_files_rejects_malformed_success_payload() -> None:
    transport = build_transport(
        [
            {
                "status": 200,
    ...
    with pytest.raises(TaskForceAIError, match="Invalid file list response from API"):
        client.list_files()
