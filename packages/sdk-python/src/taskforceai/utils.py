...
@contextmanager
def translate_http_errors(timeout_message: Optional[str] = None) -> Iterator[None]:
    try:
        yield
    except httpx.TimeoutException as exc:
        raise TaskForceAIError(timeout_message or f"Network error: {exc}") from exc
...
def resolve_terminal_task_status(status: TaskStatusResponse) -> Optional[TaskStatusResponse]:
    if status.status == "completed":
        return status
    if status.status == "failed":
        raise TaskForceAIError(getattr(status, "error", "Task failed"))
    if status.status in {"canceled", "awaiting_approval"}:
        fallback = "Task canceled" if status.status == "canceled" else "Task is awaiting approval"
        raise TaskForceAIError(
            getattr(status, "error", None) or getattr(status, "message", None) or fallback
        )
    return None
