...
def _attachment_base_url(base_url: str) -> str:
    return base_url.removesuffix("/developer")


def _decode_image_attachment(image: ImageAttachment) -> bytes:
    encoded = image.data.split(",", 1)[1] if "," in image.data else image.data
...
def _task_path_segment(task_id: TaskId) -> str:
    if not str(task_id).strip():
        raise TaskForceAIError("Task ID must be a non-empty string")
    return _path_segment(task_id)
...
def _is_retryable_poll_error(error: TaskForceAIError) -> bool:
...
def _mock_thread_response(method: str, base_endpoint: str) -> Dict[str, Any]:
    ...
    if method == "POST" and base_endpoint.endswith("/runs"):
        return {"taskId": "mock-thread-task", "status": "processing"}
    return {
        "id": 1,
...
class TaskForceAIClient:
    ...
    def __init__(
        ...
        *,
        ...
        if self._client is not None:
            self._close_finalizer = weakref.finalize(self, self._client.close)
    ...
    def submit_task(
        ...
        *,
        ...
        if images:
            resolved_attachment_ids.extend(
    ...
    def get_task_status(self, task_id: TaskId) -> Any:
        data = self._request("GET", f"/status/{_task_path_segment(task_id)}")
        return validate_task_status(data)

    def get_task_result(self, task_id: TaskId) -> Any:
        data = self._request("GET", f"/results/{_task_path_segment(task_id)}")
        return validate_task_status(data)
