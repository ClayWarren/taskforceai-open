...
import { buildUserState, loadUserProfile } from '@taskforceai/contracts/auth/auth-service';
import { storeAuthToken, clearAuthToken } from '@taskforceai/contracts/auth/auth-storage';
import type { AuthStorage, ProfileStorage } from '@taskforceai/contracts/auth/storage';
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ config, children }) => {
  const { authStorage, profileStorage, onLogout, onAuthError, gracePeriodMs = 10000 } = config;
        ...
        .then((token) => {
          if (token) {
            ...
            if (config.onAuthError) {
              config.onAuthError(new Error('token_fetch_failed'));
            }
          }
        })
        .catch((error) => {
          ...
          if (config.onAuthError) {
            config.onAuthError(error instanceof Error ? error : new Error('token_fetch_failed'));
          }
        })
        .finally(() => {
          isFetchingTokenRef.current = false;
        });
    } else if (status === 'unauthenticated' && !isSigningOut) {
      if (bootstrapLoading) {
        return;
      }
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated' && hasValidStoredToken && session?.user && !isSigningOut) {
      ...
      const fullName = session.user?.name ?? localUser?.full_name ?? null;
  ...
  useEffect(() => {
    ...
    return undefined;
  }, [hasAuthOverride, session, setUserState, status, hasValidStoredToken, isSigningOut]);
  ...
  const isLoading = bootstrapLoading || status === 'loading';
  ...
  const contextValue = useMemo(() => {
    const baseValue = {
      ...
      isAuthenticated:
        ...
        (status === 'authenticated' || hasValidStoredToken),
      isLoading,
