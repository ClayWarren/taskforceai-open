...
describe('shared/auth/auth-client', () => {
  ...
  describe('signOut', () => {
    ...
    it('submits signout without a csrf token when csrf payload is malformed', async () => {
      setTestWindow('https://app.taskforceai.chat');
      const fetchMock = installFetchResponses(
      ...
      expect(body).toBeInstanceOf(URLSearchParams);
      const params = body as URLSearchParams;
      expect(params.get('csrfToken')).toBe('');

      const headers = new Headers((signOutInit as RequestInit).headers);
      expect(headers.get('X-CSRF-Token')).toBe('');
      expect(globalThis.window.location.href).toBe('https://app.taskforceai.chat/signed-out');
    ...
    it('still submits signout when the csrf response JSON is malformed', async () => {
      setTestWindow('https://app.taskforceai.chat');
      const fetchMock = installFetchResponses(
        {
          ok: true,
