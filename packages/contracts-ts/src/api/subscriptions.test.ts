...
describe('subscriptions api helpers', () => {
  ...
  it('fetches products successfully', async () => {
    ...
    browserClient.getProducts.mockResolvedValue(products);

    await expect(fetchProducts()).resolves.toEqual({ ok: true, value: products });
  });
  ...
  it('maps create subscription failures with price details', async () => {
    browserClient.createSubscription.mockRejectedValue(
      Object.assign(new Error('checkout failed'), { status: 402 })
    );
  ...
  it('unwraps cancel subscription success results', async () => {
  ...
  it('unwraps reactivate subscription result errors', async () => {
    browserClient.reactivateSubscription.mockResolvedValue({
      ok: false,
      error: Object.assign(new Error('reactivate failed'), { status: 409 }),
    });
