...
> = {
  endpoint: TEndpoint;
  server: McpServerConfig;
  promise: Promise<ManagedSession<TEndpoint, TClient, TTransport>>;
};
...
const managedSessionClosePromises = new WeakMap<object, Promise<void>>();
...
> {
  ...
  private readonly pendingSessions = new Map<
    string,
    PendingManagedSession<TEndpoint, TClient, TTransport>
  >();
  ...
  async close(name: string): Promise<void> {
    ...
    await Promise.all([closeManagedSession(managed), closePendingManagedSession(pending)]);
  }

  async closeAll(): Promise<void> {
    const sessions = [...this.sessions.values()];
    const pendingSessions = [...this.pendingSessions.values()];
    this.sessions.clear();
    this.pendingSessions.clear();
    await Promise.all([
      ...sessions.map((session) => closeManagedSession(session)),
      ...pendingSessions.map((session) => closePendingManagedSession(session)),
    ]);
  }
  ...
  private async ensureConnected(
    ...
    const pending = this.pendingSessions.get(key);
    if (pending && isSessionMatch(pending, server, endpoint, this.isEndpointMatch)) {
      return pending.promise;
    }

    this.sessions.delete(key);
    this.pendingSessions.delete(key);
    const closePreviousSession = Promise.all([
      closeManagedSession(existing),
      closePendingManagedSession(pending),
    ]);

    const pendingSession: PendingManagedSession<TEndpoint, TClient, TTransport> = {
      endpoint,
      server: normalizedServer,
      promise: closePreviousSession
        .then(() => this.connect(server.endpoint))
        .then((session) => ({
          endpoint,
          server: normalizedServer,
          session,
    ...
    this.pendingSessions.set(key, pendingSession);

    try {
      const managed = await pendingSession.promise;
      if (this.pendingSessions.get(key) === pendingSession) {
        this.pendingSessions.delete(key);
      ...
      if (current && isSessionMatch(current, server, endpoint, this.isEndpointMatch)) {
        return current;
      }
      throw new Error(`MCP server ${name} connection was superseded.`);
    } catch (error) {
      if (this.pendingSessions.get(key) === pendingSession) {
        this.pendingSessions.delete(key);
      }
...
>(
  session:
    | ManagedSession<TEndpoint, TClient, TTransport>
    | PendingManagedSession<TEndpoint, TClient, TTransport>,
  server: McpServerConfig,
  endpoint: TEndpoint,
  isEndpointMatch: (left: TEndpoint, right: TEndpoint) => boolean
...
>(
  ...
  const existingClose = managedSessionClosePromises.get(session);
  if (existingClose) {
    await existingClose;
    return;
  }
  ...
  managedSessionClosePromises.set(session, closePromise);
  await closePromise;
};

const closePendingManagedSession = async <
  TEndpoint extends { url: URL },
  TClient extends McpClientLike,
  TTransport extends { close: () => Promise<void> },
>(
  pending: PendingManagedSession<TEndpoint, TClient, TTransport> | undefined
): Promise<void> => {
  if (!pending) {
    return;
  }
  let managed: ManagedSession<TEndpoint, TClient, TTransport>;
  try {
    managed = await pending.promise;
  } catch {
    // The pending connection failed before it could leak.
    return;
  }
  await closeManagedSession(managed);
};
