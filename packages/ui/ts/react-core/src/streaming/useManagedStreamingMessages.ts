...
>): ManagedStreamingState {
  ...
  const livePersistenceQueueRef = useRef<Promise<void>>(Promise.resolve());
  const enqueueLivePersistence = useCallback(
    (operation: () => Promise<void> | void): Promise<void> => {
      const queuedOperation = livePersistenceQueueRef.current.then(() => operation());
      livePersistenceQueueRef.current = queuedOperation.catch(() => undefined);
  ...
  useEffect(() => {
    ...
    const updateContent = async () => {
      try {
        ...
        if (cancelled || !isMountedRef.current) return;
        await enqueueLivePersistence(async () => {
          if (cancelled || !isMountedRef.current) return;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void enqueueLivePersistence(() => {
      if (cancelled || !isMountedRef.current) return;
      return persistence.persistToolEvents?.({
        conversationId,
        ids,
        toolEvents,
      });
  ...
  useEffect(() => {
    ...
    const finalize = async () => {
      try {
        await livePersistenceQueueRef.current;
