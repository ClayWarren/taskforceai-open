...
import { configureLogger as configurePackageLogger, logger as packageLogger } from './index';
...
describe('logger', () => {
  ...
  it('exposes logger configuration through the package entrypoint', () => {
    const customLogger = {
      debug: vi.fn(),
      info: vi.fn(),
      warn: vi.fn(),
    ...
    configurePackageLogger(customLogger);
    packageLogger.error('entrypoint error', { detail: true });

    expect(customLogger.error).toHaveBeenCalledWith('entrypoint error', { detail: true });
  });
});
