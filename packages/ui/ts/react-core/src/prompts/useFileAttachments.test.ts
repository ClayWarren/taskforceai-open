...
describe('useFileAttachments', () => {
  ...
  it('rejects additions that exceed the aggregate attachment budget', () => {
    const { result } = renderHook(() => useFileAttachments<MockAttachment>());
    const largeVideo = (index: number): MockAttachment => ({
    ...
    act(() => {
      result.current.addFile(largeVideo(1));
      result.current.addFile(largeVideo(2));
    });

    expect(result.current.files).toEqual([largeVideo(1)]);
    expect(result.current.error).toBe(
      `Attachments exceed the maximum total size of ${MAX_TOTAL_ATTACHMENT_SIZE_BYTES / (1024 * 1024)}MB`
    );
  });

  it('ignores empty input changes without clearing the selected value', () => {
    const { result } = renderHook(() => useFileAttachments<MockAttachment>());
    const input = document.createElement('input');
    ...
    expect(result.current.files).toEqual([]);
    expect(input.value).toBe('selected-file');
  });
  ...
  it('ignores drag events that do not contain files', () => {
    ...
    act(() => {
      result.current.handleDrop({
        ...
        dataTransfer: {
          types: ['text/plain'],
          files: [createAttachment(1)],
        },
      } as unknown as React.DragEvent<HTMLElement>);
    });

    expect(preventDefault).not.toHaveBeenCalled();
    expect(result.current.files).toEqual([createAttachment(1)]);
  });
  ...
  it('reports duplicate-only additions without changing files', () => {
    const onDuplicate = vi.fn();
    const { result } = renderHook(() =>
      useFileAttachments<MockAttachment>({
        getKey: (file) => file.name,
  ...
  it('deduplicates repeated keys within the same incoming batch', () => {
    const onDuplicate = vi.fn();
    const { result } = renderHook(() =>
      useFileAttachments<MockAttachment>({
        getKey: (file) => file.name,
    ...
    act(() => {
      result.current.addFiles([createAttachment(1), createAttachment(1), createAttachment(2)]);
    });

    expect(result.current.files).toEqual([createAttachment(1), createAttachment(2)]);
    expect(onDuplicate).toHaveBeenCalledTimes(1);
  });

  it('reports the limit when truncate strategy has no available slots', () => {
    const onLimitExceeded = vi.fn();
    const { result } = renderHook(() =>
      useFileAttachments<MockAttachment>({
        limitStrategy: 'truncate',
    ...
    act(() => {
      result.current.addFiles(
        Array.from({ length: MAX_ATTACHMENTS }, (_, index) => createAttachment(index))
      );
      result.current.addFile(createAttachment(MAX_ATTACHMENTS + 1));
    });
    ...
    expect(result.current.files).toHaveLength(MAX_ATTACHMENTS);
    expect(result.current.error).toBe(`You can only attach up to ${MAX_ATTACHMENTS} files.`);
    expect(onLimitExceeded).toHaveBeenLastCalledWith({
      availableSlots: 0,
      totalLimit: MAX_ATTACHMENTS,
    });
  });

  it('tolerates clearing and opening without an attached input element', () => {
    const { result } = renderHook(() => useFileAttachments<MockAttachment>());
