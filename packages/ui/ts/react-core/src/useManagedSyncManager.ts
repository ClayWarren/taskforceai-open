...
export function useManagedSyncManager({
  ...
  const callbackSettlementRef = useRef<Promise<void>>(Promise.resolve());
  ...
  const loggerRef = useLatestRef(logger);

  const settleLifecycleCallback = useCallback(
    (phase: 'complete' | 'error', callback: () => void | Promise<void>): Promise<void> => {
      try {
        return Promise.resolve(callback()).catch((error) => {
          loggerRef.current?.error?.('Sync lifecycle callback failed', { phase, error });
        });
      } catch (error) {
        loggerRef.current?.error?.('Sync lifecycle callback failed', { phase, error });
  ...
  const executeManagerSync = useCallback(
    async (manager: SyncManager) => {
      callbackSettlementRef.current = Promise.resolve();
      try {
        const stats = await (runSyncRef.current ? runSyncRef.current(manager) : manager.sync());
        await callbackSettlementRef.current;
        return stats;
      } catch (error) {
        await callbackSettlementRef.current;
        throw error;
  ...
  useEffect(() => {
    ...
    const managerConfig: SyncManagerConfig = {
      ...
      onSyncComplete: (stats) => {
        clearRetryTimer();
        completeSync(stats);
        callbackSettlementRef.current = settleLifecycleCallback('complete', () =>
          onSyncCompleteRef.current?.(stats)
        );
      },
      onSyncError: (error) => {
        const normalizedError = normalizeErrorRef.current(error);
        failSync(normalizedError);
        callbackSettlementRef.current = settleLifecycleCallback('error', () =>
          onSyncErrorRef.current?.({ error, normalizedError, manager })
        );
