...
const clearStoredAuthState = async (
  ...
  const [sessionResult, profileResult] = await Promise.allSettled([
    Promise.resolve().then(() => authStorage.clearSession()),
    Promise.resolve().then(() => profileStorage.clearProfile()),
  ]);

  if (sessionResult.status === 'rejected') {
    logger.error('Failed to clear session storage', { error: sessionResult.reason });
  }
  if (profileResult.status === 'rejected') {
    logger.error('Failed to clear profile storage', { error: profileResult.reason });
  }
};
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ config, children }) => {
  ...
  const handleAuthFailure = useCallback(
    async (reason?: string): Promise<void> => {
      ...
      await clearStoredAuthState(authStorage, profileStorage);
  ...
  const contextValue = useMemo(() => {
    ...
    return {
      ...baseValue,
      user: authOverride.user === undefined ? baseValue.user : authOverride.user,
