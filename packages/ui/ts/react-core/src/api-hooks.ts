...
export function useApi<T>(
  ...
  const dependencyRef = useRef<readonly unknown[] | null>(null);
  ...
  useEffect(() => {
    if (areDependencyListsEqual(dependencyRef.current, d)) {
      return;
    }
    dependencyRef.current = [...d];
    void load();
  }); // Only re-run when explicit dependencies change
