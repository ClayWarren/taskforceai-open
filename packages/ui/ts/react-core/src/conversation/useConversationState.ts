...
import { reportLatencyMark } from '../shared/latency';
import type { ConversationStore, KeyValueStorage } from '../shared/types';
...
export function useConversationState({
  ...
  const ensureActiveConversation = useCallback(async (): Promise<string> => {
    ...
    restoreRequestVersionRef.current += 1;
    isLoadingMoreRef.current = false;
    const freshId = createId(isPrivateMode ? 'private' : conversationIdPrefix);
  ...
  const handleNewChat = useCallback(async () => {
    ...
    const newId = createId(conversationIdPrefix);
    isLoadingMoreRef.current = false;
  ...
  }, [conversationIdPrefix, conversationStore, isPrivateMode, storage, activeConversationKey]);
  ...
  const loadMoreMessages = useCallback(async () => {
    ...
    try {
      ...
      if (
        requestVersion !== loadMoreRequestVersionRef.current ||
      ...
      if (requestVersion === loadMoreRequestVersionRef.current) {
