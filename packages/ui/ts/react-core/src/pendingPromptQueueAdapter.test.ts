...
describe('pendingPromptQueueAdapter', () => {
  ...
  it('strips legacy queued MCP client tools before replay', async () => {
    const runTask = vi.fn(async () => ({ task_id: 'task-1' }));
  ...
  it('strips object-shaped legacy queued MCP client tools before replay', async () => {
    const runTask = vi.fn(async () => ({ task_id: 'task-1' }));
    ...
    await adapter.runTask('replay this', {
      ...
      runPayload: {
        ...
        options: {
          clientTools: {
            mcp: [{ serverName: 'linear', toolName: 'create_issue' }],
          },
        },
      },
    });
