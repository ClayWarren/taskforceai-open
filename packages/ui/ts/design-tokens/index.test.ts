 ...
 */`;
  ...
  it('preserves keyboard focus and reduced-motion accessibility styles', () => {
    const appGlobals = fs.readFileSync(
      path.join(packageRoot, 'css/app-globals-shared.css'),
      'utf8'
    );
    const animations = fs.readFileSync(path.join(packageRoot, 'css/animations.css'), 'utf8');

    expect(appGlobals).toContain('.quick-search-panel__header input:focus-visible');
    expect(appGlobals).toContain('@media (prefers-reduced-motion: reduce)');
    expect(animations).toContain('@media (prefers-reduced-motion: reduce)');
    expect(animations).toContain('animation: none !important;');
  });
});
