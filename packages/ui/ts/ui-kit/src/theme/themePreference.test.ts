...
describe('themePreference', () => {
  ...
  it('reports unavailable theme application without required browser globals', () => {
    ...
    const missingDocument = applyThemePreference('dark');

    expect(missingDocument.ok).toBe(false);
    if (!missingDocument.ok) expect(missingDocument.error.kind).toBe('unavailable');
  ...
  it('reports unavailable subscriptions when media query events are unsupported', () => {
    ...
    const missingMatchMedia = subscribeToSystemTheme(vi.fn());

    expect(missingMatchMedia.ok).toBe(false);
    if (!missingMatchMedia.ok) expect(missingMatchMedia.error.kind).toBe('unavailable');
