        ...
        P: Serialize,
    {
        ...
        match &mut self.transport {
            AppServerTransport::Stdio {
                ...
                timeout(request_timeout, receive_stdio_response(&mut responses, id))
                    .await
                    .map_err(|_| request_timeout_error(method, request_timeout))?
            }
            AppServerTransport::Http {
                base_url,
                session_token,
...
impl AppServerRequestHandle {
    ...
    ) -> Result<VoiceTranscribeResult, AppClientError> {
        self.request("voice.transcribe", params).await
    }
        ...
        P: Serialize,
    {
        ...
        match &self.transport {
            AppServerRequestTransport::Stdio { stdin, responses } => {
                let line = encode_stdio_request(id, method, &params)?;
                let mut responses = responses.lock().await;
...
pub(crate) async fn receive_stdio_response<T>(
...
async fn receive_http_event(
    ...
    events: &mut Option<mpsc::Receiver<AppServerEvent>>,
