...
describe('createStreamingStore', () => {
  ...
  it('aborts an existing stream before starting a new one', async () => {
    ...
    adapter.connect = mock(async () => {
      calls += 1;
      return calls === 1 ? firstDisconnect : secondDisconnect;
    });
    const store = createStreamingStore(adapter);
  ...
  it('handles progress, start, approval, budget, and completion payloads', async () => {
    ...
    setup.callbacks.onMessage?.(
      JSON.stringify({
        type: 'progress',
        pending_approval: { id: 'approval-1', metadata: {} },
      })
    );
    setup.callbacks.onMessage?.(
    ...
    expect(store.getState().reasoning).toBe('Step one.');
    expect(store.getState().currentSpend).toBe(2);
    expect(store.getState().isStreaming).toBe(false);
    expect(onApproval).toHaveBeenCalledWith({ id: 'approval-1', metadata: {} });
  ...
  it('handles connection failures and timeout messages', async () => {
    adapter.connect = mock(async () => {
      throw new Error('Streaming connection timed out');
    });
    const store = createStreamingStore(adapter);
    ...
    await expect(
      store.getState().startStreaming({ taskId: 'task-2', conversationId: 'c', prompt: 'p' })
