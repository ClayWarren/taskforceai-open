...
export const applyRealtimeVoiceTranscriptEvent = (
  ...
  switch (event.type) {
    case 'speech-started':
    case 'speech-stopped':
      return controller.setActiveUserTranscript({
    ...
    case 'input-transcription-completed':
      if (typeof event.itemId !== 'string' || typeof event.transcript !== 'string') {
        return null;
      }
    ...
    case 'text-delta':
      ...
      return controller.appendAssistantText(event.itemId, event.delta, true);
