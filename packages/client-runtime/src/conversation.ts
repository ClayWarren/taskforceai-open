...
import { normalizeToolUsageEvent } from '@taskforceai/shared/streaming/normalization';
...
export async function ingestRemoteConversationSummary(options: {
  ...
  const summaryToolEvents = normalizeConversationToolEvents(conversation.toolEvents);
  ...
  const sharedSources =
    ...
    [];
  const assistantToolEvents = summaryToolEvents ?? existingAssistantMessage?.toolEvents ?? [];
  ...
  await conversationStore.upsertMessage({
    ...
    agentStatuses: summaryAgentStatuses ?? existingAgentStatusMessage?.agentStatuses ?? [],
  ...
  await conversationStore.upsertMessage({
    ...
    agentStatuses: summaryAgentStatuses ?? existingAssistantMessage?.agentStatuses ?? [],
    createdAt: timestamp,
    updatedAt: timestamp,
  });
