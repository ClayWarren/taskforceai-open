...
describe('send-message runtime', () => {
  ...
  it('blocks attachment sends while offline before mutating conversation state', async () => {
    const options = createOptions({ isOnline: false, attachmentIds: ['file-1'] });
  ...
  it('queues offline text prompts and invalidates pending prompts', async () => {
    const options = createOptions({ isOnline: false, attachmentIds: undefined });
  ...
  it('preserves MCP client tools when queueing offline prompts', async () => {
    const mcpToolItems = [
      {
        source: 'mcp' as const,
        serverName: 'filesystem',
    ...
    expect(options.enqueuePrompt).toHaveBeenCalledWith('local-conversation', ' hello ', {
      ...expectedDefaultRunPayload,
      options: {
        ...expectedDefaultRunPayload.options,
        clientTools: {
          mcp: mcpToolItems,
        },
      },
