...
```rust
use bytes::Bytes;
use taskforceai_sdk::FileUploadOptions;
...
let files = client.list_files(20, 0).await?;
let metadata = client.get_file(&uploaded.id).await?;
let content = client.download_file(&uploaded.id).await?;
...
```rust
use bytes::Bytes;
use taskforceai_sdk::TaskSubmissionOptions;

let attachment_id = client
    .upload_attachment("diagram.png", Bytes::from(image_bytes), Some("image/png"))
    .await?;
...
let task_id = client
    .submit_task(
        ...
        Some(TaskSubmissionOptions {
            attachment_ids: Some(vec![attachment_id]),
            ..Default::default()
        }),
    )
    .await?;
...
```rust
use taskforceai_sdk::{CreateThreadOptions, ThreadRunOptions};
...
let run = client
    .run_in_thread(
        ...
        ThreadRunOptions {
            ...
            model_id: Some("zai/glm-5.2".to_string()),
            stream: Some(false),
            ..Default::default()
        },
    )
...
let threads = client.list_threads(20, 0).await?;
let conversations = threads.conversations;
let messages = client.get_thread_messages(thread.id, 50, 0).await?;
```

`delete_thread()` intentionally returns an SDK error because the current Developer API does not expose a thread delete endpoint.
