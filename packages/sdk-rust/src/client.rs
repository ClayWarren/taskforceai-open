...
impl TaskForceAI {
    ...
    pub(crate) async fn ensure_success(
        ...
        if response.status().is_success() {
            ...
            Err(TaskForceAIError::Api { status, message })
        }
    }
...
#[cfg(test)]
mod tests {
    use super::TaskForceAI;
    use crate::types::TaskForceAIOptions;

    #[test]
    fn api_root_url_strips_developer_route_for_cross_service_endpoints() {
        let client = TaskForceAI::new(TaskForceAIOptions {
            api_key: Some("key".to_string()),
            base_url: Some("https://taskforceai.chat/api/v1/developer".to_string()),
            ..TaskForceAIOptions::default()
        })
        .expect("client");

        assert_eq!(client.api_root_url(), "https://taskforceai.chat/api/v1");
    }
}
