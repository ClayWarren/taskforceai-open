...
fn spawn_truncated_error_body_server() -> (String, thread::JoinHandle<()>) {
    let listener = TcpListener::bind("127.0.0.1:0").expect("error body fixture should bind");
    let address = listener.local_addr().expect("error body fixture address");
    let server = thread::spawn(move || {
        let (mut stream, _) = listener.accept().expect("error body fixture should accept");
        let mut request = Vec::new();
        let mut buffer = [0_u8; 1024];
        loop {
            let read = stream
                .read(&mut buffer)
                .expect("error body fixture should read request");
            if read == 0 {
                break;
            }
            request.extend_from_slice(&buffer[..read]);
        ...
        let request = String::from_utf8_lossy(&request);
        assert!(request.starts_with("POST /attachments/upload "));

        stream
            .write_all(
                b"HTTP/1.1 500 Internal Server Error\r\nContent-Type: text/plain\r\nContent-Length: 100\r\nConnection: close\r\n\r\npartial",
            )
            .expect("error body fixture should write truncated response");
        stream
            .flush()
            .expect("error body fixture should flush response");
    });
    (format!("http://{address}"), server)
}
...
async fn upload_attachment_error_paths_are_reported() {
    ...
    let (body_error_url, body_error_server) = spawn_truncated_error_body_server();
    let body_error_client = client_with_key(body_error_url);
    let body_error = body_error_client
        .upload_attachment("broken.bin", Bytes::from_static(b"broken"), None)
        .await;
    body_error_server
        .join()
        .expect("error body fixture should finish");
    assert!(matches!(
        body_error,
        Err(TaskForceAIError::Api { status, ref message })
