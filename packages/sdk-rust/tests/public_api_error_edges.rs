...
async fn create_thread_with_none_options_sends_empty_json_body() {
    ...
    let _mock = server
        ...
        .with_status(200)
        .with_body(r#"{"id":1,"title":"Untitled","created_at":1672531200,"updated_at":1672531200}"#)
        .create_async()
        .await;
...
async fn thread_responses_validate_required_fields() {
    ...
    let _list = server
        ...
        .with_status(200)
        .with_body(r#"{"threads":[{"id":1,"title":"","created_at":1672531200,"updated_at":1672531200}],"total":1}"#)
        .create_async()
        .await;
    let _messages = server
        .mock("GET", "/threads/1/messages?limit=1&offset=0")
        .with_status(200)
        .with_body(r#"{"messages":[{"id":1,"thread_id":1,"role":"system","content":"bad","created_at":1672531200}],"total":1}"#)
        .create_async()
        .await;
    let _run = server
    ...
    assert!(matches!(
        list_res,
        Err(TaskForceAIError::Validation(ref message)) if message.contains("title is required")
    ));
