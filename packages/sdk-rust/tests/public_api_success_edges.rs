...
async fn submit_task_sends_options_and_image_attachments() {
    ...
    let _mock = server
        ...
        .match_body(Matcher::AllOf(vec![
            ...
            Matcher::Regex(r#""attachment_ids":\["attachment-image-1"\]"#.to_string()),
        ]))
        .with_status(200)
...
async fn thread_success_paths_use_public_api_json_contract() {
    let mut server = Server::new_async().await;
    let thread_json = r#"{"id":7,"timestamp":"2023-01-01T00:00:00Z","user_input":"Research","result":"","execution_time":0,"model":"","agent_count":0,"sources":[],"agentStatuses":[],"toolEvents":[]}"#;
    let message_json = r#"{"id":11,"thread_id":7,"role":"user","content":"hello","created_at":"2023-01-01T00:00:05Z"}"#;
    ...
    let _list = server
        ...
        .with_status(200)
        .with_body(format!(
            r#"{{"conversations":[{}],"total":1,"limit":5,"offset":10,"has_more":false}}"#,
            thread_json
        ))
        .create_async()
        .await;
    ...
    assert_eq!(created.user_input, "Research");

    let listed = client.list_threads(5, 10).await.unwrap();
    assert_eq!(listed.total, 1);
    assert_eq!(listed.conversations[0].id, 7);

    let fetched = client.get_thread(7).await.unwrap();
    assert_eq!(fetched.timestamp, "2023-01-01T00:00:00Z");
