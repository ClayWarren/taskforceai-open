...
const NATIVE_COVERAGE_EXCLUSIONS = [
  ...
  'src/features/desktop-work/components/', // subviews of the native Remote modal; covered by screen/device tests
  'src/features/desktop-work/desktop-work-sections.ts', // native Remote screen section composition
  'src/features/desktop-work/live-activity.ts', // ActivityKit boundary
  'src/features/desktop-work/quick-actions.ts', // expo-quick-actions native boundary
  'src/widgets/AgentActivity.tsx', // expo-widgets native rendering boundary
...
function isExcluded(filePath: string): boolean {
  ...
  return NATIVE_COVERAGE_EXCLUSIONS.some((pattern) =>
    pattern.endsWith('/') ? normalized.includes(pattern) : normalized.endsWith(pattern)
  );
}
...
function main() {
  ...
  for (const record of merged.values()) {
    if (
      ...
      record.sourceFile.includes('.types.') ||
