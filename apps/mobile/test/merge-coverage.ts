...
const parsedThreshold = Number(
  process.env['COVERAGE_LINE_THRESHOLD'] ?? process.env['COVERAGE_THRESHOLD'] ?? '95'
);
const COVERAGE_THRESHOLD = Number.isFinite(parsedThreshold) ? parsedThreshold : 95;
...
function main() {
  ...
  for (const record of merged.values()) {
    ...
    const found = record.lineData.size;
    const hit = Array.from(record.lineData.values()).filter((h) => h > 0).length;
