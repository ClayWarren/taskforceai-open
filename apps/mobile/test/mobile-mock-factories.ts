...
export const createReactNativeMock = (
  ...
  const Animated = {
    ...
    timing: createSpy(spy, () => ({ start: spy(noop), stop: spy(noop) })),
    sequence: createSpy(spy, () => ({ start: spy(noop), stop: spy(noop) })),
    loop: createSpy(spy, () => ({ start: spy(noop), stop: spy(noop) })),
  };
  const Easing = {
    ease: 'ease',
...
export const createExpoModulesCoreMock = (spy: SpyFactory) => ({
  __esModule: true,
  uuid: {
    v4: createSpy(spy, () => 'mobile-mock-uuid-0000-0000-0000-000000000000'),
  },
  requireNativeModule: () => ({}),
...
export const createExpoFileSystemMock = () => {
  class MockExpoFile {
    uri: string;
  ...
  return {
    ...
    Paths: {
      document: { uri: 'file:///mock-dir/' },
      cache: { uri: 'file:///mock-cache/' },
    },
    File: MockExpoFile,
    Directory: MockExpoDirectory,
  };
};
...
export const createExpoNotificationsMock = (spy: SpyFactory) => ({
  ...
  getLastNotificationResponseAsync: createSpy(spy, async () => null),
