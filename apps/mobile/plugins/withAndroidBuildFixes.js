...
const ANDROIDX_RESOLUTION_MARKER = '// TaskForceAI AndroidX resolution pins';
const ANDROIDX_RESOLUTION_STRATEGY = `${ANDROIDX_RESOLUTION_MARKER}
configurations.all {
    resolutionStrategy {
...
function appendAndroidxResolutionStrategy(contents) {
  if (
    contents.includes(ANDROIDX_RESOLUTION_MARKER) ||
    contents.includes("force 'androidx.core:core:1.13.1'")
  ) {
    return contents;
  }

  return `${contents.trimEnd()}\n\n${ANDROIDX_RESOLUTION_STRATEGY}\n`;
}
...
module.exports = function withAndroidBuildFixes(config) {
  ...
  config = withAppBuildGradle(config, async (modConfig) => {
    modConfig.modResults.contents = appendAndroidxResolutionStrategy(
...
module.exports.appendAndroidxResolutionStrategy = appendAndroidxResolutionStrategy;
module.exports.patchReactNativeVoiceBuildGradle = patchReactNativeVoiceBuildGradle;
