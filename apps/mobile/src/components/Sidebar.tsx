...
import { isGuestConversationId } from '../storage/conversations/ownership';
...
export function Sidebar({
  ...
  useEffect(() => {
    ...
    void loadLocalConversations(isAuthenticated);
  }, [isAuthenticated, visible]);

  const loadLocalConversations = async (authenticated = isAuthenticated) => {
    try {
      ...
      const belongsToCurrentOwner = (conversation: LocalConversation): boolean =>
        isGuestConversationId(conversation.conversationId) !== authenticated;
      if (archivedResult.ok) {
        setArchivedConversationIds(
          new Set(
            archivedResult.value
              .filter(belongsToCurrentOwner)
      ...
      const locals = result.value.filter(belongsToCurrentOwner);
  ...
  const projectScopedConversations = useMemo(() => {
    if (!isAuthenticated) return localConversations;
            ...
            void (async () => {
              try {
                if (conversationId < 0) {
                  ...
                  await loadLocalConversations(isAuthenticated);
                } else {
  ...
  const { user } = useAuth();
  const { displayName: userName, initials: userInitials } = createSidebarIdentity({
    email: user?.email,
