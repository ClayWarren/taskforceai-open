      ...
      <StyledView className="flex-1 flex-row" style={{ backgroundColor: 'rgba(0, 0, 0, 0.6)' }}>
        ...
        <View style={{ height: '100%', width: '80%', maxWidth: 320 }}>
          ...
          >
            ...
            {isAuthenticated ? (
              <View style={styles.productNavGroup}>
                <TouchableOpacity
                  onPress={onManageProjects}
                  activeOpacity={0.72}
                  style={styles.productNavRow}
                  accessibilityRole="button"
                  accessibilityLabel={t('sidebar.openProjects', 'Open Projects')}
                >
                  <Icon name="Folder" size={18} color="#f8fafc" />
                  <Text style={styles.productNavText}>{t('sidebar.projects', 'Projects')}</Text>
                </TouchableOpacity>
                {desktopSessionsSlot}
              </View>
            ) : null}
