import { extractGeneratedMediaResult, stripGeneratedMediaMarkup } from '@taskforceai/presenters';
...
const canOpenGeneratedMediaUri = (uri: string): boolean => {
  return URL.canParse(uri) && ['https:', 'http:'].includes(new URL(uri).protocol);
};
...
export function MarkdownView({ content, isUser }: MarkdownViewProps) {
    ...
    return (
      <View style={styles.generatedMediaContainer}>
        {generatedMedia.kind === 'image' && canLoadMedia ? (
          ...
          <GeneratedVideoPreview uri={generatedMedia.uri} styles={styles} textColor={textColor} />
        ) : null}
  ...
  return <Markdown style={styles}>{safeContent}</Markdown>;
}
