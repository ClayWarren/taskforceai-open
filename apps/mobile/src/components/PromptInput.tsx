...
import { PROMPT_BUBBLE_MAX_WIDTH } from './PromptInput.internal';
import { usePromptInputState, type PromptInputProps } from './PromptInput.state';
import { PromptInputView } from './PromptInput.view';

export type { PromptInputProps } from './PromptInput.state';

export function PromptInput(props: PromptInputProps) {
  const insets = useSafeAreaInsets();
  const { width: windowWidth } = useWindowDimensions();
  const availableWidth = Math.max(windowWidth - spacingTokens.sm * 2, 320);
  const promptMaxWidth = Math.min(availableWidth, PROMPT_BUBBLE_MAX_WIDTH);
