...
import { safeArgsForDisplay } from '@taskforceai/shared/tool-usage/parsers';
import {
  formatCodeLanguageLabel,
  type ToolUsageViewItem,
} from '@taskforceai/shared/tool-usage/view-model';
import { extractDomain } from '@taskforceai/shared/utils';
...
export const SearchContent = ({
  ...
  const chips = preview.links.slice(0, MAX_SEARCH_LINKS);
      ...
      {chips.length > 0 && (
        <View style={styles.chipRow}>
          {chips.map((link, index: number) => {
            const domain =
              preview.domains?.[index] ?? extractDomain(link.url) ?? link.title ?? link.url;
            if (!domain) {
              return null;
            }
            return (
