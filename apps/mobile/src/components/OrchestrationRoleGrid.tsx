...
import { canUseModelForPlan, getAgentRoleSlots, getModelCostTier } from '@taskforceai/client-core';
          ...
          <View key={role.id} style={styles.roleCard}>
            ...
            {isExpanded && (
              <View style={styles.roleOptions}>
                {models.map((model) => {
                  const isLocked = !canUseModelForPlan(userPlan, model.usageMultiple);
                  const costTier = getModelCostTier(model.usageMultiple);
                  return (
                    <TouchableOpacity
                      key={model.id}
                      style={[
                        ...
                        isLocked && styles.modelOptionLocked,
                      ]}
                      disabled={isLocked}
                    ...
                    >
                      ...
                      {costTier ? <Text style={styles.modelCostTier}>{costTier.symbol}</Text> : null}
                      {isLocked ? (
                        <Icon name="Lock" size={12} color="#fbbf24" />
                      ) : model.badge ? (
