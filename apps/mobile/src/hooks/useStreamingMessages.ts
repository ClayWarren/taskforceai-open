  ...
  const managedPersistence = useMemo(
    () =>
      createStreamingPersistence<SourceReference, ToolUsageEvent, AgentStatus, unknown, Message>({
        upsertMessage: persistenceEnabled ? persistence.upsertMessage : async () => undefined,
        deleteMessage: persistenceEnabled ? persistence.deleteMessage : async () => undefined,
        setMessages,
    ...
    [
      getPlaceholderTimes,
      persistence.deleteMessage,
      persistence.upsertMessage,
      persistenceEnabled,
      setMessages,
    ]
  );
