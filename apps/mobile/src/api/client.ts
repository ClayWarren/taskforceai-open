...
const getGlobalClients = () => {
  if (typeof globalThis === 'undefined') {
    return { api: null, remote: null, auth: null, pinnedFetch: null };
  }
  if (!globalThis.__MOBILE_CLIENTS__) {
    globalThis.__MOBILE_CLIENTS__ = { api: null, remote: null, auth: null, pinnedFetch: null };
  }
...
const getCachedRemoteClient = () => getGlobalClients().remote;
const setCachedRemoteClient = (c: ApiClient | null) => { getGlobalClients().remote = c; };
...
const buildRemoteClient = (): ApiClient => {
  const url = getMobileRemoteBaseUrl();
  logger.info('Creating Remote API client', { url });
  return createApiClient({
    baseUrl: url,
    defaultHeaders: {
      'User-Agent': 'TaskForceAI-Mobile',
...
/** Get the API client pinned to the Sync service for Remote operations. */
export const getMobileRemoteClient = (): ApiClient => {
  const cached = getCachedRemoteClient();
  if (cached) return cached;

  const api = buildRemoteClient();
  setCachedRemoteClient(api);
  return api;
};
