...
import { isDesktopPairingDeepLink } from '../features/desktop-work/pairing/deep-link';
import { getInitialUrl, subscribeUrlEvents } from '../features/desktop-work/pairing/linking';
import {
  remoteThreadIdFromUrl,
...
export function AppRoot() {
  ...
  const handleOpenDesktopSessions = () => {
    handleCloseSidebar();
    router.push('/remote' as Href);
  };

  const handleOpenRemoteThread = useCallback(
    (threadId: string) => {
      handleCloseSidebar();
      router.push({ pathname: '/remote/thread/[threadId]', params: { threadId } } as unknown as Href);
    },
  ...
  const handleDesktopPairingLink = useCallback(
    (url: string) => {
      const linkedRemoteThreadId = remoteThreadIdFromUrl(url);
      if (linkedRemoteThreadId) {
        handleOpenRemoteThread(linkedRemoteThreadId);
        return;
      }
    ...
    [handleCloseSidebar, handleOpenRemoteThread]
  );
