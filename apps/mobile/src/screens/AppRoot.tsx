...
export function AppRoot() {
  ...
  const pendingDesktopSettingsSection = useRef<SettingsSectionId | 'home' | null>(null);
  const desktopDismissFallback = useRef<ReturnType<typeof globalThis.setTimeout> | null>(null);
  ...
  const requestSettingsAfterDesktopDismiss = (section: SettingsSectionId | 'home') => {
    pendingDesktopSettingsSection.current = section;
    setIsDesktopWorkVisible(false);
    if (desktopDismissFallback.current !== null) {
      globalThis.clearTimeout(desktopDismissFallback.current);
    }
    if (process.env.EXPO_OS === 'android') {
      desktopDismissFallback.current = globalThis.setTimeout(handleDesktopWorkDismissed, 700);
    }
  };

  const handleDesktopWorkDismissed = () => {
    const section = pendingDesktopSettingsSection.current;
    if (section === null) return;
    pendingDesktopSettingsSection.current = null;
    if (desktopDismissFallback.current !== null) {
      globalThis.clearTimeout(desktopDismissFallback.current);
      desktopDismissFallback.current = null;
    }
    setDesktopPairingPayload(null);
    setSettingsInitialSection(section === 'home' ? null : section);
    setIsSettingsVisible(true);
  };
  ...
  const handleOpenPlugins = () => {
    handleCloseSidebar();
    globalThis.setTimeout(() => setIsPluginsVisible(true), 0);
  };
  ...
  return (
    <Fragment>
      ...
      <Sidebar
        ...
        onPluginsPress={handleOpenPlugins}
      />
      <ChatScreen
      ...
      />
        ...
        <DesktopWorkScreen
          ...
          onDismiss={handleDesktopWorkDismissed}
          onOpenSettings={() => requestSettingsAfterDesktopDismiss('home')}
        />
      ...
      <PluginsScreen visible={isPluginsVisible} onClose={() => setIsPluginsVisible(false)} />
    </Fragment>
  );
}
