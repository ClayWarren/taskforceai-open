...
type PendingMFAChallenge = {
  mfaToken: string;
  userProfile: ReturnType<typeof buildUserState>;
};
...
export function LoginScreen({
  ...
  const completeAuthExchange = async (
    response: AuthExchangeResponse,
    fallbackProfile: ReturnType<typeof buildUserState>
  ) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const fallbackProfile = buildUserState({
  ...
  const handleMFAVerify = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await getMobileClient().verifyAuthenticatorMFALogin(code, pendingMFA.mfaToken);
