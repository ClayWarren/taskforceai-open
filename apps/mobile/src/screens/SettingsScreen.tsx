...
export function SettingsScreen({
  ...
  const subscriptionQuery = useSubscriptionQuery({
    enabled: visible && (activeSection === 'subscription' || activeSection === 'usage') && !!user,
  });
  ...
  const productsQuery = useProductsQuery({
    enabled:
      visible &&
      (activeSection === 'subscription' || activeSection === 'usage') &&
      !!user &&
      user.plan !== 'super',
  });
  ...
  React.useEffect(() => {
    if (!visible) return;
    Keyboard.dismiss();
    setActiveSection(desktopPairingPayload ? 'apps' : initialSection);
  }, [desktopPairingPayload, initialSection, visible]);
    ...
    >
      {activeSection === null ? (
          ...
          <SettingsSectionContent
            ...
            onPrivacyPolicy={() => void openPrivacyPolicy()}
            onTermsOfService={() => void openTermsOfService()}
            onContactSupport={() => void openSupportContact()}
