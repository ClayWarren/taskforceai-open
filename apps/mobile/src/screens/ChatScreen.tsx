...
const resolveRealtimeVoiceHandler = (
  enabled: boolean,
  handler: () => void,
): (() => void) | undefined => (enabled ? handler : undefined);

const resolveRealtimeVoiceActive = (enabled: boolean, active: boolean): boolean =>
  enabled && active;

const resolveRealtimeVoiceDisabled = (
  enabled: boolean,
  isStreaming: boolean,
  isSidebarVisible: boolean,
  isAuthenticated: boolean,
): boolean => !enabled || isStreaming || isSidebarVisible || !isAuthenticated;
...
export function ChatScreen({
  ...
  const realtimeVoiceEnabled = taskMode === "chat" && !privateChat;
  ...
  }, [connectRealtimeVoice, onRealtimeVoiceStart, realtimeVoiceIsActive]);

  useEffect(() => {
    if (!isAuthenticated || !realtimeVoiceEnabled) {
      return;
    }
  ...
  }, [isAuthenticated, prewarmRealtimeVoice, realtimeVoiceEnabled]);

  useEffect(() => {
    if (realtimeVoiceEnabled || !realtimeVoiceIsActive) {
      return;
    }
    disconnectRealtimeVoice();
  }, [disconnectRealtimeVoice, realtimeVoiceEnabled, realtimeVoiceIsActive]);
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      <PromptInput
        ...
        onRealtimeVoice={resolveRealtimeVoiceHandler(realtimeVoiceEnabled, handleRealtimeVoice)}
        realtimeVoiceActive={resolveRealtimeVoiceActive(
          realtimeVoiceEnabled,
          realtimeVoiceIsActive,
        )}
        realtimeVoiceDisabled={resolveRealtimeVoiceDisabled(
          realtimeVoiceEnabled,
          isStreaming,
          isSidebarVisible,
          isAuthenticated,
