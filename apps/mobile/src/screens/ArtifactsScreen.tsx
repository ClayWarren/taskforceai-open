...
const artifactActionDescriptors = [
  ['Download', 'Download', 'Download artifact'],
  ['Share', 'Copy link', 'Copy public link for'],
  ['Trash2', 'Delete', 'Delete artifact'],
] as const satisfies readonly [IconName, string, string][];

const MAX_IMAGE_ARTIFACT_PREVIEW_BYTES = 20 * 1024 * 1024;
const MAX_TEXT_ARTIFACT_PREVIEW_BYTES = 2 * 1024 * 1024;

const buildArtifactFileUri = (
  directory: string,
  prefix: 'artifact-download' | 'artifact-preview',
  artifact: MobileArtifact,
  version: MobileArtifactVersion,
  filename: string
): string =>
  `${directory}${prefix}-${sanitizeFilename(artifact.id)}-${sanitizeFilename(version.id)}-${filename}`;
...
export function ArtifactsScreen({ visible, onClose }: ArtifactsScreenProps) {
  ...
  const openArtifact = async (artifact: MobileArtifact) => {
    ...
    try {
      ...
      if (contentUrl && currentVersion) {
        const canShare = await Sharing.isAvailableAsync();
        const directory = canShare ? FileSystem.cacheDirectory : FileSystem.documentDirectory;
        const filename = sanitizeFilename(currentVersion.filename ?? `${artifact.title}.txt`);
        const fileUri = buildArtifactFileUri(
          directory,
          'artifact-download',
          artifact,
          currentVersion,
        ...
        const result = await downloadMobileArtifactContent(contentUrl, fileUri, {
          expectedBytes: currentVersion.bytes,
        });
        if (!result.ok) {
          throw result.error;
        }
        if (canShare) {
          try {
            await Sharing.shareAsync(result.value);
          } finally {
            await FileSystem.deleteAsync(result.value, { idempotent: true });
          }
          return;
        }
        Alert.alert('Artifact saved', result.value);
        return;
      }
  ...
  return (
    <View style={[styles.artifactRow, { backgroundColor: theme.colors.cardBackground }]}>
      ...
      <View style={styles.artifactActions}>
        {artifactActionDescriptors.map((descriptor, index) => (
          <ArtifactActionButton
            key={descriptor[0]}
            descriptor={descriptor}
            title={artifact.title}
            onPress={[onOpen, onCopyPublicLink, onDelete][index]!}
            busy={busy}
          />
        ))}
      </View>
...
function ArtifactActionButton({
  backgroundColor,
  busy,
  descriptor: [icon, label, accessibilityPrefix],
  detail = false,
  onPress,
  title,
}: {
  ...
  busy: boolean;
  descriptor: (typeof artifactActionDescriptors)[number];
  detail?: boolean;
  onPress: () => void;
  title: string;
}) {
  return (
    <TouchableOpacity
      onPress={onPress}
      disabled={busy}
      style={detail ? [styles.detailActionButton, { backgroundColor }] : styles.iconButton}
      accessibilityRole="button"
      accessibilityLabel={`${accessibilityPrefix} ${title}`}
    >
      <Icon name={icon} size={detail ? 18 : 17} color="#f8fafc" />
      {detail ? <Text style={styles.detailActionText}>{label}</Text> : null}
    </TouchableOpacity>
  );
}
...
function ArtifactDetailModal({
  ...
  React.useEffect(() => {
    let active = true;
    let previewFileUri: string | null = null;
    if (!artifact) {
    ...
    void (async () => {
      try {
        if (mimeType.startsWith('image/')) {
          ...
          const uri = buildArtifactFileUri(
          ...
          const result = await downloadMobileArtifactContent(contentUrl, uri, {
            expectedBytes: currentVersion.bytes,
            maxBytes: MAX_IMAGE_ARTIFACT_PREVIEW_BYTES,
          });
          if (!result.ok) {
            throw result.error;
          }
          previewFileUri = result.value;
          if (active) {
            setPreview({ status: 'image', uri: result.value });
          } else {
            await FileSystem.deleteAsync(result.value, { idempotent: true });
            previewFileUri = null;
          }
        ...
        if (mimeType.startsWith('text/') || mimeType === 'application/json') {
          const filename = sanitizeFilename(currentVersion.filename ?? `${artifact.id}.txt`);
          const uri = buildArtifactFileUri(
            FileSystem.cacheDirectory,
            'artifact-preview',
            artifact,
          ...
          const result = await downloadMobileArtifactContent(contentUrl, uri, {
            expectedBytes: currentVersion.bytes,
            maxBytes: MAX_TEXT_ARTIFACT_PREVIEW_BYTES,
          });
          if (!result.ok) {
            throw result.error;
          }
          const content = await FileSystem.readAsStringAsync(result.value);
          await FileSystem.deleteAsync(result.value, { idempotent: true });
          if (active) {
            setPreview({ status: 'text', content: truncatePreview(content) });
          }
          return;
        }
    ...
    return () => {
      active = false;
      if (previewFileUri) {
        void FileSystem.deleteAsync(previewFileUri, { idempotent: true });
      }
    };
  }, [artifact]);
          ...
          <View style={styles.detailActions}>
            {artifactActionDescriptors.map((descriptor, index) => (
              <ArtifactActionButton
                ...
                descriptor={descriptor}
                title={artifact.title}
                onPress={() => [onOpen, onCopyPublicLink, onDelete][index]!(artifact)}
                busy={busy}
                detail
