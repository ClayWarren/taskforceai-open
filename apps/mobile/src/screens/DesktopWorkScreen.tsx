...
export function DesktopWorkScreen({ visible, initialThreadId = null, onClose }: DesktopWorkScreenProps) {
...
function WorkspaceListView({
  ...
  const workspaceGroups = makeWorkspaceGroups(projects, filteredThreads, activeProjectId);
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.remoteFilters}>
        {remoteThreadFilters.map((candidate) => (
          <TouchableOpacity
            ...
            onPress={() => setFilter(candidate.value)}
            style={[
              styles.remoteFilter,
