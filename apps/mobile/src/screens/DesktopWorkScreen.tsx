...
export function DesktopWorkScreen({ visible, onClose }: DesktopWorkScreenProps) {
  ...
  const threads = workState?.status === 'connected' ? workState.threads : [];
  const projects = workState?.status === 'connected' ? workState.projects : [];
  const pendingChanges = workState?.status === 'connected' ? workState.pendingChanges : [];
  const selectedThread = selectedThreadId
    ? threads.find((thread) => thread.sessionId === selectedThreadId) ??
      (createdThread?.sessionId === selectedThreadId ? createdThread : null)
    : threads[0] ?? null;
  const machineName = workState?.status === 'connected' ? workState.machineName : 'Desktop';
          ...
          <View style={styles.headerCenter}>
            ...
            {activeView === 'session' && selectedThread ? (
              <Text style={[styles.headerSubtitle, { color: theme.colors.textMuted }]} numberOfLines={1}>
                {activeProjectName(projects, workState?.activeProjectId ?? null)} · {machineName}
              </Text>
            ) : null}
          </View>
        ...
        >
          {activeView === 'newThread' ? (
            ...
            <WorkspaceListView
              ...
              projects={projects}
              threads={threads}
              activeProjectId={workState?.activeProjectId ?? null}
