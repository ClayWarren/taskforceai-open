...
const memoryUpdatedAt = (memory: Memory): number =>
  new Date(memory.updated_at ?? memory.created_at).getTime();
...
export function useSettingsMemories({ t }: UseSettingsMemoriesOptions): SettingsMemoriesState {
  ...
  const loadMemories = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const nextMemories = await listMobileMemories();
      setMemories(
        sortedCopy(
          nextMemories,
          (left, right) => memoryUpdatedAt(right) - memoryUpdatedAt(left)
        )
      );
    } catch (loadError) {
  ...
  const submit = React.useCallback(async () => {
    ...
    try {
      if (editingMemory) {
        const updatedMemory = await updateMobileMemory(editingMemory.id, {
          content,
  ...
  const deleteMemory = React.useCallback(
    async (memory: Memory) => {
      ...
      try {
        await deleteMobileMemory(memory.id);
