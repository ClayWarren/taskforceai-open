...
function formatStorageCount(categoryId: string, count: number): string {
  if (count === 1) {
...
function getVisibleCategories(summary: StorageSummary) {
  const fileCategory = summary.categories.find((category) => category.id === 'files') ?? {
    id: 'files',
    label: 'Files',
    bytes: 0,
    count: 0,
  };
  const imageCategory = summary.categories.find((category) => category.id === 'images') ?? {
    id: 'images',
    label: 'Images',
    bytes: 0,
    count: 0,
...
export function StorageSection({ summary, loading, error, onRetry }: StorageSectionProps) {
  const { theme } = useTheme();
  const { t } = useTranslation();
  const effectiveSummary = summary ?? { usedBytes: 0, quotaBytes: 0, categories: [] };
  ...
  const progressWidth = `${Math.max(usagePercent, usedBytes > 0 ? 2 : 0)}%` as DimensionValue;
  const visibleCategories = getVisibleCategories(effectiveSummary);
  const usageLabel = `${formatStorageBytes(usedBytes)} ${t('mobile.settings.storage.of', {
    defaultValue: 'of',
    ...
    return (
      <View style={styles.centerState}>
        ...
        <Text style={[styles.stateText, { color: theme.colors.textMuted }]}>
          {t('mobile.settings.storage.loading', { defaultValue: 'Loading storage...' })}
        </Text>
      </View>
    );
  }
  ...
  if (error && !summary) {
    return (
      <View style={[styles.errorCard, { borderColor: theme.colors.error }]}>
        <Text style={[styles.stateText, { color: theme.colors.text }]}>{error}</Text>
        <TouchableOpacity
          onPress={onRetry}
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.usageBlock, { borderBottomColor: theme.colors.border }]}>
        <Text selectable style={[styles.usageText, { color: theme.colors.text }]}>
          {usageLabel}
        </Text>
        <View
          style={[styles.progressTrack, { backgroundColor: theme.colors.cardBackground }]}
      ...
      <View style={[styles.categoryCard, { backgroundColor: theme.colors.cardBackground }]}>
        {visibleCategories.map((category, index) => {
          const isLast = index === visibleCategories.length - 1;
          const categorySummary = `${formatStorageBytes(category.bytes)} - ${formatStorageCount(
            category.id,
            category.count
          )}`;
          return (
            <View key={category.id}>
              <View style={styles.categoryRow}>
                <View style={styles.categoryText}>
                  ...
                  <Text selectable style={[styles.categoryValue, { color: theme.colors.textMuted }]}>
                    {categorySummary}
                  </Text>
                </View>
