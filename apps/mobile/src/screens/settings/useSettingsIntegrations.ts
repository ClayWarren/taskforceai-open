...
export function useSettingsIntegrations({
  ...
  const loadIntegrations = React.useCallback(async () => {
    ...
    try {
      const result = await getMobileClient().getIntegrations();
  ...
  const handleDisconnectIntegration = async (provider: string) => {
    ...
    try {
      await getMobileClient().disconnectIntegration(provider);
  ...
  const handleAddMcpServer = async () => {
    ...
    const nextServers = await persistMobileMcpServers(upsertMcpServerByName(mcpServers, input.value));
  ...
  const handleRemoveMcpServer = async (serverName: string) => {
    const nextServers = await persistMobileMcpServers(removeMcpServerByName(mcpServers, serverName));
    setMcpServers(nextServers);
