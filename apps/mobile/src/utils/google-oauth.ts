...
const readGoogleUserInfo = async (response: Response): Promise<GoogleUserInfo> => {
  let rawUserInfo: unknown;
  try {
    rawUserInfo = await response.json();
  } catch (error) {
    logger.warn('Failed to parse Google user info response', { error });
    throw new Error('Invalid Google user info response', { cause: error });
  }

  const parsedUserInfo = GoogleUserInfoSchema.safeParse(rawUserInfo);
  if (!parsedUserInfo.success) {
    logger.warn('Google user info response failed validation', {
...
export async function signInWithGoogle(): Promise<GoogleAuthResult> {
  try {
    ...
    if (promptResult.type === 'success') {
      ...
      const userInfo = await readGoogleUserInfo(userInfoResponse);
