    ...
    <View style={{ gap: 14 }}>
      {items.map((item) => {
        ...
        return (
          ...
          >
            {text ? <MarkdownView content={text} isUser={fromUser} /> : null}
            {imageUri ? (
...
function RemoteInlineDiff({ diff }: { diff: string }) {
  const rows = inlineDiffRows(diff);
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator>
      <View accessibilityLabel="Remote inline diff" style={{ minWidth: '100%', paddingVertical: 4, backgroundColor: '#05070a', borderRadius: 10 }}>
        {rows.map((row, index) => (
          <View
            key={`${index}:${row.text}`}
            style={{
              ...
              flexDirection: 'row',
              paddingHorizontal: 8,
              backgroundColor: diffLineBackground(row.text),
            }}
          >
            <Text selectable style={{ width: 35, color: '#4b5563', fontFamily: 'monospace', fontSize: 10, lineHeight: 18 }}>
              {row.oldLine ?? ''}
            </Text>
            <Text selectable style={{ width: 35, color: '#4b5563', fontFamily: 'monospace', fontSize: 10, lineHeight: 18 }}>
              {row.newLine ?? ''}
            </Text>
            <Text selectable style={{ color: diffLineColor(row.text), fontFamily: 'monospace', fontSize: 11, lineHeight: 18 }}>
              {row.text || ' '}
            </Text>
          </View>
        ))}
...
type InlineDiffRow = { text: string; oldLine: number | null; newLine: number | null };

export const inlineDiffRows = (diff: string): InlineDiffRow[] => {
  let oldLine = 0;
  let newLine = 0;
  return diff.split('\n').map((text) => {
    if (text.startsWith('@@')) {
    ...
    if (text.startsWith('+') && !text.startsWith('+++')) {
      const row = { text, oldLine: null, newLine };
      newLine += 1;
      return row;
    }
    if (text.startsWith('-') && !text.startsWith('---')) {
      const row = { text, oldLine, newLine: null };
      oldLine += 1;
      return row;
    }
    if (text.startsWith(' ')) {
      const row = { text, oldLine, newLine };
      oldLine += 1;
      newLine += 1;
      return row;
    }
...
const diffLineBackground = (line: string) => {
  if (line.startsWith('+') && !line.startsWith('+++')) return 'rgba(34,197,94,0.12)';
  if (line.startsWith('-') && !line.startsWith('---')) return 'rgba(239,68,68,0.12)';
  if (line.startsWith('@@')) return 'rgba(59,130,246,0.12)';
  return 'transparent';
};
