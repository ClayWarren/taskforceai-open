...
import { useRespondDesktopInteractionMutation, type DesktopInteractionRequest } from '../data/desktop-work';
...
function RemoteInteractionCard({ interaction, pending, onRespond }: {
  ...
  pending: boolean;
  onRespond: (response: { decision?: 'accept' | 'acceptForSession' | 'decline'; response?: unknown }) => void;
}) {
  ...
  return (
    <View style={[styles.card, { backgroundColor: theme.colors.cardBackground }]}>
      ...
      {isUserInput ? questions.map((question, index) => {
        ...
        return (
          <View key={id} style={{ gap: 7 }}>
            ...
            <TextInput value={answers[id] ?? ''} onChangeText={(value) => setAnswers((current) => ({ ...current, [id]: value }))} placeholder={options.length > 0 ? 'Or type another answer' : 'Type your response'} placeholderTextColor={theme.colors.textMuted} accessibilityLabel={`Answer ${prompt}`} style={[styles.input, { color: theme.colors.text, borderColor: theme.colors.border }]} />
          </View>
        );
      }) : null}
      <View style={styles.actions}>
        ...
        {!isUserInput ? (
          <TouchableOpacity accessibilityRole="button" accessibilityLabel={`Approve desktop request ${interaction.id} for session`} disabled={pending} onPress={() => onRespond({ decision: 'acceptForSession' })} style={[styles.button, { borderColor: theme.colors.border }]}>
            <Text style={{ color: theme.colors.text }}>Allow session</Text>
          </TouchableOpacity>
        ) : null}
