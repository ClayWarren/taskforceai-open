...
import { AuthProvider as SharedAuthProvider } from '@taskforceai/contracts/auth/AuthProvider';
import { useAuth as useSharedAuth } from '@taskforceai/contracts/auth/AuthProvider';
...
import { ACTIVE_CONVERSATION_KEY } from '../hooks/useConversationState';
import { clearDesktopPairingSession } from '../desktop-pairing/session-store';
...
const clearMobileAuthSessionState = async (): Promise<void> => {
  queryClient.clear();
  ...
  await Promise.allSettled([
    ...
    for (const result of results) {
      if (result.status === 'rejected') {
        mobileLogger.error('[AuthContext] Failed to clear mobile auth session state', {
          error: result.reason,
        });
      }
    }
...
const mobileProfileStorage = {
  ...
  clearProfile: async () => {
    await clearMobileAuthSessionState();
