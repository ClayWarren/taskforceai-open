...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children }) => {
    ...
    const initTheme = async () => {
      try {
        ...
        if (isMounted && savedMode) {
          themeModeRef.current = savedMode;
  ...
  const persistThemeMode = useCallback((mode: ThemeMode): Promise<void> => {
    const write = persistenceQueueRef.current.then(() => storeThemeMode(mode));
  ...
  const toggleTheme = useCallback(async () => {
    const newMode = themeModeRef.current === 'dark' ? 'light' : 'dark';
