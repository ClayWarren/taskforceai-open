...
// Bump this key whenever the provider disclosure changes materially.
const AI_DATA_SHARING_CONSENT_KEY = '@taskforceai:ai-data-sharing-consent:v2';

const AI_DATA_SHARING_DISCLOSURE =
  'TaskForceAI sends your prompt, attachments, conversation context, and selected model settings to TaskForceAI servers and third-party AI providers including OpenAI, Anthropic, Google, xAI, Mistral, Z.ai, and Vercel AI Gateway to generate responses. Do not include sensitive personal data unless you want it processed for your request.';

async function hasAcceptedAiDataSharing(): Promise<boolean> {
  return (await AsyncStorage.getItem(AI_DATA_SHARING_CONSENT_KEY)) === 'accepted';
}

async function recordAiDataSharingConsent(): Promise<void> {
