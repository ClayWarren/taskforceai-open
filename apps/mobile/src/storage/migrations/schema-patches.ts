...
const dedupeRowsByColumn = (
  ...
  db.execSync(
    `DELETE FROM "${table}" WHERE rowid IN (SELECT rowid FROM (SELECT rowid, ROW_NUMBER() OVER (PARTITION BY "${column}" ORDER BY "${orderColumn}" DESC, rowid DESC) AS duplicate_rank FROM "${table}") WHERE duplicate_rank > 1)`,
  );
};

const isUniqueConstraintError = (error: unknown): boolean => {
  if (!(error instanceof Error)) return false;
  return /(?:UNIQUE constraint failed|SQLITE_CONSTRAINT_(?:UNIQUE|PRIMARYKEY))/i.test(
    error.message,
  );
};
...
const ensureUniqueIndex = (
  ...
  assertSafeIdentifier(config.dedupeOrderColumn);
  ...
  db.execSync("BEGIN IMMEDIATE;");
  try {
    dedupeRowsByColumn(db, config.table, config.column, config.dedupeOrderColumn);
    db.execSync(indexSql);
    db.execSync(`DROP INDEX IF EXISTS "${config.name}_nonunique"`);
    db.execSync("COMMIT;");
  } catch (error) {
