...
const ensureRequiredTables = (rawDb: SQLite.SQLiteDatabase): void => {
  const finalTables = rawDb.getAllSync<{ name: string }>(
    'SELECT name FROM sqlite_master WHERE type="table"'
  );
  const finalTableNames = new Set(finalTables.map((table) => table.name));
  const missingTables = REQUIRED_TABLES.filter((table) => !finalTableNames.has(table));
...
export async function runDrizzleMigrations(
  ...
  if (!hasProjectId) {
    mobileLogger.info('[MigrationRunner] Adding missing conversations.project_id column');
    rawDb.execSync('ALTER TABLE conversations ADD COLUMN project_id INTEGER;');
  }
