...
(table) => [
	...
	index("conversations_conversation_id_idx").on(table.conversationId),
...
export const messages = sqliteTable("messages", {
	...
	conversationId: text("conversation_id").notNull().references(() => conversations.conversationId, { onDelete: "cascade", onUpdate: "cascade" } ),
	role: text().notNull(),
...
export const pendingPrompts = sqliteTable("pending_prompts", {
	...
	prompt: text().notNull(),
	conversationId: text("conversation_id").references(() => conversations.conversationId, { onDelete: "set null", onUpdate: "cascade" } ),
	createdAt: integer("created_at").notNull(),
...
(table) => [
	index("pending_prompts_created_at_idx").on(table.createdAt),
]);
