...
import { type MobileSchema, mobileSchema } from './schema';
...
class DatabaseManager {
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      // Step 2: Apply legacy schema patches (only once per migration tag)
      const lastPatchTag = await AsyncStorage.getItem(SCHEMA_PATCH_VERSION_KEY);
      if (lastPatchTag !== LATEST_MIGRATION_TAG) {
        const tablesResult = this.rawDb.getAllSync<{ name: string }>('SELECT name FROM sqlite_master WHERE type="table"');
        const tableNames = tablesResult.map(t => t.name);
        mobileLogger.info('[DatabaseManager] Existing tables before patches', { tableNames });

        applyLegacySchemaPatches(this.rawDb, tableNames);

        // Close and reopen to clear internal statement/schema caches
