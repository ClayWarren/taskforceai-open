...
import type { MobileSchema } from './schema';
...
export async function runDrizzleMigrations(
    ...
    if (!hasUserTables && (effectiveAppliedTag === null || effectiveAppliedTag === 'none')) {
    ...
    if (hasAppTables && (effectiveAppliedTag === null || effectiveAppliedTag === 'none')) {
    ...
    if (effectiveAppliedTag !== LATEST_MIGRATION_TAG) {
        ...
        try {
            rawDb.execSync('BEGIN EXCLUSIVE;');
            ...
            await migrate(orm, cloneResult.value);
