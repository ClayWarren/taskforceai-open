...
import {
  ...
  ENCRYPTION_MIGRATION_PENDING_KEY,
  ENCRYPTED_DB_MARKER_KEY,
  LEGACY_PLAINTEXT_MODE_KEY,
} from './encryption-migration';
import { runDrizzleMigrations, MIGRATION_STATE_KEY_EXPORT } from '../migrations/migration-runner';
import { applyLegacySchemaPatches } from '../migrations/schema-patches';
...
class DatabaseManager {
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      // Step 1: Get the key and safely migrate legacy plaintext data when needed.
      let encryptionKey: string | null = await getOrCreateEncryptionKey();
      const encryptionMode = await handleEncryptionMigration(encryptionKey);
  ...
  async resetDatabase(): Promise<void> {
    ...
    if (typeof sqliteWithDelete.deleteDatabaseSync === 'function') {
      ...
      sqliteWithDelete.deleteDatabaseSync(ENCRYPTION_MIGRATION_BACKUP_DB_NAME);
    ...
    await Promise.all([
      ...
      AsyncStorage.removeItem(ENCRYPTION_MIGRATION_PENDING_KEY),
