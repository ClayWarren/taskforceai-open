...
import { messages } from '@taskforceai/db-sync/drizzle/schema';
...
export class MessageRepository implements IMessageStore {
  ...
  async getMessage(messageId: string): Promise<Result<StorageMessage, StorageReadError>> {
    return withRepoResult(
      ...
      async () => {
        ...
        return row ? ok(mapMessageRow(row)) : err(storageNotFoundError('Message not found'));
      },
      { messageId },
      storageFailureError
    );
