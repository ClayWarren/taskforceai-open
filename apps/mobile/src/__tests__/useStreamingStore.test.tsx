...
import type { StreamingPayload } from '@taskforceai/client-core/streaming/types';
...
describe('useStreamingStore', () => {
  ...
  it('handles streaming lifecycle correctly', async () => {
    ...
    expect(mobileMetrics.incrementCounter).toHaveBeenCalledWith(
      'streaming.sse.connection.opened',
      { transport: 'sse' }
    );
    expect(mobileMetrics.incrementCounter).toHaveBeenCalledWith(
      'streaming.sse.message.received',
      { transport: 'sse' }
    );
    expect(mobileMetrics.incrementCounter).toHaveBeenCalledWith(
      'streaming.sse.connection.closed',
      expect.objectContaining({ outcome: 'terminal_message' })
    );
  ...
  it('keeps an opened EventSource alive after a recoverable error', async () => {
    let startPromise!: Promise<void>;
    await act(async () => {
      startPromise = useStreamingStore.getState().startStreaming({
        taskId: 'run-retry',
  ...
  it('aborts a stream that receives no events before the watchdog deadline', async () => {
    let startPromise!: Promise<void>;
    await act(async () => {
      startPromise = useStreamingStore.getState().startStreaming({
        taskId: 'run-stalled',
        conversationId: 'conv-stalled',
        prompt: 'stall transport',
      });
    });

    await act(async () => {
      jest.advanceTimersByTime(60_000);
      try { await startPromise; } catch { /* expected watchdog failure */ }
    });

    expect(mobileMetrics.incrementCounter).toHaveBeenCalledWith(
      'streaming.sse.watchdog.timeout',
      { transport: 'sse', outcome: 'failed' }
    );
  });
});
