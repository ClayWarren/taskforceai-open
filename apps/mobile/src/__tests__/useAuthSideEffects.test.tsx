...
describe('useAuthSideEffects', () => {
  ...
  it('configures RevenueCat with the stable backend user id', async () => {
  ...
  it('re-configures RevenueCat when user id changes', () => {
    ...
    expect(configureRevenueCatMock).toHaveBeenCalledWith('user-2');
    
    cleanup();
    cleanup2();
  });

  it('contains RevenueCat configuration failures', async () => {
    configureRevenueCatMock.mockRejectedValueOnce(new Error('billing unavailable'));
