...
describe('PreferencesContext', () => {
  ...
  it('loads, clamps, and persists Remote display preferences', async () => {
    mockStorage['@taskforceai:remoteTextScale'] = '0.2';
    mockStorage['@taskforceai:remoteCodeScale'] = '1.8';
    mockStorage['@taskforceai:remoteWordWrap'] = 'true';
    const { getValue } = await renderWithProvider();
    ...
    await act(async () => {
      await getValue().setRemoteTextScale(1.14);
      await getValue().setRemoteCodeScale(Number.NaN);
      await getValue().setRemoteWordWrap(false);
    });
    expect(mockStorage).toMatchObject({
      '@taskforceai:remoteTextScale': '1.1',
      '@taskforceai:remoteCodeScale': '1',
      '@taskforceai:remoteWordWrap': 'false',
    });
  });
  ...
  it('does not update state when storage loading finishes after unmount', async () => {
    const AsyncStorage = require('@react-native-async-storage/async-storage');
    let resolveStorage!: (value: string | null) => void;
    const pendingStorage = new Promise<string | null>((resolve) => {
      resolveStorage = resolve;
    });
    AsyncStorage.getItem.mockReturnValue(pendingStorage);
