...
describe('certificate-pinning', () => {
  describe('isPinnedDomain', () => {
    ...
    it('returns true for sync.taskforceai.chat', async () => {
      const { isPinnedDomain } = await import('../../security/certificate-pinning');
      expect(isPinnedDomain('https://sync.taskforceai.chat/api/v1/remote/pair')).toBe(true);
    });
  ...
  describe('assertProductionDomain', () => {
    ...
    it('does not throw in prod mode for pinned domains', async () => {
      ...
      expect(() =>
        assertProductionDomain('https://sync.taskforceai.chat/api/v1/remote/pair')
      ).not.toThrow();
    ...
    it('reports malformed production URLs as pinning failures', async () => {
      (globalThis as any).__DEV__ = false;
      const { assertProductionDomain } = await import('../../security/certificate-pinning');

      expect(() => assertProductionDomain('not-a-url')).toThrow(/invalid URL/i);
    });
  });
  ...
  describe('PINNED_SPKI_HASHES', () => {
    ...
    it('keeps leaf and CA pins classified separately', async () => {
      ...
      expect(PINNED_CA_SPKI_HASHES).toHaveLength(2);
  ...
  describe('Expo plugin guard', () => {
    ...
    it('exports separate iOS leaf and CA pin groups', async () => {
      ...
      expect(plugin.__internal.CA_SPKI_HASHES).toHaveLength(2);
      expect(plugin.__internal.IOS_PINNED_HASHES).toEqual(plugin.__internal.CA_SPKI_HASHES);
    });

    it('keeps the Expo plugin pins synchronized with runtime pins', async () => {
      const plugin = require('../../../plugins/withCertificatePinning.js');
      const { PINNED_LEAF_SPKI_HASHES, PINNED_CA_SPKI_HASHES } = await import(
        '../../security/certificate-pinning'
      );

      expect(plugin.__internal.LEAF_SPKI_HASHES).toEqual(PINNED_LEAF_SPKI_HASHES);
      expect(plugin.__internal.CA_SPKI_HASHES).toEqual(PINNED_CA_SPKI_HASHES);
      expect(plugin.__internal.IOS_PINNED_HASHES).toEqual(PINNED_CA_SPKI_HASHES);
    });
    ...
    it('generates Android domain-config entries with domain children', async () => {
      ...
      expect(productionDomainConfig?.[1]).toContain(
        '<domain includeSubdomains="true">sync.taskforceai.chat</domain>'
      );
      expect(productionDomainConfig?.[1]).toContain(
