...
describe('database encryption helpers', () => {
  ...
  it('returns an existing encryption key without generating a new one', async () => {
    secureStoreState.value = 'existing-key';
    const { getOrCreateEncryptionKey } = require('../../storage/encryption');

    await expect(getOrCreateEncryptionKey()).resolves.toBe('existing-key');
  ...
  it('applies SQLCipher hex key pragma', () => {
    ...
    const { applyEncryptionKey } = require('../../storage/encryption');
    ...
    expect(execCalls).toEqual([`PRAGMA key = "x'abc123'"`]);
  });
});
