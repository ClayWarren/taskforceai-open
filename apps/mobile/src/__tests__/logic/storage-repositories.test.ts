...
const { ConversationRepository } = require(
  '../../storage/database/repositories/ConversationRepository'
) as typeof import('../../storage/database/repositories/ConversationRepository');
const { MessageRepository } = require(
  '../../storage/database/repositories/MessageRepository'
) as typeof import('../../storage/database/repositories/MessageRepository');
const { SyncRepository } = require(
  '../../storage/database/repositories/SyncRepository'
) as typeof import('../../storage/database/repositories/SyncRepository');
const { SessionRepository } = require(
  '../../storage/database/repositories/SessionRepository'
) as typeof import('../../storage/database/repositories/SessionRepository');
const { UserRepository } = require(
  '../../storage/database/repositories/UserRepository'
) as typeof import('../../storage/database/repositories/UserRepository');
...
describe('storage repositories', () => {
  ...
  it('ConversationRepository rethrows unexpected list errors', async () => {
    ...
    ormState.selectPlans.push({ throwAtWhere: 'non-error failure' as unknown as Error });
    await expect(repo.getConversations()).rejects.toBe('non-error failure');

    ormState.selectPlans.push({ throwAtWhere: 'archive failure' as unknown as Error });
    await expect(repo.getArchivedConversations()).rejects.toBe('archive failure');
  });
  ...
  it('UserRepository normalizes legacy flags and falls back invalid plans to free', async () => {
    ormState.selectPlans.push({
      rows: [
        createProfileRow({
          id: 202,
