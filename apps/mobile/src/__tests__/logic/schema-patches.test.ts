...
import { applyLegacySchemaPatches } from '../../storage/migrations/schema-patches';
...
describe('schema-patches', () => {
  ...
  it('uses SQL defaults and non-null fallbacks when rebuilding missing legacy columns', () => {
    const { db, execCalls } = createMockDb({
    ...
    expect(insertSql).toContain("'free'");
    expect(insertSql).toContain("'{}'");
    expect(insertSql).toContain('0');
    expect(insertSql).toContain('NULL');
  });

  it('preserves text user identifiers when rebuilding auth sessions', () => {
    const { db, execCalls } = createMockDb({
      tables: ['auth_sessions'],
      columnsByTable: {
        auth_sessions: ['accessToken', 'expiresAt', 'userId', 'email', 'createdAt'],
      },
    });

    applyLegacySchemaPatches(db as any, []);

    const createSql = execCalls.find((sql) => sql.includes('CREATE TABLE auth_sessions_new'));
  ...
  it('dedupes rows and retries unique indexes if unique index creation fails', () => {
    ...
    const { db, execCalls } = createMockDb({
      ...
      throwOnExec: (sql) => {
        ...
        if (uniqueIndexAttempts === 1) {
          throw new Error('UNIQUE constraint failed: conversations.conversation_id');
        }
        return false;
      },
    });
    ...
    expect(
      execCalls.some((sql) =>
        sql.includes(
          'PARTITION BY "conversation_id" ORDER BY "updated_at" DESC, rowid DESC'
        )
      )
    ).toBe(true);
    expect(
  ...
  it('does not delete rows when index creation fails for a non-constraint error', () => {
    const { db, execCalls } = createMockDb({
      tables: ['conversations'],
      columnsByTable: {
        conversations: [
      ...
      throwOnExec: (sql) => {
        if (sql.includes('CREATE UNIQUE INDEX')) throw new Error('database is locked');
        return false;
      },
    });

    expect(() => applyLegacySchemaPatches(db as any, [])).toThrow('database is locked');
    expect(execCalls.some((sql) => sql.startsWith('DELETE FROM'))).toBe(false);
    expect(execCalls).not.toContain('BEGIN IMMEDIATE;');
  });

  it('rolls back deduplication when the unique-index retry fails', () => {
    let uniqueIndexAttempts = 0;
    const { db, execCalls } = createMockDb({
      tables: ['messages'],
      columnsByTable: {
      ...
      throwOnExec: (sql) => {
        if (!sql.includes('CREATE UNIQUE INDEX')) return false;
        uniqueIndexAttempts += 1;
        if (uniqueIndexAttempts === 1) {
    ...
    expect(() => applyLegacySchemaPatches(db as any, [])).toThrow('disk I/O error');
    expect(execCalls.some((sql) => sql.startsWith('DELETE FROM "messages"'))).toBe(true);
