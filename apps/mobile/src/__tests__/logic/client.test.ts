...
mock.module('../../config/base-url', () => ({
  ...
  getMobileRemoteBaseUrl: () => 'https://sync.test',
}));
...
import {
  getMobileAuthClient,
  getMobileClient,
  getMobilePinnedFetch,
  getMobileRemoteClient,
} from '../../api/client';

describe('Mobile API client bootstrap', () => {
  beforeEach(() => {
    (globalThis as any).__MOBILE_CLIENTS__ = {
      api: null,
      remote: null,
      auth: null,
      pinnedFetch: null,
  ...
  it('provides a separately cached Remote client targeting Sync', () => {
    const client1 = getMobileRemoteClient();
    const client2 = getMobileRemoteClient();

    expect(client1).toBe(client2);
    expect(controller.createApiClient).toHaveBeenCalledTimes(1);
    expect(controller.createApiClient.mock.calls[0]?.[0].baseUrl).toBe('https://sync.test');
  });

  it('initializes the global cache when a host has not created it yet', async () => {
    (globalThis as any).__MOBILE_CLIENTS__ = undefined;

    const moduleUrl = new URL(`../../api/client.ts?fresh=${Date.now()}`, import.meta.url);
    const fresh = await import(moduleUrl.href);

    expect(fresh.getMobileClient()).toBe(controller.apiClient);
  });
