...
describe('token exchange uses pinned fetch', () => {
  ...
  it('exchanges Apple token through pinned fetch', async () => {
    ...
    expect(init.method).toBe('POST');
    expect(JSON.parse(String(init.body as string))).toEqual(
      expect.objectContaining({ nonce: 'apple-nonce' })
    );
  });
  ...
  it('wraps invalid success JSON in a stable exchange error', async () => {
    testState.fetchImpl.mockImplementation(
      async () =>
    ...
    ).rejects.toThrow('Invalid response from google token exchange');
  });

  it('wraps schema-invalid success payloads in a stable exchange error', async () => {
    testState.fetchImpl.mockImplementation(
      async () =>
    ...
    ).rejects.toThrow('Invalid response from apple token exchange');
  });

  it('preserves network failures from pinned fetch', async () => {
    testState.fetchImpl.mockRejectedValueOnce(new Error('network unavailable'));
