...
} = await import('../../hooks/api/desktopWork');
...
const createQueryHarness = <T>(useHook: () => T) => {
...
describe('desktop work query data', () => {
  ...
  it('reads desktop work through the React Query hook when enabled', async () => {
    currentSession = null;
    const { result } = createQueryHarness(() => useDesktopWorkStateQuery(true));
  ...
  it('sends desktop turns and invalidates desktop work state', async () => {
    ...
    try {
      const { result, invalidations } = createQueryHarness(() => useSendDesktopTurnMutation());
  ...
  it('rejects desktop turns while unpaired', async () => {
    currentSession = null;
    const { result } = createQueryHarness(() => useSendDesktopTurnMutation());
  ...
  it('starts desktop threads with a title derived from the objective', async () => {
      ...
      return jsonResponse({
        result: {
          thread: {
            ...
            objective: body.params.objective,
            state: 'running',
            source: 'mobile',
            createdAt: 1,
            updatedAt: 2,
    ...
    try {
      const { result, invalidations } = createQueryHarness(() => useStartDesktopThreadMutation());
  ...
  it('rejects desktop mutations while unpaired', async () => {
    currentSession = null;
    const { result } = createQueryHarness(() => useStartDesktopThreadMutation());
