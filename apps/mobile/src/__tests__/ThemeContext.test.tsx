...
jest.mock('../utils/theme-storage', () => ({
  loadThemeMode: async () => {
    if (mockForceGetItemFailure) {
      throw new Error('forced theme load failure');
    }
    if (mockDeferGetItem) {
      return await new Promise<string | null>((resolve) => {
        const wait = () => {
          if (!mockDeferGetItem) {
            resolve(mockAsyncState[THEME_KEY] ?? null);
            return;
          }
          setTimeout(wait, 0);
        };
    ...
    return mockAsyncState[THEME_KEY] ?? null;
  },
...
let mockForceGetItemFailure = false;
let mockDeferGetItem = false;

const applyAsyncStorageBindings = () => {
  asyncStorage.setItem.mockImplementation(async (key: string, value: string) => {
    setItemCalls.push({ key, value });
    mockAsyncState[key] = value;
  });
  ...
  asyncStorage.getItem.mockImplementation(async (key: string) => {
    ...
    if (mockDeferGetItem) {
      return await new Promise<string | null>((resolve) => {
        const wait = () => {
          if (!mockDeferGetItem) {
            resolve(mockAsyncState[key] ?? null);
            return;
          }
          setTimeout(wait, 0);
        };
...
beforeEach(() => {
  ...
  resetTracking();
  mockAsyncState[THEME_KEY] = null;
  mockForceGetItemFailure = false;
  mockDeferGetItem = false;
  applyAsyncStorageBindings();
});
...
async function testLoadsStoredTheme() {
  resetTracking();
  mockAsyncState[THEME_KEY] = 'light';
...
async function testToggleUpdatesStorage() {
  ...
  assert.equal(mockAsyncState[THEME_KEY], 'light', 'light mode should persist to storage');
...
async function testRapidTogglesPersistInOrder() {
  ...
  assert.equal(mockAsyncState[THEME_KEY], 'dark', 'rapid toggles should persist in invocation order');
...
async function testSetThemeModePersists() {
  ...
  assert.equal(mockAsyncState[THEME_KEY], 'dark', 'dark mode should persist to storage');
