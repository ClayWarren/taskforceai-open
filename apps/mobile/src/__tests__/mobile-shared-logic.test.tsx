...
import { getJwtExpiryMs } from '@taskforceai/api-client/auth/session-expiry';
...
import { serializeError } from '@taskforceai/persistence/storage/value-utils';
...
describe('mobile shared logic', () => {
  ...
  it('creates and connects mobile MCP clients', async () => {
    ...
    expect(mockMcpClientConstructor).toHaveBeenCalledWith(
      { name: 'taskforceai-mobile', version: '0.5.2' },
      { capabilities: {} }
    );
  ...
  it('fulfills pending MCP approvals through the shared denial core', async () => {
    ...
    mockFulfillPendingMcpApprovalCore.mockImplementationOnce(async (options) => {
      expect(options.executeTool).toBeUndefined();
      expect(options.logFailure).toBeUndefined();
      return true;
    });
  ...
  it('returns typography font readiness', async () => {
    const loaded = await renderHook(() => useTypography());
  ...
  it('normalizes streaming parser errors and renders message timestamps', async () => {
    const rendered = await render(<MessageTimestamp timestamp={new Date('2026-01-02T03:04:05Z')} isUser={false} />);
    expect(rendered.toJSON()).toBeTruthy();
  });

  it('wires the mobile MCP catalog hook to a manager and registry factory', async () => {
