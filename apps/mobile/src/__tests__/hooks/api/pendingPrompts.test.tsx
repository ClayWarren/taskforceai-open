...
const customRenderHook = async <T,>(useHook: () => T) => {
...
describe('usePendingPromptsQuery', () => {
  ...
  it('calls listPendingPrompts', async () => {
    ...
    await customRenderHook(() => usePendingPromptsQuery());
  ...
  it('returns pending prompts data', async () => {
    ...
    const { result } = await customRenderHook(() => usePendingPromptsQuery());
  ...
  it('uses correct query key', async () => {
    jest.mocked(storage.listPendingPrompts).mockResolvedValueOnce({ ok: true, value: [] });
    const { queryClient } = await customRenderHook(() => usePendingPromptsQuery());
...
describe('useClearPendingPromptsMutation', () => {
  ...
  it('calls clearPendingPrompts', async () => {
    const { result } = await customRenderHook(() => useClearPendingPromptsMutation());
  ...
  it('invalidates pending prompts query on success', async () => {
    const { result, queryClient } = await customRenderHook(() => useClearPendingPromptsMutation());
...
describe('useRemovePendingPromptMutation', () => {
  ...
  it('calls removePrompt with id', async () => {
    const { result } = await customRenderHook(() => useRemovePendingPromptMutation());
  ...
  it('invalidates pending prompts query on success', async () => {
    const { result, queryClient } = await customRenderHook(() => useRemovePendingPromptMutation());
    const invalidateSpy = jest.spyOn(queryClient, 'invalidateQueries');
