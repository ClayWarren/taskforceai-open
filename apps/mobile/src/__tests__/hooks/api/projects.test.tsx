...
describe('useProjectsQuery', () => {
  ...
  it('loads projects with the projects query key', async () => {
    const { result } = await renderHookWithQueryClient(() => useProjectsQuery());
...
describe('useCreateProjectMutation', () => {
  ...
  it('calls createProject with request data', async () => {
    ...
    const { result } = await renderHookWithQueryClient(() => useCreateProjectMutation());
  ...
  it('invalidates projects query on success', async () => {
    const { result, queryClient } = await renderHookWithQueryClient(() => useCreateProjectMutation());
...
describe('useDeleteProjectMutation', () => {
  ...
  it('calls deleteProject with projectId', async () => {
    const { result } = await renderHookWithQueryClient(() => useDeleteProjectMutation());
  ...
  it('invalidates projects query on success', async () => {
    const { result, queryClient } = await renderHookWithQueryClient(() => useDeleteProjectMutation());
    const invalidateSpy = jest.spyOn(queryClient, 'invalidateQueries');
