...
describe('mobile artifact API helpers', () => {
  ...
  it('loads artifact versions and keeps the query idle without an artifact id', async () => {
    const version = artifactFixture().currentVersion!;
    mockPinnedFetch.mockResolvedValueOnce(jsonResponse([version]));

    const loaded = await renderHookWithQueryClient(() =>
      useArtifactVersionsQuery('artifact/1', true)
    );

    await waitFor(() => expect(loaded.result.current.data).toEqual([version]));
    expect(mockPinnedFetch).toHaveBeenCalledWith(
      'https://mobile.example/api/v1/artifacts/artifact%2F1/versions',
      expect.objectContaining({ headers: expect.any(Headers) })
    );

    mockPinnedFetch.mockClear();
    const idle = await renderHookWithQueryClient(() => useArtifactVersionsQuery(null, true));
    expect(idle.result.current.fetchStatus).toBe('idle');
    await expect(idle.result.current.refetch()).resolves.toEqual(
  ...
  it('surfaces invalid artifact version responses', async () => {
    mockPinnedFetch.mockResolvedValueOnce(jsonResponse([{ id: 1 }]));

    const invalid = await renderHookWithQueryClient(() =>
      useArtifactVersionsQuery('artifact-1', true)
    );

    await waitFor(() =>
      expect(invalid.result.current.error).toEqual(new Error('Invalid response from server'))
    );
  });
  ...
  it('rejects downloads that do not produce a valid bounded file', async () => {
    mockGetFileInfo.mockResolvedValueOnce({
      exists: false,
      uri: 'file:///cache/missing-artifact',
    });

    const missing = await downloadMobileArtifactContent(
      'https://mobile.example/file',
      'file:///cache/missing-artifact',
      { maxBytes: 10 }
    );

    expect(missing.ok).toBe(false);
    if (!missing.ok) {
      expect(missing.error.message).toBe('Artifact download did not produce a valid file');
    }
    expect(mockDeleteFile).toHaveBeenCalledWith('file:///cache/missing-artifact', {
      idempotent: true,
    });
    ...
    mockGetFileInfo.mockResolvedValueOnce({
      exists: true,
      size: 11,
      uri: 'file:///cache/oversized-artifact',
    });
    const oversized = await downloadMobileArtifactContent(
      'https://mobile.example/file',
      'file:///cache/oversized-artifact',
      { maxBytes: 10 }
    );
    expect(oversized.ok).toBe(false);
    if (!oversized.ok) {
      expect(oversized.error.message).toContain('mobile download limit');
    }
  });
  ...
  it('loads text previews and handles malformed error bodies', async () => {
    ...
    mockPinnedFetch.mockRejectedValueOnce('offline');
    const unavailable = await fetchArtifactContentText('https://mobile.example/preview');
    expect(unavailable.ok).toBe(false);
    if (!unavailable.ok) {
      expect(unavailable.error.message).toBe('offline');
    }
  });
