...
describe('useStorageSummaryQuery', () => {
  ...
  it('loads the mobile storage summary and caches it under the storage query key', async () => {
    ...
    const { result, queryClient } = renderHookWithQueryClient(() => useStorageSummaryQuery());
  ...
  it('does not call the mobile client when disabled', () => {
    const { result } = renderHookWithQueryClient(() => useStorageSummaryQuery({ enabled: false }));
