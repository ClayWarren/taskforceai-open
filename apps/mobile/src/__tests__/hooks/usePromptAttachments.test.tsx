...
describe('usePromptAttachments', () => {
    ...
    it('takes a photo with camera permission', async () => {
        ImagePicker.requestCameraPermissionsAsync.mockResolvedValueOnce({ granted: true });
        ImagePicker.launchCameraAsync.mockResolvedValueOnce({
            canceled: false,
            assets: [{ fileName: 'camera.jpg', uri: 'file:///camera.jpg', fileSize: 240, mimeType: 'image/jpeg', type: 'image' }],
        });
        ...
        expect(result.current.attachments).toHaveLength(1);
        expect(result.current.attachments[0].name).toBe('camera.jpg');
    });
