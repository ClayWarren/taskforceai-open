...
describe('useCacheCleanup', () => {
  ...
  it('deletes only stale attachment-like files older than 24 hours', async () => {
    ...
    await renderHook(() => useCacheCleanup());
  ...
  it('contains cache enumeration failures', async () => {
    ...
    await waitFor(() => expect(FileSystem.readDirectoryAsync).toHaveBeenCalledTimes(1));
    expect(FileSystem.deleteAsync).not.toHaveBeenCalled();
  });
});
