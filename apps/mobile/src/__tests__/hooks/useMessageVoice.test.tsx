...
describe('useMessageVoice', () => {
  ...
  it('starts playback from the first speech chunk while prefetching the next chunk', async () => {
    ...
    const { result } = renderHook(() => useMessageVoice(`${firstChunk}\n\n${secondChunk}`));
  ...
  it('keeps pause state false when playback has not started yet', () => {
    const { result } = renderHook(() => useMessageVoice('Hello world'));
  ...
  it('clears playback on unmount', async () => {
    const { result, unmount } = renderHook(() => useMessageVoice('Hello world'));
