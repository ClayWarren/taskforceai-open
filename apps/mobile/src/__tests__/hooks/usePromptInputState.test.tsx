...
import { configureLogger } from '../../../../../packages/react-core/src/logger';
...
jest.mock('../../utils/orchestration-preference', () => ({
    readStoredOrchestrationConfig: (...args: unknown[]) => mockReadStoredOrchestrationConfig(...args),
    persistOrchestrationConfig: (...args: unknown[]) => mockPersistOrchestrationConfig(...args),
}));
...
const defaultProps = {
    onSend: jest.fn<(message: string, metadata?: any, attachments?: any[]) => Promise<void>>().mockResolvedValue(undefined as never),
};
...
describe('usePromptInputState', () => {
    ...
    it('initializes with default state', async () => {
        const { result } = renderHook(() => usePromptInputState(defaultProps), {
    ...
    it('waits for orchestration hydration before persisting config', async () => {
        ...
        renderHook(() => usePromptInputState(defaultProps), {
            wrapper: createWrapper(),
        });
    ...
    it('sends message and clears state', async () => {
        ...
        const { result } = renderHook(() => usePromptInputState({ ...defaultProps, onSend }), {
            wrapper: createWrapper(),
        });
    ...
    it('does not upload attachments before the auth gate', async () => {
        ...
        const { result } = renderHook(
            () => usePromptInputState({ ...defaultProps, onSend, isAuthenticated: false }),
            { wrapper: createWrapper() }
        );
        ...
        expect(onSend).not.toHaveBeenCalled();
        expect(alertSpy).toHaveBeenCalledWith('Send Failed', 'Please sign in to start chatting.');
    });
    ...
    it('shows alert when file upload called without callback and at max attachments', async () => {
        ...
        expect(alertSpy).toHaveBeenCalledWith('Add Attachment', 'Choose a source', expect.any(Array));
    });
