...
const mockLogger = (jest.requireMock('../../logger') as {
    mockLogger: {
        debug: jest.Mock;
        error: jest.Mock;
        info: jest.Mock;
...
describe('useProfileActions', () => {
    ...
    it('shows the Stripe fallback when portal creation throws', async () => {
        mockUseAuth.mockReturnValue({
            user: { email: 'test@example.com', id: '1', name: 'Test', subscription_source: 'stripe' },
            logout: mockLogout,
        } as any);
        mockCreatePortalSession.mockRejectedValueOnce(new Error('network failed'));
    ...
    it('handles external link failures and exposes every legal destination', async () => {
        (Linking.openURL as jest.Mock)
            .mockRejectedValueOnce(new Error('browser unavailable'))
            .mockRejectedValueOnce('terms unavailable')
        ...
        await act(async () => {
            await Promise.all([
                ...
                result.current.openSupportContact(),
            ]);
        });
    ...
    it('offers a copyable email fallback when no email handler is available', async () => {
        (Linking.canOpenURL as jest.Mock).mockResolvedValueOnce(false);
        const { result } = await renderHook(() => useProfileActions(onCloseMock));

        await act(async () => {
            await result.current.openSupportContact();
        });
        ...
        const copyButton = (Alert.alert as jest.Mock).mock.calls[0][2][0];
        await act(async () => {
            copyButton.onPress();
        });
        expect(Clipboard.setStringAsync).toHaveBeenCalledWith('support@taskforceai.chat');
    });

    it('uses the email fallback without reporting an error when opening mail fails', async () => {
        (Linking.openURL as jest.Mock).mockRejectedValueOnce(new Error('mail unavailable'));
        const { result } = await renderHook(() => useProfileActions(onCloseMock));
        ...
        await waitFor(() => expect(mockLogger.warn).toHaveBeenCalledWith(
            'Support email handler unavailable',
            expect.objectContaining({ error: expect.any(Error) })
        ));
        expect(mockLogger.error).not.toHaveBeenCalled();
    });

    it('reports clipboard failures from the support email fallback', async () => {
        const clipboardError = new Error('clipboard unavailable');
        (Linking.canOpenURL as jest.Mock).mockResolvedValueOnce(false);
        (Clipboard.setStringAsync as jest.Mock).mockRejectedValueOnce(clipboardError);
        const { result } = await renderHook(() => useProfileActions(onCloseMock));
