...
describe('ChatScreen', () => {
  ...
  it('hides and disconnects realtime voice in Work mode', async () => {
    mockRealtimeVoiceState = {
      ...mockRealtimeVoiceState,
      isActive: true,
      isCapturing: true,
    ...
    await render(<ChatScreen {...baseProps} taskMode="work" />);

    expect(mockRealtimePrewarm).not.toHaveBeenCalled();
    expect(mockRealtimeDisconnect).toHaveBeenCalledTimes(1);
    expect(screen.queryByTestId('realtime-voice-dock')).toBeNull();
    expect(latestPromptInputProps.onRealtimeVoice).toBeUndefined();
    expect(latestPromptInputProps.realtimeVoiceActive).toBe(false);
    expect(latestPromptInputProps.realtimeVoiceDisabled).toBe(true);
    expect(latestPromptInputProps.quickModeEnabled).toBe(true);
  });

  it('changes agent-team orchestration independently from task mode', async () => {
    await render(
      <ChatScreen
        {...baseProps}
        taskMode="work"
    ...
    await act(() => {
      latestPromptInputProps.onQuickModeToggle();
    });

    expect(latestPromptInputProps.quickModeEnabled).toBe(false);
  });
