...
describe('ChatScreen', () => {
  ...
  it('keeps realtime voice setup warm while authenticated', () => {
    render(<ChatScreen {...baseProps} />);
  ...
  it('starts realtime voice before host conversation bookkeeping', async () => {
    ...
    render(<ChatScreen {...baseProps} onRealtimeVoiceStart={onRealtimeVoiceStart} />);
  ...
  it('keeps realtime voice in the chat surface instead of showing the empty logo', () => {
    mockRealtimeVoiceState = {
      ...mockRealtimeVoiceState,
      isActive: true,
      isCapturing: true,
    ...
    const { toJSON } = render(<ChatScreen {...baseProps} />);
  ...
  it('renders realtime voice transcript messages as normal chat bubbles above the orb', () => {
    mockRealtimeVoiceState = {
      ...mockRealtimeVoiceState,
      isActive: true,
      isCapturing: true,
  ...
  it('notifies the host when realtime voice transcript messages change', () => {
  ...
  it('does not duplicate realtime transcript messages already mirrored into chat state', () => {
    mockRealtimeVoiceState = {
      ...mockRealtimeVoiceState,
      isActive: true,
  ...
  it('clears stale realtime voice session state when the reset key changes', () => {
    const { rerender } = render(<ChatScreen {...baseProps} realtimeVoiceResetKey={0} />);

    expect(mockRealtimeResetSession).not.toHaveBeenCalled();

    rerender(<ChatScreen {...baseProps} realtimeVoiceResetKey={1} />);
