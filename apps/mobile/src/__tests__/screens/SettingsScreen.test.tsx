...
describe('SettingsScreen', () => {
  ...
  it('opens usage settings from the root list', async () => {
    ...
    expect(getByText('2 task credits per hour')).toBeTruthy();
    expect(getByText('Metered by throughput')).toBeTruthy();
    expect(getByText('Model cost tiers')).toBeTruthy();
  });
  ...
  it('opens connected apps when a desktop pairing payload is pending', async () => {
    const { getByText, getByDisplayValue } = await render(
      <SettingsScreen
        visible={true}
        onClose={jest.fn()}
        desktopPairingPayload="taskforceai://desktop-pairing?code=WV8D-JMXH"
      />
    );

    expect(getByText('Remote')).toBeTruthy();
    expect(getByDisplayValue('WV8D-JMXH')).toBeTruthy();
  });

  it('opens Connected Apps directly for the Remote Add connection flow', async () => {
    const { getAllByText, getByText } = await render(
      <SettingsScreen visible={true} onClose={jest.fn()} initialSection="apps" />
    );

    expect(getAllByText('Connected Apps')).toHaveLength(2);
    expect(getByText('Remote')).toBeTruthy();
  });
