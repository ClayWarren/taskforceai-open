...
describe('PluginsScreen', () => {
  ...
  it('reports persistence failures when toggling an installed plugin', async () => {
    const persistenceError = new Error('disk full');
    mockPersistMobileMcpServers.mockRejectedValueOnce(persistenceError);
    const alertSpy = jest.spyOn(Alert, 'alert').mockImplementation(() => undefined);
    const view = await render(<PluginsScreen visible onClose={jest.fn()} />);

    await waitFor(() => expect(view.getByLabelText('GitHub plugin')).toBeTruthy());
    await fireEvent.press(view.getByLabelText('GitHub plugin'));
