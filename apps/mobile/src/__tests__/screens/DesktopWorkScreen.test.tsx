...
describe('DesktopWorkScreen', () => {
  ...
  it('keeps Code-only review and file tabs out of Work threads', async () => {
    mockUseDesktopThreadQuery.mockReturnValue({
    ...
    expect(getByLabelText('Remote activity')).toBeTruthy();
    expect(queryByLabelText('Remote changes')).toBeNull();
    expect(queryByLabelText('Remote files')).toBeNull();
  });
