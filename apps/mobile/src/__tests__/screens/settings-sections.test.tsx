...
describe('Settings sections', () => {
  ...
  it('wires Data Controls actions', async () => {
    ...
    await act(async () => {
      ...
      await manageArchivedButton!.props.onPress();
      await archiveAllButton!.props.onPress();
    ...
    await act(async () => {
      await deleteButtons.find((button) => button.text === 'Delete all')?.onPress?.();
    });
  ...
  it('wires subscription actions for free and paid plans', () => {
    ...
    const proButton = freeButtons.find((button) => getButtonText(button) === 'Upgrade to Pro');
    const superButton = freeButtons.find((button) => getButtonText(button) === 'Upgrade to Super');
    ...
    expect(freeTextNodes.some((text) => text.includes('Pro plan') && text.includes('$20/mo'))).toBe(true);
    expect(freeTextNodes.some((text) => text.includes('Super plan') && text.includes('$50/mo'))).toBe(true);
