...
jest.mock('../../features/desktop-work/pairing/linking', () => ({
...
jest.mock('../../screens/CloudTasksScreen', () => ({
  CloudTasksScreen: ({ visible, onClose, onCreate }: any) => {
    const react = require('react');
    const { Text, TouchableOpacity, View } = require('react-native');
    return react.createElement(
      View,
      null,
      react.createElement(Text, null, `cloud-tasks-visible:${String(visible)}`),
      react.createElement(
        TouchableOpacity,
        { testID: 'cloud-tasks-close', onPress: onClose },
        react.createElement(Text, null, 'close-cloud-tasks')
      ),
      react.createElement(
        TouchableOpacity,
        { testID: 'cloud-tasks-create', onPress: onCreate },
        react.createElement(Text, null, 'create-cloud-task')
      )
    );
  },
}));
...
describe('AppRoot', () => {
  ...
  it('opens desktop work from the sidebar desktop entry', async () => {
    ...
    expect(mockHandleCloseSidebar).toHaveBeenCalledTimes(1);
    expect(mockRouterPush).toHaveBeenCalledWith('/remote');
    expect(getByText('settings-visible:false')).toBeTruthy();
