...
jest.mock('../../screens/ChatScreen', () => ({
  ChatScreen: ({ messages, isSidebarVisible, modelLabel, realtimeVoiceResetKey, userPlan }: any) => {
    const react = require('react');
...
describe('AppRoot', () => {
  ...
  it('opens artifacts from the sidebar artifacts entry', () => {
    const { getByTestId, getByText } = render(<AppRoot />);

    expect(getByText('artifacts-visible:false')).toBeTruthy();
    fireEvent.press(getByTestId('sidebar-artifacts'));
  ...
  it('opens finance from the sidebar finance entry', () => {
    const { getByTestId, getByText } = render(<AppRoot />);

    expect(getByText('finance-visible:false')).toBeTruthy();
    fireEvent.press(getByTestId('sidebar-finance'));
  ...
  it('closes finance screen when onClose is pressed', () => {
    const { getByTestId, getByText } = render(<AppRoot />);

    fireEvent.press(getByTestId('sidebar-finance'));
    expect(getByText('finance-visible:true')).toBeTruthy();

    fireEvent.press(getByTestId('finance-close'));
    expect(getByText('finance-visible:false')).toBeTruthy();
  });

  it('closes artifacts screen when onClose is pressed', () => {
    const { getByTestId, getByText } = render(<AppRoot />);

    fireEvent.press(getByTestId('sidebar-artifacts'));
    expect(getByText('artifacts-visible:true')).toBeTruthy();

    fireEvent.press(getByTestId('artifacts-close'));
    expect(getByText('artifacts-visible:false')).toBeTruthy();
  });
