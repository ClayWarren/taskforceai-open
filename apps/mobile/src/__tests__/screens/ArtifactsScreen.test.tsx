...
const mockFetchArtifactContentBytes = jest.fn();
const mockFetchArtifactContentText = jest.fn();
...
jest.mock('../../utils/file-system', () => ({
  ...
  writeBytesAsync: jest.fn(async () => undefined),
}));
...
jest.mock('../../hooks/api/artifacts', () => ({
  ...
  fetchArtifactContentBytes: (...args: unknown[]) => mockFetchArtifactContentBytes(...args),
  fetchArtifactContentText: (...args: unknown[]) => mockFetchArtifactContentText(...args),
...
describe('ArtifactsScreen', () => {
  beforeEach(() => {
    ...
    mockFetchArtifactContentBytes.mockResolvedValue({ ok: true, value: new Uint8Array([1, 2, 3]) });
    mockFetchArtifactContentText.mockResolvedValue({ ok: true, value: 'Preview body' });
  ...
  it('lists artifacts and writes binary content before sharing', async () => {
    const { getByLabelText, getByText } = render(<ArtifactsScreen visible={true} onClose={jest.fn()} />);

    expect(getByText('Launch brief')).toBeTruthy();
    fireEvent.press(getByLabelText('Download artifact Launch brief'));

    await waitFor(() => {
      expect(mockFetchArtifactContentBytes).toHaveBeenCalledWith('https://api.test/files/file-1');
      expect(FileSystem.writeBytesAsync).toHaveBeenCalledWith(
        'file:///mock-documents/launch-brief.pdf',
        new Uint8Array([1, 2, 3])
      );
      expect(Sharing.shareAsync).toHaveBeenCalledWith('file:///mock-documents/launch-brief.pdf');
    });
  });
  ...
  it('opens a detail view with inline text preview metadata', async () => {
    ...
    fireEvent.press(getByLabelText('View artifact Launch brief'));

    await waitFor(() => {
      expect(mockFetchArtifactContentText).toHaveBeenCalledWith('https://api.test/files/file-1');
  ...
  it('creates and copies a public link', async () => {
    ...
    const { getByLabelText } = render(<ArtifactsScreen visible={true} onClose={jest.fn()} />);

    fireEvent.press(getByLabelText('Copy public link for Launch brief'));
  ...
  it('deletes an artifact after confirmation', async () => {
    ...
    fireEvent.press(getByLabelText('Delete artifact Launch brief'));
