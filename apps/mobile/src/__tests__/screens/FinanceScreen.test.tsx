...
describe('FinanceScreen', () => {
  ...
  it('renders connected finance dashboard data', () => {
    const { getByLabelText, getByText } = render(<FinanceScreen visible={true} onClose={jest.fn()} />);
  ...
  it('syncs finance data from the action button', async () => {
    const { getByLabelText } = render(<FinanceScreen visible={true} onClose={jest.fn()} />);

    fireEvent.press(getByLabelText('Sync finance data'));
  ...
  it('confirms before disconnecting a connection', async () => {
    ...
    fireEvent.press(getByLabelText('Disconnect Demo Bank'));
  ...
  it('keeps the connect action honest until native Plaid Link exists', () => {
    const alertSpy = jest.spyOn(Alert, 'alert').mockImplementation(() => undefined);
    const { getByLabelText } = render(<FinanceScreen visible={true} onClose={jest.fn()} />);

    fireEvent.press(getByLabelText('Connect finance account'));
