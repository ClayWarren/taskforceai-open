...
describe('useMessageSender', () => {
  ...
  const handleMobileLocalMcpCommandMock = jest.mocked(handleMobileLocalMcpCommand);
  const upsertMessageMock = jest.mocked(upsertMessage);
  const fulfillPendingMcpApprovalMock = jest.mocked(fulfillPendingMcpApproval);
  ...
  it('runs local MCP commands while offline without queueing them', async () => {
    const conversation = createConversationMock();
    const streaming = createStreamingMock();
    const triggerRunTask = jest.fn<RunTaskMutation>();
    handleMobileLocalMcpCommandMock.mockResolvedValueOnce(true);
    ...
    await hook.handleSendMessage('/mcp list');

    expect(handleMobileLocalMcpCommandMock).toHaveBeenCalledWith(
    ...
    expect(triggerRunTask).not.toHaveBeenCalled();
    expect(streaming.setErrorMessage).not.toHaveBeenCalledWith(
      'Network error. Prompt saved locally for retry.'
    );
    cleanup();
  });
  ...
  it('fulfills pending MCP approval metadata returned with a task', async () => {
    const conversation = createConversationMock();
    const streaming = createStreamingMock();
    const approval = { id: 'approval-1', permission: 'write' };
    const triggerRunTask = jest.fn<RunTaskMutation>().mockResolvedValue({
    ...
    await hook.handleSendMessage('Approve the tool');
    const streamOptions = (streaming.startStreaming as jest.Mock).mock.calls[0]?.[0];
    await streamOptions.onApproval(approval);
  ...
  it('does not queue offline prompts that require MCP client tools', async () => {
    ...
    await hook.handleSendMessage('Use local MCP offline');
  ...
  it('marks private sends and skips durable assistant persistence', async () => {
    const conversation = createConversationMock();
    const streaming = createStreamingMock();
    const triggerRunTask = jest
      .fn<RunTaskMutation>()
      .mockResolvedValue({ task_id: 'task-private', status: 'completed', result: 'Private done' });
    ...
    await hook.handleSendMessage('Private prompt');
    ...
    expect(conversation.setMessages).toHaveBeenCalledWith(expect.any(Function));
  ...
  it('includes model metadata when invoking runTask', async () => {
    ...
    await hook.handleSendMessage('Model online', { modelId: 'openai/gpt-5.6-sol' });
