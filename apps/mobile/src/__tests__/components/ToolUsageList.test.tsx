...
describe('ToolUsageList', () => {
  ...
  it('returns null for empty array', () => {
    const { toJSON } = render(<ToolUsageList toolEvents={[]} />);
    expect(toJSON()).toBeNull();
  });

  it('renders default heading and hides it for embedded variant', () => {
    const events = [
    ...
    const { getByText, queryByText, rerender } = render(<ToolUsageList toolEvents={events} />);
    expect(getByText(/Tool Usage \(3\)/)).toBeTruthy();

    rerender(<ToolUsageList toolEvents={events} variant="embedded" />);
    expect(queryByText(/Tool Usage/)).toBeNull();
  });

  it('displays non-search tool metadata and errors', () => {
    const events = [
      createToolEvent({
    ...
    const { getByText } = render(<ToolUsageList toolEvents={events} />);
  ...
  it('renders specialized content for search, code, and generic events', () => {
