...
jest.mock('../../components/Sidebar.view', () => ({
  SidebarView: (props: any) => {
    ...
    return react.createElement(
      ...
      react.createElement(Text, null, `projects:${props.projects.length}`),
...
describe('Sidebar', () => {
  it('does not expose cached local or remote conversations while signed out', async () => {
    mockConversationData = [
      {
        id: 7,
    ...
    mockListConversations.mockResolvedValueOnce({
      ok: true,
      value: [localConversation('guest-local-1')],
    });

    const { getByText, queryByText } = renderSidebar({ isAuthenticated: false });

    await waitFor(() => {
      expect(getByText('filtered:0')).toBeTruthy();
    });
    expect(mockListConversations).not.toHaveBeenCalled();
    expect(latestSidebarViewProps.filteredConversations).toEqual([]);
  ...
  it('shows authenticated product sidebar rows', async () => {
    ...
    const { getByLabelText, getByText } = renderSidebar({ onDesktopSessionsPress, onArtifactsPress });

    await waitFor(() => {
      expect(getByText('Desktop')).toBeTruthy();
      expect(getByText('Artifacts')).toBeTruthy();
    });
    fireEvent.press(getByLabelText('Open Artifacts'));
    fireEvent.press(getByLabelText('Open Desktop'));
  ...
  it('filters archived mirrored remote conversations from the active list', async () => {
    ...
    const { getByText } = renderSidebar({ isAuthenticated: true });
  ...
  it('maps negative local ids back to local conversation id when selecting', async () => {
    ...
    const { getByTestId, getByText } = renderSidebar();

    await waitFor(() => {
      expect(getByText('filtered:1')).toBeTruthy();
    });

    fireEvent.press(getByTestId('sidebar-view-select-first'));
  ...
  it('ingests remote summaries only when timestamps change', async () => {
    ...
    const { rerender } = renderSidebar({ isAuthenticated: true });
  ...
  it('deletes local conversations via confirmation dialog', async () => {
    ...
    fireEvent.press(getByTestId('sidebar-view-delete-first'));
  ...
  it('deletes mirrored local remote conversation after remote delete succeeds', async () => {
    ...
    const { getByTestId, getByText } = renderSidebar({ isAuthenticated: true });
  ...
  it('loads projects when authenticated and visible, and opens projects modal on manage action', async () => {
    mockUseProjectsQuery.mockReturnValueOnce({
      data: [
    ...
    await waitFor(() => {
      expect(getByText('projects:2')).toBeTruthy();
    });

    fireEvent.press(getByTestId('sidebar-view-manage-projects'));

    expect(getByText('projects-visible:true')).toBeTruthy();

    fireEvent.press(getByTestId('projects-screen-select'));
