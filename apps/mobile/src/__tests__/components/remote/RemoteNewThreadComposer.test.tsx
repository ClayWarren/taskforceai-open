...
jest.mock('../../../features/desktop-work/data/desktop-work', () => ({
  useAttachDesktopWorkspaceMutation: () => ({ mutate: jest.fn(), isPending: false }),
  useCreateDesktopWorktreeMutation: () => ({ mutate: jest.fn(), isPending: false }),
  useDesktopGitBranchesQuery: () => ({ data: { branches: [] } }),
  useDesktopGitStatusQuery: () => ({
    data: mockGitBranch ? { branch: mockGitBranch } : undefined,
    isLoading: mockGitLoading,
  }),
  useDesktopGitWorktreesQuery: () => ({ data: { worktrees: [] } }),
  useDesktopHostsQuery: () => ({ data: [] }),
  useDesktopSkillsQuery: () => ({ data: { skills: [] } }),
  useDesktopWorkspaceFilesQuery: () => ({ data: { files: [] }, isLoading: false }),
  useSelectDesktopHostMutation: () => ({ mutate: jest.fn(), isPending: false }),
