...
const mockReviewActionMutate = jest.fn();
const mockGitFinishMutate = jest.fn();
...
let mockGitStatusState: Record<string, unknown>;
let mockGitBranchesState: Record<string, unknown>;
...
jest.mock('../../../contexts/PreferencesContext', () => ({
  usePreferences: () => ({ remoteCodeScale: 1, remoteWordWrap: false }),
}));
...
jest.mock('../../../features/desktop-work/data/desktop-work', () => ({
  ...
  useDesktopGitStatusQuery: () => mockGitStatusState,
  useDesktopGitBranchesQuery: () => mockGitBranchesState,
  useDesktopGitFinishMutation: () => ({ mutate: mockGitFinishMutate, isPending: false, error: null }),
  useDesktopReviewActionMutation: () => ({ mutate: mockReviewActionMutate, isPending: false, error: null }),
...
import { RemoteActionIcon, RemoteActionPill, RemoteErrorText, RemoteStatusText } from '../../../features/desktop-work/components/RemoteControls';
import {
  RemoteFilesPanel,
  sourceSegments,
} from '../../../features/desktop-work/components/RemoteFilesPanel';
import { RemoteInteractionCards } from '../../../features/desktop-work/components/RemoteInteractionCards';
import { RemoteGitPanel } from '../../../features/desktop-work/components/RemoteGitPanel';
import {
  RemoteChangeSummaryPill,
  RemoteReviewPanel,
  reviewRangeTarget,
  summarizeDesktopReview,
} from '../../../features/desktop-work/components/RemoteReviewPanel';
import { RemoteThreadActions } from '../../../features/desktop-work/components/RemoteThreadActions';
import { RemoteThreadActivity } from '../../../features/desktop-work/components/RemoteThreadActivity';
import { RemoteThreadDetail } from '../../../features/desktop-work/components/RemoteThreadDetail';
...
const screen = {
  ...
  queryByLabelText: (...args: Parameters<typeof currentRender.queryByLabelText>) =>
    currentRender.queryByLabelText(...args),
...
describe('Remote panels', () => {
  beforeEach(() => {
    ...
    mockGitStatusState = { isLoading: false, error: null, data: null };
    mockGitBranchesState = { isLoading: false, error: null, data: { branches: [] } };
    mockGitFinishMutate.mockImplementation((_input, options: { onSuccess?: (result: { message: string }) => void }) =>
      options?.onSuccess?.({ message: 'Done.' })
    );
  });
  ...
  it('opens the changes bottom sheet from the compact diff pill', async () => {
    ...
    expect(reviewRangeTarget('src/app.ts', 18, 12)).toEqual({
      path: 'src/app.ts',
      line: 12,
      endLine: 18,
    });
    expect(sourceSegments('const ready = true; // status')).toEqual(
      expect.arrayContaining([
        { text: 'const', color: '#c4b5fd' },
        { text: 'true', color: '#93c5fd' },
  ...
  it('browses remote workspace files and preview states', async () => {
    ...
    await fireEvent.press(screen.getByLabelText('Copy remote file contents'));
    ...
    await rerender(<RemoteFilesPanel />);
    expect(screen.getByLabelText('Preview of src/app.ts')).toBeTruthy();
    mockFileState = {
      isLoading: false,
      error: null,
      data: {
    ...
    await rerender(<RemoteFilesPanel />);
    expect(screen.getByText('This image is larger than the 1 MB preview limit.')).toBeTruthy();
    expect(screen.queryByLabelText('Preview of src/app.ts')).toBeNull();
  ...
  it('runs the paired-desktop Git finish loop from a code workspace', async () => {
    mockGitStatusState = {
      isLoading: false,
      error: null,
      data: {
        ...
        files: [
          { path: 'src/app.ts', staged: true, unstaged: false, untracked: false },
          { path: 'src/view.tsx', staged: false, unstaged: true, untracked: false },
        ],
        pullRequest: null,
      },
    };
    mockGitBranchesState = {
      isLoading: false,
      error: null,
      data: { branches: [{ name: 'main', current: true, remote: false }, { name: 'feature', current: false, remote: false }] },
    };
    jest.spyOn(Alert, 'alert').mockImplementation((_title, _message, buttons) => buttons?.[1]?.onPress?.());
    await renderRemote(<RemoteGitPanel workspace="/workspace" />);

    await fireEvent.press(screen.getByText('Stage all'));
    expect(mockReviewActionMutate).toHaveBeenCalledWith({ kind: 'stage', workspace: '/workspace', paths: ['src/view.tsx'], staged: true });
    await fireEvent.changeText(screen.getByLabelText('Remote Git commit message'), 'Ship mobile Git');
    await fireEvent.press(screen.getByText('Commit staged changes'));
    expect(mockGitFinishMutate).toHaveBeenCalledWith(
      { kind: 'commit', workspace: '/workspace', message: 'Ship mobile Git' },
      expect.any(Object)
    );
    await fireEvent.press(screen.getByText('Push'));
    expect(mockGitFinishMutate).toHaveBeenCalledWith(
      { kind: 'push', workspace: '/workspace' },
      expect.any(Object)
    );
    await fireEvent.press(screen.getByLabelText('Check out feature'));
    expect(mockGitFinishMutate).toHaveBeenCalledWith(
      { kind: 'checkout', workspace: '/workspace', branch: 'feature', remote: false },
      expect.any(Object)
    );
  });
