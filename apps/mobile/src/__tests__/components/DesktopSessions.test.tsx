...
describe('DesktopSessions', () => {
  ...
  it('returns null when there are no sessions and empty state is not requested', () => {
    mockUseDesktopSessionsQuery.mockReturnValue({
      data: [],
      refetch: mockRefetch,
    } as any);

    const { toJSON } = render(<DesktopSessions />);
  ...
  it('renders the empty state when requested', () => {
    ...
    const { getByText } = render(<DesktopSessions showEmpty />);

    expect(getByText('No desktop work is active.')).toBeTruthy();
    expect(getByText('0 active')).toBeTruthy();
  });

  it('opens the desktop work surface from the sidebar empty state', () => {
    mockUseDesktopSessionsQuery.mockReturnValue({
      data: [],
      refetch: mockRefetch,
    } as any);
    ...
    fireEvent.press(getAllByLabelText('Open desktop work')[0]);

    expect(onOpen).toHaveBeenCalledTimes(1);
  });

  it('renders pending desktop session details and refreshes on request', () => {
    const { getByLabelText, getByText } = render(<DesktopSessions />);
    ...
    fireEvent.press(getByLabelText('Refresh desktop sessions'));

    expect(mockRefetch).toHaveBeenCalledTimes(1);
  });

  it('opens the desktop work surface from a sidebar session row', () => {
    const onOpen = jest.fn();
    const { getByLabelText } = render(<DesktopSessions variant="sidebar" onOpen={onOpen} />);

    fireEvent.press(getByLabelText('Open desktop work: Deploy the preview build'));

    expect(onOpen).toHaveBeenCalledTimes(1);
  });

  it('submits approve and deny decisions from the approval controls', () => {
    const { getByLabelText } = render(<DesktopSessions />);

    fireEvent.press(getByLabelText('Approve desktop session action'));
    fireEvent.press(getByLabelText('Deny desktop session action'));
