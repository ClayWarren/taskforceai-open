...
const mockFileSystemGetInfoAsync = jest.fn().mockResolvedValue({ exists: false });
const mockFileSystemDeleteAsync = jest.fn().mockResolvedValue(undefined);
...
jest.mock('../../utils/file-system', () => ({
  ...
  getInfoAsync: mockFileSystemGetInfoAsync,
  deleteAsync: mockFileSystemDeleteAsync,
}));
...
describe('ErrorBoundary', () => {
  ...
  it('hard reset clears auth token before reloading', async () => {
    let renderer: TestRenderer.ReactTestRenderer;
    ...
    expect(mockFileSystemGetInfoAsync).toHaveBeenCalledWith('/tmp/SQLite/taskforceai.db');
