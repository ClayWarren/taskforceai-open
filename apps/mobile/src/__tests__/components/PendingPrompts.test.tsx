...
describe('PendingPrompts', () => {
    it('returns null when loading (no spinner)', async () => {
        mockUsePendingPromptsQuery.mockReturnValue({
            data: undefined,
            isLoading: true,
        } as any);

        const { toJSON } = await render(<PendingPrompts />);
        expect(toJSON()).toBeNull();
    });

    it('returns null when no pending prompts', async () => {
        mockUsePendingPromptsQuery.mockReturnValue({
            data: [],
            isLoading: false,
        } as any);
    ...
    it('renders pending prompts with status counts', async () => {
        mockUsePendingPromptsQuery.mockReturnValue({
            data: samplePrompts,
            isLoading: false,
        } as any);

        const { getByText } = await render(<PendingPrompts />);
        ...
        expect(
            getByText('Queued prompts retry automatically. Failed prompts stay saved until removed.')
        ).toBeTruthy();
    });

    it('shows modelId when present', async () => {
        mockUsePendingPromptsQuery.mockReturnValue({
            data: samplePrompts,
            isLoading: false,
        } as any);
    ...
    it('calls clearAll when Clear All is pressed', async () => {
        ...
        const { getByLabelText } = await render(<PendingPrompts />);
        await act(async () => {
            await fireEvent.press(getByLabelText('Clear All'));
        });
    ...
    it('handles remove with non-number id gracefully', async () => {
        ...
        const { getByText, getByLabelText } = await render(<PendingPrompts />);
        expect(getByText('No id')).toBeTruthy();
        fireEvent.press(getByLabelText('Remove prompt'));
        expect(mockRemoveMutateAsync).not.toHaveBeenCalled();
    });

    it('removes a saved prompt', async () => {
        mockUsePendingPromptsQuery.mockReturnValue({
            data: [samplePrompts[0]],
            isLoading: false,
        } as any);
        const first = await render(<PendingPrompts />);

        await act(async () => {
            fireEvent.press(first.getByLabelText('Remove prompt'));
    ...
    it('uses the failed-only retention hint', async () => {
        mockUsePendingPromptsQuery.mockReturnValue({
            data: [samplePrompts[2]],
            isLoading: false,
        } as any);

        const { getByText } = await render(<PendingPrompts />);

        expect(getByText('Failed prompts stay saved until removed.')).toBeTruthy();
    });

    it('contains a saved prompt removal failure', async () => {
        mockRemoveMutateAsync.mockRejectedValueOnce(new Error('remove failed'));
        mockUsePendingPromptsQuery.mockReturnValue({
            data: [samplePrompts[0]],
            isLoading: false,
        } as any);
