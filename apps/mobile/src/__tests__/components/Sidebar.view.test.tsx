...
describe('SidebarView', () => {
  it('renders unauthenticated empty state', async () => {
    const props = createProps({
      isAuthenticated: false,
      filteredConversations: [],
    });

    const { getByText } = await render(<SidebarView {...props} />);
  ...
  it('renders authenticated search empty state', async () => {
    const props = createProps({
      searchQuery: 'missing',
      filteredConversations: [],
    });
  ...
  it('handles clear search and new chat actions', async () => {
    ...
    const { getByLabelText } = await render(<SidebarView {...props} />);
  ...
  it('handles conversation press and long press deletion', async () => {
    ...
    const { getByLabelText, getByText } = await render(<SidebarView {...props} />);

    const conversation = getByLabelText('Alpha summary');
    await fireEvent.press(conversation);
    await fireEvent(conversation, 'longPress');
  ...
  it('renders project actions, settings footer, and pagination footer', async () => {
    ...
    const { getByLabelText, getByText, getByTestId } = await render(<SidebarView {...props} />);

    await fireEvent.press(getByLabelText('General project'));
    await fireEvent.press(getByLabelText('Project Project Eleven'));
    await fireEvent.press(getByLabelText('Manage projects'));
    await fireEvent.press(getByLabelText('Open settings'));
    await fireEvent.press(getByTestId('flash-list-end-reached'));
  ...
  it('hides project actions but keeps settings visible for guest users', async () => {
    const onSettingsPress = jest.fn();
    const onManageProjects = jest.fn();
    ...
    const { getByLabelText, getByText, queryByLabelText } = await render(<SidebarView {...props} />);
  ...
  it('renders desktop sessions slot inside the sidebar', async () => {
    const props = createProps({
  ...
  it('keeps settings available when authenticated user metadata is not available yet', async () => {
    const props = createProps({
      isAuthenticated: true,
      userName: undefined,
      userInitials: undefined,
