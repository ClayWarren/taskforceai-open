...
describe('AgentStatusMessage', () => {
  ...
  it('returns null for null/undefined message', () => {
    const { toJSON } = render(<AgentStatusMessage message={null as any} />);
    expect(toJSON()).toBeNull();
  });

  it('renders baseline agent execution without optional lists or errors', () => {
  ...
  it('routes agent statuses and suppresses fallback tool list', () => {
  ...
  it('renders fallback tools, sources, and error when no agent statuses are present', () => {
    const message = createMessage({
      toolEvents: [{ toolName: 'test', success: true } as any],
      sources: [{ url: 'https://example.com' }],
      error: 'Warning',
    });
    const { getByTestId, getByText } = render(<AgentStatusMessage message={message} />);
    expect(getByTestId('agent-execution')).toBeTruthy();
  ...
  it('handles empty and undefined agent statuses', () => {
    ...
    rerender(<AgentStatusMessage message={createMessage({ agentStatuses: undefined })} />);
    expect(getByTestId('agent-execution')).toBeTruthy();
  });
});
