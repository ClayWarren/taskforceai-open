...
describe('MoreOptionsSheet', () => {
  it('calls mode toggle handlers from option rows', () => {
    ...
    const { getByLabelText } = render(<MoreOptionsSheet {...props} />);

    fireEvent.press(getByLabelText('Autonomous. Self-directed task execution'));
    fireEvent.press(getByLabelText('Direct Chat. Standard single-assistant responses'));
    fireEvent.press(getByLabelText('Computer Use. Enable desktop automation'));
  ...
  it('renders free-plan agent count options and applies selection', () => {
  ...
  it('renders extended agent count options for super plan', () => {
  ...
  it('invokes delayed custom models and budget actions after closing', () => {
    jest.useFakeTimers();
    ...
    fireEvent.press(getByLabelText('Custom Models - Assign models to agent roles'));
    fireEvent.press(getByLabelText('Set Budget - Configure autonomous spending limit'));
  ...
  it('hides optional rows when feature callbacks are not provided', () => {
