...
jest.mock('../../components/MessageBubble/MessageActions', () => ({
  MessageActions: ({ onSpeakPress, onCopyPress, onSharePress, onRatingPress, rating: _rating, copied, isSpeaking }: any) => {
    const ReactMod = require('react');
    const { Text: TextComp, View: ViewComp, TouchableOpacity } = require('react-native');
    return ReactMod.createElement(ViewComp, { testID: 'message-actions' },
      ReactMod.createElement(TouchableOpacity, { onPress: onSpeakPress, testID: 'speak-btn' },
      ...
      ReactMod.createElement(TouchableOpacity, { onPress: onSharePress, testID: 'share-btn' },
        ReactMod.createElement(TextComp, null, 'Share')
      ),
      ReactMod.createElement(TouchableOpacity, { onPress: () => onRatingPress(1), testID: 'rate-up' },
        ReactMod.createElement(TextComp, null, 'Up')
      ),
      ReactMod.createElement(TouchableOpacity, { onPress: () => onRatingPress(-1), testID: 'rate-down' },
...
describe('StandardMessage', () => {
  ...
  it('returns null when message is null', () => {
    const { toJSON } = render(<StandardMessage message={null as any} />);
    expect(toJSON()).toBeNull();
  });

  it('renders assistant message with actions, tools, sources, and rating updates', () => {
    ...
    const { getByText, getByTestId, rerender } = render(<StandardMessage message={message} />);
    ...
    rerender(<StandardMessage message={{ ...message, rating: 1 }} />);
    expect(getByTestId('message-actions')).toBeTruthy();
  });

  it('renders user message without assistant-only affordances', () => {
    const message = createMessage({
      role: 'user',
      sources: [{ url: 'https://example.com' }],
      toolEvents: [{ toolName: 'test', success: true } as any],
  ...
  it('omits empty or undefined tool/source lists', () => {
    const message = createMessage({
      role: 'assistant',
      sources: [],
      toolEvents: [],
    });
    const { queryByTestId } = render(<StandardMessage message={message} />);
    expect(queryByTestId('tool-usage')).toBeNull();
    expect(queryByTestId('sources-list')).toBeNull();
    ...
    const omitted = render(<StandardMessage message={omittedMessage} />);
    expect(omitted.queryByTestId('tool-usage')).toBeNull();
    expect(omitted.queryByTestId('sources-list')).toBeNull();
  });

  it('wires assistant action controls', () => {
    const message = createMessage({ role: 'assistant' });
    const { getByTestId } = render(<StandardMessage message={message} />);
    fireEvent.press(getByTestId('copy-btn'));
    fireEvent.press(getByTestId('speak-btn'));
    fireEvent.press(getByTestId('share-btn'));
    fireEvent.press(getByTestId('rate-up'));
  });

  it('renders realtime assistant transcripts without a bubble and with icon-only actions', () => {
    const message = createMessage({
    ...
    fireEvent.press(getByLabelText('Message details'));
    expect(queryByTestId('message-actions')).toBeNull();
  });
});
