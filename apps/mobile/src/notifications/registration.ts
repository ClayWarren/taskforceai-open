...
export async function ensurePushRegistration(
  ...
  try {
    ...
    await syncTokenWithDesktop(token);
...
const readStoredPushToken = (): Promise<string | null> =>
  AsyncStorage.getItem(PUSH_TOKEN_STORAGE_KEY);

export const syncStoredPushTokenWithDesktop = async (): Promise<void> => {
  try {
    const token = await readStoredPushToken();
    if (token) await syncTokenWithDesktop(token);
  } catch (error) {
    logger.warn('Stored push token could not be registered with paired desktops', { error });
  }
};
...
export async function unregisterPushNotifications(): Promise<void> {
  ...
  try {
    const desktopHosts = await readDesktopPairingHosts();
    await Promise.allSettled(
      desktopHosts.map((host) => unregisterDesktopRemotePushToken(host.session))
    );
...
async function syncTokenWithDesktop(token: string): Promise<void> {
  const hosts = await readDesktopPairingHosts();
  const sessions = hosts.length > 0
    ? hosts.map((host) => host.session)
    : [await readDesktopPairingSession()].filter((session) => session !== null);
  const results = await Promise.allSettled(
    sessions.map((session) => registerDesktopRemotePushToken(session, token))
  );
  const failures = results.filter((result) => result.status === 'rejected');
  if (failures.length > 0) {
    logger.warn('Some paired desktops could not register Remote notifications', {
      failedHostCount: failures.length,
    });
  }
}
