...
bun run eas build --profile development --platform ios
```
...
# Configure project
bun run eas build:configure

# Build for iOS
bun run eas build --profile development --platform ios

# Build for Android
bun run eas build --profile development --platform android
```
...
bun run eas build --profile production --platform ios
...
bun run eas build --profile production --platform android
...
- **When to rebuild:** Add new native modules or change `app.config`? Run a fresh `bun run eas build` for each platform and submit through the stores.
