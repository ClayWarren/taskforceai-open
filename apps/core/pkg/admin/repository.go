...
type UpdateAdminUserFieldsInput = db.UpdateAdminUserFieldsParams
...
type AdminQueries interface {
	...
	UpdateAdminUserFields(ctx context.Context, arg UpdateAdminUserFieldsInput) error
...
func (r *RepositoryImpl) ListUsers(ctx context.Context, limit, offset int) (*AdminUsersPage, error) {
	limit32, offset32, err := paginationInt32(limit, offset)
	if err != nil {
		return nil, err
	}
...
func (r *RepositoryImpl) FetchInsightsData(ctx context.Context, since24h, since5m time.Time) (*AdminInsightsData, error) {
	...
	fetchAsync(g, gCtx, &userStats, func() (UserStatsRow, error) { return r.q.GetUserStats(gCtx) })
	fetchAsync(g, gCtx, &messages24h, func() (int64, error) { return r.q.CountMessagesSince(gCtx, since24hTS) })
	fetchAsync(g, gCtx, &conversationAgg, func() (ConversationAggregateRow, error) {
		return r.q.GetConversationAggregateSince(gCtx, since24hTS)
	})
	fetchAsync(g, gCtx, &modelUsageRows, func() ([]ModelUsageRow, error) {
		return r.q.GetModelUsageSince(gCtx, TimestampLimitInput{Timestamp: since24hTS, Limit: 10})
	})
	fetchAsync(g, gCtx, &slowestRows, func() ([]SlowConversationRow, error) {
		return r.q.GetSlowestConversationsSince(gCtx, TimestampLimitInput{Timestamp: since24hTS, Limit: 10})
	})
	fetchAsync(g, gCtx, &inProgress, func() (int64, error) {
		return r.q.CountInProgressConversationsSince(gCtx, since5mTS)
	})
	fetchAsync(g, gCtx, &planCountsRows, func() ([]PlanCountRow, error) { return r.q.GetPlanCounts(gCtx) })
	fetchAsync(g, gCtx, &topUsersRows, func() ([]TopUserMessageCountRow, error) {
		return r.q.GetTopUsersByMessageCount(gCtx, 10)
	})
	fetchAsync(g, gCtx, &tokens24h, func() (TokenAggregateRow, error) {
		return r.q.GetTokenAggregateSince(gCtx, since24hTS)
	})
	fetchAsync(g, gCtx, &tokensAllTime, func() (TokenAggregateAllTimeRow, error) {
		return r.q.GetTokenAggregateAllTime(gCtx)
	})
	fetchAsync(g, gCtx, &tokensByModel, func() ([]TokensByModelRow, error) {
		return r.q.GetTokensByModelSince(gCtx, CreatedAtLimitInput{CreatedAt: since24hTS, Limit: 10})
	})
	fetchAsync(g, gCtx, &toolUsageRows, func() ([]ToolUsageRow, error) {
		return r.q.GetToolUsageSince(gCtx, since24hTS)
	})
	fetchAsync(g, gCtx, &toolSuccessRows, func() ([]ToolSuccessRow, error) {
...
func (r *RepositoryImpl) UpdateUserPlan(ctx context.Context, email, plan string) (*AdminDashboardUser, error) {
	...
	return adminDashboardUserSummary(u), nil
}
...
func (r *RepositoryImpl) UpdateUser(ctx context.Context, input AdminUserUpdate) error {
	err := r.q.UpdateAdminUserFields(ctx, UpdateAdminUserFieldsInput{
		UserID:  input.UserID,
		Email:   input.Email,
		Plan:    input.Plan,
	...
	if err != nil {
		slog.Error("Failed to atomically update admin user fields", "userId", input.UserID, "email", input.Email, "error", err)
	}
	return err
}
...
func adminDashboardUserSummary(u AdminUserRecord) *AdminDashboardUser {
	return &AdminDashboardUser{ID: int(u.ID), Email: u.Email, Plan: &u.Plan, IsAdmin: u.IsAdmin}
}
...
func fetchAsync[T any](group *errgroup.Group, ctx context.Context, target *T, query func() (T, error)) {
	group.Go(func() error {
		value, err := postgres.WithRetry(ctx, query)
		*target = value
		return err
	})
}

func paginationInt32(limit, offset int) (int32, int32, error) {
	limit32, err := toInt32(limit, "limit")
	if err != nil {
		return 0, 0, err
	}
	offset32, err := toInt32(offset, "offset")
	return limit32, offset32, err
}
