...
func TestRepository_UpdateUser_UsesSingleAtomicQuery(t *testing.T) {
	...
	repo = NewRepository(&mockAdminQueries{
		updateAdminUserFieldsFunc: func(context.Context, UpdateAdminUserFieldsInput) error {
			return expectedErr
		},
	})
	require.ErrorIs(t, repo.UpdateUser(context.Background(), AdminUserUpdate{Email: "user@example.com"}), expectedErr)
}
