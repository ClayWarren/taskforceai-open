...
func httpTriggerForTest(ctx context.Context, url, token string) func(string, string) error {
	trigger := newHTTPTrigger(url, token)
	return func(agentID, reason string) error {
		return trigger(ctx, agentID, reason)
	}
}

func TestHTTPTrigger_Success(t *testing.T) {
	...
	trigger := httpTriggerForTest(context.Background(), server.URL, "test-token")
...
func TestHTTPTrigger_NoToken(t *testing.T) {
	var receivedAuth string
	...
	trigger := httpTriggerForTest(context.Background(), server.URL, "")
...
func TestHTTPTrigger_DrainsResponseBodyForConnectionReuse(t *testing.T) {
...
func TestHTTPTrigger_Non2xxStatus(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestHTTPTrigger_RetriesRetryableStatusAndSucceeds(t *testing.T) {
...
func TestHTTPTrigger_CanceledParentContextFailsFast(t *testing.T) {
	...
	trigger := httpTriggerForTest(ctx, server.URL, "")
	if err := trigger("agent-1", "test"); err == nil {
...
func TestHTTPTrigger_CancellationDuringRetryBackoff(t *testing.T) {
	...
	originalJitter := secureJitterInt
	secureJitterInt = func(io.Reader, *big.Int) (*big.Int, error) { return big.NewInt(0), nil }
	t.Cleanup(func() { secureJitterInt = originalJitter })

	ctx, cancel := context.WithCancel(context.Background())
	done := make(chan error, 1)
	go func() { done <- newHTTPTrigger(server.URL, "")(ctx, "agent", "timer") }()
	<-requested
...
func TestHTTPTrigger_RetriesOwnRequestDeadline(t *testing.T) {
	originalTimeout := httpTriggerRequestTimeout
	originalJitter := secureJitterInt
	t.Cleanup(func() {
		httpTriggerRequestTimeout = originalTimeout
		secureJitterInt = originalJitter
	})
	httpTriggerRequestTimeout = 5 * time.Millisecond
	...
	trigger := httpTriggerForTest(context.Background(), server.URL, "")
	if err := trigger("agent-1", "heartbeat"); !errors.Is(err, context.DeadlineExceeded) {
		t.Fatalf("Expected request deadline error after retries, got %v", err)
	}
	if got := calls.Load(); got < 2 || got > 3 {
		t.Fatalf("Expected own request deadline to retry, got %d observed server calls", got)
	}
}

func TestHTTPTrigger_OpensCircuitAfterRetryableFailures(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestHTTPTrigger_InvalidURL(t *testing.T) {
	trigger := httpTriggerForTest(context.Background(), "http://invalid-url-that-does-not-exist:99999", "")
	err := trigger("agent-1", "test")
...
func TestHTTPTrigger_InvalidRequestURL(t *testing.T) {
	trigger := httpTriggerForTest(context.Background(), "://bad-url", "")
	err := trigger("agent-1", "test")
...
func TestHTTPTrigger_NonRetryableClientErrorDoesNotRetry(t *testing.T) {
	var calls atomic.Int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls.Add(1)
...
func TestHTTPTrigger_NonRetryableClientErrorsDoNotOpenCircuit(t *testing.T) {
	var calls atomic.Int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls.Add(1)
		var payload struct {
...
func TestHTTPTrigger_SuccessResetsCircuitFailures(t *testing.T) {
	var calls atomic.Int32
	success := atomic.Bool{}
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls.Add(1)
