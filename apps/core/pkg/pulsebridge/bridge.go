...
const cronPersistenceTimeout = 5 * time.Second
...
type Bridge struct {
	...
	triggerWithContext contextInteractionTrigger
...
func NewBridgeWithRedis(parentCtx context.Context, db DB, redis Redis, engineURL, engineToken string) *Bridge { //nolint:contextcheck // Nil is a supported compatibility fallback.
	...
	triggerWithContext := newHTTPTrigger(engineURL, engineToken)
	trigger := func(agentID, reason string) error {
		return triggerWithContext(ctx, agentID, reason)
	}
	b.trigger = trigger
	b.triggerWithContext = triggerWithContext
...
func (b *Bridge) statusCheck(agentID string) bool {
	return b.statusCheckContext(b.ctx, agentID)
}

func (b *Bridge) statusCheckContext(ctx context.Context, agentID string) bool {
	if b.redis != nil {
		// Use a fast path if redis is available
		status, err := b.redis.Get(ctx, "agent_status:"+agentID)
...
func (b *Bridge) CronTick(ctx context.Context) error {
	...
	for _, agent := range agents {
		...
		if b.statusCheckContext(ctx, agent.ID) {
		...
		handler.Go("pulseHeartbeat_"+agent.ID, func() {
			...
			case <-ctx.Done():
				errMu.Lock()
				runErrs = append(runErrs, fmt.Errorf("schedule %s: %w", agent.ID, ctx.Err()))
				errMu.Unlock()
				return
			}
			defer func() { <-sem }()
			...
			claimed, err := b.db.ClaimAgentPulse(ctx, ClaimAgentPulseInput{
				ID:        agent.ID,
			...
			if err := b.triggerWithContext(ctx, agent.ID, "heartbeat"); err != nil {
			...
			// Give final persistence a short cleanup window after a successful
			// trigger, even if the incoming request disconnects.
			persistCtx, persistCancel := context.WithTimeout(context.WithoutCancel(ctx), cronPersistenceTimeout)
			defer persistCancel()
			if err := b.db.UpdateAgentPulseState(persistCtx, UpdateAgentPulseStateInput{
				ID:        agent.ID,
