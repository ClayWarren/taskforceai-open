...
func setBridgeTrigger(b *Bridge, trigger func(string, string) error) {
	b.trigger = trigger
	b.triggerWithContext = func(_ context.Context, agentID, reason string) error {
		return trigger(agentID, reason)
	}
}
...
func TestBridge_CronTick_PersistsWithBoundedContextAfterRequestCancellation(t *testing.T) {
	m := new(MockDB)
	b := NewBridgeWithRedis(context.Background(), m, nil, "http://localhost:8080", "token")
	reqCtx, cancel := context.WithCancel(context.Background())
	setBridgeTrigger(b, func(_ string, _ string) error {
		cancel()
		return nil
	})
...
func TestBridge_CronTick_CanceledRequestStopsBeforeClaim(t *testing.T) {
	m := new(MockDB)
	b := NewBridgeWithRedis(context.Background(), m, nil, "http://localhost:8080", "token")
	agents := []AgentRecord{{
		ID:            "agent-1",
	...
	require.ErrorIs(t, b.CronTick(reqCtx), context.Canceled)
	m.AssertNotCalled(t, "ClaimAgentPulse", mock.Anything, mock.Anything)
}

func TestBridge_CronTick_ReturnsErrorWhenHeartbeatFails(t *testing.T) {
	m := new(MockDB)
	b := NewBridgeWithRedis(context.Background(), m, nil, "http://localhost:8080", "token")
	setBridgeTrigger(b, func(_ string, _ string) error { return fmt.Errorf("engine unavailable") })
...
func TestBridge_CronTick_ReturnsPersistError(t *testing.T) {
	m := new(MockDB)
	b := NewBridgeWithRedis(context.Background(), m, nil, "http://localhost:8080", "token")
	setBridgeTrigger(b, func(_ string, _ string) error { return nil })
...
func TestNewBridgeInitializesRunner(t *testing.T) {
	...
	assert.False(t, b.statusCheck("agent-1"))
	b.Stop()
	require.Error(t, b.trigger("agent-1", "test"))
}

func TestBridge_CronTick_CancelsWorkWaitingForConcurrencySlot(t *testing.T) {
	m := new(MockDB)
	b := NewBridgeWithRedis(context.Background(), m, nil, "u", "t")
	release := make(chan struct{})
	started := make(chan struct{}, 10)
	setBridgeTrigger(b, func(string, string) error {
		started <- struct{}{}
		<-release
		return nil
	})
	agents := make([]AgentRecord, 11)
	for i := range agents {
		agents[i] = AgentRecord{ID: fmt.Sprintf("agent-%d", i), Timezone: "UTC", ActiveStart: "00:00", ActiveEnd: "23:59", ActiveDays: []int32{0, 1, 2, 3, 4, 5, 6}, CheckInterval: 300}
	}
	m.On("ListAgentsDueForPulse", mock.Anything).Return(agents, nil).Once()
	m.On("ClaimAgentPulse", mock.Anything, mock.AnythingOfType("pulsebridge.ClaimAgentPulseInput")).Return(true, nil).Times(10)
	m.On("UpdateAgentPulseState", mock.Anything, mock.AnythingOfType("pulsebridge.UpdateAgentPulseStateInput")).Return(nil).Times(10)

	ctx, cancel := context.WithCancel(context.Background())
	done := make(chan error, 1)
	go func() { done <- b.CronTick(ctx) }()
	for range 10 {
		<-started
	}
	cancel()
