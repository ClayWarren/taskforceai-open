...
func RegisterHandlers(api huma.API, checkDatabase func(context.Context) error) {
	...
	huma.Register(api, huma.Operation{
		...
		var options []contractspkg.ModelOptionSummary
		if err := copyModelOptions(&options, &cfg.Models.Options); err != nil {
			return nil, huma.Error500InternalServerError("Failed to map models: " + err.Error())
		}
