...
type Service interface {
	ExportUserDataByEmail(ctx context.Context, email string) (platform.GdprExport, error)
...
func RegisterHandlers(api huma.API, svc Service) {
	...
	huma.Register(api, huma.Operation{
		...
		export, err := svc.ExportUserDataByEmail(ctx, input.User.Email)
		if err != nil {
			slog.Error("GDPR export failed", "email", input.User.Email, "error", err)
