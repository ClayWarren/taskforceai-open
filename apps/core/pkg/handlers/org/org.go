...
func registerUpdateMemberRole(api huma.API, service identity.Service) {
	...
	huma.Register(api, huma.Operation{
		...
		if err := service.UpdateMemberRole(ctx, *ids.OrgID32, input.UserID, ids.UserID32, input.Body.Role); err != nil {
			...
			if errors.Is(err, identity.ErrUnauthorized) || errors.Is(err, identity.ErrOwnerRoleRequiresOwner) || errors.Is(err, identity.ErrOwnerRoleProtected) {
				return nil, huma.Error403Forbidden(err.Error())
			}
			if errors.Is(err, identity.ErrCannotRemoveLastOwner) {
...
func registerRemoveMember(api huma.API, service identity.Service) {
	...
	huma.Register(api, huma.Operation{
		...
		if err := service.RemoveMember(ctx, *ids.OrgID32, input.UserID, ids.UserID32); err != nil {
			if errors.Is(err, identity.ErrUnauthorized) || errors.Is(err, identity.ErrCannotRemoveSelf) || errors.Is(err, identity.ErrOwnerRoleProtected) {
				return nil, huma.Error403Forbidden(err.Error())
			}
			if errors.Is(err, identity.ErrCannotRemoveLastOwner) {
				return nil, huma.Error409Conflict(err.Error())
