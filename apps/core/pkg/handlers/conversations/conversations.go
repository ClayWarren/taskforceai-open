...
func conversationResponse(conversation conversations.ConversationApiView) ConversationResponse {
	return ConversationResponse{
		ID: conversation.ID, Timestamp: conversation.Timestamp, UserInput: conversation.UserInput,
		Result: conversation.Result, ExecutionTime: conversation.ExecutionTime, Model: conversation.Model,
		AgentCount: conversation.AgentCount, ProjectID: conversation.ProjectID, Sources: conversation.Sources,
		AgentStatuses: conversation.AgentStatuses, ToolEvents: conversation.ToolEvents,
	}
}

func conversationResponses(conversations []conversations.ConversationApiView) []ConversationResponse {
	responses := make([]ConversationResponse, len(conversations))
	for i, conversation := range conversations {
		responses[i] = conversationResponse(conversation)
	}
	return responses
}
...
func RegisterHandlers(api huma.API, service conversations.Service) {
	...
	huma.Register(api, huma.Operation{
		...
		return &struct{ Body ConversationsListResponse }{Body: ConversationsListResponse{
			Conversations: conversationResponses(page.Conversations),
			Total:         page.Total,
			Limit:         page.Limit,
			Offset:        page.Offset,
	...
	huma.Register(api, huma.Operation{
		...
		return &struct{ Body ConversationResponse }{Body: conversationResponse(*conv)}, nil
	})
	...
	huma.Register(api, huma.Operation{
		...
		srvInput := conversations.ConversationUpdateInput(input.Body)
