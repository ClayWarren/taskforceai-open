...
func validateAgentInput(input AgentInput) (string, error) {
	...
	if _, err := pulse.NormalizeActiveHoursTime(input.ActiveStart); err != nil {
		return "", fmt.Errorf("invalid activeStart: %w", err)
	}
	if _, err := pulse.NormalizeActiveHoursTime(input.ActiveEnd); err != nil {
		return "", fmt.Errorf("invalid activeEnd: %w", err)
	}
