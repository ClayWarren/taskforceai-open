...
func RegisterHandlers(api huma.API, service projects.Service) {
	...
	huma.Register(api, huma.Operation{
		...
		proj, err := service.UpdateProject(ctx, projects.UpdateProjectInput{
			ID:             int32(projectID),
			UserID:         ids.UserID32,
			OrganizationID: ids.OrgID32,
			Name:           input.Body.Name,
		...
		if err != nil {
			slog.Error("Failed to update project", "projectId", input.ID, "userId", ids.UserID32, "orgId", ids.OrgID, "error", err)
