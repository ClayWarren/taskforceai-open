...
func RegisterHandlers(api huma.API, service Service) {
	...
	registerArtifactDeletion(api, artifactDeletionRoute{
		operationID: "revoke-artifact-public-links", path: "/api/v1/artifacts/{id}/share/public", summary: "Revoke public artifact links", failureMessage: "Failed to revoke public artifact links", action: service.RevokePublicLinks,
	})
	registerGetPublicArtifact(api, service)
	registerGetPublicArtifactContent(api, service)
	registerArtifactDeletion(api, artifactDeletionRoute{
		operationID: "delete-artifact", path: "/api/v1/artifacts/{id}", summary: "Delete artifact", failureMessage: "Failed to delete artifact", action: service.DeleteArtifact,
	})
}
...
type artifactDeletionRoute struct {
	operationID, path, summary, failureMessage string
	action                                     func(context.Context, string, int32, *int32) error
}

func registerArtifactDeletion(api huma.API, route artifactDeletionRoute) {
	huma.Register(api, huma.Operation{
		...
		if err := route.action(ctx, input.ID, ids.UserID32, ids.OrgID32); err != nil {
			slog.Error(route.failureMessage, "artifactId", input.ID, "userId", ids.UserID32, "orgId", ids.OrgID, "error", err)
			return nil, mapArtifactError(err, route.failureMessage)
		}
		return &struct{}{}, nil
	})
}
...
func registerGetPublicArtifactContent(api huma.API, service Service) {
	huma.Register(api, huma.Operation{
		...
		headers := server.BuildBinaryDownloadHeaders(contentType, input.Disposition, sanitizeFilename(file.Filename))
