...
func updateAdminUserPlan(ctx context.Context, repo admin.Repository, userID *int32, targetEmail, plan string) error {
	if userID != nil {
		_, err := repo.UpdateUserPlanByID(ctx, *userID, plan)
		return err
	}
	_, err := repo.UpdateUserPlan(ctx, targetEmail, plan)
	return err
}

func updateAdminUserRole(ctx context.Context, repo admin.Repository, userID *int32, targetEmail string, isAdmin bool) error {
	if userID != nil {
		_, err := repo.UpdateUserAdminByID(ctx, *userID, isAdmin)
		return err
	}
	_, err := repo.UpdateUserAdmin(ctx, targetEmail, isAdmin)
	return err
}
...
func registerUpdateUser(api huma.API, repo admin.Repository) {
	...
	huma.Register(api, huma.Operation{
		...
		if input.Body.Plan != nil {
			...
			if err := updateAdminUserPlan(ctx, repo, input.Body.UserID, targetEmail, plan); err != nil {
				slog.Error("Admin user update failed", "actorEmail", input.User.Email, "targetEmail", targetEmail, "targetID", input.Body.UserID, "field", "plan", "error", err)
				return nil, huma.Error500InternalServerError("Failed to update user")
			}
		}

		if input.Body.IsAdmin != nil {
			if err := updateAdminUserRole(ctx, repo, input.Body.UserID, targetEmail, *input.Body.IsAdmin); err != nil {
				slog.Error("Admin user update failed", "actorEmail", input.User.Email, "targetEmail", targetEmail, "targetID", input.Body.UserID, "field", "isAdmin", "error", err)
				return nil, huma.Error500InternalServerError("Failed to update user")
			}
		}
