...
func TestStatusBlobPublisher(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		assert.True(t, putter.options.AllowOverwrite)
...
func TestVercelStatusBlobPutterUsesRESTUploadContract(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "1", r.Header.Get("x-allow-overwrite"))
...
func TestVercelStatusBlobPutterErrorPaths(t *testing.T) {
	...
	t.Run("transport error", func(t *testing.T) {
		...
		_, err := (vercelStatusBlobPutter{baseURL: "https://blob.test", client: client}).Put(
			context.Background(), "status.json", strings.NewReader("{}"), vercelblob.PutCommandOptions{IfMatch: "etag"},
		)
