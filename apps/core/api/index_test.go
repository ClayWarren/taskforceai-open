...
func setTestRedis(t *testing.T) {
	t.Helper()
	infraredis.SetClient(newEvalRateLimitRedis())
...
func memoryValues() []any {
	return []any{int32(11), int32(20), int32Ptr(30), "remember this", "preference", []byte(`{}`), testTimestamp(), testTimestamp()}
}
...
func TestProjectStoreAdapter_CoversQueriesAndMapping(t *testing.T) {
	q, backing := newQueuedQueries(projectValues())
...
func TestMemoryAndNotificationAdapters_CoverQueriesAndMapping(t *testing.T) {
	q, backing := newQueuedQueries(memoryValues(), memoryValues(), memoryValues())
	backing.rows = [][][]any{{memoryValues()}, {memoryValues()}}
	memoriesAdapter := memoryStoreAdapter{q: q}

	memories, err := memoriesAdapter.GetUserMemories(context.Background(), 20)
	require.NoError(t, err)
	require.Len(t, memories, 1)
	assert.Equal(t, "remember this", memories[0].Content)
	assert.Equal(t, "2026-05-21T12:00:00Z", memories[0].CreatedAt)
	assert.Equal(t, "2026-05-21T12:00:00Z", memories[0].UpdatedAt)

	memories, err = memoriesAdapter.GetUserMemoriesWithOrg(context.Background(), memoriespkg.GetUserMemoriesWithOrgInput{UserID: 20, OrganizationID: int32Ptr(30)})
	require.NoError(t, err)
	require.Len(t, memories, 1)

	require.NoError(t, memoriesAdapter.CreateMemory(context.Background(), memoriespkg.CreateMemoryInput{UserID: 20, Content: "remember this", Type: "preference"}))
	updated, err := memoriesAdapter.UpdateMemory(context.Background(), memoriespkg.UpdateMemoryStoreInput{ID: 11, UserID: 20, Content: "updated", Type: "preference", Metadata: []byte(`{"source":"test"}`)})
	require.NoError(t, err)
	assert.Equal(t, int32(11), updated.ID)
	assert.Equal(t, "remember this", updated.Content)
	updated, err = memoriesAdapter.UpdateMemoryWithOrg(context.Background(), memoriespkg.UpdateMemoryWithOrgStoreInput{ID: 11, UserID: 20, OrganizationID: int32Ptr(30), Content: "updated", Type: "preference", Metadata: []byte(`{"source":"test"}`)})
	require.NoError(t, err)
	require.NotNil(t, updated.OrganizationID)
	assert.Equal(t, int32(30), *updated.OrganizationID)
	assert.Empty(t, pgTimestampString(pgtype.Timestamp{}))
	assert.NoError(t, memoriesAdapter.DeleteMemory(context.Background(), memoriespkg.DeleteMemoryInput{ID: 11, UserID: 20}))
	assert.NoError(t, memoriesAdapter.DeleteMemoryWithOrg(context.Background(), memoriespkg.DeleteMemoryWithOrgInput{ID: 11, UserID: 20, OrganizationID: int32Ptr(30)}))
