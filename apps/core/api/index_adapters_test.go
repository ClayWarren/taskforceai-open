...
func TestAdapters_ReturnDatabaseErrors(t *testing.T) {
	...
	memoryAdapter := memoryStoreAdapter{q: q}
	_, err = memoryAdapter.GetUserMemories(ctx, 20)
	require.Error(t, err)
	_, err = memoryAdapter.GetUserMemoriesWithOrg(ctx, memoriespkg.GetUserMemoriesWithOrgInput{UserID: 20, OrganizationID: int32Ptr(30)})
	require.Error(t, err)
	require.Error(t, memoryAdapter.CreateMemory(ctx, memoriespkg.CreateMemoryInput{UserID: 20, Content: "content", Type: "type"}))
	_, err = memoryAdapter.UpdateMemory(ctx, memoriespkg.UpdateMemoryStoreInput{ID: 1, UserID: 20, Content: "content", Type: "type"})
	require.Error(t, err)
	_, err = memoryAdapter.UpdateMemoryWithOrg(ctx, memoriespkg.UpdateMemoryWithOrgStoreInput{ID: 1, UserID: 20, OrganizationID: int32Ptr(30), Content: "content", Type: "type"})
	require.Error(t, err)
	require.Error(t, memoryAdapter.DeleteMemory(ctx, memoriespkg.DeleteMemoryInput{ID: 1, UserID: 20}))
	require.Error(t, memoryAdapter.DeleteMemoryWithOrg(ctx, memoriespkg.DeleteMemoryWithOrgInput{ID: 1, UserID: 20, OrganizationID: int32Ptr(30)}))
