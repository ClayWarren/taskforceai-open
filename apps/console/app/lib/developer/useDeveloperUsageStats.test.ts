...
describe('useDeveloperUsageStats', () => {
  ...
  it('waits for authentication and clears stats on logout', async () => {
    const { result, rerender } = renderHook(
      ({ isAuthenticated, isAuthLoading }) =>
        useDeveloperUsageStats({ isAuthenticated, isAuthLoading, userScope: 'id-7' }),
  ...
  it('logs result and thrown failures without replacing stats', async () => {
    ...
    const { result } = renderHook(() =>
      useDeveloperUsageStats({ isAuthenticated: true, userScope: 'id-7' })
    );
    await act(async () => {
  ...
  it('invalidates prior work and reloads stats when the user identity changes', async () => {
    const first = deferred<ReturnType<typeof okResult<UsageStats>>>();
    const secondStats = buildStats(900);
    mockRefreshUsageStats
      .mockReturnValueOnce(first.promise)
      .mockResolvedValueOnce(okResult(secondStats));

    const { result, rerender } = renderHook(
      ({ userScope }) => useDeveloperUsageStats({ isAuthenticated: true, userScope }),
      { initialProps: { userScope: 'id-7' } }
    );

    rerender({ userScope: 'id-8' });
    await act(async () => {
      await Promise.resolve();
    });
    expect(result.current.stats).toBe(secondStats);

    await act(async () => {
      first.resolve(okResult(buildStats(100)));
      await first.promise;
    });
    expect(result.current.stats).toBe(secondStats);
  });
