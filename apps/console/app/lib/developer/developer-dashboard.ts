...
export {
  createApiKey as createDeveloperApiKey,
  revokeApiKey as revokeDeveloperApiKey,
} from '../api/developer';
...
export const refreshUsageStats = async (): Promise<Result<UsageStats, ApiError>> => {
  const requestCacheKey = getActiveScopedCacheKey();
  const result = await fetchUsageStats();
  if (result.ok && getActiveScopedCacheKey() === requestCacheKey) {
    writeStorageItem(requestCacheKey, JSON.stringify(result.value));
  }
  return result;
};
