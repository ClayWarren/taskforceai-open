...
import { loadStoredUser } from '../utils/auth-storage';
import { readStorageItem, removeStorageItem, writeStorageItem } from '../platform/browser-storage';
...
const resolveUserCacheScope = (): string => {
  const userResult = loadStoredUser();
  if (!userResult.ok) {
  ...
  if (typeof userResult.value.email === 'string' && userResult.value.email.length > 0) {
    return `email-${normalizeScopeSegment(userResult.value.email)}`;
  }
...
const getActiveScopedCacheKey = (): string => {
  const scope = resolveUserCacheScope();
...
export const clearCachedUsageStats = (): Result<true, UsageStatsCacheError> => {
  const keysToRemove = new Set<string>([
    ...
    getScopedCacheKey(ANONYMOUS_SCOPE),
    getScopedCacheKey(resolveUserCacheScope()),
  ]);
