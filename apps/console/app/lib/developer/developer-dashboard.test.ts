...
describe('developer-dashboard cache scoping', () => {
  ...
  it('does not cache a delayed response after the authenticated user changes', async () => {
    const stats = buildUsageStats();
    const refresh = deferred<ReturnType<typeof ok<UsageStats>>>();
    fetchUsageStatsMock.mockReturnValue(refresh.promise);

    const refreshPromise = refreshUsageStats();
    loadStoredUserMock.mockReturnValue({
      ok: true,
      value: { id: 8, email: 'next@taskforceai.chat' },
    });
    refresh.resolve(ok(stats));
    await refreshPromise;

    expect(window.localStorage.getItem(`${DEVELOPER_STATS_CACHE_KEY}:id-7`)).toBeNull();
    expect(window.localStorage.getItem(`${DEVELOPER_STATS_CACHE_KEY}:id-8`)).toBeNull();
  });
