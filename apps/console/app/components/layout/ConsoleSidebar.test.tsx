...
describe('ConsoleSidebar', () => {
  ...
  it('closes on Escape and prevents focus on the closed mobile sidebar', () => {
    const onClose = vi.fn();
    const view = render(<ConsoleSidebar isOpen={true} onClose={onClose} />);

    expect(screen.getByRole('link', { name: /TaskForce AI Console/i })).toHaveFocus();
    fireEvent.keyDown(document, { key: 'Escape' });
    expect(onClose).toHaveBeenCalledTimes(1);

    view.rerender(<ConsoleSidebar isOpen={false} onClose={onClose} />);
    const sidebar = screen.getByLabelText('Console navigation');
