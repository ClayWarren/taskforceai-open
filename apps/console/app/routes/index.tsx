...
import { ArrowUpRight, KeyRound, BarChart3, Zap, TrendingUp } from 'lucide-react';
...
function OverviewPage() {
  ...
  const usagePercent = visibleStats
    ? Math.min((visibleStats.requestsThisMonth / Math.max(visibleStats.monthlyQuota, 1)) * 100, 100)
    : 0;
      ...
      <div className="relative grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-8 transition-colors hover:bg-white/[0.04]">
            ...
            <div className="grid flex-1 grid-cols-3 gap-4">
              <div>
                ...
                <p className="mt-1 text-sm font-bold text-white">
                  {visibleStats?.requestsThisMonth?.toLocaleString() ?? '0'}
                </p>
              ...
              <div>
                ...
                <p className="mt-1 text-sm font-bold text-white">
                  {visibleStats?.monthlyRemaining?.toLocaleString() ?? '0'}
                </p>
          ...
          <div className="mt-12 flex items-center justify-between border-t border-white/5 pt-6 text-xs">
            ...
            <span className="font-mono font-bold text-white">
              {visibleStats?.totalRequests?.toLocaleString() ?? '0'}
            </span>
          </div>
          <div className="mt-2 flex items-center justify-between text-xs">
            <span className="text-slate-400">API Keys active</span>
            <span className="font-mono font-bold text-white">
              {visibleStats?.apiKeys?.filter((k) => !k.revokedAt).length ?? 0}
            </span>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-8 transition-colors hover:bg-white/[0.04]">
          ...
          <div className="mt-8 flex h-32 items-end justify-between gap-2">
            {(visibleStats?.usageHistory?.length
              ? visibleStats.usageHistory
              : Array(12).fill({ count: 0 })
            ).map((day, i) => {
              const max = Math.max(...(visibleStats?.usageHistory?.map((d) => d.count) ?? []), 1);
              const height = visibleStats?.usageHistory?.length ? (day.count / max) * 100 : 10;
              return (
                <div
                  key={i}
                  className={cn(
                    'w-full rounded-sm transition-all duration-500',
                    i === (visibleStats?.usageHistory?.length ?? 0) - 1
                      ? 'bg-blue-600'
                      : 'bg-white/10'
                  )}
          ...
          <div className="mt-12 flex flex-col gap-1 border-t border-white/5 pt-6">
            ...
            <p className="text-xs text-slate-400">
              ...
              <span className="font-bold text-slate-300">
                {visibleStats?.usageHistory?.length ?? 0}
              </span>{' '}
              reporting periods
            </p>
          </div>
