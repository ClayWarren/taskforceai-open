...
import { billingQueryKeys } from '../lib/billing/query-keys';
...
function BillingLayout() {
  ...
  const previousUserScopeRef = useRef(userScope);

  useEffect(() => {
    const previousUserScope = previousUserScopeRef.current;
    if (!isAuthenticated || previousUserScope !== userScope) {
      queryClient.removeQueries({ queryKey: billingQueryKeys.scope(previousUserScope) });
    }
    previousUserScopeRef.current = userScope;
  }, [isAuthenticated, queryClient, userScope]);
