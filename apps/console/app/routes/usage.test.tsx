...
describe('usage route', () => {
  ...
  it('refreshes usage automatically while the page remains visible', async () => {
    const setIntervalSpy = vi.spyOn(globalThis, 'setInterval');

    renderUsagePage();
    await waitFor(() => expect(mockRefreshUsageStats).toHaveBeenCalledTimes(1));

    const intervalCall = setIntervalSpy.mock.calls.find((call) => call[1] === 30_000);
    expect(intervalCall).toBeDefined();
    const refresh = intervalCall?.[0];
    if (typeof refresh !== 'function') {
      throw new Error('Usage refresh interval callback was not registered');
    }
    ...
    await waitFor(() => expect(mockRefreshUsageStats).toHaveBeenCalledTimes(2));

    document.dispatchEvent(new Event('visibilitychange'));

    await waitFor(() => expect(mockRefreshUsageStats).toHaveBeenCalledTimes(3));
  ...
  it('ignores stale refresh responses after auth state changes', async () => {
    ...
    const staleStats = buildStats({
      requestsThisMonth: 111,
      totalRequests: 1111,
    });
    const freshStats = buildStats({
      requestsThisMonth: 900,
      totalRequests: 9000,
    });
