...
const renderBillingLayout = () => {
  const BillingLayout = getBillingLayout();
...
describe('billing route layout', () => {
  ...
  it('removes the previous user billing cache when identity changes', () => {
    const BillingLayout = getBillingLayout();
    mockUseAuth.mockReturnValue({
      isAuthenticated: true,
      isLoading: false,
    ...
    const view = render(<BillingLayout />);
    ...
    view.rerender(<BillingLayout />);

    expect(mockRemoveQueries).toHaveBeenCalledWith({ queryKey: ['billing', 'id-7'] });
  });
});
