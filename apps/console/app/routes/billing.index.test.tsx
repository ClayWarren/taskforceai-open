...
vi.mock('@taskforceai/api-client/api/subscriptions', () => ({
...
const errResult = (message: string) => ({
  ok: false as const,
  error: { kind: 'server' as const, message },
});
...
describe('billing overview route', () => {
  ...
  it('enables auto recharge with configured amount and threshold', async () => {
    mockUseQuery.mockReturnValue({
      data: buildBalance({ autoRechargeEnabled: false, autoRechargeAmount: 25 }),
      isLoading: false,
    });
  ...
  it('alerts failed auto-recharge responses and rejected auto-recharge mutations', async () => {
    mockUpdateAutoRecharge.mockResolvedValueOnce(errResult('Auto recharge unavailable'));

    const { unmount } = renderBillingOverviewPage();

    fireEvent.click(screen.getByRole('button', { name: 'Enable auto recharge' }));

    await waitFor(() => {
      expect(mockShowAlert).toHaveBeenCalledWith(
        'Failed to update auto recharge: Auto recharge unavailable'
      );
    });
    expect(mockInvalidateQueries).not.toHaveBeenCalledWith({ queryKey: ['billing', 'balance'] });
    ...
    mockUpdateAutoRecharge.mockRejectedValueOnce(new Error('network failed'));
    mockShowAlert.mockReturnValue(okResult(undefined));
    ...
    await waitFor(() => {
      expect(mockShowAlert).toHaveBeenCalledWith(
        'Failed to update auto recharge. Please try again.'
      );
    });
  });
  ...
  it('warns and alerts when cancellation confirmation cannot be shown', () => {
    mockConfirmAction.mockReturnValue({
      ok: false,
      error: { kind: 'unavailable', message: 'confirm unavailable' },
    });
    ...
    expect(mockCancelSubscription).not.toHaveBeenCalled();
    expect(mockShowAlert).toHaveBeenCalledWith('Unable to confirm cancellation. Please try again.');
  });

  it('alerts failed cancel responses and logs alert delivery failures', async () => {
    mockCancelSubscription.mockResolvedValue(errResult('Cannot cancel right now'));
    mockShowAlert.mockReturnValue({ ok: false, error: { kind: 'unavailable' } });
    ...
    await waitFor(() => {
      expect(mockShowAlert).toHaveBeenCalledWith('Failed to cancel plan: Cannot cancel right now');
    });
  });

  it('alerts when cancel mutations reject', async () => {
    ...
    await waitFor(() => {
      expect(mockShowAlert).toHaveBeenCalledWith('Failed to cancel plan. Please try again.');
    });
  });
  ...
  it('alerts failed reactivation responses and rejected reactivation mutations', async () => {
    mockUseQuery.mockReturnValue({
      data: buildBalance({
        subscriptionId: 'sub_canceling',
        cancelAtPeriodEnd: true,
    ...
    mockReactivateSubscription.mockResolvedValueOnce(errResult('Cannot reactivate right now'));
    ...
    await waitFor(() => {
      expect(mockShowAlert).toHaveBeenCalledWith(
        'Failed to reactivate plan: Cannot reactivate right now'
      );
    });
    ...
    mockReactivateSubscription.mockRejectedValueOnce(new Error('reactivate failed'));
    mockShowAlert.mockReturnValue(okResult(undefined));
    ...
    await waitFor(() => {
      expect(mockShowAlert).toHaveBeenCalledWith('Failed to reactivate plan. Please try again.');
    });
  });
