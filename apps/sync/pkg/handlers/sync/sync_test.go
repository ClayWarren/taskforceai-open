...
func TestSyncPush_RejectsOutOfScopeProject(t *testing.T) {
	repo := &stubRepo{}
	service := sync.NewService(repo, nil, nil, nil, nil, nil)
	r := setupAPI(service, repo, testUser())

	body := `{"conversations":[{"id":0,"local_id":"local-1","project_id":9001,"timestamp":"2026-01-01T00:00:00Z","user_input":"prompt","agent_count":1,"sync_version":0,"last_synced_at":"2026-01-01T00:00:00Z","is_deleted":false,"updated_at":"2026-01-01T00:00:00Z"}],"messages":[],"deletions":[]}`
	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/api/v1/sync/push", strings.NewReader(body))
	req.Header.Set("X-Device-Id", "device1")
	r.ServeHTTP(w, req)
	...
	assert.Equal(t, 1, repo.projectChecks)
	assert.Equal(t, "123", repo.projectUserID)
	assert.Zero(t, repo.conversationCreates)
	assert.Zero(t, repo.conversationUpdates)
}

func TestSyncPush_AllowsOwnedPersonalProject(t *testing.T) {
	repo := &stubRepo{projectAllowed: true}
	service := sync.NewService(repo, nil, nil, nil, nil, nil)
	r := setupAPI(service, repo, testUser())

	body := `{"conversations":[{"id":0,"local_id":"local-1","project_id":17,"timestamp":"2026-01-01T00:00:00Z","user_input":"prompt","agent_count":1,"sync_version":0,"last_synced_at":"2026-01-01T00:00:00Z","is_deleted":false,"updated_at":"2026-01-01T00:00:00Z"}],"messages":[],"deletions":[]}`
	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/api/v1/sync/push", strings.NewReader(body))
	req.Header.Set("X-Device-Id", "device1")
	r.ServeHTTP(w, req)
	...
	assert.Equal(t, 1, repo.projectChecks)
	assert.Equal(t, "123", repo.projectUserID)
	assert.Nil(t, repo.projectOrgID)
	assert.Equal(t, 1, repo.conversationCreates)
}
