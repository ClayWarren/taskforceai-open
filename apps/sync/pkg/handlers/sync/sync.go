...
func resolveSyncRequest(ctx context.Context, resolve DependencyResolver, user *adapterauth.AuthenticatedUser) (Dependencies, string, error) {
	deps, err := resolve(ctx)
	if err != nil {
		slog.Error("Sync dependencies unavailable", "error", err)
		return Dependencies{}, "", huma.Error503ServiceUnavailable("Sync service unavailable")
	}
	userID, err := syncUserID(user)
	if err != nil {
		return Dependencies{}, "", huma.Error401Unauthorized("Unauthorized")
	}
	return deps, userID, nil
}
...
func handleSyncPull(ctx context.Context, resolve DependencyResolver, input *syncPullInput) (*struct{ Body *SyncPullResponse }, error) {
	deps, userID, err := resolveSyncRequest(ctx, resolve, input.User)
	if err != nil {
		return nil, err
	}
...
func handleSyncPush(ctx context.Context, resolve DependencyResolver, input *syncPushInput) (*struct{ Body *SyncPushResponse }, error) {
	...
	if err != nil {
		...
		if errors.Is(err, sync.ErrProjectAccessDenied) {
			return nil, huma.Error403Forbidden("Project is not accessible")
		}
		slog.Error(
			"Sync push failed",
			"userId",
...
func syncPullResponseFromDomain(resp *sync.SyncPullResponse) *SyncPullResponse {
	normalized := *resp
	if normalized.Conversations == nil {
		normalized.Conversations = []sync.ConversationSyncPayload{}
	}
	if normalized.Messages == nil {
		normalized.Messages = []sync.MessageSyncPayload{}
	}
	if normalized.Deletions == nil {
		normalized.Deletions = []sync.DeletionRecord{}
	}
	converted := SyncPullResponse(normalized)
	return &converted
}
...
func syncPushResponseFromDomain(resp *sync.SyncPushResponse) *SyncPushResponse {
	converted := SyncPushResponse(*resp)
	return &converted
}
