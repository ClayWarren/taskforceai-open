...
func Handler(w http.ResponseWriter, r *http.Request) {
	...
	if rawVersions, present := r.URL.Query()["last_version"]; present {
		lastVersion := parseNonNegativeVersion(rawVersions)
		if latestVersion, versionErr := getLatestSyncVersion(r.Context(), userID, orgID); versionErr != nil {
			slog.Warn("Failed to load durable realtime sync version", "userId", userID, "orgId", orgID, "error", versionErr)
		} else {
			response.LatestVersion = latestVersion
			if latestVersion > lastVersion && !containsSyncVersion(response.Messages, latestVersion) {
				response.Messages = append(response.Messages, SyncMessage{
					Type: "sync_required", Version: latestVersion, ID: fmt.Sprintf("db:%d", latestVersion),
				})
			}
		}
	}
...
func parseNonNegativeVersion(values []string) int {
	if len(values) == 0 {
		return 0
	}
	version, err := strconv.Atoi(strings.TrimSpace(values[0]))
...
func getLatestSyncVersion(ctx context.Context, userID, orgID string) (int, error) {
	queries, err := getQueries(ctx)
	if err != nil {
		return 0, err
	}
	if orgID == "" {
		version, err := queries.GetLatestSyncVersion(ctx, &userID)
		return int(version), err
	}
	parsedOrgID, err := strconv.ParseInt(orgID, 10, 32)
	if err != nil {
		return 0, fmt.Errorf("parse organization id: %w", err)
	}
	value := int32(parsedOrgID)
	version, err := queries.GetLatestOrgSyncVersion(ctx, &value)
	return int(version), err
}

func containsSyncVersion(messages []SyncMessage, version int) bool {
	for _, message := range messages {
		if message.Type == "sync_required" && message.Version >= version {
			return true
		}
	}
	return false
