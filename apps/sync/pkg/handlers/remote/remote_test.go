...
type sequencedPairingRedis struct {
	*remoteRedisStub
	gets        int
	secondValue string
	secondErr   error
...
func (s *sequencedPairingRedis) Get(ctx context.Context, key string) (string, error) {
	s.gets++
	if s.gets == 2 {
		if s.secondErr != nil {
			return "", s.secondErr
		...
		if s.secondValue != "" {
			return s.secondValue, nil
		}
	}
	return s.remoteRedisStub.Get(ctx, key)
}

func TestReservePairingChallengeRemainingBranches(t *testing.T) {
	ctx := context.Background()
	challenge := `{"userId":"42","targetDeviceId":"mac-1"}`

	_, release, complete, err := reservePairingChallenge(ctx, newRemoteRedisStub(), "missing", "42")
	require.ErrorIs(t, err, redis.ErrKeyNotFound)
	release()
	require.NoError(t, complete())

	mismatch := newRemoteRedisStub()
	mismatch.data[pairingKey("code")] = `{"userId":"other","targetDeviceId":"mac-1"}`
	_, release, complete, err = reservePairingChallenge(ctx, mismatch, "code", "42")
	require.ErrorIs(t, err, errPairingAccountMismatch)
	release()
	require.NoError(t, complete())
	...
	randomFailure := newRemoteRedisStub()
	randomFailure.data[pairingKey("code")] = challenge
	_, release, complete, err = reservePairingChallenge(ctx, randomFailure, "code", "42")
	require.ErrorIs(t, err, assert.AnError)
	release()
	require.NoError(t, complete())
	readRandom = originalRead

	setFailure := newRemoteRedisStub()
	setFailure.data[pairingKey("code")] = challenge
	setFailure.operation, setFailure.err = "setnx", assert.AnError
	_, release, complete, err = reservePairingChallenge(ctx, setFailure, "code", "42")
	require.ErrorIs(t, err, assert.AnError)
	release()
	require.NoError(t, complete())

	claimed := newRemoteRedisStub()
	claimed.data[pairingKey("code")] = challenge
	claimed.data[pairingKey("code")+":claim"] = "held"
	_, release, complete, err = reservePairingChallenge(ctx, claimed, "code", "42")
	require.ErrorIs(t, err, errPairingClaimed)
	release()
	require.NoError(t, complete())

	changedBase := newRemoteRedisStub()
	changedBase.data[pairingKey("code")] = challenge
	changed := &sequencedPairingRedis{remoteRedisStub: changedBase, secondValue: `{"userId":"42","targetDeviceId":"mac-2"}`}
	_, release, complete, err = reservePairingChallenge(ctx, changed, "code", "42")
	require.ErrorIs(t, err, redis.ErrKeyNotFound)
	release()
	require.NoError(t, complete())

	readBase := newRemoteRedisStub()
	readBase.data[pairingKey("code")] = challenge
	readFailure := &sequencedPairingRedis{remoteRedisStub: readBase, secondErr: assert.AnError}
	_, release, complete, err = reservePairingChallenge(ctx, readFailure, "code", "42")
	require.ErrorIs(t, err, assert.AnError)
	release()
	require.NoError(t, complete())

	evalFailure := newRemoteRedisStub()
	evalFailure.data[pairingKey("code")] = challenge
	_, release, complete, err = reservePairingChallenge(ctx, evalFailure, "code", "42")
	require.NoError(t, err)
	defer release()
	evalFailure.operation, evalFailure.err = "eval", assert.AnError
	require.ErrorIs(t, complete(), assert.AnError)

	expiredClaim := newRemoteRedisStub()
	expiredClaim.data[pairingKey("code")] = challenge
	_, release, complete, err = reservePairingChallenge(ctx, expiredClaim, "code", "42")
	require.NoError(t, err)
	defer release()
	expiredClaim.data[pairingKey("code")+":claim"] = "replaced"
	require.ErrorContains(t, complete(), "claim expired")
}

func TestRemoteLastEventID(t *testing.T) {
	require.Equal(t, "$", remoteLastEventID("  "))
	require.Equal(t, "1-0", remoteLastEventID(" 1-0 "))
}
