...
type remoteWebSocketConnection interface {
...
func WebSocketHandler(resolveQueries QueriesResolver, resolveRedis RedisResolver) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		lastID := remoteLastEventID(r.URL.Query().Get("lastId"))
...
func remoteLastEventID(value string) string {
...
func serveRemoteWebSocket(
	ctx context.Context,
	conn remoteWebSocketConnection,
	q *db.Queries,
	...
	return serveRemoteWebSocketWithHeartbeat(
		ctx, conn, q, client, lease, userID, deviceID, lastID, remoteWebSocketWait,
	)
}

func serveRemoteWebSocketWithHeartbeat(
	ctx context.Context,
	conn remoteWebSocketConnection,
	q *db.Queries,
	client redis.Cmdable,
...
func readRemoteWebSocket(conn remoteWebSocketConnection, results chan<- remoteWebSocketEnvelope, readErrors chan<- error) {
	for {
...
func decodeRemoteCommands(
	...
	for _, message := range messages {
		...
		now := time.Now().UTC()
		if command.CreatedAt.IsZero() || command.CreatedAt.Before(now.Add(-remoteCommandMaxAge)) || command.CreatedAt.After(now.Add(time.Minute)) {
			continue
		}
