...
func TestRepository_ProjectExistsInScope(t *testing.T) {
	t.Run("owned personal project", func(t *testing.T) {
		mockPool := dbtest.NewMockPool(t)
		repo := NewRepository(db.New(mockPool))
		now := pgtype.Timestamp{Time: time.Now(), Valid: true}
		mockPool.ExpectQuery("SELECT (.+) FROM projects").
			WithArgs(int32(17), int32(123)).
			WillReturnRows(pgxmock.NewRows([]string{
				"id", "user_id", "organization_id", "name", "description", "custom_instructions", "created_at", "updated_at",
			}).AddRow(int32(17), int32(123), nil, "Owned", nil, nil, now, now))

		allowed, err := repo.ProjectExistsInScope(context.Background(), 17, "123", nil)
	...
	t.Run("out of scope organization project", func(t *testing.T) {
		mockPool := dbtest.NewMockPool(t)
		repo := NewRepository(db.New(mockPool))
		organizationID := int32(7)
		mockPool.ExpectQuery("SELECT (.+) FROM projects").
			WithArgs(int32(41), int32(123), &organizationID).
			WillReturnRows(pgxmock.NewRows([]string{
				"id", "user_id", "organization_id", "name", "description", "custom_instructions", "created_at", "updated_at",
			}))

		allowed, err := repo.ProjectExistsInScope(context.Background(), 41, "123", &organizationID)
	...
	t.Run("invalid authenticated user id", func(t *testing.T) {
		repo := NewRepository(db.New(repositoryErrorDB{err: assert.AnError}))

		allowed, err := repo.ProjectExistsInScope(context.Background(), 17, "not-a-number", nil)

		assert.False(t, allowed)
		require.ErrorContains(t, err, "parse project owner user id")
	})
}

func TestRepository_GetSyncPushResultLocksBeforeReading(t *testing.T) {
	mock := dbtest.NewMockPool(t)
	repo := NewRepository(db.New(mock))
	userID := "user-123"
	idempotencyKey := "device-1:request-1"
	encoded, err := json.Marshal(SyncPushResponse{Success: true, Version: 42})
	require.NoError(t, err)

	mock.ExpectExec("SELECT PG_ADVISORY_XACT_LOCK").
		WithArgs(&userID, &idempotencyKey).
		WillReturnResult(pgxmock.NewResult("SELECT", 1))
	mock.ExpectQuery("SELECT response FROM sync_push_results").
		WithArgs(userID, idempotencyKey).
		WillReturnRows(pgxmock.NewRows([]string{"response"}).AddRow(encoded))

	result, err := repo.GetSyncPushResult(context.Background(), userID, idempotencyKey)
