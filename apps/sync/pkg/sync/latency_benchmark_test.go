...
func reportNamedSyncLatencyProfile(b *testing.B, prefix string, samples []time.Duration) {
	...
	b.ReportMetric(float64(syncPercentileDuration(ordered, 0.50).Microseconds()), syncMetricName(prefix, "p50_us"))
	b.ReportMetric(float64(syncPercentileDuration(ordered, 0.95).Microseconds()), syncMetricName(prefix, "p95_us"))
	b.ReportMetric(float64(syncPercentileDuration(ordered, 0.99).Microseconds()), syncMetricName(prefix, "p99_us"))
}
...
func syncPercentileDuration(ordered []time.Duration, percentile float64) time.Duration {
	if len(ordered) == 0 {
		return 0
	}
