...
func (s *Service) syncConversations(
	...
	if err := validateUniqueLocalConversationIDs(conversations); err != nil {
	...
	for _, incoming := range conversations {
		...
		if causality == After {
			conflicts = s.appendConflict(ctx, conflicts, "conversation", fmt.Sprintf("%d", incoming.ID), "server_newer", existing.SyncVersion, incoming.SyncVersion)
			continue
		}
...
func validateUniqueLocalConversationIDs(conversations []ConversationSyncPayload) error {
	seen := make(map[string]struct{}, len(conversations))
	for _, conversation := range conversations {
		if conversation.ID != 0 || conversation.LocalID == nil {
			continue
		}
		localID := strings.TrimSpace(*conversation.LocalID)
		if localID == "" {
			continue
		}
		if _, exists := seen[localID]; exists {
			return fmt.Errorf("duplicate conversation local_id %q", localID)
		}
		seen[localID] = struct{}{}
	}
	return nil
...
func (s *Service) handleNewConversation(ctx context.Context, repo SyncRepository, userID string, deviceID string, version int32, incoming ConversationSyncPayload, accepted *[]string, mappings map[string]int32) (int32, error) {
	localID := ""
	if incoming.LocalID != nil {
		localID = strings.TrimSpace(*incoming.LocalID)
		if _, exists := mappings[localID]; localID != "" && exists {
			return version, fmt.Errorf("duplicate conversation local_id %q", localID)
		}
	}
	var err error
	version, err = nextSyncVersion(ctx, repo, version)
	...
	if localID != "" {
		mappings[localID] = createdID
		*accepted = append(*accepted, fmt.Sprintf("conversation:%s", localID))
	} else {
