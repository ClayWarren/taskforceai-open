...
func (s *Service) applyPushTransaction(ctx context.Context, userID, deviceID, durableKey string, req SyncPushRequest) (*SyncPushResponse, int32, error) {
	...
	err := s.repo.WithTransaction(ctx, func(txRepo SyncRepository) error {
		...
		if err := validateConversationProjectScopes(ctx, txRepo, userID, req.OrganizationID, req.Conversations); err != nil {
			return err
		}
...
func validateConversationProjectScopes(ctx context.Context, repo SyncRepository, userID string, organizationID *int32, conversations []ConversationSyncPayload) error {
	projectIDs := make(map[int32]struct{})
	for _, conversation := range conversations {
		if conversation.ProjectID != nil {
			projectIDs[*conversation.ProjectID] = struct{}{}
		}
	}
	if len(projectIDs) == 0 {
		return nil
	}
	projectRepo, ok := repo.(ProjectScopeRepository)
	if !ok {
		return errors.New("sync repository does not support project scope validation")
	}
	for projectID := range projectIDs {
		allowed, err := projectRepo.ProjectExistsInScope(ctx, projectID, userID, organizationID)
		if err != nil {
			return fmt.Errorf("validate project %d scope: %w", projectID, err)
		}
		if !allowed {
			return fmt.Errorf("%w: project %d", ErrProjectAccessDenied, projectID)
		}
	}
	return nil
}
