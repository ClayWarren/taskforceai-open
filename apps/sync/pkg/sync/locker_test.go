...
func TestRedisLocker_ReleaseFailureLeavesLockToExpire(t *testing.T) {
	client := new(mocks.Cmdable)
	locker := &RedisLocker{client: client}
	...
	release()
	client.AssertExpectations(t)
	client.AssertNotCalled(t, "Get", mock.Anything, mock.Anything)
	client.AssertNotCalled(t, "Del", mock.Anything, mock.Anything)
}
