...
type ProjectScopeRepository interface {
	ProjectExistsInScope(ctx context.Context, projectID int32, userID string, organizationID *int32) (bool, error)
}
...
func (r *Repository) ProjectExistsInScope(ctx context.Context, projectID int32, userID string, organizationID *int32) (bool, error) {
	numericUserID, err := strconv.ParseInt(userID, 10, 32)
	if err != nil {
		return false, fmt.Errorf("parse project owner user id: %w", err)
	}
	if organizationID == nil {
		_, err = r.q.GetProjectByID(ctx, db.GetProjectByIDParams{ID: projectID, UserID: int32(numericUserID)})
	} else {
		_, err = r.q.GetProjectByUserOrgAndID(ctx, db.GetProjectByUserOrgAndIDParams{
			ID: projectID, UserID: int32(numericUserID), OrganizationID: organizationID,
		})
	}
...
var marshalDurableSyncPushResponse = json.Marshal

func (r *Repository) SaveSyncPushResult(ctx context.Context, userID, idempotencyKey string, response SyncPushResponse) error {
	raw, err := marshalDurableSyncPushResponse(ensurePushResponseDefaults(response))
	if err != nil {
