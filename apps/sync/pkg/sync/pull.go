...
func trimPullResponseToJSONBudget(
	...
	if len(conversations)+len(messages) > 0 {
		...
		compacted := compactPullResponsePayload(singleChangeResponse)
		if pullResponseFitsJSONBudget(compacted, budgetBytes) {
...
func compactPullResponsePayload(response SyncPullResponse) SyncPullResponse {
	response.Messages = append([]MessageSyncPayload(nil), response.Messages...)
	response.Conversations = append([]ConversationSyncPayload(nil), response.Conversations...)
	for i := range response.Messages {
		message := &response.Messages[i]
		if message.Trace != nil || message.ToolEvents != nil || message.AgentStatuses != nil || message.Sources != nil {
			message.ContentTruncated = true
		}
		message.Trace = nil
		message.ToolEvents = nil
		message.AgentStatuses = nil
		message.Sources = nil
		message.VectorClock = nil
		message.Patches = nil
	}
	for i := range response.Conversations {
		conversation := &response.Conversations[i]
		if conversation.Result != nil {
			conversation.ContentTruncated = true
		}
		conversation.Result = nil
		conversation.VectorClock = nil
		conversation.Patches = nil
	}
	return response
}
...
func (s *Service) calculateStateHash(ctx context.Context, userID string, orgID *int32) (string, error) {
	convCount, msgCount, err := s.getCounts(ctx, userID, orgID)
	if err != nil {
		return "", err
	}
...
func (s *Service) getCounts(ctx context.Context, userID string, orgID *int32) (int64, int64, error) {
	convCount, msgCount, err := s.repo.GetSyncCounts(ctx, userID, orgID)
	if err != nil {
		return 0, 0, fmt.Errorf("get sync counts: %w", err)
	}
