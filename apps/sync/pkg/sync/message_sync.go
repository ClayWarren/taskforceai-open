...
func (s *Service) syncMessages(
	...
	for _, incoming := range messages {
		...
		if err := s.handleMessageUpdateWithMessage(ctx, repo, userID, deviceID, version, organizationID, incoming); err != nil {
...
func (s *Service) handleMessageUpdateWithMessage(
	ctx context.Context,
	repo SyncRepository,
	userID string,
	deviceID string,
	...
	if len(incoming.Patches) > 0 {
		patched, err := s.handleMessagePatchWithMessage(ctx, repo, incoming, userID, organizationID, nil)
		if err != nil {
			return err
		}
		patched.Patches = nil
