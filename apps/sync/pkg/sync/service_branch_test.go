...
func TestService_TrimPullResponseToJSONBudget_TrimsRowsBeforeCompaction(t *testing.T) {
	...
	compactedFull := compactPullResponsePayload(full)
	budget := max(jsonPayloadSize(oneRow), jsonPayloadSize(compactedFull)) + 10
...
func TestService_CompactPullResponsePayload_MarksTruncatedContent(t *testing.T) {
	result := strings.Repeat("result", 10)
		...
		Messages: []MessageSyncPayload{
			{
				MessageID:     "m1",
				Sources:       []any{"source"},
				ToolEvents:    map[string]any{"tool": "search"},
	...
	compacted := compactPullResponsePayload(response)

	require.Len(t, compacted.Conversations, 1)
	assert.True(t, compacted.Conversations[0].ContentTruncated)
	assert.Nil(t, compacted.Conversations[0].Result)
	assert.Nil(t, compacted.Conversations[0].VectorClock)
	assert.Nil(t, compacted.Conversations[0].Patches)
	assert.NotNil(t, response.Conversations[0].Result)
	assert.NotNil(t, response.Conversations[0].VectorClock)
	assert.NotNil(t, response.Conversations[0].Patches)

	require.Len(t, compacted.Messages, 2)
	assert.True(t, compacted.Messages[0].ContentTruncated)
	assert.Nil(t, compacted.Messages[0].Sources)
	assert.Nil(t, compacted.Messages[0].ToolEvents)
	assert.Nil(t, compacted.Messages[0].AgentStatuses)
	assert.Nil(t, compacted.Messages[0].Trace)
	assert.Nil(t, compacted.Messages[0].VectorClock)
	assert.Nil(t, compacted.Messages[0].Patches)
	assert.False(t, compacted.Messages[1].ContentTruncated)
	assert.NotNil(t, response.Messages[0].Sources)
	assert.NotNil(t, response.Messages[0].ToolEvents)
	assert.NotNil(t, response.Messages[0].AgentStatuses)
	assert.NotNil(t, response.Messages[0].Trace)
	assert.NotNil(t, response.Messages[0].VectorClock)
	assert.NotNil(t, response.Messages[0].Patches)
}

func TestService_TrimPullResponseToJSONBudget_CompactsSingleOversizedMessage(t *testing.T) {
	updatedAt := timestampForTest(time.Unix(10, 0))
	messages := []MessageRecord{
		{
			MessageID:      "m1",
	...
	require.NoError(t, err)
	require.Len(t, response.Messages, 1)
	assert.True(t, response.Messages[0].ContentTruncated)
	assert.Equal(t, "visible content", response.Messages[0].Content)
	assert.Nil(t, response.Messages[0].Trace)
	assert.Nil(t, response.Messages[0].ToolEvents)
	assert.Nil(t, response.Messages[0].AgentStatuses)
	assert.Nil(t, response.Messages[0].Sources)
	assert.Equal(t, int32(4), response.LatestVersion)
}
...
func TestService_TrimPullResponseToJSONBudget_CompactsOversizedConversationResult(t *testing.T) {
	updatedAt := timestampForTest(time.Unix(10, 0))
	...
	assert.True(t, response.Conversations[0].ContentTruncated)
	assert.Nil(t, response.Conversations[0].Result)
	assert.Equal(t, "prompt", response.Conversations[0].UserInput)
	assert.Equal(t, int32(3), response.LatestVersion)
}
