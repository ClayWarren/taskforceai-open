...
type organizationExchangeRepo struct {
	*testutils.MockRepository
	user *auth.DeviceLoginUser
	err  error
}

func (r *organizationExchangeRepo) FindUserByIDForOrganization(context.Context, int, int) (*auth.DeviceLoginUser, error) {
	return r.user, r.err
}
...
func TestDeviceLoginService_ExchangeDeviceToken_UsesOrganizationRepository(t *testing.T) {
	secret := "secret_must_be_32_chars_long_exactly!!"
	t.Setenv("AUTH_PRIVATE_KEY", "")
	t.Setenv("AUTH_PUBLIC_KEY", "")
	t.Setenv("AUTH_PUBLIC_KEYS", "")
	...
	t.Cleanup(auth.ResetJWTKeysForTest)
	userID, orgID := 123, 77
	base := &testutils.MockRepository{
		DeviceLoginRecord:   &auth.DeviceLoginRecord{ID: 1, Status: auth.DeviceStatusAuthorized, UserID: &userID, InternalOrgID: &orgID, ExpiresAt: time.Now().Add(time.Hour)},
		MarkCompletedResult: true,
	}
	repo := &organizationExchangeRepo{MockRepository: base, user: &auth.DeviceLoginUser{ID: userID, Email: "org@example.com", InternalOrgID: &orgID}}
...
func TestDeviceLoginService_ExchangeDeviceToken_ErrorBranches(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name: "organization repository unsupported",
			repo: &testutils.MockRepository{
				DeviceLoginRecord: &auth.DeviceLoginRecord{
					ID:            1,
					Status:        auth.DeviceStatusAuthorized,
					UserID:        new(123),
					InternalOrgID: new(77),
					ExpiresAt:     time.Now().Add(time.Hour),
				},
			},
			wantErr: "repository does not support organization-scoped login",
		},
		{
			name: "missing user",
			repo: &testutils.MockRepository{
...
func TestDeviceLoginService_StartDeviceLogin_Errors(t *testing.T) {
	...
	t.Run("maximum attempts exceeded", func(t *testing.T) {
