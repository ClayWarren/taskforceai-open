...
func revokeTokenOnLogout(ctx context.Context, rawToken string) error {
	if rawToken == "" {
		return nil
	}
	...
	if err == nil {
		...
		if revoker == nil {
			if handler.IsProductionEnv() {
				return errors.New("token revocation store unavailable")
			}
			return nil
		}
	...
	// Invalid or expired local tokens have no valid server-side session to revoke.
	// Logout still clears every authentication cookie in the HTTP handler.
	return nil
}
...
func LogoutHandler(w http.ResponseWriter, r *http.Request) {
	...
	revokeErr := revokeTokenOnLogout(r.Context(), handler.ExtractToken(r))
	...
	if revokeErr != nil {
		handler.JSONError(w, http.StatusServiceUnavailable, "Session could not be revoked; local cookies were cleared")
		return
	}
