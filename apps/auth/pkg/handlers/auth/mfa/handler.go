...
func verifyAuthenticatorMFA(ctx context.Context, user *adapterauth.AuthenticatedUser, code string) (StatusResponse, error) {
	settings, err := verifyCurrentUserCodeSettings(ctx, user, code)
	if err != nil {
	...
	if _, err := q.EnableUserMFAIfSecretMatches(ctx, db.EnableUserMFAIfSecretMatchesParams{ID: userID, MfaTotpSecret: settings.MfaTotpSecret}); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return StatusResponse{}, huma.Error409Conflict("Authenticator setup changed; verify the current secret")
		}
...
func disableAuthenticatorMFA(ctx context.Context, user *adapterauth.AuthenticatedUser, code string) (StatusResponse, error) {
	...
	if _, err := q.DisableUserMFAIfSecretMatches(ctx, db.DisableUserMFAIfSecretMatchesParams{ID: userID, MfaTotpSecret: settings.MfaTotpSecret}); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return StatusResponse{}, huma.Error409Conflict("Authenticator settings changed; retry")
		}
...
func verifyCurrentUserCode(ctx context.Context, user *adapterauth.AuthenticatedUser, code string) error {
	_, err := verifyCurrentUserCodeSettings(ctx, user, code)
	return err
}

func verifyCurrentUserCodeSettings(ctx context.Context, user *adapterauth.AuthenticatedUser, code string) (db.GetUserMFASettingsRow, error) {
	settings, err := loadMFASettings(ctx, user)
	if err != nil {
		return db.GetUserMFASettingsRow{}, err
	}
	if settings.MfaTotpSecret == nil {
		return db.GetUserMFASettingsRow{}, huma.Error400BadRequest("Authenticator app setup has not been started")
	}
	...
	if err != nil {
		...
		return db.GetUserMFASettingsRow{}, huma.Error500InternalServerError("Unable to verify authenticator code")
	}
	if !verifyTOTPCode(secret, code, time.Now()) {
		return db.GetUserMFASettingsRow{}, huma.Error401Unauthorized("Invalid authenticator code")
	}
	return settings, nil
}
