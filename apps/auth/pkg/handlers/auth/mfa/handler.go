...
func RegisterHandlers(api huma.API) {
	...
	huma.Register(api, huma.Operation{
		OperationID: "verify-authenticator-mfa",
		Method:      http.MethodPost,
		Path:        "/api/v1/auth/mfa/authenticator/verify",
		Summary:     "Verify and enable authenticator app MFA",
		Tags:        []string{"Auth"},
	}, func(ctx context.Context, input *struct {
		adapterhandler.AuthContext
		requestInfo
		...
		if err := checkMFARateLimit(ctx, input.User, input.ClientIP, "verify", mfaVerifyMaxRequests); err != nil {
			return nil, err
		}
		body, err := verifyAuthenticatorMFA(ctx, input.User, input.Body.Code)
		if err != nil {
			return nil, err
		}
	...
	huma.Register(api, huma.Operation{
		OperationID: "disable-authenticator-mfa",
		...
		if err := checkMFARateLimit(ctx, input.User, input.ClientIP, "disable", mfaDisableMaxRequests); err != nil {
			return nil, err
		}
		body, err := disableAuthenticatorMFA(ctx, input.User, input.Body.Code)
		if err != nil {
			return nil, err
		}
		return &struct{ Body StatusResponse }{Body: body}, nil
...
func (r *requestInfo) Resolve(ctx huma.Context) []error {
	...
	r.ClientIP = clientIPFromRemoteAddr(ctx.RemoteAddr())
	return nil
}
