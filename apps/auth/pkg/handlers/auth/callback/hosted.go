...
func (h *HostedHandlerStruct) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if resp.OrganizationID != "" {
		...
		// Resolve internal ID (skip if DB is unavailable)
		if q != nil {
			// This lookup is intentionally keyed by organizations.workos_organization_id,
			// which is uniquely indexed in the shared schema
			// (organizations_workos_organization_id_key).
			if org, err := q.GetOrganizationByWorkosID(r.Context(), &resp.OrganizationID); err == nil {
				id := int(org.ID)
...
func LinkOrCreateWorkOSUserWithTM(ctx context.Context, q *db.Queries, workosUser usermanagement.User, tm TransactionManager) (*auth.AuthUser, error) {
	...
	userRepo := auth.NewAuthUserRepository(qtx)
