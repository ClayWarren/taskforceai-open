...
func TestGoogleDriveCallbackHandler_Success(t *testing.T) {
	...
	expiresAt := int32(expiry.Unix()) // #nosec G115 -- OAuth expiry is within the supported database epoch range.
	mockGoogle := &testutils.MockGoogleClient{
		Token: &oauth2.Token{AccessToken: "token", Expiry: expiry},
	...
	h := &GoogleDriveCallbackHandlerStruct{
		...
		GetQueries: func(ctx context.Context) (*db.Queries, error) {
			...
			insertArgs := callbackInsertArgs()
			insertArgs[7] = &expiresAt
			mockPool.ExpectQuery("INSERT INTO accounts").
				...
				WillReturnRows(pgxmock.NewRows([]string{
					...
					"id_token",
					"session_state",
				}).AddRow("acc_1", int32(123), "oauth", "google-drive", "google-user-id", nil, new("token"), &expiresAt, nil, nil, nil, nil))
	...
	for _, c := range cookies {
		if c.Name == "oauth_state_google_drive" && c.MaxAge == -1 {
			stateCookieCleared = true
			break
		}
	}
	if !stateCookieCleared {
		t.Error("expected Google Drive OAuth state cookie to be cleared after successful callback")
	}
}
...
func TestGoogleDriveCallbackHandler_ExchangeError(t *testing.T) {
	...
	req.AddCookie(&http.Cookie{Name: "oauth_state_google_drive", Value: "s"})
	w := serve(h, req)
