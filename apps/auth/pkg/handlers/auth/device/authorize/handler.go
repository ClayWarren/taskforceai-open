...
func RegisterHandler(api huma.API) {
	huma.Register(api, huma.Operation{
		...
		return authorizeDeviceLoginForOrganization(ctx, input.User.ID, input.User.OrgID, input.requestInfo, input.Body, defaultDeps(q))
	})
}
...
func authorizeDeviceLogin(ctx context.Context, userID int, req requestInfo, input AuthorizeRequest, deps Deps) (*struct{ Body AuthorizeResponse }, error) {
	return authorizeDeviceLoginForOrganization(ctx, userID, nil, req, input, deps)
}

func authorizeDeviceLoginForOrganization(ctx context.Context, userID int, internalOrgID *int, req requestInfo, input AuthorizeRequest, deps Deps) (*struct{ Body AuthorizeResponse }, error) {
	...
	if internalOrgID != nil {
		tenantService, ok := deps.Service.(auth.TenantAwareDeviceService)
		if !ok {
			return nil, huma.Error500InternalServerError("internal_error")
		}
		err = tenantService.AuthorizeDeviceLoginForOrganization(ctx, userID, internalOrgID, input.UserCode)
	} else {
