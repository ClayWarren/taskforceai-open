...
var (
	appleClientCache       sync.Map
	newAppleClient         = cachedAppleClient
	defaultAppleQueries    = appdatabase.GetQueries
	defaultLinkAppleUser   = linkOrCreateAppleUser
	loadOrStoreAppleClient = appleClientCache.LoadOrStore
)

func cachedAppleClient(clientID string) providers.AppleProvider {
	if cached, ok := appleClientCache.Load(clientID); ok {
		if client, valid := cached.(providers.AppleProvider); valid {
			return client
		}
		appleClientCache.Delete(clientID)
	}

	client := providers.AppleProvider(providers.NewAppleClient(clientID))
	actual, _ := loadOrStoreAppleClient(clientID, client)
	if cached, valid := actual.(providers.AppleProvider); valid {
		return cached
	}

	appleClientCache.Store(clientID, client)
	return client
}
...
func (h *AppleHandlerStruct) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	writeMobileSessionResponseAt(w, r, user, auditLog, "Apple", time.Unix(claims.AuthTime, 0).UTC())
}
