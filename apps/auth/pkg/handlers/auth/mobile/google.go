...
func (h *GoogleHandlerStruct) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	writeMobileSessionResponseAt(w, r, user, auditLog, "Google", googleAuthenticationTime(payload))
}

func googleAuthenticationTime(payload *idtoken.Payload) time.Time {
	if payload == nil {
		return time.Time{}
	}
	if unix, ok := authClaimUnix(payload.Claims["auth_time"]); ok {
		return time.Unix(unix, 0).UTC()
	}
	if payload.IssuedAt > 0 {
		return time.Unix(payload.IssuedAt, 0).UTC()
	}
	return time.Time{}
}

func authClaimUnix(value any) (int64, bool) {
