...
var (
	ensureSAMLMember       = ensureSAMLMembership
...
func (h *CallbackHandlerStruct) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	user, txErr := h.linkUserAndMembership(r.Context(), q, profile)
	if txErr != nil {
...
func (h *CallbackHandlerStruct) linkUserAndMembership(ctx context.Context, q *db.Queries, profile sso.Profile) (*auth.AuthUser, error) {
	...
	err = postgres.WithTx(ctx, p, func(tx pgx.Tx) error {
		...
		user = linkedUser
		return ensureSAMLMember(ctx, txQ, user.ID, profile.OrganizationID)
	})
	return user, err
}
