...
func TestCallbackHandlerSignedStateAndSessionBranches(t *testing.T) {
	...
	t.Run("signed state split and full name", func(t *testing.T) {
		...
		for _, cookie := range rr.Result().Cookies() {
			if cookie.Name == auth.SessionCookieName || cookie.Name == auth.SecureSessionCookieName {
				rawSession = cookie.Value
				break
			}
		}
		require.NotEmpty(t, rawSession)
		parsed, err := auth.VerifyToken(rawSession)
		require.NoError(t, err)
		claims, ok := parsed.Claims.(jwt.MapClaims)
		require.True(t, ok)
		assert.Equal(t, float64(99), claims["org_id"])
		assert.Equal(t, "org_full_name", claims["workos_org_id"])
...
func TestSigninHandlerStateGenerationErrorsAndWrapperNilQueries(t *testing.T) {
	...
	t.Run("random failure", func(t *testing.T) {
		...
		original := readStateRandom
		readStateRandom = func([]byte) (int, error) { return 0, errors.New("entropy unavailable") }
		t.Cleanup(func() { readStateRandom = original })
...
func expectSAMLTransactionWithOrgIDZero(mockPool pgxmock.PgxPoolIface, workosID string) {
	mockPool.ExpectBeginTx(pgx.TxOptions{})
	ts := pgtype.Timestamp{Time: time.Now(), Valid: true}
	expectSAMLOrgRow(mockPool, workosID, 99, ts)
	mockPool.ExpectQuery("SELECT (.+) FROM memberships").
		...
		WillReturnRows(pgxmock.NewRows([]string{"id", "organization_id", "user_id", "role", "created_at", "updated_at"}).
			AddRow(int32(1), int32(99), int32(1), db.OrganizationRoleMEMBER, ts, ts))
	mockPool.ExpectCommit()
}
