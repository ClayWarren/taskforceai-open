...
func updateSettings(ctx context.Context, user *adapterauth.AuthenticatedUser, req UpdateSettingsRequest) error {
	...
	if err := invalidateUserSettingsCache(ctx, userID); err != nil {
		return huma.Error503ServiceUnavailable("Settings saved, but cache invalidation failed")
	}
	slog.Info("User settings updated", "userId", user.ID)
	return nil
}

func invalidateUserSettingsCache(ctx context.Context, userID int32) error {
