...
func TestProcessEvent_GroupUserAddedSuccess(t *testing.T) {
	...
	mockVal.On("ValidatePayload", mock.Anything, mock.Anything).
		Return(`{"id":"evt_add","event":"dsync.group.user_added","data":{"user":{"organization_id":"org_1","email":"member@example.com","id":"u1"}}}`, nil)
...
func TestProcessEvent_GroupUserRemovedSuccess(t *testing.T) {
	mockVal := new(webhookValidatorMock)
	mockVal.On("ValidatePayload", mock.Anything, mock.Anything).
		Return(`{"id":"evt_remove","event":"dsync.group.user_removed","data":{"user":{"organization_id":"org_1","email":"member@example.com","id":"u1"}}}`, nil)
