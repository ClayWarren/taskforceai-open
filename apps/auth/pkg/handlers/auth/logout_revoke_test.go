...
func TestRevokeTokenOnLogout_RequiresRevokerInProduction(t *testing.T) {
	t.Setenv("NODE_ENV", "production")
	t.Setenv("AUTH_SECRET", "test-secret-value-that-is-long-enough")
	authpkg.ResetJWTKeysForTest()
	t.Cleanup(authpkg.ResetJWTKeysForTest)
	token, err := authpkg.EncodeSessionToken(authpkg.SessionUser{ID: "1", Email: "user@example.com"}, os.Getenv("AUTH_SECRET"), 3600)
	require.NoError(t, err)

	original := getTokenRevoker
	getTokenRevoker = func() adapterauth.TokenRevoker { return nil }
	t.Cleanup(func() { getTokenRevoker = original })

	err = revokeTokenOnLogout(context.Background(), token)
	require.ErrorContains(t, err, "token revocation store unavailable")
}
