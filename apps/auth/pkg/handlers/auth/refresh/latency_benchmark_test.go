...
func BenchmarkRefreshHandlerLatencyProfile(b *testing.B) {
	...
	b.StopTimer()
	reportRefreshLatencyProfile(b, samples)
}
...
func reportRefreshLatencyProfile(b *testing.B, samples []time.Duration) {
	b.Helper()
	if len(samples) == 0 {
		return
	}
	...
	b.ReportMetric(float64(refreshPercentileDuration(ordered, 0.50).Microseconds()), "p50_us")
	b.ReportMetric(float64(refreshPercentileDuration(ordered, 0.95).Microseconds()), "p95_us")
	b.ReportMetric(float64(refreshPercentileDuration(ordered, 0.99).Microseconds()), "p99_us")
}

func refreshPercentileDuration(ordered []time.Duration, percentile float64) time.Duration {
	if len(ordered) == 0 {
		return 0
	}
