...
The desktop app owns its Tauri host and desktop-only TypeScript UI while reusing the shared TaskForceAI product UI and Rust app-server. Desktop-specific code owns these capabilities:
...
Desktop-only TypeScript belongs in `apps/desktop/ui`. The desktop entrypoint may reuse product UI from `apps/web/app` while platform-neutral contracts, client logic, adapters, and primitives belong in `packages/*`. Browser-only implementation remains in `apps/web`.
