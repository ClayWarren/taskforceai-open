...
#[derive(Debug, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct DesktopWorktreeResetParams {
    repository: Option<String>,
    path: String,
    base_ref: Option<String>,
}
...
#[derive(Debug, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct DesktopWorktreeRemoveParams {
    repository: Option<String>,
    path: String,
    force: Option<bool>,
}
...
#[derive(Debug, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct DesktopWorkspaceCheckpointCaptureParams {
    root: Option<String>,
...
#[derive(Debug, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct DesktopWorkspaceCheckpointRestoreParams {
    conversation_id: String,
...
) -> Result<crate::worktrees::DesktopGitWorktreeListResult, String> {
    let repository =
...
) -> Result<crate::worktrees::DesktopGitWorktreeCreateResult, String> {
...
#[tauri::command]
pub async fn desktop_worktree_reset(
    state: tauri::State<'_, AppState>,
    params: DesktopWorktreeResetParams,
) -> Result<crate::worktrees::DesktopGitWorktreeActionResult, String> {
    let repository = repository_path_from_params(&state, params.repository)?;
    crate::worktrees::reset_git_worktree(&repository, &params.path, params.base_ref.as_deref())
}

#[tauri::command]
pub async fn desktop_worktree_remove(
    state: tauri::State<'_, AppState>,
    params: DesktopWorktreeRemoveParams,
) -> Result<crate::worktrees::DesktopGitWorktreeActionResult, String> {
    let repository = repository_path_from_params(&state, params.repository)?;
    crate::worktrees::remove_git_worktree(&repository, &params.path, params.force.unwrap_or(false))
}

#[tauri::command]
pub async fn desktop_workspace_checkpoint_capture(
    app: tauri::AppHandle,
    state: tauri::State<'_, AppState>,
    params: DesktopWorkspaceCheckpointCaptureParams,
) -> Result<crate::workspace_checkpoints::WorkspaceCheckpointResult, String> {
    let root = selected_workspace_root(&state, params.root.as_deref())?;
    ...
    tauri::async_runtime::spawn_blocking(move || {
        crate::workspace_checkpoints::capture(&app_data_dir, &root, &params.conversation_id)
    })
    .await
...
#[tauri::command]
pub async fn desktop_workspace_checkpoint_restore(
    app: tauri::AppHandle,
    ...
    let app_data_dir = app
        ...
        .map_err(|error| format!("Failed to resolve app data directory: {error}"))?;
    tauri::async_runtime::spawn_blocking(move || {
        crate::workspace_checkpoints::restore(
            &app_data_dir,
