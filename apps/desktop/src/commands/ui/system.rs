...
fn authorize_desktop_computer_use_observe(window: &tauri::WebviewWindow) -> Result<(), String> {
    ...
    if desktop_privileged_origin_allowed(&url) {
        Ok(())
    } else {
        Err(
...
fn desktop_privileged_origin_allowed(url: &Url) -> bool {
    match url.scheme() {
        "tauri" | "asset" => true,
        "http" | "https" => {
...
mod tests {
    ...
    fn desktop_privileged_origin_allows_packaged_and_local_urls() {
        for url in [
            ...
            let parsed = Url::parse(url).expect("fixture URL should parse");
            assert!(
                desktop_privileged_origin_allowed(&parsed),
