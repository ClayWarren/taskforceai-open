...
fn browser_action_scripts_are_generated_from_typed_actions() {
    ...
    assert!(select_script.contains("Browser preview selection cancelled."));
    assert!(select_script.contains("document.addEventListener('keydown', cancelSelection, true)"));
    assert!(select_script.contains("clearTimeout(selectionTimeout)"));
    assert!(select_script.contains("setTimeout(() => cancelSelection(null), 55000)"));
}
