...
mod tests {
    ...
    fn validate_renderer_server_config_allows_supported_endpoints() {
        ...
        let aliased_server = DesktopMcpServerConfig {
            name: "innocent-alias".to_string(),
            endpoint: "taskforceai:local-computer-use".to_string(),
            enabled: true,
        };
        validate_renderer_server_config(&aliased_server)
            .expect_err("renaming the built-in server must not bypass authorization");
