...
fn screen_memory_status_many_captures_benchmark() {
    let (manager, root) = test_manager("status-many-captures");
    fs::create_dir_all(&manager.capture_dir).expect("capture dir");
    for index in 0..1_200 {
        fs::write(
            manager.capture_dir.join(format!("screen-{index:04}.png")),
            [index as u8; 16],
        )
        .expect("write capture");
    }
    fs::write(manager.capture_dir.join("ignore.txt"), b"not a capture").expect("write text");
    ...
    for _ in 0..80 {
        ...
        assert_eq!(status.capture_count, 1_200);
        assert_eq!(status.bytes, 1_200 * 16);
    }

    timings.sort_unstable();
    let median = timings[timings.len() / 2];
    println!(
        "screen_memory_status_many_captures median={}us iterations={}",
        median.as_micros(),
        timings.len()
    );
