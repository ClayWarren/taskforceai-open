...
#[tokio::test]
async fn voice_listen_timeout_covers_descendants_holding_output_pipes() {
    let started = std::time::Instant::now();
    let err = voice_listen_command(
        "sleep 5 & printf 'partial transcript'",
        Duration::from_millis(50),
    )
    .await
    .expect_err("inherited output pipe should time out");
...
async fn voice_listen_output_reports_truncation_after_limit_is_full() {
    ...
    let output = read_limited_output(input, 4, OUTPUT_READ_ERROR)
        .await
        .expect("read output");
...
async fn voice_listen_output_reports_read_errors() {
    let err = read_limited_output(FailingReader, 8, OUTPUT_READ_ERROR)
        .await
