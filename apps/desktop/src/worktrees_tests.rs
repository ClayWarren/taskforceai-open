use super::{
    ...
    git_error, git_success, git_text, list_git_worktrees, parse_worktree_porcelain,
    remove_git_worktree, reset_git_worktree, resolve_repository_root,
};
...
#[test]
fn reset_and_remove_manage_secondary_worktree_lifecycle() {
    let _guard = lock_env();
    let root = unique_test_dir("git-worktree-lifecycle");
    let repo = root.join("repo");
    init_git_repo(&repo);
    let target = root.join("lifecycle-worktree");
    let created = create_git_worktree(
        &repo,
        Some(target.to_string_lossy().as_ref()),
        Some("codex/lifecycle"),
        Some("HEAD"),
    )
    .expect("create lifecycle worktree");

    std::fs::write(target.join("README.md"), "dirty").expect("dirty tracked file");
    std::fs::write(target.join("untracked.txt"), "temporary").expect("write untracked file");
    reset_git_worktree(&repo, &created.worktree.path, None).expect("reset worktree");
    assert!(!target.join("untracked.txt").exists());
    assert_ne!(
        std::fs::read_to_string(target.join("README.md")).expect("read reset file"),
        "dirty"
    );

    remove_git_worktree(&repo, &created.worktree.path, true).expect("remove worktree");
    assert!(!target.exists());
    let branch_status = Command::new("git")
        .args(["show-ref", "--verify", "refs/heads/codex/lifecycle"])
        .current_dir(&repo)
        .status()
...
#[test]
fn reset_and_remove_reject_primary_and_unregistered_worktrees() {
    let _guard = lock_env();
    let root = unique_test_dir("git-worktree-rejections");
    let repo = root.join("repo");
    init_git_repo(&repo);

    let primary_error = reset_git_worktree(&repo, repo.to_string_lossy().as_ref(), None)
        .expect_err("primary worktree reset should fail");
    assert!(primary_error.contains("primary worktree"));

    let unregistered = root.join("unregistered");
    std::fs::create_dir_all(&unregistered).expect("create unregistered directory");
    let unregistered_error =
        remove_git_worktree(&repo, unregistered.to_string_lossy().as_ref(), false)
            .expect_err("unregistered worktree removal should fail");
    assert!(unregistered_error.contains("not a registered worktree"));
    std::fs::remove_dir_all(root).ok();
}

#[test]
fn git_text_and_success_report_command_failures() {
    let _guard = lock_env();
    let root = unique_test_dir("git-worktree-command-errors");
    let repo = root.join("repo");
    init_git_repo(&repo);

    assert!(
        git_text(&repo, &["rev-parse", "refs/heads/definitely-missing"])
            .expect_err("missing ref lookup should fail")
            .contains("git rev-parse refs/heads/definitely-missing failed")
    );
    assert!(git_success(&repo, &["definitely-not-a-command"])
