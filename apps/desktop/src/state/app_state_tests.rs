...
#[test]
fn remote_keep_awake_tracks_restarts_and_shutdown() {
    let state = app_state();

    state
        .sync_remote_keep_awake(true)
        .expect("start Remote keep-awake process");
    state
        .sync_remote_keep_awake(true)
        .expect("reuse running Remote keep-awake process");
    ...
    #[cfg(target_os = "macos")]
    {
        let mut process = state
            .remote_keep_awake
            .lock()
            .expect("lock Remote keep-awake process");
        let child = process.as_mut().expect("running caffeinate process");
        child.kill().expect("stop caffeinate process");
        child.wait().expect("reap caffeinate process");
    }

    state
        .sync_remote_keep_awake(true)
        .expect("restart completed Remote keep-awake process");
    state
        .sync_remote_keep_awake(false)
        .expect("stop Remote keep-awake process");
    state
        .sync_remote_keep_awake(false)
        .expect("ignore an already stopped Remote keep-awake process");
}

#[test]
fn remote_keep_awake_reports_poisoned_state() {
    let state = app_state();
    let process = Arc::clone(&state.remote_keep_awake);
    assert!(std::thread::spawn(move || {
        let _guard = process.lock().expect("lock Remote keep-awake state");
        panic!("poison Remote keep-awake state");
    })
    .join()
    .is_err());
    ...
    assert!(state
        .sync_remote_keep_awake(false)
        .expect_err("poisoned Remote keep-awake state should fail")
        .contains("state is unavailable"));
}

#[test]
fn remote_keep_awake_reports_process_status_errors() {
    let error = remote_keep_awake_is_running(Err(std::io::Error::other("status unavailable")))
        .expect_err("process status failures should be reported");

    assert!(error.contains("Check Remote keep-awake process: status unavailable"));
}

#[test]
fn remote_keep_awake_reports_process_start_errors() {
    let error = remote_keep_awake_process(Err(std::io::Error::other("spawn unavailable")))
        .expect_err("process start failures should be reported");

    assert!(error.contains("Start Remote keep-awake process: spawn unavailable"));
}
...
fn session_workspace_persistence_reports_io_and_encoding_failures() {
    ...
    let temporary_path = root.join(format!("bindings.tmp-{}", std::process::id()));
    std::fs::create_dir(&temporary_path).expect("create temporary-file blocker");
    let temporary_write_error = save_session_workspaces(&bindings_path, &BTreeMap::new())
        .expect_err("a directory cannot be overwritten as the temporary file");
    assert!(temporary_write_error.contains("temporary file"));
    std::fs::remove_dir(&temporary_path).expect("remove temporary-file blocker");
...
fn session_workspace_methods_report_state_and_persistence_failures() {
    ...
    assert!(state
        .local_coding_workspace_for_session("thread-1")
        .expect("failed persistence should leave readable state")
        .is_none());

    state
        .local_coding_session_workspaces
        .write()
        .expect("session workspace state")
        .insert(
            "thread-1".to_string(),
            vec![PathBuf::from("/tmp/original-project")],
        );
    let replacement_error = state
        .bind_local_coding_session("thread-1", PathBuf::from("/tmp/replacement-project"))
        .expect_err("failed replacement persistence should restore the previous binding");
    assert!(replacement_error.contains("Failed to persist"));
    assert_eq!(
        state
            .local_coding_workspace_for_session("thread-1")
            .expect("restored session workspace state"),
        Some(PathBuf::from("/tmp/original-project"))
    );
