...
pub struct AppState {
    ...
    remote_keep_awake: Arc<Mutex<Option<Child>>>,
}
...
impl AppState {
    pub fn new(
        ...
        Self {
            ...
            remote_keep_awake: Arc::new(Mutex::new(None)),
        }
    }

    pub fn sync_remote_keep_awake(&self, enabled: bool) -> Result<(), String> {
        let mut process = self
            .remote_keep_awake
            .lock()
            .map_err(|_| "Remote keep-awake state is unavailable.".to_string())?;
        ...
        if let Some(child) = process.as_mut() {
            if remote_keep_awake_is_running(child.try_wait())? {
                return Ok(());
            }
            *process = None;
        }
        ...
        #[cfg(target_os = "macos")]
        {
            let child = Command::new("/usr/bin/caffeinate")
                ...
                .stderr(Stdio::null())
                .spawn();
            let child = remote_keep_awake_process(child)?;
            *process = Some(child);
        }
    ...
    pub fn bind_local_coding_session_roots(
        ...
        let previous = workspaces.insert(session_id.clone(), workspace_roots);
        if let Some(path) = &self.local_coding_session_workspaces_path {
            if let Err(error) = save_session_workspaces(path, &workspaces) {
                match previous {
                    Some(previous) => {
                        workspaces.insert(session_id, previous);
                    }
                    None => {
...
fn remote_keep_awake_is_running(
    status: std::io::Result<Option<std::process::ExitStatus>>,
) -> Result<bool, String> {
    match status {
        Ok(None) => Ok(true),
        Ok(Some(_)) => Ok(false),
        Err(error) => Err(format!("Check Remote keep-awake process: {error}")),
    }
}

fn remote_keep_awake_process(process: std::io::Result<Child>) -> Result<Child, String> {
    process.map_err(|error| format!("Start Remote keep-awake process: {error}"))
}
...
fn save_session_workspaces(
    ...
    std::fs::write(&temporary_path, raw).map_err(|error| {
        format!(
            "Failed to persist local coding session workspaces temporary file {}: {error}",
