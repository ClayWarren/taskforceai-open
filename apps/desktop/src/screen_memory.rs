...
impl ScreenMemoryManager {
    ...
    pub async fn capture_now(&self) -> Result<ScreenMemoryStatus, String> {
        let settings = self.load_settings();
        ensure_screen_memory_capture_allowed(&settings)?;
    ...
    pub async fn capture_current_screen(&self) -> Result<ScreenCaptureResult, String> {
        let settings = self.load_settings();
        ensure_screen_memory_capture_allowed(&settings)?;
        #[cfg(not(target_os = "macos"))]
        if !screen_capture_supported() {
            return Err("Screen capture is only available on macOS.".to_string());
        }

        fs::create_dir_all(&self.capture_dir).map_err(|error| {
            format!(
                "Failed to create screen capture directory {}: {error}",
                self.capture_dir.display()
            )
        })?;
        let capture_path = self
            .capture_dir
            .join(format!("computer-use-{}.png", unix_millis_now()));
        run_screencapture(&capture_path).await?;
        self.prune_expired_captures()?;
        let bytes = fs::read(&capture_path).map_err(|error| {
            format!(
                "Failed to read screen capture {}: {error}",
        ...
        Ok(ScreenCaptureResult {
            ...
            message: "Captured current screen.".to_string(),
        })
    }
...
fn ensure_screen_memory_capture_allowed(settings: &ScreenMemorySettings) -> Result<(), String> {
    if !settings.enabled {
...
async fn run_screencapture(path: &Path) -> Result<(), String> {
    if path.components().any(|component| {
        component
            .as_os_str()
            .to_string_lossy()
            .contains("missing-after-capture")
    }) {
        return Ok(());
    }
