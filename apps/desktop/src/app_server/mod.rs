...
use taskforceai_app_client::{
    ...
    AppServerSpawnOptions, ManagedAppServerRuntime, RuntimeUpdateError,
};
...
const DESKTOP_KEYCHAIN_SERVICE: &str = "com.taskforceai.desktop.auth";
const DESKTOP_DEV_KEYCHAIN_SERVICE: &str = "com.taskforceai.desktop.dev.auth";

const fn desktop_keychain_service() -> &'static str {
    if cfg!(debug_assertions) {
        DESKTOP_DEV_KEYCHAIN_SERVICE
    } else {
...
pub struct DesktopAppServer {
    ...
    runtime_manager: Option<Arc<ManagedAppServerRuntime>>,
}
...
impl DesktopAppServer {
    pub fn new(api_base_url: String) -> Self {
        Self {
            ...
            options: AppServerSpawnOptions {
                ...
                run_store_path: None,
                keychain_service: Some(desktop_keychain_service().to_string()),
    ...
    pub fn with_managed_runtime(
        api_base_url: String,
        runtime_directory: std::path::PathBuf,
    ) -> Self {
        let mut server = Self::new(api_base_url.clone());
        if std::env::var_os("TASKFORCEAI_APP_SERVER").is_none() {
            server.runtime_manager = Some(Arc::new(ManagedAppServerRuntime::new(
                &api_base_url,
                runtime_directory,
    ...
    pub async fn http_pairing_info(
        ...
        let binary = self.selected_binary().await;
        let server = match start_http_app_server(&binary, &self.options).await {
            Ok(server) => server,
            Err(error) if self.is_managed_binary(&binary) => {
                if let Some(manager) = &self.runtime_manager {
                    manager.rollback(&binary).await;
                    start_http_app_server(manager.bundled_binary(), &self.options).await?
                } else {
                    return Err(error);
                }
            }
    ...
    pub async fn thread_handoff(
        ...
        if has_active_turn {
            if let Err(error) = source
                ...
                .await
            {
                return Err(format!("Interrupt source turn before handoff: {error}"));
            }
            exported = source
                .thread_read(ThreadIDParams {
                    thread_id: thread_id.to_string(),
                ...
                .await
                .map_err(|error| format!("Reread interrupted source thread: {error}"))?;
        }

        let imported = target
            .thread_import(ThreadImportParams {
                thread: exported.thread,
                overwrite: false,
            })
            .await
    ...
    async fn local_request_handle(&self) -> Result<AppServerRequestHandle, AppClientError> {
        ...
        if guard.is_none() {
            let client = self.spawn_local_client().await?;
    ...
    pub(crate) async fn refresh_managed_runtime(&self) -> Result<bool, RuntimeUpdateError> {
        let Some(manager) = &self.runtime_manager else {
            return Ok(false);
        };
        if !manager.check_for_update(&self.options).await? {
            return Ok(false);
        }

        let mut client = self.client.lock().await;
        if let Some(mut managed) = client.take() {
            managed.client.kill().await;
        }
        drop(client);
        *self.http_server.lock().await = None;
        Ok(true)
    }

    async fn spawn_local_client(&self) -> Result<AppServerClient, AppClientError> {
        let binary = self.selected_binary().await;
        match self.spawn_initialized(&binary).await {
            Ok(client) => Ok(client),
            Err(error) if self.is_managed_binary(&binary) => {
                if let Some(manager) = &self.runtime_manager {
                    warn!(
                        ...
                        error = %error,
                        "Managed app-server launch failed; retrying bundled runtime"
                    );
                    manager.rollback(&binary).await;
                    self.spawn_initialized(manager.bundled_binary()).await
                } else {
                    Err(error)
                }
            }
    ...
    ) -> Result<AppServerClient, AppClientError> {
        info!(
            target: "app_server",
            binary = %binary.display(),
    ...
    async fn selected_binary(&self) -> std::path::PathBuf {
        match &self.runtime_manager {
            Some(manager) => manager.active_binary().await,
            None => default_app_server_binary(),
        }
    }

    fn is_managed_binary(&self, binary: &std::path::Path) -> bool {
        self.runtime_manager
            .as_ref()
            .is_some_and(|manager| binary != manager.bundled_binary())
    }
}
...
mod tests {
    ...
    use super::{
        DesktopActiveEnvironment, DesktopAppServer, DesktopAppServerEnvironmentStatus,
        DesktopHttpAppServerError, DesktopHttpPairingInfo, DesktopRemoteAppServer,
        DesktopSshConnectError, DesktopSshConnectParams, DesktopSshConnectResult,
        DesktopSshProbeError, DESKTOP_DEV_KEYCHAIN_SERVICE,
    };

    #[cfg(unix)]
    #[tokio::test]
    async fn reusing_ssh_connection_reactivates_remote_environment() {
        let spawn_sleeper = || {
            let mut command = tokio::process::Command::new("sh");
            command.arg("-c").arg("sleep 30").kill_on_drop(true);
            command.spawn().expect("spawn SSH process fixture")
        };
        let result = DesktopSshConnectResult {
            target: "user@example.com".to_string(),
            remote_base_url: "http://127.0.0.1:4123".to_string(),
            local_base_url: "http://127.0.0.1:5123".to_string(),
            local_port: 5123,
            remote_port: 4123,
            pairing: DesktopHttpPairingInfo {
                base_url: "http://127.0.0.1:5123".to_string(),
        ...
        *server.remote_server.lock().await = Some(DesktopRemoteAppServer {
            target: result.target.clone(),
            remote_child: spawn_sleeper(),
            forward_child: spawn_sleeper(),
            result: result.clone(),
            session_token: "token".to_string(),
        });
        *server.active_environment.lock().await = DesktopActiveEnvironment::Local;

        let reused = server
            .ssh_connect(DesktopSshConnectParams {
                target: result.target.clone(),
                app_server_path: None,
                remote_port: None,
            })
            .await
            .expect("reuse running SSH connection");

        assert_eq!(reused, result);
        let status = server.environment_status().await;
        assert_eq!(status.active, "remote");
        assert_eq!(status.target.as_deref(), Some("user@example.com"));
    }
    ...
    fn desktop_app_server_stores_api_base_url_option() {
        ...
        assert_eq!(
            server.options.keychain_service.as_deref(),
            Some(DESKTOP_DEV_KEYCHAIN_SERVICE)
        );
    }
