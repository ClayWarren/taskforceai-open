...
pub(super) async fn start_remote_app_server(
    ...
    let remote_port = port_from_url(&startup.base_url)?;
    let local_port = allocate_local_port().map_err(DesktopSshConnectError::AllocateLocalPort)?;
    let forward_spec = format!("127.0.0.1:{local_port}:127.0.0.1:{remote_port}");
    let forward_child = Command::new("ssh")
        .arg("-N")
        .arg("-o")
        .arg("BatchMode=yes")
        .arg("-o")
    ...
    let session_token = pair_remote_app_server(&local_base_url, &pairing_code)
        .await
        .map_err(|error| {
            let _ = remote_child.start_kill();
            DesktopSshConnectError::Pair(error)
        })?;
...
async fn pair_remote_app_server(
    local_base_url: &str,
    pairing_code: &str,
