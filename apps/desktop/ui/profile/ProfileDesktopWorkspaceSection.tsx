...
export function WorkspaceSections(props?: { mode?: WorkspaceSectionMode }) {
  ...
  const [worktreeRepository, setWorktreeRepository] = useState('');
  ...
  const hydrateLocalEnvironmentStatus = useCallback((status: DesktopLocalEnvironmentStatus) => {
    ...
    if (firstAction) {
      ...
      setLocalEnvironmentActionName('Test');
      setLocalEnvironmentActionScript('');
    }
  }, []);
  ...
  const refreshWorktrees = useCallback(async () => {
    ...
    try {
      setWorktrees(
        await listDesktopWorktrees(
          worktreeRepository.trim() ? { repository: worktreeRepository.trim() } : {}
        )
      );
  ...
  useEffect(() => {
    ...
    setWorktreeRepository((current) => current || roots[0] || '');
  ...
  const saveLocalEnvironment = async () => {
    ...
    try {
      const currentConfig = localEnvironment?.config ?? {};
      const currentActions = currentConfig.actions ?? [];
      const currentFirstAction = currentActions[0];
      const nextFirstAction =
      ...
      const next = await saveDesktopLocalEnvironment({
        config: {
          ...
          actions: [...(nextFirstAction ? [nextFirstAction] : []), ...currentActions.slice(1)],
        },
      });
      hydrateLocalEnvironmentStatus(next);
  ...
  const createWorktree = async () => {
    ...
    try {
      const result = await createDesktopWorktree({
        repository: worktreeRepository.trim() || null,
  ...
  const resetWorktree = async (path: string) => {
    if (
      !window.confirm(
        'Reset this worktree? Uncommitted and untracked changes in it will be removed.'
      )
    ) {
      return;
    }
    setWorktreeActionStatus('resetting');
    setWorktreeError(null);
    setWorktreeMessage(null);
    try {
      const result = await resetDesktopWorktree({
        repository: worktreeRepository.trim() || null,
        path,
        baseRef: worktreeBaseRef.trim() || null,
      });
      ...
      setWorktreeError(caught instanceof Error ? caught.message : 'Worktree reset failed.');
      setWorktreeActionStatus('error');
    }
  };

  const removeWorktree = async (path: string) => {
    if (!window.confirm('Remove this worktree? Its branch will be kept.')) return;
    setWorktreeActionStatus('removing');
    setWorktreeError(null);
    setWorktreeMessage(null);
    try {
      const result = await removeDesktopWorktree({
        repository: worktreeRepository.trim() || null,
        path,
        force: true,
      });
      ...
      setWorktreeError(caught instanceof Error ? caught.message : 'Worktree removal failed.');
      setWorktreeActionStatus('error');
    }
  };
        ...
        <div className="space-y-3 border-t border-border pt-4">
          ...
          <div className="grid gap-2 sm:grid-cols-[minmax(0,0.45fr)_minmax(0,0.25fr)_minmax(0,0.3fr)]">
            {workspaceRoots.length > 1 ? (
              <label className="grid gap-1 text-xs text-slate-200/80 sm:col-span-3">
                Repository
                <select
                  className="rounded-md border border-border bg-black/20 px-2 py-1.5 text-xs text-white"
                  value={worktreeRepository}
                  onChange={(event) => {
                    setWorktreeRepository(event.currentTarget.value);
                    setWorktrees(null);
                  }}
                >
              ...
              {worktrees.worktrees.slice(0, 5).map((worktree) => (
                ...
                >
                  ...
                  <div className="flex shrink-0 items-center gap-1">
                    ...
                    {worktree.path !== worktrees.repositoryRoot ? (
                      <>
                        <button
                          type="button"
                          ...
                          onClick={() => void resetWorktree(worktree.path)}
                        >
                          Reset
                        </button>
                        <button
                          type="button"
                          className="rounded-md border border-rose-400/30 px-2 py-1 text-xs text-rose-200 transition-colors hover:bg-rose-500/10"
