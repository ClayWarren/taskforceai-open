...
export function DesktopProjectsSidebar(props: {
  ...
  activeConversationActivity?: React.ComponentProps<
    typeof ConversationList
  >['activeConversationActivity']; // coverage-ignore-line -- type-only indexed access
  ...
  const addExistingFolder = async () => {
    ...
    try {
      ...
      const created = existing
        ? null
        : (await createDesktopAppServerProject({ name, workspaceRoots: [root] })).project;
      const projectId = existing?.id ?? created?.id;
      ...
      if (created) {
        ...
        upsertProject({
          ...
          custom_instructions: created.customInstructions ?? null,
          created_at: created.createdAt ?? timestamp,
          updated_at: created.updatedAt ?? created.createdAt ?? timestamp,
        });
      }
    ...
    <>
      ...
      <section className="mt-5 min-h-0 flex-1 overflow-y-auto" aria-label="Tasks">
        ...
        <ConversationList
          ...
          activeConversationActivity={props.activeConversationActivity}
