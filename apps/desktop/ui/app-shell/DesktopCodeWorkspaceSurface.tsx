...
export function DesktopCodeWorkspaceSurface(props: {
  ...
  const rawDiff = diff?.rawDiff ?? '';
  const parsedDiff = useMemo(() => parseReviewDiff(rawDiff), [rawDiff]);
  const stats = useMemo(() => countDiffLines(rawDiff), [rawDiff]);
  const fileCount = Math.max(parsedDiff.length, status?.files.length ?? 0);
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              ...
              <div className="min-h-0 flex-1 overflow-auto p-4">
                {loading && parsedDiff.length === 0 ? (
                  ...
                  <ReviewCodeSurface rawDiff={rawDiff} />
                ) : (
