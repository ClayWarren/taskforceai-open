...
import { openRealtimeVoiceSocket } from '@taskforceai/web/app/lib/api/realtime-voice-socket';
import { logger } from '@taskforceai/web/app/lib/logger';
import { createVoiceGatewayRequestOptions } from '../platform/voice-gateway';
import {
  RealtimeBrowserMicrophone,
  RealtimeBrowserPcmPlayer,
} from '@taskforceai/web/app/components/chat/prompt-form/realtime/realtimeBrowserAudio';
...
export function useDesktopRealtimeVoiceSession({
  ...
  const finishSession = useCallback(
    (recordEnded: boolean, finalStatus: DesktopRealtimeStatus) => {
  ...
  const disconnect = useCallback(
    (recordEnded = true) => finishSession(recordEnded, 'disconnected'),
    [finishSession]
  );
