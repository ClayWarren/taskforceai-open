...
export interface DesktopStreamingRuntimeDependencies {
  waitForBridge: typeof waitForTauriBridge;
  getRunStatus: typeof getDesktopAppServerRunStatus;
  cancelRun: typeof cancelDesktopAppServerRun;
  pollIntervalMs: number;
  completionRampStepMs: number;
}

const defaultDependencies: DesktopStreamingRuntimeDependencies = {
  waitForBridge: waitForTauriBridge,
  getRunStatus: getDesktopAppServerRunStatus,
  cancelRun: cancelDesktopAppServerRun,
  pollIntervalMs: POLL_INTERVAL_MS,
  completionRampStepMs: COMPLETION_RAMP_STEP_MS,
};
...
class DesktopStreamingRuntime implements StreamingRuntime {
  ...
  constructor(private readonly dependencies: DesktopStreamingRuntimeDependencies) {}

  async startStreaming(taskId: string, handlers: StreamingRuntimeHandlers): Promise<void> {
    await this.dependencies.waitForBridge();
    this.stopStreaming();
  ...
  async cancelTask(taskId: string): Promise<void> {
    await this.dependencies.cancelRun(taskId);
  }
  ...
  private async pollRunStatus(
    ...
    while (!signal.aborted) {
      try {
        ...
        const { run } = await this.dependencies.getRunStatus(taskId);
        if (signal.aborted) {
          return;
        }
  ...
  private async emitCompletionRamp(
    ...
    for (const step of steps) {
      ...
      await sleep(this.dependencies.completionRampStepMs, signal);
    }
  }
}

export const createDesktopStreamingRuntime = (
  overrides: Partial<DesktopStreamingRuntimeDependencies> = {}
): StreamingRuntime => {
  return new DesktopStreamingRuntime({ ...defaultDependencies, ...overrides });
};
