...
async fn run_headless_prompt(
    ...
    let prompt = if options.local_coding {
        local_coding::contextualize_prompt(options.workspace.as_deref(), &options.prompt)
    } else {
        options.prompt
    };
    let result = client
...
async fn run_terminal(
    ...
    let result = app::run_event_loop(client, state, &mut terminal, keyboard_enhancement_enabled)
        .await
...
mod tests {
    use super::{
        headless_agent_count, headless_quick_mode, sanitize_terminal_text, Cli, CliCommand,
        HeadlessPromptOptions,
    };
    use clap::Parser;
