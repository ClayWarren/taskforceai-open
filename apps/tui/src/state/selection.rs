...
use super::{AppState, EffortSelectorState, FocusArea, ModelSelectorState, ModelSelectorTarget};
...
impl AppState {
    ...
    pub fn open_nested_effort_selector(&mut self, selector: EffortSelectorState) {
        self.agent_mode_selector = None;
        self.team_config = None;
        self.command_output = None;
        self.clear_prompt();
    ...
    pub fn model_selector_target(&self) -> Option<ModelSelectorTarget> {
        let selector = self.model_selector.as_ref()?;
        Some(match selector.selected_index {
            index if index < selector.options.len() => ModelSelectorTarget::Model,
            index if index == selector.options.len() => ModelSelectorTarget::Effort,
            _ => ModelSelectorTarget::AgentMode,
        })
    }
    ...
    pub(super) fn select_model_by_delta(&mut self, delta: isize) {
        ...
        let last = selector.options.len().saturating_add(1) as isize;
        let next = (selector.selected_index as isize + delta).clamp(0, last) as usize;
        selector.selected_index = next;
        self.status_line = selector.options.get(next).map_or_else(
            || {
                if next == selector.options.len() {
                    "Reasoning effort".to_string()
                } else {
                    "Agent mode".to_string()
                }
            },
            |option| format!("Model {}", option.id),
        );
    }
    ...
    pub(super) fn remove_run(&mut self, run_id: &str) {
        ...
        if self.selected_run_id.as_deref() != Some(run_id) {
            if let Some(index) = self.find_selected_run_index() {
                self.selected_run_index = Some(index);
                return;
            }
