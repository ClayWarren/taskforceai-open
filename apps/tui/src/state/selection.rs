...
use super::{AppState, FocusArea, ModelSelectorState};
...
impl AppState {
    ...
    pub fn selected_run(&self) -> Option<&RunRecord> {
        ...
        self.runs.iter().find(|run| &run.id == selected)
    }

    pub fn set_current_model(&mut self, model_id: impl Into<String>) {
        let model_id = model_id.into();
        self.current_model_id = if model_id.trim().is_empty() {
            "default".to_string()
        } else {
            model_id
        };
    ...
    pub(super) fn select_by_delta(&mut self, delta: isize) {
        if self.runs.is_empty() {
            ...
            self.status_line = "No runs to select".to_string();
            return;
        }
        ...
        self.command_output = None;
        self.status_line = format!("Selected {}", self.runs[next].id);
    }
    ...
    pub(super) fn remove_run(&mut self, run_id: &str) {
        let removed_index = self.runs.iter().position(|run| run.id == run_id);
        ...
        if let Some(selected) = &self.selected_run_id {
            if selected != run_id && self.runs.iter().any(|run| &run.id == selected) {
                return;
            }
        }
        ...
        self.selected_run_id = self.runs.get(next_index).map(|run| run.id.clone());
        self.detail_scroll_offset = 0;
    }
    ...
    pub(super) fn toggle_focus(&mut self) {
        ...
        self.status_line = match self.focus {
            FocusArea::Runs => "Run focus".to_string(),
