...
pub struct PendingInteraction {
    ...
    pub approval_target: Option<crate::permissions::ApprovalTarget>,
}

impl PendingInteraction {
    pub fn from_request(request: JsonRpcServerRequest) -> Result<Self, String> {
        let approval_target = crate::permissions::approval_target(&request);
    ...
    fn approval(
        ...
        message: String,
        approval_target: Option<crate::permissions::ApprovalTarget>,
    ) -> Self {
        Self {
            request_id,
            method,
            ...
            options: vec![
                ...
                option(
                    "Always allow matching",
                    "Persist this exact command, path, or permission rule",
                    "alwaysAllow",
                ),
                option("Decline", "Reject this request", "decline"),
                option(
                    "Always deny matching",
                    "Persist a denial for this exact command, path, or permission rule",
                    "alwaysDeny",
                ),
...
mod tests {
    ...
    fn approval_requests_open_and_serialize_decisions() {
        ...
        state
            .open_interaction(request(
                "item/commandExecution/requestApproval",
                json!({
                    "threadId": "thread", "turnId": "turn", "itemId": "second",
                    "command": ["cargo", "test"]
                }),
            ))
            .expect("persistent approval request");
        state.move_interaction_selection(2);
        let target = state
            .pending_interaction
            .as_ref()
            .and_then(|interaction| interaction.approval_target.as_ref())
            .expect("approval target");
        assert_eq!(target.kind, "command");
        assert_eq!(target.value, "cargo test");
        let (_, response) = state
            .submit_interaction()
            .expect("submit persistent")
            .expect("persistent response");
    ...
    fn concurrent_requests_are_queued_without_replacing_the_active_interaction() {
        ...
        let mut third = request(
            "item/permissions/requestApproval",
            json!({"threadId":"c","turnId":"tc","itemId":"ic","permissions":{"filesystem":true}}),
        );
        third.id = json!(9);
        state.open_interaction(first).expect("first interaction");
        state.open_interaction(second).expect("queued interaction");
        state
            .open_interaction(third)
            .expect("second queued interaction");
        ...
        assert_eq!(state.queued_interactions.len(), 2);
        ...
        assert!(state.status_line.contains("(1 queued)"));
        state
            .cancel_interaction()
            .expect("cancel second interaction");
        assert_eq!(state.queued_interactions.len(), 0);
        assert_eq!(state.status_line, "Action required: Permission request");
    }
    ...
    fn interaction_text_and_validation_edges_restore_the_modal() {
        ...
        .expect("MCP action request");
        interaction.options.clear();
        state.pending_interaction = Some(interaction);
        assert!(state
            .submit_interaction()
            .expect_err("missing MCP action")
            .contains("no selected action"));

        let mut interaction = PendingInteraction::approval(
            json!(9),
            "approval".into(),
            "Title",
            "Message".into(),
        ...
        assert!(state
            .submit_interaction()
            .expect_err("invalid question index")
            .contains("out of range"));
        let mut interaction = state.pending_interaction.take().expect("restored question");
        if let InteractionKind::UserInput { question_index, .. } = &mut interaction.kind {
            *question_index = 0;
        }
        interaction.options.clear();
