...
impl AppState {
    ...
    pub fn cancel_interaction(&mut self) -> Option<(Value, Value)> {
        ...
        self.status_line = "Interaction canceled".to_string();
    ...
    pub fn submit_interaction(&mut self) -> Result<Option<(Value, Value)>, String> {
        ...
        let response = match &mut interaction.kind {
            InteractionKind::Approval => {
                let decision = selected_value
                    .clone()
                    .ok_or_else(|| "approval has no selected decision".to_string())?;
                Some(json!({"decision": decision}))
            }
            InteractionKind::McpElicitation { .. } => {
                let action = selected_value
                    .clone()
                    .ok_or_else(|| "MCP request has no selected action".to_string())?;
            ...
            InteractionKind::UserInput {
                ...
                let question = questions
                    .get(*question_index)
                    .ok_or_else(|| "question index is out of range".to_string())?;
                let answer = if question.options.is_empty() {
                    ...
                    selected_value
                        .clone()
                        .ok_or_else(|| "question has no selected answer".to_string())?
                };
                answers.insert(
                    question.id.clone(),
                    UserInputAnswer {
        ...
        self.status_line = "Interaction answered".to_string();
        Ok(response.map(|response| (interaction.request_id, response)))
    }
}
