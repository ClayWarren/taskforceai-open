...
impl AppState {
    ...
    pub(super) fn refresh_command_suggestions(&mut self) {
        self.command_suggestions = command_suggestions(&self.prompt_input, self.authenticated);
...
fn command_suggestions(input: &str, authenticated: bool) -> Vec<&'static str> {
    ...
    fn perf_command_suggestion_refresh() {
        ...
        for _ in 0..(ITERATIONS / prefixes.len()) {
            for prefix in prefixes {
                black_box(command_suggestions(prefix, true));
                rendered += 1;
            }
        }
