...
async fn handle_input_action(
    ...
    match action {
        ...
        InputAction::SubmitPrompt => {
            ...
            {
                let input = expand_workspace_mentions(client, state, &prompt).await;
                let steered = client
...
pub(super) async fn submit_task_prompt(
    ...
    let expanded_prompt = expand_workspace_mentions(client, state, &prompt).await;
    ...
    let result = client
        .turn_start(TurnStartParams {
            ...
            quick_mode: Some(state.task_mode == crate::state::TaskMode::Chat),
            autonomous: Some(false),
...
async fn expand_workspace_mentions(
    ...
    let mut paths = prompt
        ...
        .map(|path| {
            path.trim_matches(|character: char| matches!(character, ',' | ';' | ':' | ')' | ']'))
        })
