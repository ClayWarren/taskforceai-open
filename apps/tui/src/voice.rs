...
static ACTIVE_PLAYBACKS: OnceLock<Mutex<Vec<PlaybackControl>>> = OnceLock::new();
...
pub fn speak_with_platform_voice(text: &str) -> Result<(), VoiceError> {
    ...
    spawn_tracked_playback(platform_speak_command(text), None)
}
// coverage:ignore-end

// coverage:ignore-start -- spawns host speech/audio cancellation commands.
pub fn cancel_speech() -> Result<(), VoiceError> {
    let mut playbacks = active_playbacks()
        .lock()
        .map_err(|_| VoiceError::CommandFailed("voice playback registry is unavailable".into()))?;
    for playback in playbacks.drain(..) {
        if !playback.finished.load(Ordering::Acquire) {
            let _ = playback.cancel.send(());
        }
    }
    Ok(())
}
...
fn play_audio_bytes(bytes: &[u8], media_type: &str) -> Result<(), VoiceError> {
    ...
    let command = platform_audio_play_command(&path)?;
    spawn_tracked_playback(command, Some(path))
}
// coverage:ignore-end

fn active_playbacks() -> &'static Mutex<Vec<PlaybackControl>> {
    ACTIVE_PLAYBACKS.get_or_init(|| Mutex::new(Vec::new()))
}

// coverage:ignore-start -- owns host playback children until completion or cancellation.
fn spawn_tracked_playback(
    mut command: Command,
    ...
    let mut playbacks = active_playbacks()
        .lock()
        .map_err(|_| VoiceError::CommandFailed("voice playback registry is unavailable".into()))?;
    playbacks.retain(|playback| !playback.finished.load(Ordering::Acquire));
    playbacks.push(PlaybackControl { cancel, finished });
    Ok(())
}
// coverage:ignore-end
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        command
            .args([
                ...
                "-Command",
                &format!(
                    "Add-Type -AssemblyName PresentationCore; $p=New-Object System.Windows.Media.MediaPlayer; $p.Open([uri]'{escaped}'); $p.Play(); Start-Sleep -Seconds 30"
                ),
            ])
            .stdin(Stdio::null())
            .stdout(Stdio::null())
