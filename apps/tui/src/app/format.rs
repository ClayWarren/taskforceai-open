...
pub(crate) fn format_generated_media_output(output: &str) -> Cow<'_, str> {
    ...
    if is_media_only_video(trimmed) {
        if let Some(uri) = extract_video_uri(trimmed) {
            return Cow::Owned(format!("Generated video: {uri}"));
        }
    }
    if is_media_only_image(trimmed) {
        if let Some(uri) = extract_markdown_image_uri(trimmed) {
            if is_data_image_uri(&uri) {
                return Cow::Owned(compact_data_image(&uri));
            }
            return Cow::Owned(format!("Generated image: {uri}"));
        }
    }
    if let Some(compacted) = compact_inline_data_images(output) {
        return Cow::Owned(compacted);
    }
    Cow::Borrowed(output)
}

fn is_media_only_video(output: &str) -> bool {
    let lower = output.to_ascii_lowercase();
    (lower.starts_with("<video") && lower.ends_with('>'))
        || (lower.starts_with("<source") && lower.ends_with('>'))
        || ((lower.starts_with("[generated video]")
            || lower.starts_with("[download generated video]"))
            && lower.ends_with(')'))
}

fn is_media_only_image(output: &str) -> bool {
    if !output.starts_with("![") {
        return false;
    }
    let Some(uri_start) = output.find("](") else {
        return false;
    };
    output[uri_start + 2..]
        .find(')')
        .is_some_and(|relative_end| uri_start + 2 + relative_end + 1 == output.len())
}
...
fn is_data_image_uri(uri: &str) -> bool {
    uri.get(.."data:image/".len())
        .is_some_and(|prefix| prefix.eq_ignore_ascii_case("data:image/"))
}

fn compact_inline_data_images(output: &str) -> Option<String> {
    let mut compacted = String::new();
    let mut copied_through = 0;
    let mut search_from = 0;

    while let Some(relative_image_start) = output[search_from..].find("![") {
        let image_start = search_from + relative_image_start;
        let Some(relative_uri_start) = output[image_start + 2..].find("](") else {
            break;
        };
        let uri_start = image_start + 2 + relative_uri_start + 2;
        let Some(relative_uri_end) = output[uri_start..].find(')') else {
            break;
        };
        let uri_end = uri_start + relative_uri_end;
        let uri = output[uri_start..uri_end].trim();

        if is_data_image_uri(uri) {
            compacted.push_str(&output[copied_through..image_start]);
            compacted.push_str(&compact_data_image(uri));
            copied_through = uri_end + 1;
        }
        search_from = uri_end + 1;
    }

    if copied_through == 0 {
        return None;
    }
    compacted.push_str(&output[copied_through..]);
    Some(compacted)
}
...
mod tests {
    ...
    #[test]
    fn generated_media_output_compacts_mixed_inline_data_images() {
        assert_eq!(
        ...
        assert_eq!(
            format_generated_media_output(
                "![first](data:image/png;base64,abc) and ![second](data:image/jpeg;base64,defg)"
            ),
            "Generated image: inline image/png (3 base64 chars) and Generated image: inline image/jpeg (4 base64 chars)"
        );
        assert_eq!(
            format_generated_media_output("Caption ![image](DATA:IMAGE/PNG;BASE64,abcdef)"),
            "Caption Generated image: inline image (6 base64 chars)"
        );
    }

    #[test]
    fn generated_media_output_bounds_large_mixed_inline_data_images() {
        let encoded = "a".repeat(1_000_000);
        let output = format!("Caption\n![Generated Image](data:image/png;base64,{encoded})\nDone");
        let formatted = format_generated_media_output(&output);

        assert_eq!(
            formatted,
            "Caption\nGenerated image: inline image/png (1000000 base64 chars)\nDone"
        );
    ...
    fn generated_media_output_handles_single_quote_video_and_malformed_links() {
        ...
        for malformed in [
            "![broken",
            "Caption ![image](data:image/png;base64,abc",
            "[Generated video] missing)\n[other](https://example.test/video.mp4)",
            "[Generated video](missing\ntrailing)",
        ] {
            assert_eq!(format_generated_media_output(malformed), malformed);
        }
    }
