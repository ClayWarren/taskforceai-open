...
#[tokio::test]
async fn mouse_wheel_over_task_scrolls_details_without_recalling_prompt_history() {
    let mut client =
        AppServerClient::connect_http("http://127.0.0.1:1", "unused").expect("client construction");
    let mut state = crate::state::AppState::new(initialized(), Vec::new());
    state.prompt_history = vec!["/status".to_string()];
    ...
    .await
    .expect("mouse scrolling should be local UI state");
...
async fn work_submission_uses_threads_without_enabling_agent_teams_and_supports_steer_queue() {
    ...
    assert_eq!(turn_start["params"]["displayInput"], "Plan launch");
    ...
    assert_eq!(turn_steer["params"]["displayInput"], "focus on risks");
    let queued = requests
        .iter()
