...
fn begin_test_login(state: &mut crate::state::AppState, result: DeviceLoginStartResult) -> u64 {
...
#[tokio::test]
async fn first_realtime_poll_pulls_missed_updates_even_without_a_new_event() {
    let (base_url, server) = start_rpc_sequence_server(vec![
        (
            "sync.realtimePoll",
            json!({"hasUpdates": false, "lastEventId": "evt-startup"}),
        ),
        (
            "sync.pull",
            json!({
                ...
                "latestVersion": 9,
                "conversations": [{
                    "conversationId": "missed-while-closed",
                    "title": "Missed while closed",
                    "createdAt": 1,
                    "updatedAt": 2,
                    "lastMessagePreview": "Pulled on startup"
                }],
                "messages": [],
                "deletions": [],
                "hasMore": false
        ...
        (
            "history.list",
            json!({"runs": [run_json("missed-while-closed", RunStatus::Completed)]}),
        ),
    ]);
    let mut client = AppServerClient::connect_http(base_url, "session-token")
        .expect("test client should connect");
    ...
    poll_sync_if_due(&mut client, &mut state)
        .await
        .expect("initial sync poll should succeed");

    assert_eq!(state.last_sync_event_id.as_deref(), Some("evt-startup"));
    assert_eq!(state.selected_run_id(), Some("missed-while-closed"));
    assert_eq!(
        state.status_line,
        "Sync pulled 1 conversations and 0 messages"
    );
    server.join().expect("rpc sequence should finish");
}
...
fn background_task_result_and_space_helpers_cover_edges() {
    ...
    state.prompt_input = "abc".to_string();
    state.prompt_cursor = 1;
    let mut dictation = SpaceDictationState::Idle;
    handle_space_dictation_pressed(&mut state, &mut dictation);
    let SpaceDictationState::Pending { space_index, .. } = dictation else {
        panic!("space dictation should be pending");
    };
    assert_eq!(space_index, 1);
    remove_pending_space(&mut state, space_index);
    assert_eq!(state.prompt_input, "abc");
    assert_eq!(state.prompt_cursor, 1);
}

#[test]
fn background_prompt_and_login_results_cover_all_non_host_outcomes() {
    fn thread(mode: &str) -> ThreadRecord {
        serde_json::from_value(json!({
            "id": format!("thread-{mode}"), "title":"Task", "objective":"Objective",
            "state":"active", "archived":false, "source":"test", "taskMode":mode,
            "parentThreadId":null, "turns":[], "createdAt":1, "updatedAt":1
        }))
        .expect("thread")
    }

    for mode in ["chat", "work", "code"] {
        let mut state = crate::state::AppState::new(initialized(), Vec::new());
        let id = state.begin_prompt_submission(format!("submit {mode}"));
        apply_background_task_result(
            &mut state,
            BackgroundTaskResult::PromptSubmission(Box::new(PromptSubmissionResult {
                id,
                outcome: Ok(PromptSubmissionOutcome::Turn {
                    thread: thread(mode),
                    run: run(&format!("run-{mode}"), RunStatus::Processing),
                }),
            })),
        );
    ...
    let mut state = crate::state::AppState::new(initialized(), Vec::new());
    let id = state.begin_prompt_submission("run".to_string());
    apply_background_task_result(
        &mut state,
        BackgroundTaskResult::PromptSubmission(Box::new(PromptSubmissionResult {
            id,
            outcome: Ok(PromptSubmissionOutcome::Run(run("run", RunStatus::Queued))),
        })),
    );
    let id = state.begin_prompt_submission("steer".to_string());
    apply_background_task_result(
        &mut state,
        BackgroundTaskResult::PromptSubmission(Box::new(PromptSubmissionResult {
            id,
            outcome: Ok(PromptSubmissionOutcome::Steer(thread("work"))),
        })),
    );
    let id = state.begin_prompt_submission("fail".to_string());
    apply_background_task_result(
        &mut state,
        BackgroundTaskResult::PromptSubmission(Box::new(PromptSubmissionResult {
            id,
    ...
    assert!(state.status_line.contains("failed"));

    let stale_start: DeviceLoginStartResult = serde_json::from_value(json!({
        "deviceCode":"device", "userCode":"user", "verificationUri":"https://example.test",
        "verificationUriComplete":"https://example.test/complete", "expiresIn":600, "interval":5
    }))
    .expect("login start");
    apply_background_task_result(
        &mut state,
        BackgroundTaskResult::LoginStart {
            attempt_id: 999,
            result: Ok(stale_start),
        },
    );
    apply_background_task_result(
        &mut state,
        BackgroundTaskResult::LoginStart {
            attempt_id: 999,
            result: Err("start failed".into()),
        },
    );

    let attempt_id = state.begin_login_attempt();
    for status in ["pending", "approved"] {
        let poll: taskforceai_app_protocol::DeviceLoginPollResult = serde_json::from_value(json!({
            "status":status, "authenticated": status == "approved"
        }))
        .expect("login poll");
        let approved = apply_background_task_result(
            &mut state,
            BackgroundTaskResult::LoginPoll {
                attempt_id,
                result: Ok(poll),
            },
        );
...
async fn startup_update_helpers_cover_status_and_join_paths() {
    ...
    apply_finished_startup_update(&mut state, None, false).await;
    ...
    state.status_line = "Current command".to_string();
    apply_finished_startup_update(
        &mut state,
        Some(tokio::spawn(async {
            StartupUpdateResult {
                command_output: Some("Update\nlate".to_string()),
                status_line: "late update".to_string(),
            }
        })),
        true,
    )
    ...
    assert_eq!(state.status_line, "Current command");
    ...
    apply_finished_startup_update(&mut state, Some(aborted_update), false).await;
    assert!(state
        .command_output
        .as_deref()
        .expect("panic output")
