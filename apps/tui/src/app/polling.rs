...
pub(super) async fn poll_sync_if_due(
    ...
    match result {
        Ok(result) => {
            ...
            if has_updates {
                let pulled = client.sync_pull(SyncPullParams { limit: Some(50) }).await?;
                let history = client.history_list(HistoryListParams { limit: 50 }).await?;
...
pub(super) async fn replay_pending_prompt_if_due(
    ...
    match result {
        Ok(result) => state.apply(UiAction::PendingPromptReplayed(result)),
