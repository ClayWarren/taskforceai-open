...
// coverage:ignore-start -- top-level slash-command adapter dispatches live RPC/process/browser operations.
pub(super) async fn handle_local_command(
    ...
    match parse_local_command(prompt) {
        ...
        LocalCommand::Help => {
            let message =
                crate::state::commands::command_help(state.authenticated, state.task_mode);
            show_command(state, "Help", message);
            Ok(true)
        }
        LocalCommand::Shortcuts => {
        ...
        LocalCommand::Login(args) => {
            handle_login_command(client, state, args, background_tasks).await
        }
        LocalCommand::Logout => {
            let status = client.auth_logout().await?;
            state.cancel_login("Signed out");
        ...
        LocalCommand::Agent(args) => threads::handle_agent_navigation(client, state, args).await,
        LocalCommand::Permissions(args) => handle_permissions_command(state, args).await,
        LocalCommand::Skills(args) => handle_skills_command(client, state, args).await,
        ...
        LocalCommand::Plan(args) => Ok(features::handle_plan(state, args)),
        LocalCommand::Compact(args) => features::handle_compact(client, state, args).await,
        LocalCommand::Hooks(args) => Ok(features::handle_hooks(state, args).await),
        LocalCommand::Thinking(args) => Ok(features::handle_thinking(state, args)),
        LocalCommand::Theme(args) => Ok(features::handle_theme(state, args)),
        LocalCommand::Shell(command) => Ok(features::handle_shell(state, command).await),
...
fn parse_local_command(prompt: &str) -> LocalCommand<'_> {
    if let Some(command) = prompt.strip_prefix('!') {
        return LocalCommand::Shell(command);
    }
    ...
    match command {
        ...
        "/shortcuts" | "/keys" => LocalCommand::Shortcuts,
        "/editor" | "/edit" => LocalCommand::Editor,
        ...
        "/agent" => LocalCommand::Agent(parts.collect()),
        "/permissions" => LocalCommand::Permissions(parts.collect()),
        "/skills" | "/skill" => LocalCommand::Skills(parts.collect()),
        ...
        "/plan" => LocalCommand::Plan(parts.collect()),
        "/compact" => LocalCommand::Compact(parts.collect()),
        "/hooks" => LocalCommand::Hooks(parts.collect()),
        "/thinking" => LocalCommand::Thinking(parts.collect()),
        "/theme" => LocalCommand::Theme(parts.collect()),
        _ if command.starts_with('/') => LocalCommand::AppServerCommand,
        _ => LocalCommand::Prompt,
    }
}

// coverage:ignore-start -- command adapters bridge persistent filesystem/RPC capabilities.
async fn handle_permissions_command(
    state: &mut AppState,
    args: Vec<&str>,
) -> Result<bool, AppClientError> {
    let message = crate::permissions::handle_command(state.workspace.as_deref(), &args)
        .await
        .unwrap_or_else(|error| error);
    show_command(state, "Permissions", message);
    Ok(true)
}
...
async fn handle_skills_command(
    ...
    let result = if matches!(action, "enable" | "disable") {
        let requested = args.get(1).copied().unwrap_or_default();
        let current = client.skill_list().await?;
        let Some(skill) = current
            .skills
            .iter()
            .find(|skill| skill.name == requested || skill.path == requested)
        else {
            show_command(
                state,
                "Skills",
                "Skill not found. Use /skills list to see available names.",
            );
            return Ok(true);
        };
        client
    ...
    let message = if lines.is_empty() {
        ...
        format!(
            "Invoke an enabled skill with $skill-name. Relevant skills are also selected automatically.\n{}",
            lines.join("\n")
        )
    };
    show_command(state, "Skills", message);
    Ok(true)
}
...
async fn handle_login_command(
    ...
    start_login(client.request_handle(), state, background_tasks);
    Ok(true)
}
// coverage:ignore-end

// coverage:ignore-start -- spawns a live device-login request task.
pub(super) fn start_login(
    ...
    background_tasks.push(tokio::spawn(async move {
        ...
        BackgroundTaskResult::LoginStart { attempt_id, result }
    }));
}
// coverage:ignore-end
    ...
    show_command(
        ...
        match mode {
            ...
            crate::state::TaskMode::Work => {
                "Work mode enabled. It shares Code execution capabilities while hiding coding-specific UI. Agent Teams remains off until explicitly enabled.".to_string()
            }
