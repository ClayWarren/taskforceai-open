...
async fn handle_code_command(
    ...
    if workspace_arg.trim().is_empty() {
        show_command(state, "Code", "Usage: /code <project-directory>");
        return Ok(true);
    }
    ...
    client
        .quick_mode_set(QuickModeSetParams { enabled: false })
        .await?;
    client
...
async fn handle_task_mode_command(
    ...
    show_command(
        ...
        match mode {
            ...
            crate::state::TaskMode::Code => "Workspace Code mode enabled.".to_string(),
        },
    );
    Ok(true)
}
