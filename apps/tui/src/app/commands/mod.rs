...
async fn handle_usage_command(
    ...
    state.apply(UiAction::CommandExecuted {
        ...
        message: format_usage_summary(&result),
    });
    Ok(true)
}
...
async fn handle_effort_command(
    ...
    let Some(option) = result
        ...
        state.apply(UiAction::CommandExecuted {
            ...
            message: format!("Model {current_model} is not present in the model catalog."),
        });
        return Ok(true);
    };
    if option.reasoning_effort_levels.is_empty() {
        state.apply(UiAction::CommandExecuted {
            title: "Reasoning Effort".to_string(),
            message: format!(
                "Model {current_model} does not expose configurable reasoning effort."
            ),
        });
        return Ok(true);
    }
    ...
    if matches!(action.as_deref(), Some("status")) {
        ...
        state.apply(UiAction::CommandExecuted {
            title: "Reasoning Effort".to_string(),
            message: format!("Model: {current_model}\nSelected: {selected}"),
        });
        return Ok(true);
    }
