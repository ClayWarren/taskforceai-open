...
async fn apply_next_background_task(state: &mut AppState, tasks: &mut UiTaskQueue) {
    ...
    match result {
        ...
        BackgroundTaskResult::PromptSubmission(_)
        | BackgroundTaskResult::LoginStart { .. }
        | BackgroundTaskResult::LoginPoll { .. } => {
            panic!("command tests do not submit prompts")
        }
    }
}
...
fn command_classifier_keeps_ui_only_commands_local() {
    ...
    assert_eq!(parse_local_command("/help"), LocalCommand::Help);
}
...
fn command_classifier_preserves_args_for_local_handlers() {
    ...
    assert_eq!(
        parse_local_command("/plan status"),
        LocalCommand::Plan(vec!["status"])
    );
    assert_eq!(
        parse_local_command("/compact decisions only"),
        LocalCommand::Compact(vec!["decisions", "only"])
    );
    assert_eq!(
        parse_local_command("/hooks run pre_tool"),
        LocalCommand::Hooks(vec!["run", "pre_tool"])
    );
    assert_eq!(
        parse_local_command("/theme nord"),
        LocalCommand::Theme(vec!["nord"])
    );
    assert_eq!(
        parse_local_command("!git status --short"),
        LocalCommand::Shell("git status --short")
    );
    assert_eq!(
    ...
    assert_eq!(
        parse_local_command("/thinking hide"),
        LocalCommand::Thinking(vec!["hide"])
    );
