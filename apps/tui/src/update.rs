...
pub fn auto_update_disabled_reason() -> Option<&'static str> {
    ...
    if env::var_os("npm_package_name").is_some()
        || env::args()
            .next()
            .is_some_and(|arg| arg.contains("node_modules"))
    {
...
fn platform_archive_name() -> Result<String, UpdateError> {
    let os = match env::consts::OS {
        ...
        "linux" => "linux",
        "windows" => "windows",
        other => return Err(UpdateError::UnsupportedPlatform(other, env::consts::ARCH)),
    };
    let arch = match env::consts::ARCH {
        "x86_64" => "amd64",
        "aarch64" => "arm64",
        other => return Err(UpdateError::UnsupportedPlatform(env::consts::OS, other)),
    };
...
fn file_name_is_tui_binary(path: &Path) -> bool {
    ...
    name == "taskforceai"
        ...
        || (name.starts_with(TUI_BINARY_PREFIX) && !name.starts_with(APP_SERVER_BINARY_PREFIX))
}
