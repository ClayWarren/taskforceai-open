...
pub enum UpdateError {
    ...
    WindowsSelfUpdate,
    #[error("self-update is disabled: {0}")]
...
pub fn auto_update_disabled_reason() -> Option<&'static str> {
    ...
    if env::var_os("npm_package_name").is_some()
        || arg_points_to_managed_install(env::args().next().as_deref())
        || current_exe_points_to_managed_install()
    {
        return Some("managed-install");
    }
    None
}

fn current_exe_points_to_managed_install() -> bool {
    env::current_exe().is_ok_and(|path| path_points_to_managed_install(&path))
}

fn arg_points_to_managed_install(arg: Option<&str>) -> bool {
    arg.is_some_and(|arg| path_points_to_managed_install(Path::new(arg)))
}

fn path_points_to_managed_install(path: &Path) -> bool {
    path_has_component(path, "node_modules") || path_has_component(path, "Cellar")
}
...
mod tests {
    ...
    #[tokio::test]
    async fn apply_update_respects_disable_env_before_network_access() {
        ...
        let err = apply_update(&check)
            .await
            .expect_err("apply should honor disable env before network access");
        assert!(matches!(
            err,
            UpdateError::AutoUpdateDisabled("disabled-env")
        ));
    ...
    #[test]
    fn auto_update_disabled_reason_prefers_disable_and_managed_install_context() {
        let _guard = UPDATE_ENV_TEST_LOCK.lock().expect("update env test lock");
        unsafe {
            env::set_var("TASKFORCEAI_DISABLE_AUTOUPDATE", "1");
            env::remove_var("npm_package_name");
        ...
        assert_eq!(auto_update_disabled_reason(), Some("managed-install"));
        assert!(arg_points_to_managed_install(Some(
            "/repo/node_modules/.bin/taskforceai"
        )));
        assert!(arg_points_to_managed_install(Some(
            "/opt/homebrew/Cellar/taskforceai-cli/0.11.11/bin/taskforceai"
        )));
        assert!(!arg_points_to_managed_install(Some(
            "/usr/local/bin/taskforceai"
        )));
        assert!(!arg_points_to_managed_install(None));
