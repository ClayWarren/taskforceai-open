...
pub use layout::{run_index_at, run_scroll_offset, task_pane_at, TaskPane};
pub(crate) use runs::url_at;

use self::composer::render_composer;
use self::effort::render_effort_selector;
use self::footer::render_footer;
use self::header::render_header;
use self::interaction::render_interaction;
pub(crate) use self::launch::auth_url_at as launch_auth_url_at;
use self::launch::render_launch;
use self::layout::root_chunks;
use self::model::render_model_selector;
use self::runs::render_runs;
use self::style::bg;
use self::team::{render_agent_mode_selector, render_team_config};

pub fn render(frame: &mut Frame<'_>, state: &AppState) {
    let area = frame.area();
    frame.render_widget(Block::default().style(Style::default().bg(bg())), area);
    if state.raw_output_mode {
        raw::render_raw_output(frame, state);
        return;
    }
    if state.launch_screen_visible
        && !state.picker_active()
        && !state.model_selector_active()
        && !state.effort_selector_active()
        && !state.file_suggestions_active()
        && !state.command_suggestions_active()
        && state.command_output.is_none()
        && !state.interaction_active()
    {
        render_launch(frame, area, state);
        return;
    }
    ...
    render_model_selector(frame, state);
    render_effort_selector(frame, state);
    render_agent_mode_selector(frame, state);
    render_team_config(frame, state);
    render_interaction(frame, state);
}
...
mod tests {
    ...
    use serde_json::json;
    use taskforceai_app_protocol::{ModelOptionRecord, RunRecord, RunStatus};
    ...
    fn render_draws_full_tui_surface() {
        ...
        let mut state = AppState::new(initialized(), vec![run()]);
        ...
        state.open_picker(
            ...
            vec![crate::state::PickerOption::new(
                "thread",
                "Saved conversation",
                "work · local",
                "saved",
        ...
        terminal
            .draw(|frame| render(frame, &state))
            .expect("picker should render over every task mode");
        state.close_picker("closed");
        state.raw_output_mode = true;
        terminal
            .draw(|frame| render(frame, &state))
            .expect("raw TUI render should succeed");
    }
    ...
    fn detail_lines_include_stream_metadata() {
        ...
        assert!(lines
            .iter()
            .any(|line| line == "Sources Docs (https://example.test/docs)"));
    ...
    fn detail_lines_compact_generated_media_and_show_errors() {
        ...
        assert!(lines.iter().any(|line| line == "Conversation failed"));
        assert!(lines
            .iter()
    ...
    #[test]
    fn detail_lines_never_expose_mixed_inline_data_image_payloads() {
        let mut run = run();
        run.output = Some(
            "Caption before\n![Generated Image](data:image/png;base64,abcdef)\nCaption after"
                .to_string(),
        );
        ...
        assert!(lines.iter().any(|line| line == "Caption before"));
        assert!(lines
            .iter()
            .any(|line| line == "Generated image: inline image/png (6 base64 chars)"));
        assert!(lines.iter().any(|line| line == "Caption after"));
        assert!(lines.iter().all(|line| !line.contains("data:image/")));
    }
    ...
    #[test]
    fn detail_lines_preserve_multiline_prompts_and_responses() {
        let mut run = run();
        run.prompt = "first prompt\nsecond prompt".to_string();
        run.output = Some("first response\n\nthird response".to_string());
        let lines = run_detail_lines(&run)
            .into_iter()
            .map(line_text)
            .collect::<Vec<_>>();
        assert!(lines
            .windows(2)
            .any(|lines| lines == ["first prompt", "second prompt"]));
        assert!(lines
            .windows(3)
            .any(|lines| lines == ["first response", "", "third response"]));
    }
    ...
    fn run_hit_test_maps_clicks_to_visible_run_rows() {
        ...
        assert_eq!(run_index_at(area, 2, 5, 3, 0, false), Some(0));
        assert_eq!(run_index_at(area, 2, 6, 3, 0, false), Some(1));
        assert_eq!(run_index_at(area, 2, 5, 0, 0, false), None);
        assert_eq!(run_index_at(area, 2, 9, 3, 0, false), None);
        assert_eq!(run_index_at(area, 60, 6, 3, 0, false), None);
        assert_eq!(run_index_at(area, 2, 5, 20, 12, false), Some(12));
        assert_eq!(run_index_at(area, 2, 5, 3, 0, true), None);
    }
    ...
    fn footer_hit_test_maps_clicks_to_actions() {
        ...
        assert_eq!(
            footer_action_at(area, 48, 28),
            Some(FooterAction::ToggleSidebar)
        );
        assert_eq!(footer_action_at(area, 105, 28), Some(FooterAction::Dismiss));
        assert_eq!(footer_action_at(area, 75, 28), None);
        assert_eq!(footer_action_at(area, 95, 28), None);
        assert_eq!(footer_action_at(area, 114, 28), None);
    }
    ...
    fn run_scroll_keeps_selected_run_visible() {
        ...
        assert_eq!(offset, 10);
        assert_eq!(
            run_index_at(area, 2, 5, state.runs.len(), offset, false),
            Some(10)
        );
        assert_eq!(
            run_scroll_offset(
        ...
        assert_eq!(run_scroll_offset(area, &state), 0);
        assert_eq!(
            run_scroll_offset(
                Rect {
                    x: 0,
    ...
    fn compact_model_id_preserves_short_values_and_truncates_long_values() {
        assert_eq!(compact_model_id("openai/gpt-5.6-sol"), "openai/gpt-5.6-sol");
        assert_eq!(
    ...
    fn command_suggestion_spans_keep_selected_option_visible_within_width() {
        ...
        state.selected_command_suggestion = None;
        assert!(!command_suggestion_spans(&state, 80).is_empty());
        state.selected_command_suggestion = Some(0);
        let narrow = command_suggestion_spans(&state, 12)
            .into_iter()
            .map(|span| span.content.into_owned())
            .collect::<String>();
    ...
    fn model_selector_scroll_counts_description_lines() {
        let mut state = crate::state::AppState::new(initialized(), Vec::new());
        assert_eq!(model_selector_scroll(Rect::new(0, 0, 40, 8), &state), 0);
        ...
        if let Some(selector) = &mut state.model_selector {
            ...
            selector.remote_catalog = true;
            selector.options[0].usage_multiple = Some(2.25);
            selector.options[0].description = Some("fast model".to_string());
        }
        let lines = model_selector_lines(&state);
        assert!(lines.iter().any(|line| line.to_string().contains("remote")));
        assert!(lines.iter().any(|line| line.to_string().contains("2.25x")));

        state.model_selector.as_mut().unwrap().selected_index = 2;
        assert!(!model_selector_lines(&state).is_empty());

        assert_eq!(
            super::runs::picker_scroll(Rect::new(0, 0, 40, 8), &state),
            0
        );
        assert_eq!(
            super::runs::picker_lines(&state)[0].to_string(),
            "No picker is open."
        );
        state.open_picker(
            crate::state::PickerKind::Resume,
            "Resume",
            vec![
                crate::state::PickerOption::new("a", "Alpha", "first", "one"),
                crate::state::PickerOption::new("b", "Beta", "second", "two"),
            ],
            None,
        );
        state.picker.as_mut().unwrap().selected_index = 1;
        assert!(!super::runs::picker_lines(&state).is_empty());
        assert!(super::runs::picker_scroll(Rect::new(0, 0, 40, 3), &state) > 0);
        state.picker.as_mut().unwrap().query = "missing".to_string();
        assert!(super::runs::picker_lines(&state)
            .iter()
            .any(|line| line.to_string().contains("No matching")));
    }
