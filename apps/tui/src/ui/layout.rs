...
pub fn run_index_at(
    ...
    let runs_area = run_chunks(root_chunks(area)[1])[0];
...
pub fn run_scroll_offset(area: Rect, state: &AppState) -> usize {
    ...
    let selected_index = state
        .selected_run_id()
        .and_then(|selected| state.runs.iter().position(|run| run.id == selected))
        .unwrap_or(0);
...
pub(super) fn run_chunks(area: Rect) -> std::rc::Rc<[Rect]> {
    Layout::default()
