...
fn footer_spans() -> Vec<Span<'static>> {
    vec![
        ...
        Span::styled("  Up/Down or j/k select", Style::default().fg(TEXT_MUTED)),
        Span::styled("  PgUp/PgDn scroll", Style::default().fg(TEXT_MUTED)),
        Span::raw("  "),
        key_hint("[Esc] Quit"),
    ]
}
...
mod tests {
    ...
    fn footer_spans_and_render_cover_control_surface() {
        ...
        let state = AppState::new(initialized(), Vec::new());
