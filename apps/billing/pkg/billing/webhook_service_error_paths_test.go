...
func TestWebhookService_HandleSubscriptionUpdate_UserNotFoundByCustomer(t *testing.T) {
	...
	assert.Equal(t, ErrRetryableEvent, err)
}
...
func TestWebhookService_HandleEvent_UnknownPriceCanReplayAfterConfigurationBackfill(t *testing.T) {
	repo := newPoisonRepo()
	ctx := context.Background()
	...
	require.Equal(t, ErrRetryableEvent, firstErr)
	assert.Nil(t, firstResult)
	assert.False(t, repo.processed[event.ID])
	assert.Zero(t, repo.updates)

	replaySvc := NewWebhookService(WebhookDependencies{
		Repo: repo,
		GetPlanByPriceID: func(priceID *string) *BillingPlanDefinition {
			if priceID != nil && *priceID == "price_new_unmapped" {
				return &BillingPlanDefinition{Plan: PlanPro}
			}
			return nil
		},
	})
	replayResult, replayErr := replaySvc.HandleEvent(ctx, event)
	require.Empty(t, replayErr)
	require.NotNil(t, replayResult)
	assert.True(t, replayResult.Processed)
	assert.True(t, repo.processed[event.ID])
	assert.Equal(t, 1, repo.updates)
}

func TestWebhookService_HandleEvent_InvalidSubscriptionCompletesTerminalClaim(t *testing.T) {
	mockRepo := new(MockWebhookRepository)
	svc := NewWebhookService(WebhookDependencies{Repo: mockRepo})
	ctx := context.Background()
	event := &stripe.Event{
	...
	mockRepo.On("CompleteEvent", ctx, event.ID).Return(nil).Once()
...
func TestWebhookService_FindUserForSubscription_ErrorBranches(t *testing.T) {
	...
	t.Run("missing user without customer retries", func(t *testing.T) {
		mockRepo := new(MockWebhookRepository)
		svc := NewWebhookService(WebhookDependencies{Repo: mockRepo})
		userID := 42
		mockRepo.On("FindUserByID", ctx, userID).Return(nil, ErrBillingUserNotFound).Once()

		user, werr := svc.findUserForSubscription(ctx, &parsedSubscription{UserID: &userID})

		assert.Equal(t, ErrRetryableEvent, werr)
		assert.Nil(t, user)
		mockRepo.AssertExpectations(t)
	})
}

func TestWebhookService_InvalidEventCompletionFailure(t *testing.T) {
	ctx := context.Background()
	mockRepo := new(MockWebhookRepository)
	mockRepo.On("CompleteEvent", ctx, "evt_invalid").Return(assert.AnError).Once()
	svc := NewWebhookService(WebhookDependencies{Repo: mockRepo})

	result, werr := svc.handleEventFailure(ctx, "evt_invalid", "unknown", "claim", ErrInvalidEvent)
...
func TestSubscriptionPlanForUpdate_NilSubscription(t *testing.T) {
	svc := NewWebhookService(WebhookDependencies{Repo: new(MockWebhookRepository)})
	plan, err := svc.subscriptionPlanForUpdate(nil, nil)
	assert.Equal(t, string(PlanFree), plan)
	assert.Equal(t, ErrInvalidEvent, err)
}
...
func TestWebhookStatusHelpers_DefaultBranches(t *testing.T) {
	...
	assert.False(t, isOpenSubscriptionStatus("incomplete"))
