...
func TestHandleEventResult_Struct(t *testing.T) {
	...
	result2 := HandleEventResult{
		Processed: false,
	}
	assert.False(t, result2.Processed)
}
...
func TestWebhookError_Constants(t *testing.T) {
	assert.Equal(t, ErrDBError, WebhookError("DB_ERROR"))
	assert.Equal(t, ErrInvalidEvent, WebhookError("INVALID_EVENT"))
	assert.Equal(t, ErrStripeError, WebhookError("STRIPE_ERROR"))
}
