...
func TestStripeClient_GetCustomer(t *testing.T) {
	...
	mockBackend.On("CustomerGet", "cus_123", mock.MatchedBy(func(params *stripe.CustomerParams) bool {
		return params != nil && params.Context == ctx
	})).Return(expected, nil).Once()

	customer, err := client.GetCustomer(ctx, "cus_123")

	require.NoError(t, err)
	assert.Same(t, expected, customer)
}

func TestStripeClient_GetCustomer_EmptyCustomer(t *testing.T) {
	client, mockBackend := newStripeTestClient()
	...
	mockBackend.AssertNotCalled(t, "CustomerGet", mock.Anything, mock.Anything)
}
