...
const (
	...
	revenueCatEntitlementProID   = "entl8dfecfaf08"
	revenueCatEntitlementSuperID = "entl5f2cdbca7e"
)
...
func ResolvePlanFromRevenueCat(entitlementID, productID *string) *BillingPlanDefinition {
	if entitlementID != nil {
		lookupKey := ""
		switch strings.TrimSpace(*entitlementID) {
		case revenueCatEntitlementProID:
			lookupKey = "pro"
		case revenueCatEntitlementSuperID:
			lookupKey = "super"
		}
		if lookupKey != "" {
			entitlementID = &lookupKey
		}
	}
