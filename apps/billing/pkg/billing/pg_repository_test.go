...
func TestPgAccountSettingsRepository_DelegatesToPaymentsRepository(t *testing.T) {
	mock := dbtest.NewMockPool(t)

	queries := db.New(mock)
	repo := NewAccountSettingsRepository(queries)
	...
	byEmail, err := repo.FindUserByEmail(context.Background(), "billing@example.com")
	require.NoError(t, err)
	...
	require.NoError(t, repo.UpdateCustomerID(context.Background(), 100, "cus_new"))
	...
	assert.NoError(t, repo.UpdateSubscription(context.Background(), 100, SubscriptionUpdate{
		SubscriptionStatus: &status,
		CurrentPeriodStart: &now,
