...
func TestMapBillingUserToPaymentsAccountUser_IncludesRevenueCatOwnership(t *testing.T) {
	appUserID := "rcapp_123"

	user := mapBillingUserToPaymentsAccountUser(account.User{
		ID:                  100,
		Email:               "test@example.com",
		Plan:                string(PlanPro),
		RevenueCatAppUserID: &appUserID,
	...
	assert.Equal(t, string(PlanPro), user.Plan)
...
func TestPgPaymentsRepository_UpdateSubscription_Clear(t *testing.T) {
	mock := dbtest.NewMockPool(t)
	...
	require.ErrorIs(t, repo.UpdateSubscription(context.Background(), 100, SubscriptionUpdate{
		ClearSubscription: true,
		Plan:              &plan,
	}), assert.AnError)
}

func TestPgPaymentsRepository_UpdateSubscription_Reconcile(t *testing.T) {
	mock := dbtest.NewMockPool(t)
	repo := NewPaymentsRepository(db.New(mock))
	plan := string(PlanPro)
	subscriptionID := "sub_123"
	...
	mock.ExpectExec("UPDATE users SET").
		WithArgs(
			int32(100), &subscriptionID, &status, pgxmock.AnyArg(),
			pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(),
			pgxmock.AnyArg(), &plan, &priceID, pgxmock.AnyArg(), pgxmock.AnyArg(),
		).
		WillReturnResult(pgxmock.NewResult("UPDATE", 1))
	...
	mock.ExpectExec("UPDATE users SET").
		WithArgs(
			int32(100), &subscriptionID, pgxmock.AnyArg(), pgxmock.AnyArg(),
			pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(),
