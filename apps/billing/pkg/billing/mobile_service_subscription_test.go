...
func TestMobileSubscriptionService_SyncByUserID_NoRevenueCatKey(t *testing.T) {
	...
	// Ensure the v2 secret key is not set.
...
func TestMobileSubscriptionService_SyncByUserID_ResetsV2EntitlementWithoutStoreMetadata(t *testing.T) {
	mockRepo := new(MockMobileSubscriptionRepository)
	mockFetcher := new(MockRevenueCatFetcher)
	...
	user := mobileUser("free")
	futureExpiry := time.Now().Add(30 * 24 * time.Hour)
	activeSubscriber := &revenuecat.Subscriber{
		Entitlements: map[string]revenuecat.Entitlement{
			revenueCatEntitlementProID: {ExpiresDate: &futureExpiry},
		},
	}
	appUserID := "1"
	status := "active"
	paidUser := &MobileSyncUser{
		ID:                  user.ID,
		Email:               user.Email,
	...
	freeUser := mobileUser("free")

	mockRepo.On("FindUserByID", ctx, user.ID).Return(user, nil).Once()
	mockFetcher.On("FetchSubscriber", mock.Anything, appUserID).Return(activeSubscriber, RevenueCatError("")).Once()
	mockRepo.On("UpdateUser", ctx, user.ID, mock.MatchedBy(func(update MobileSubscriptionUpdate) bool {
		return update.Plan != nil && *update.Plan == string(PlanPro) &&
			update.SubscriptionSource == nil && update.RevenueCatAppUserID != nil &&
			*update.RevenueCatAppUserID == appUserID
	})).Return(paidUser, nil).Once()

	activeResult, err := svc.SyncMobileSubscriptionByUserID(ctx, user.ID)
	require.NoError(t, err)
	require.Equal(t, string(PlanPro), activeResult.Plan)

	mockRepo.On("FindUserByID", ctx, user.ID).Return(paidUser, nil).Once()
	mockFetcher.On("FetchSubscriber", mock.Anything, appUserID).Return(&revenuecat.Subscriber{
		Entitlements: map[string]revenuecat.Entitlement{},
	}, RevenueCatError("")).Once()
	mockRepo.On("UpdateUser", ctx, user.ID, mock.MatchedBy(func(update MobileSubscriptionUpdate) bool {
		return update.ClearSubscription && update.Plan != nil && *update.Plan == string(PlanFree)
	})).Return(freeUser, nil).Once()

	expiredResult, err := svc.SyncMobileSubscriptionByUserID(ctx, user.ID)
	require.NoError(t, err)
	assert.Equal(t, string(PlanFree), expiredResult.Plan)
	mockRepo.AssertExpectations(t)
	mockFetcher.AssertExpectations(t)
}
