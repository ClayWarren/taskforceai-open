...
// RevenueCatFetcher fetches the normalized RevenueCat customer snapshot.
...
// FetchSubscriber fetches customer data from RevenueCat using the default client.
...
func (s *MobileSubscriptionService) syncMobileSubscription(ctx context.Context, user *MobileSyncUser) (*MobileSyncResult, error) {
	...
	if revenueCatAPIKey() == "" {
		return nil, fmt.Errorf("RevenueCat v2 API key not configured")
	}
	...
	// Handle RevenueCat errors or missing customer.
...
func (s *MobileSubscriptionService) resetSubscription(ctx context.Context, user *MobileSyncUser) (*MobileSyncResult, error) {
	if hasMobileSubscriptionProvenance(user) {
		freePlan := "free"
		falseVal := false
...
func hasMobileSubscriptionProvenance(user *MobileSyncUser) bool {
	if user == nil {
		return false
	}
	if user.SubscriptionSource != nil {
		return mobileSources[*user.SubscriptionSource]
	}
	return user.RevenueCatAppUserID != nil && strings.TrimSpace(*user.RevenueCatAppUserID) != ""
}
...
func (s *MobileSubscriptionService) buildSubscriptionUpdate(user *MobileSyncUser, sub *revenuecat.Subscriber, activeEntitlement *revenuecat.Entitlement, appUserID string, planDef *BillingPlanDefinition) MobileSubscriptionUpdate {
	...
	source := resolveMobileSubscriptionSource(user, subDetail, planDef, activeEntitlement.ProductIdentifier)
	...
	if subDetail != nil {
		value := subDetail.UnsubscribeDetectedAt != nil
	...
	if activeEntitlement.ProductIdentifier != "" {
		update.PriceID = &activeEntitlement.ProductIdentifier
		update.MobileProductID = &activeEntitlement.ProductIdentifier
	}
...
func resolveMobileSubscriptionSource(user *MobileSyncUser, subDetail *revenuecat.Subscription, planDef *BillingPlanDefinition, productID string) *SubscriptionSource {
	var source *SubscriptionSource
	if subDetail != nil {
		mappedSource, err := MapStoreToSource(subDetail.Store)
		if err != "" {
	...
	if source == nil {
		source = sourceFromPlanProduct(planDef, productID)
	}
	if source == nil && user.SubscriptionSource != nil && mobileSources[*user.SubscriptionSource] {
