...
const (
	...
	ErrRetryableEvent WebhookError = "RETRYABLE_EVENT"
)
...
func (s *WebhookService) handleEventFailure(
	...
	} else if werr == ErrInvalidEvent {
		if err := s.deps.Repo.CompleteEvent(ctx, eventID, claim); err != nil {
			slog.Error("Failed to complete terminal webhook event", "eventId", eventID, "error", err)
			recordStripeWebhook(ctx, false, eventType)
			return nil, ErrDBError
		}
	}
...
func isRetryableWebhookError(werr WebhookError) bool {
	return werr == ErrDBError || werr == ErrStripeError || werr == ErrRetryableEvent
}
...
func (s *WebhookService) handleSubscriptionUpdate(ctx context.Context, data json.RawMessage, eventCreatedAt *time.Time) WebhookError {
	...
	plan, werr := s.subscriptionPlanForUpdate(sub, user)
	if werr != "" {
		return werr
	}
...
func (s *WebhookService) subscriptionPlanForUpdate(sub *parsedSubscription, user *WebhookUser) (string, WebhookError) {
	...
	slog.Error("Unrecognized price ID for active subscription; deferring webhook for configuration backfill", "priceId", ptrOr(sub.PriceID, "nil"), "subId", sub.ID, "userId", userIDForLog(user))
	return "", ErrRetryableEvent
}
