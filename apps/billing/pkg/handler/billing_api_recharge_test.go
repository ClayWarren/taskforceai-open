...
func TestGetPaymentMethods_SkipsNonCardMethods(t *testing.T) {
	...
	mockStripe.On("GetCustomer", mock.Anything, customerID).Return(&stripe.Customer{}, nil)
...
func TestGetPaymentMethods_GetCustomerError(t *testing.T) {
	dbMock := newBillingDBMock(t)
	mockStripe := newBillingStripeMock(t)
	h := newBillingAPITest(t, 1)

	customerID := "cus_123"
	expectGetUserByID(dbMock, dbtest.User{Billing: &dbtest.UserBilling{CustomerID: &customerID}})
	mockStripe.On("ListPaymentMethods", mock.Anything, customerID).Return([]*stripe.PaymentMethod{}, nil)
	mockStripe.On("GetCustomer", mock.Anything, customerID).Return((*stripe.Customer)(nil), assert.AnError)
...
func TestGetPaymentMethods_Success(t *testing.T) {
	...
	mockStripe.On("GetCustomer", mock.Anything, customerID).Return(&stripe.Customer{
