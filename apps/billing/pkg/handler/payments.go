...
func RedirectToLogin(w http.ResponseWriter, r *http.Request, plan string) {
	...
	loginURL := MarketingBaseURL + LoginPath + "?plan=" + url.QueryEscape(plan) + "&callbackUrl=" + url.QueryEscape(callbackURL)
...
func buildSubscriptionCheckoutParams(
	...
	return &stripe.CheckoutSessionParams{
		Params: stripe.Params{
			IdempotencyKey: stripe.String(checkoutIdempotencyKey(userID, priceID)),
		},
...
func checkoutIdempotencyKey(userID int, priceID string) string {
	digest := sha256.Sum256([]byte(strings.TrimSpace(priceID)))
...
func createSubscriptionCheckoutSession(
	...
	params := buildSubscriptionCheckoutParams(opts.UserID, customerID, opts.PriceID, opts.SuccessURL, opts.CancelURL)
