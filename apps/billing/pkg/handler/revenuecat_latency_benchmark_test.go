...
func reportRevenueCatLatencyProfile(b *testing.B, samples []time.Duration) {
	...
	b.ReportMetric(revenueCatDurationMicroseconds(revenueCatPercentileDuration(ordered, 0.50)), "p50_us")
	b.ReportMetric(revenueCatDurationMicroseconds(revenueCatPercentileDuration(ordered, 0.95)), "p95_us")
	b.ReportMetric(revenueCatDurationMicroseconds(revenueCatPercentileDuration(ordered, 0.99)), "p99_us")
}

func revenueCatPercentileDuration(ordered []time.Duration, percentile float64) time.Duration {
	if len(ordered) == 0 {
		return 0
	}
