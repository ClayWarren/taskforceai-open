...
func authenticatedBillingAccount(ctx context.Context, userID int) (*billingAccount, error) {
...
func getBillingAccountOrError(ctx context.Context, repo billing.PaymentsRepository, userID int) (*billingAccount, error) {
	...
	account := &billingAccount{
		...
		AutoRechargeEnabled:   dbUser.AutoRechargeEnabled,
		AutoRechargeAmount:    dbUser.AutoRechargeAmount,
		AutoRechargeThreshold: dbUser.AutoRechargeThreshold,
		SubscriptionStatus:    dbUser.SubscriptionStatus,
		SubscriptionID:        dbUser.SubscriptionID,
		CancelAtPeriodEnd:     dbUser.CancelAtPeriodEnd,
		CustomerID:            dbUser.CustomerID,
		PaymentMethodBrand:    dbUser.PaymentMethodBrand,
		PaymentMethodLast4:    dbUser.PaymentMethodLast4,
	}
	if dbUser.CurrentPeriodStart != nil {
		account.CurrentPeriodStart = dbUser.CurrentPeriodStart
	}
	if dbUser.CurrentPeriodEnd != nil {
		account.CurrentPeriodEnd = dbUser.CurrentPeriodEnd
	}

	return account, nil
}

func balanceResponseFromAccount(account *billingAccount) BalanceResponse {
...
func RegisterBillingHandlers(api huma.API) {
	...
	registerCancelSubscription(api)
	registerReactivateSubscription(api)
...
func registerCancelSubscription(api huma.API) {
	huma.Register(api, huma.Operation{
		OperationID: "cancel-payment-subscription",
		Method:      http.MethodPost,
		...
		if err := changeSubscriptionCancellation(ctx, billing.NewPaymentsRepository(dbQueries), input.User.ID, true, "CancelSub"); err != nil {
			return nil, err
		}

		return billingOK(MessageResponse{Message: "Subscription set to cancel at end of period"}), nil
	})
}

func registerReactivateSubscription(api huma.API) {
	huma.Register(api, huma.Operation{
		OperationID: "reactivate-payment-subscription",
		Method:      http.MethodPost,
		Path:        "/api/v1/payments/reactivate-subscription",
		Summary:     "Reactivate current subscription",
		Tags:        []string{"Billing"},
	}, func(ctx context.Context, input *struct {
		adapterhandler.AuthContext
	}) (*billingAPIResponse[MessageResponse], error) {
		...
		if err := changeSubscriptionCancellation(ctx, billing.NewPaymentsRepository(dbQueries), input.User.ID, false, "ReactivateSub"); err != nil {
			return nil, err
		}

		return billingOK(MessageResponse{Message: "Subscription reactivated successfully"}), nil
	})
}
...
func registerGetPaymentMethods(api huma.API) {
	huma.Register(api, huma.Operation{
		...
		for _, pm := range paymentMethods {
			...
			isDefault := account.PaymentMethodLast4 != nil && *account.PaymentMethodLast4 == pm.Card.Last4 &&
				account.PaymentMethodBrand != nil && *account.PaymentMethodBrand == string(pm.Card.Brand)
