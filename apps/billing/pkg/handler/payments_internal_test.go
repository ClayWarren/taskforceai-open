...
func TestRemainingSubscriptionMappingBranches(t *testing.T) {
	appUserID := "rc-user"
	pro := string(billing.PlanPro)
	assert.True(t, hasOpenRevenueCatSubscription(&billing.PaymentsAccountUser{
		RevenueCatAppUserID: &appUserID, Plan: pro,
	}))
	unknown := "expired"
	assert.False(t, hasOpenRevenueCatSubscription(&billing.PaymentsAccountUser{
		RevenueCatAppUserID: &appUserID, Plan: pro, SubscriptionStatus: &unknown,
	}))

	assert.Equal(t, billing.SubscriptionUpdate{}, stripeSubscriptionUpdate(nil))

	priceID := "price_covered"
	originalPlans := billing.BillingPlans
	t.Cleanup(func() { billing.BillingPlans = originalPlans })
	billing.BillingPlans = []billing.BillingPlanDefinition{{Plan: billing.PlanPro, StripePriceID: &priceID}}
	active := stripeSubscriptionUpdate(&stripe.Subscription{
		ID: "sub_active", Status: stripe.SubscriptionStatusActive,
		Items: &stripe.SubscriptionItemList{Data: []*stripe.SubscriptionItem{{Price: &stripe.Price{ID: priceID}}}},
	})
	require.NotNil(t, active.PriceID)
	require.NotNil(t, active.Plan)
	assert.Equal(t, string(billing.PlanPro), *active.Plan)

	inactive := stripeSubscriptionUpdate(&stripe.Subscription{ID: "sub_paused", Status: stripe.SubscriptionStatusPaused})
	require.NotNil(t, inactive.Plan)
	assert.Equal(t, string(billing.PlanFree), *inactive.Plan)
	assert.Equal(t, priceID, ptrString(&priceID))
}
