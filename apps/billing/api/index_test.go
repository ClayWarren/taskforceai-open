...
func TestHandleHealth_DeepRequiresAuthentication(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/api/v1/billing/health?deep=true", nil)
	w := httptest.NewRecorder()
...
func TestNewRouter_PostRoutesEnforceCSRF(t *testing.T) {
	t.Setenv("DATABASE_URL", "")
	...
	tests := []struct {
		...
		{"auto recharge", "/api/v1/billing/auto-recharge", `{"enabled":true}`},
		{"legacy subscription", "/api/v1/payments/create-subscription", `{"price_id":"price_123"}`},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			req := httptest.NewRequestWithContext(context.Background(), http.MethodPost, test.path, bytes.NewBufferString(test.body))
