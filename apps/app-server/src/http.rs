...
async fn http_runtime_loop(
    ...
    let mut remote_poll = tokio::time::interval(Duration::from_millis(750));
    ...
    loop {
        tokio::select! {
            ...
            _ = remote_poll.tick() => {
                if let Err(error) = process_remote_commands(
                    &mut runtime,
                    &mut connection,
                    &runtime_outputs,
                    &state,
...
async fn process_remote_commands(
    ...
    let poll = runtime
        .remote_api_client()
        .remote_poll_commands(&token, &device_id, &last_id)
        .await?;
    ...
    for command in poll.commands {
        let method = command
            ...
            .and_then(Value::as_str)
            .unwrap_or_default();
        let response_value = if !mobile_rpc_method_allowed(Some(method)) {
            json!({
                "jsonrpc": JSONRPC_VERSION,
                "id": command.request.get("id").cloned().unwrap_or(Value::Null),
                "error": { "code": -32601, "message": "Method not allowed for Remote" }
            })
        } else if method == "remote.event.snapshot" {
            let events = state.event_backlog.lock().await.clone();
            json!({
                "jsonrpc": JSONRPC_VERSION,
                "id": command.request.get("id").cloned().unwrap_or(Value::Null),
                "result": { "events": events }
            })
        } else if method == "remote.interaction.respond" {
            remote_interaction_response(&command.request, state).await
        } else {
            let request: JsonRpcRequest = serde_json::from_value(command.request.clone())
        ...
        runtime
            ...
            .remote_submit_result(
                ...
                &device_id,
                &command.id,
                &command.controller_device_id,
                &response_value,
            )
            .await?;
    }
    runtime.set_remote_last_command_id(&poll.last_id)?;
    Ok(())
}

async fn remote_interaction_response(request: &Value, state: &HttpServerState) -> Value {
...
fn mobile_rpc_method_allowed(method: Option<&str>) -> bool {
    matches!(
        method,
        Some(
            "server.ping"
