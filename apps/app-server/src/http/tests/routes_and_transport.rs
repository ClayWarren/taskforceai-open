...
async fn runtime_unavailable_edges_return_json_rpc_internal_errors() {
    ...
    let (response, action) = handle_rpc_request("session-token".to_string(), request, state).await;
...
async fn authorized_event_stream_writes_backlog_and_live_events() {
    ...
    assert!(text.contains("\"eventId\":1"));
    assert!(text.contains("\"eventId\":2"));
