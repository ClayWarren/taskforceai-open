...
#[test]
fn event_connections_filter_requests_and_allow_responses() {
...
async fn test_state(pairing_code: &str, session_token: &str) -> Arc<HttpServerState> {
    test_state_with_mobile_workspaces(pairing_code, session_token, Vec::new()).await
}

async fn test_state_with_mobile_workspaces(
    pairing_code: &str,
    session_token: &str,
    ...
    if !workspace_roots.is_empty() {
        runtime
            .set_metadata_json(
                ...
                &BTreeMap::from([(1_i64, workspace_roots)]),
            )
            .expect("mobile workspace roots should persist");
    }
    ...
    for raw in [
        ...
        let (response, _) =
            handle_rpc_request(session_token.to_string(), request, Arc::clone(&state)).await;
...
#[tokio::test]
async fn http_connections_keep_initialization_state_per_session() {
    let state = test_state("pair", "session-one").await;
    let raw = r#"{"jsonrpc":"2.0","id":1,"method":"server.ping","params":{}}"#;
    let request: JsonRpcRequest = serde_json::from_str(raw).expect("ping should parse");
    let (ready, _) =
        handle_rpc_request("session-one".to_string(), request, Arc::clone(&state)).await;
    assert!(ready.is_some_and(|response| response.error.is_none()));

    let request: JsonRpcRequest = serde_json::from_str(raw).expect("ping should parse");
    let (uninitialized, _) = handle_rpc_request("session-two".to_string(), request, state).await;
    assert_eq!(
        uninitialized
...
fn process_output_notification(delta: &str) -> OutgoingMessage {
    OutgoingMessage::Notification(crate::protocol::JsonRpcNotification {
        jsonrpc: JSONRPC_VERSION.to_string(),
        method: "process/outputDelta".to_string(),
        params: json!({ "processId": "process-one", "delta": delta, "cursor": delta.len() }),
    })
}
...
include!("tests/pairing_and_push.rs");
