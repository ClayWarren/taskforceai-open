...
fn push_lifecycle_notifications(messages: &mut Vec<OutgoingMessage>, event: &AppServerEvent) {
    match event {
        AppServerEvent::TurnStarted { thread_id, run } => {
            ...
            push_run_item_notification(messages, "item/started", Some(thread_id), run);
        }
        AppServerEvent::TurnInterrupted { thread_id, run } => {
            ...
            push_run_item_notification(messages, "item/completed", Some(thread_id), run);
        }
...
fn push_run_lifecycle_notifications(
    ...
    match run.status {
        ...
        crate::protocol::RunStatus::Completed => {
            ...
            push_run_item_notification(messages, "item/completed", None, run);
        }
...
fn push_run_item_notification(
    ...
    messages.push(notification(method, params));
}
