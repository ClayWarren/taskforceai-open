...
{"jsonrpc":"2.0","id":6,"method":"thread/list","params":{}}"#,
    ...
    assert_eq!(turn["result"]["thread"]["sessionId"], "thread-runtime");
    assert_eq!(
        turn["result"]["run"]["prompt"],
        "Check local runtime status"
    );
    assert_eq!(
        notifications_by_method(&lines, "event")[1]["params"]["type"],
        "run_updated"
    );
    ...
    assert_eq!(
        steer["result"]["thread"]["lastMessage"],
        "Focus on app-server protocol"
    );
    assert_eq!(
        fork["result"]["thread"]["parentSessionId"],
        start["result"]["thread"]["sessionId"]
    );
    assert_eq!(archive["result"]["thread"]["state"], "cancelled");
    assert_eq!(
        list["result"]["threads"]
            .as_array()
...
{"jsonrpc":"2.0","id":3,"method":"turn/interrupt","params":{"threadId":"thread-interrupt"}}"#,
    ...
    assert_eq!(notifications_by_method(&lines, "turn/interrupted").len(), 2);
    assert_eq!(notifications_by_method(&lines, "item/completed").len(), 1);
    assert_eq!(
...
{"jsonrpc":"2.0","id":2,"method":"promptQueue.add","params":{"conversationId":"conv-1","prompt":"follow up","status":"queued","dispatchTiming":"after_response","createdAt":1,"updatedAt":2,"modelId":"gpt-5","attachmentIds":["att-1"]}}
