...
#[tokio::test]
async fn workspace_file_methods_are_not_exposed_over_json_rpc() {
    let (output, _logs) = run(
        r#"{"jsonrpc":"2.0","id":1,"method":"workspace.file.list","params":{"workspace":"/"}}
{"jsonrpc":"2.0","id":2,"method":"workspace.file.read","params":{"workspace":"/","path":"etc/passwd"}}"#,
    )
    .await;
    ...
    for (index, line) in lines.iter().enumerate() {
        ...
        assert_eq!(line["error"]["code"], -32601);
        assert_eq!(line["error"]["message"], "Method not found");
    }
}
