...
impl super::AppRuntime {
    ...
    pub fn workflow_run_pause(
        ...
        let run = self.update_workflow_run_state(&params.run_id, WorkflowRunState::Paused)?;
        Ok(workflow_run_response(run, Vec::new()))
    }

    pub async fn workflow_run_resume(
        &mut self,
        params: WorkflowRunIDParams,
    ) -> Result<AppResponse, RuntimeError> {
        let resumed = self.update_workflow_run_state(&params.run_id, WorkflowRunState::Running)?;
        let mut events = vec![AppServerEvent::WorkflowRunUpdated {
            run: Box::new(resumed.clone()),
        }];

        for run_id in resumed.agent_run_ids.clone() {
            let Some(child_run) = self.runs.get(&run_id).cloned() else {
                continue;
            };
            for workflow_run in self.update_workflow_runs_for_run(&child_run)? {
        ...
        let reconciled = self.find_workflow_run(&params.run_id)?;
        if matches!(
            reconciled.state,
            WorkflowRunState::Completed | WorkflowRunState::Failed | WorkflowRunState::Cancelled
        ) {
            return Ok(AppResponse::WithEvents {
                result: to_value(WorkflowRunResult {
                    run: reconciled.clone(),
                }),
                events,
            });
        }
        let workflow = self.find_workflow(&reconciled.workflow_id)?;
        let (run, mut start_events) = self
            .start_ready_workflow_phases(&workflow, reconciled)
            .await?;
        events.append(&mut start_events);
        Ok(AppResponse::WithEvents {
            result: to_value(WorkflowRunResult { run }),
            events,
        })
    }
    ...
    pub fn workflow_run_cancel(
        ...
        let workflow_run = self.find_workflow_run(&params.run_id)?;
        let mut events = Vec::new();
        for run_id in &workflow_run.agent_run_ids {
            let should_cancel = self.runs.get(run_id).is_some_and(|run| {
                !matches!(
                    run.status,
            ...
            if let AppResponse::WithEvents {
                events: mut run_events,
                ..
            } = self.run_cancel(RunIDParams {
                run_id: run_id.clone(),
            })? {
                events.append(&mut run_events);
            }
        }
        let run = self.update_workflow_run_state(&params.run_id, WorkflowRunState::Cancelled)?;
        Ok(workflow_run_response(run, events))
    }
    ...
    ) -> Result<WorkflowRunRecord, RuntimeError> {
        let mut runs = self.workflow_runs()?;
        let run = runs
            .iter_mut()
            .find(|run| run.run_id == run_id)
...
fn workflow_run_response(run: WorkflowRunRecord, mut events: Vec<AppServerEvent>) -> AppResponse {
    events.push(AppServerEvent::WorkflowRunUpdated {
        run: Box::new(run.clone()),
    });
    AppResponse::WithEvents {
        result: to_value(WorkflowRunResult { run }),
        events,
    }
}
