...
pub(crate) fn format_plugins(plugins: Vec<PluginRecord>) -> String {
    if plugins.is_empty() {
        return "No plugins discovered. Use /plugins install <source> to add one.".to_string();
    }
    let mut lines = plugins
        .into_iter()
        .take(50)
        .map(|plugin| {
            let mut line = format!(
        ...
        .join("\n");
    lines.push_str(
        "\n\nInstall: /plugins install <source> · Update: /plugins update <id> · Remove: /plugins uninstall <id>",
    );
    lines
}
