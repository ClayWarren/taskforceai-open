...
impl super::AppRuntime {
    pub(crate) fn handle_logging_settings(
        ...
        let update = match args[0].to_ascii_lowercase().as_str() {
            ...
            _ => {
                return Ok(CommandExecuteResult {
                    handled: false,
                    title: "Settings".to_string(),
                    message: "Usage: /settings logging <level|format> ...".to_string(),
                })
            }
        };
        ...
        Ok(CommandExecuteResult {
            handled: true,
            title: "Settings".to_string(),
            message: "Logging settings updated.".to_string(),
        })
    }
    ...
    pub(crate) async fn remote_settings_account(
        ...
        Ok(CommandExecuteResult {
            handled: true,
            title: "Settings".to_string(),
            message: format_account_settings(&user),
        })
    }
    ...
    pub(crate) async fn remote_settings_personalization(
        ...
        if args.is_empty() {
            ...
            return Ok(CommandExecuteResult {
                handled: true,
                title: "Settings".to_string(),
                message: format_personalization_settings(&user),
            });
        }
        if args.len() < 2 {
    ...
    pub(crate) fn handle_goal_command(
        ...
        if args.is_empty() {
            return Ok(CommandExecuteResult {
                ...
                message: format_goal_state(self.goal_record()?),
            });
        }
        ...
        match args.to_ascii_lowercase().as_str() {
            "pause" => {
                ...
                Ok(CommandExecuteResult {
                    handled: goal.is_some(),
                    title: "Goal".to_string(),
                    message: goal
                        .map(|goal| format!("Goal paused: {}", goal.objective))
            ...
            "resume" => {
                ...
                Ok(CommandExecuteResult {
                    ...
                    title: "Goal".to_string(),
                    message: goal
                        .map(|goal| format!("Goal resumed: {}", goal.objective))
            ...
            _ => {
                ...
                Ok(CommandExecuteResult {
                    handled: true,
                    title: "Goal".to_string(),
                    message: format!("Goal set: {}", goal.objective),
                })
            }
        }
    }
    ...
    pub(crate) fn handle_pet_command(
        ...
        Ok(CommandExecuteResult {
            handled: true,
            title: "Companion".to_string(),
            message: format_pet_state(&result.pet),
        })
    }
        ...
        match action.as_str() {
            "use" | "select" | "set" => {
                ...
                Ok(CommandExecuteResult {
                    ...
                    message: format!("Active project set to {project_id}."),
                })
            }
            "clear" | "none" | "unset" => {
                self.project_clear()?;
                Ok(CommandExecuteResult {
                    handled: true,
                    title: "Project".to_string(),
                    message: "Active project cleared.".to_string(),
                })
            }
            "status" | "list" | "ls" => {
            ...
            "create" | "new" | "add" => {
                ...
                if name.trim().is_empty() {
                    return Ok(CommandExecuteResult {
                        handled: false,
                        title: "Project".to_string(),
                        message: "Usage: /project create <name>".to_string(),
                    });
                }
            ...
            _ => Ok(CommandExecuteResult {
                handled: false,
                title: "Project".to_string(),
                message: "Usage: /project [status|use <id>|clear]".to_string(),
            }),
        }
    }
    ...
    pub(crate) async fn handle_mcp_command(
        ...
        let action = args
            ...
            .unwrap_or_else(|| "list".to_string());
        match action.as_str() {
            "list" | "ls" => Ok(CommandExecuteResult {
                handled: true,
                title: "MCP".to_string(),
                message: format_mcp_servers(&self.mcp_servers()?),
            }),
            "add" => {
                // coverage:ignore-line
                ...
                Ok(CommandExecuteResult {
                    handled: true,
                    title: "MCP".to_string(),
                    message: format!("Configured MCP server {}.", result["server"]["name"]),
                })
            }
            "remove" | "rm" => {
                let name = args
            ...
            "enable" => {
                ...
                Ok(CommandExecuteResult {
                    handled: true,
                    title: "MCP".to_string(),
                    message: format!("Enabled MCP server {name}."),
                })
            }
            "disable" => {
                let name = args
                ...
                Ok(CommandExecuteResult {
                    handled: true,
                    title: "MCP".to_string(),
                    message: format!("Disabled MCP server {name}."),
                })
            }
            "available" => Ok(CommandExecuteResult {
                handled: true,
                title: "MCP".to_string(),
                message: format_mcp_available(&self.mcp_available_result()?), // coverage:ignore-line
            }), // coverage:ignore-line
            "tools" => {
                let name = args.get(1).ok_or_else(|| {
                    // coverage:ignore-start
                ...
                Ok(CommandExecuteResult {
                    handled: true,
                    title: "MCP".to_string(),
                    message: format!("Updated tools for MCP server {}.", result.server.name),
                })
            }
            "inspect" => {
                let name = args
                ...
                Ok(CommandExecuteResult {
                    handled: true,
                    title: "MCP".to_string(), // coverage:ignore-line
                    message: format_mcp_inspect(&from_value_response(
            ...
            "call" => {
                ...
                Ok(CommandExecuteResult {
                    handled: true, // coverage:ignore-line
                    // coverage:ignore-start
                    title: "MCP".to_string(),
                    message: format_mcp_call_result(&from_value_response(
