    ...
    pub(crate) async fn handle_workflow_command(
        ...
        let message = match action.as_str() {
            ...
            "resume" => {
                ...
                let result: WorkflowRunResult = from_response_result(
                    self.workflow_run_resume(WorkflowRunIDParams {
                        run_id: id.to_string(),
                    })
                    .await?,
                )?; // coverage:ignore-line -- Async response conversion is exercised by the workflow command test.
        ...
        match action.as_str() {
            ...
            "inspect" => {
                ...
                Ok(command_message(
                    ...
                    format_mcp_inspect(&from_response_result(
