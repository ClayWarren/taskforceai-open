...
pub(crate) fn local_model_list_result(selected_model_id: Option<String>) -> ModelListResult {
    ModelListResult {
        ...
        default_model_id: "zai/glm-5.2".to_string(),
        selected_model_id,
...
mod tests {
    ...
    #[test]
    fn ollama_status_memory_and_pull_event_mappers_cover_all_variants() {
        ...
        assert!(matches!(
            map_ollama_pull_event(OllamaPullEvent::Status {
                message: "pulling".to_string()
            }),
            OllamaPullEventRecord::Status { .. }
        ));
        assert!(matches!(
            map_ollama_pull_event(OllamaPullEvent::Progress {
        ...
        assert!(matches!(
            map_ollama_pull_event(OllamaPullEvent::Success),
            OllamaPullEventRecord::Success
        ));
        assert!(matches!(
            map_ollama_pull_event(OllamaPullEvent::Error {
