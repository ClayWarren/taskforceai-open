        ...
        let (run, events) = match response {
            AppResponse::WithEvents { result, events } => {
                let result: SubmitRunResult =
                    serde_json::from_value(result) // coverage:ignore-line
                        .map_err(|err| RuntimeError::storage(err.to_string()))?; // coverage:ignore-line
                (result.run, events) // coverage:ignore-line
            } // coverage:ignore-line
            // coverage:ignore-start
...
mod resume_tests {
    ...
    fn short_text_file_prompts_do_not_route_to_video() {
        ...
        assert!(
            resolve_generated_media_route("Create a video showing the changed files", true)
                .is_some()
        );
        assert_eq!(
