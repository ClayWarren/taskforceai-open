...
use taskforceai_core::orchestration as core_orchestration;
...
pub(crate) fn default_orchestration_roles() -> Vec<OrchestrationRole> {
    core_orchestration::default_orchestration_roles()
        .into_iter()
        .map(|role| OrchestrationRole {
...
pub(crate) fn merge_default_orchestration_roles(config: &mut OrchestrationConfig) {
    for default_role in default_orchestration_roles() {
        if !config // coverage:ignore-line
            .roles // coverage:ignore-line
            .iter()
...
pub(crate) fn normalize_orchestration_role(role: &str) -> Result<String, RuntimeError> {
    core_orchestration::normalize_orchestration_role(role)
        .map(str::to_string)
