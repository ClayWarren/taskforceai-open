...
#[test]
fn permission_profiles_filter_configured_and_explicit_tools() {
    let servers = filter_mcp_servers_for_permission(
        vec![McpServerRecord {
            name: "workspace".to_string(),
            endpoint: "stdio:server".to_string(),
            tools: vec!["read_file".to_string(), "write_file".to_string()],
            enabled: true,
        }],
        Some(PermissionProfile::ReadOnly),
    );
    assert_eq!(servers[0].tools, ["read_file"]);
    assert!(filter_client_mcp_tools_for_permission(
        vec![ClientMcpTool {
