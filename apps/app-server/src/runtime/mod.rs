...
use crate::protocol::{
    AppServerEvent, AttachmentRecord, PendingPromptRecord, PermissionGrantRecord, RunRecord,
    ServerInfo,
};
use taskforceai_app_store::SqliteRunStore;

pub(crate) const DEFAULT_KEYCHAIN_SERVICE: &str = "com.taskforceai.app-server.auth";
pub(crate) const DESKTOP_KEYCHAIN_SERVICE: &str = "com.taskforceai.desktop.auth";
pub(crate) const LEGACY_KEYCHAIN_SERVICE: &str = "taskforceai";
...
pub(crate) const PROJECT_WORKSPACES_METADATA_KEY: &str = "project_workspaces";
...
pub(crate) struct TestAuthKeychain {
    tokens: Arc<StdMutex<BTreeMap<String, String>>>,
    fail_get: bool,
    fail_set: bool,
    fail_delete: bool,
    fail_delete_services: HashSet<String>,
}
...
impl TestAuthKeychain {
    ...
    pub(crate) fn with_failures(
        ...
        if let Some(token) = token {
            tokens.insert(DEFAULT_KEYCHAIN_SERVICE.to_string(), token.to_string());
        }
        Self {
            ...
            fail_set,
            fail_delete,
            fail_delete_services: HashSet::new(),
        }
    }

    pub(crate) fn with_delete_failure_for_service(service: &str) -> Self {
        let mut keychain = Self::new(None);
        keychain.fail_delete_services.insert(service.to_string());
        keychain
    }

    pub(crate) fn with_legacy_token(token: &str) -> Self {
        Self::with_legacy_token_and_failures(token, false, false, false)
    }

    pub(crate) fn with_legacy_token_and_failures(
        token: &str,
        fail_get: bool,
        fail_set: bool,
        fail_delete: bool,
    ) -> Self {
        let keychain = Self::with_failures(None, fail_get, fail_set, fail_delete);
        keychain
            .tokens
            .lock()
            .expect("test keychain should not be poisoned")
            .insert(LEGACY_KEYCHAIN_SERVICE.to_string(), token.to_string());
        keychain
    }

    pub(crate) fn token_for_service(&self, service: &str) -> Option<String> {
        self.tokens
            .lock()
            .expect("test keychain should not be poisoned")
            .get(service)
    ...
    pub(crate) fn get(&self, service: &str) -> Result<Option<String>, String> {
        if self.fail_get {
    ...
    pub(crate) fn set(&self, service: &str, token: &str) -> Result<(), String> {
        if self.fail_set {
            ...
            self.tokens
                .lock()
                .expect("test keychain should not be poisoned")
                .insert(service.to_string(), token.to_string());
            Ok(())
        }
    }

    pub(crate) fn delete(&self, service: &str) -> Result<(), String> {
        if self.fail_delete || self.fail_delete_services.contains(service) {
...
pub struct AppRuntime {
    ...
    pub(crate) process_manager: Arc<impl_process::ProcessManager>,
    pub(crate) watch_manager: Arc<impl_compat::FsWatchManager>,
    pub(crate) approval_grants: Arc<StdMutex<BTreeMap<String, PermissionGrantRecord>>>,
}
...
mod impl_git_review;
pub(crate) use impl_git_review::mobile_git_repository_root;
mod impl_goals_agents;
mod impl_history_status;
mod impl_lifecycle;
pub(crate) use impl_lifecycle::protocol_schema;
mod impl_process;
mod impl_remote;
mod impl_runs;
mod impl_settings;
