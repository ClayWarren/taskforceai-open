...
const VOICE_REQUEST_TIMEOUT: Duration = Duration::from_secs(60);
const MAX_VOICE_JSON_RESPONSE_SIZE: usize = 1024 * 1024;
...
impl super::AppRuntime {
    pub async fn voice_transcribe(
        ...
        let payload = voice_json_response::<VoiceTranscribePayload>(response).await?;
    ...
    pub async fn voice_speech_generate(
        ...
        let bytes = bounded_voice_response(response, MAX_AUDIO_SIZE).await?;
        if bytes.is_empty() {
            return Err(RuntimeError::network(
    ...
    pub async fn voice_realtime_setup(
        ...
        let payload = voice_json_response::<VoiceRealtimeSetupPayload>(response).await?;
...
fn voice_client() -> Result<reqwest::Client, RuntimeError> {
...
async fn voice_json_response<T: DeserializeOwned>(
    response: reqwest::Response,
) -> Result<T, RuntimeError> {
    let bytes = bounded_voice_response(response, MAX_VOICE_JSON_RESPONSE_SIZE).await?;
    serde_json::from_slice(&bytes).map_err(|err| RuntimeError::network(err.to_string()))
}

async fn bounded_voice_response(
    response: reqwest::Response,
    max_bytes: usize,
        ...
        .is_some_and(|length| length > max_bytes as u64)
    {
        return Err(RuntimeError::network(format!(
            "voice gateway response exceeds the {max_bytes} byte limit"
        )));
    }
    let mut bytes = Vec::new();
    let mut stream = response.bytes_stream();
    while let Some(chunk) = stream.next().await {
        let chunk = chunk.map_err(|err| RuntimeError::network(err.to_string()))?;
...
async fn voice_gateway_error_message(response: reqwest::Response) -> String {
    ...
    match bounded_voice_response(response, MAX_VOICE_JSON_RESPONSE_SIZE).await {
        Ok(bytes) => serde_json::from_slice::<VoiceErrorPayload>(&bytes)
            .ok()
...
mod tests {
    ...
    use super::{
        bounded_voice_response, voice_gateway_base_url_from_api_base, voice_gateway_error_message,
    };
    ...
    fn response_with_headers(headers: &str, body: &str) -> (String, thread::JoinHandle<()>) {
        let listener = TcpListener::bind("127.0.0.1:0").expect("voice helper should bind");
        let address = listener.local_addr().expect("voice helper address");
        let headers = headers.to_string();
        let body = body.to_string();
        let handle = thread::spawn(move || {
            let (mut stream, _) = listener.accept().expect("voice helper request");
            let mut request = [0_u8; 1024];
            let _ = stream
                .read(&mut request)
                .expect("read voice helper request");
            let response = format!(
                "HTTP/1.1 500 Internal Server Error\r\n{headers}\r\nConnection: close\r\n\r\n{body}"
            );
            stream
                .write_all(response.as_bytes())
                .expect("write voice helper response");
        });
        (format!("http://{address}"), handle)
    }

    #[tokio::test]
    async fn bounded_voice_responses_cover_stream_and_error_fallback_limits() {
        let (url, server) =
            response_with_headers("Transfer-Encoding: chunked", "4\r\ntest\r\n0\r\n\r\n");
        let response = reqwest::get(url).await.expect("chunked voice response");
        assert!(bounded_voice_response(response, 3).await.is_err());
        server.join().expect("chunked voice server");

        let oversized = super::MAX_VOICE_JSON_RESPONSE_SIZE + 1;
        let (url, server) = response_with_headers(&format!("Content-Length: {oversized}"), "");
        let response = reqwest::get(url).await.expect("oversized error response");
        assert!(voice_gateway_error_message(response)
            .await
            .contains("HTTP 500"));
        server.join().expect("oversized error server");
    }
}
