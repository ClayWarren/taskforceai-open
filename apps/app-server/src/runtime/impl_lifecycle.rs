...
impl super::AppRuntime {
    ...
    pub fn try_new(config: RuntimeConfig) -> Result<Self, RuntimeError> {
        ...
        let mut runtime = Self {
            ...
            mock_server: None,
            process_manager: Arc::new(super::impl_process::ProcessManager::default()),
            watch_manager: Arc::new(super::impl_compat::FsWatchManager::default()),
            approval_grants: Arc::new(StdMutex::new(BTreeMap::new())),
        };
        runtime.apply_runtime_config_from_store()?;
        Ok(runtime)
    }

    /// Creates an isolated request view whose durable store, API client, MCP
    /// manager, event channel, and interaction broker remain shared. It is used
    /// only for registry-declared concurrent methods; mutations to ordinary
    /// in-memory runtime state stay on the serial runtime.
    pub(crate) fn concurrent_snapshot(&self) -> Self {
        let auth_token_cache = self
            .auth_token_cache
            .lock()
            .expect("auth token cache lock should not be poisoned")
            .clone();
        Self {
            config: self.config.clone(),
            run_store: self.run_store.clone(),
            runs: self.runs.clone(),
            private_run_ids: self.private_run_ids.clone(),
            pending_prompts: self.pending_prompts.clone(),
            active_attachments: self.active_attachments.clone(),
            memory_metadata: self.memory_metadata.clone(),
            auth_token_cache: StdMutex::new(auth_token_cache),
            ...
            approval_grants: Arc::clone(&self.approval_grants),
        }
    }
    ...
    pub fn server_describe(&self) -> AppResponse {
        let schema: Value = serde_json::from_str(protocol_schema()).unwrap_or(Value::Null);
        let methods = schema
            .pointer("/$defs/AppServerRpcMethod/enum")
        ...
        let schema_id = schema
            ...
            .unwrap_or("https://taskforceai.dev/schemas/app-server-protocol.schema.json")
            .to_string();
        value(ServerDescribeResult {
            server: self.config.server_info.clone(),
            protocol_version: taskforceai_app_protocol::PROTOCOL_VERSION.to_string(),
            schema_id,
            capabilities: server_capabilities(),
            methods,
            experimental_methods: vec!["thread/rollback".to_string()],
        })
    }
...
pub(crate) fn protocol_schema() -> &'static str {
    include_str!("../../../../packages/contracts/rust/schema/app-server-protocol.schema.json")
}
