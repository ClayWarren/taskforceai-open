...
impl super::AppRuntime {
    pub(crate) fn handle_orchestration_command(
        ...
        match action.as_str() {
            "list" | "status" => Ok(command_message(
                "Orchestration",
                format_orchestration_config(&self.orchestration_config()?),
            )),
            "clear" | "reset" => {
                self.orchestration_clear()?;
                Ok(command_message(
                    "Orchestration",
                    "Custom orchestration config cleared.",
                ))
            }
            "budget" => {
                let Some(raw_budget) = args.get(1) else {
                    return Ok(command_message(
                        "Orchestration",
                        format_orchestration_budget(self.orchestration_config()?.budget),
                    ));
                };
