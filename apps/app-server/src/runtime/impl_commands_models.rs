...
use super::models::{
    format_model_list, local_model_list_result, map_ollama_pull_event, map_ollama_status,
};
use super::util::*;
    ...
    pub(crate) fn handle_model_command(
        ...
        let Some(action) = args.first().map(|value| value.to_ascii_lowercase()) else {
            return Ok(CommandExecuteResult {
                ...
                message: format!("Current model: {current}\nUse /model set <model-id>."),
            });
        };
        match action.as_str() {
            "list" => Ok(CommandExecuteResult {
                handled: true,
                title: "Model".to_string(),
                message: format_model_list(&local_model_list_result(self.default_model_id()?)),
            }),
            "reset" | "default" => {
                ...
                Ok(CommandExecuteResult {
                    handled: true,
                    title: "Model".to_string(),
                    message: "Model reset to default.".to_string(),
                })
            }
            "set" | "select" => {
            ...
            if let Ok(catalog) = self.api_client.list_models().await {
                return Ok(ModelListResult {
                    ...
                    options: catalog
                        .options
                        .into_iter()
                        .map(|option| ModelOptionRecord {
                            id: option.id,
                            label: option.label,
                            badge: option.badge,
                            description: option.description,
                            usage_multiple: option.usage_multiple,
                            reasoning_effort_levels: option.reasoning_effort_levels,
