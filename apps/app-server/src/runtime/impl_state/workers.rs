            ...
            match stream_task.await {
                ...
                Ok(Err(err)) => {
                    ...
                    log_runtime(
                        ...
                        json!({ "runId": failed.id, "error": failed.error }), // coverage:ignore-line
                    ); // coverage:ignore-line
                    send_run_update(&sender, failed, "remote stream failure").await;
                    // coverage:ignore-line
                } // coverage:ignore-line
                // coverage:ignore-start
                Err(err) => {
                    ...
                    send_run_update(&sender, failed, "remote stream join failure").await;
                    // coverage:ignore-line
                }
            }
        });
