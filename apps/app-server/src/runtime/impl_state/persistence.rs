...
impl crate::runtime::AppRuntime {
    ...
    pub(crate) fn auth_token(&self) -> Result<Option<String>, RuntimeError> {
        if let Some(token) = self
            ...
            .clone()
        {
            return Ok(token.filter(|token| !auth_token_is_expired(token)));
        }
        if let Some(token) = self
            .memory_metadata
            .get("auth_token")
        ...
        {
            return Ok((!auth_token_is_expired(&token)).then_some(token));
            // coverage:ignore-line
        }
        ...
        *self
        ...
        Ok(token.filter(|token| !auth_token_is_expired(token)))
    }
