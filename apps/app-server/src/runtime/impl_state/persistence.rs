    ...
    pub(crate) fn auth_token(&self) -> Result<Option<String>, RuntimeError> {
        ...
        let token = match self.config.auth_token_storage {
            ...
            AuthTokenStorage::KeyringWithMetadataFallback => match keychain_get_auth_token() {
                Ok(Some(token)) => Some(token), // coverage:ignore-line
                Ok(None) | Err(_) => self
                    .metadata_value("auth_token")?
                    .and_then(|value| non_empty_string(&value)),
            },
            ...
            AuthTokenStorage::KeyringWithMetadataFallback => match token {
                Some(token) => {
                    ...
                    *self
                    ...
                    if let Err(error) = keychain_set_auth_token(token) {
                        // coverage:ignore-start
                        log::warn!(
                                            target: "auth",
                        // coverage:ignore-end
                                            "Failed to store desktop auth token in keychain; using metadata fallback: {error}"
                                        );
                    }
                ...
                None => {
                    ...
                    *self
                    ...
                    if let Err(error) = keychain_delete_auth_token() {
                        log::warn!(
                                            target: "auth",
                        // coverage:ignore-end
