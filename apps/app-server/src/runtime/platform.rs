...
pub(crate) fn skill_markdown_files(root: &Path) -> Result<Vec<PathBuf>, RuntimeError> {
    ...
    let entries = fs::read_dir(root).map_err(|err| RuntimeError::storage(err.to_string()))?;
    for entry in entries {
        let entry = entry.map_err(|err| RuntimeError::storage(err.to_string()))?;
        let path = entry.path();
        if path.is_dir() {
...
pub(crate) fn parse_skill_file(
    ...
    let content = fs::read_to_string(path).map_err(|err| RuntimeError::storage(err.to_string()))?;
...
pub(crate) fn parse_plugin_manifest(path: &Path) -> Result<Option<PluginRecord>, RuntimeError> {
    let content = fs::read_to_string(path).map_err(|err| RuntimeError::storage(err.to_string()))?;
    let manifest: Value =
        serde_json::from_str(&content).map_err(|err| RuntimeError::storage(err.to_string()))?;
