...
pub(crate) fn conversation_sync_value(conversation: &ConversationRecord) -> Value {
    desktop_conversation_sync_value(conversation)
}

pub(crate) fn message_sync_value(message: &MessageRecord) -> Value {
    desktop_message_sync_value(message)
}
...
pub(crate) fn desktop_conversation_sync_value(conversation: &ConversationRecord) -> Value {
    ...
    json!({
        ...
        "timestamp": rfc3339_timestamp(i64::try_from(conversation.created_at).unwrap_or(i64::MAX)),
        "user_input": conversation.title,
        "result": conversation.last_message_preview,
        "project_id": conversation.project_id,
        "sync_version": conversation.sync_version,
        "last_synced_at": rfc3339_timestamp(conversation.last_synced_at),
        "device_id": conversation.device_id,
        "is_deleted": conversation.is_deleted,
        "is_archived": conversation.is_archived,
        "updated_at": rfc3339_timestamp(i64::try_from(conversation.updated_at).unwrap_or(i64::MAX)),
    })
}
...
pub(crate) fn desktop_message_sync_value(message: &MessageRecord) -> Value {
    ...
    json!({
        ...
        "created_at": rfc3339_timestamp(i64::try_from(message.created_at).unwrap_or(i64::MAX)),
        "updated_at": rfc3339_timestamp(i64::try_from(message.updated_at).unwrap_or(i64::MAX)),
        "error": message.error,
        ...
        "last_synced_at": rfc3339_timestamp(message.last_synced_at),
...
pub(crate) fn conversation_record_from_sync_value(
    ...
    Ok(ConversationRecord {
        ...
        project_id: number_field(&value, "projectId")
            .or_else(|| number_field(&value, "project_id")),
...
mod tests {
    ...
    fn conversation_sync_serializers_preserve_archive_state() {
        ...
        assert_eq!(app_value["is_archived"], json!(true));
    ...
    fn record_merge_title_and_field_helpers_cover_empty_and_alias_paths() {
        ...
        assert_eq!(
            millis_field(&json!({ "millis": "1970-01-01T00:00:01Z" }), "millis"),
            Some(1_000)
        );
        assert_eq!(
    ...
    fn message_sync_serializers_and_parsers_cover_camel_snake_and_numeric_ids() {
        ...
        assert_eq!(app_value["conversation_local_id"], json!("conv-local"));
        assert_eq!(app_value["tool_events"], json!([{ "type": "tool" }]));
