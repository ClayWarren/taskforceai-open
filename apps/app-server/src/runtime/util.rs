...
use crate::protocol::{
    AgentSessionRecord, AgentSessionRunResult, AppResponse, AppServerEvent, CommandExecuteResult,
    DiagnosticItem, RunRecord, RunStatus, RunStatusResult, SubmitRunResult,
};
...
pub(crate) fn command_message(
    ...
    command_result(true, title, message)
}

pub(crate) fn command_unhandled(
    title: impl Into<String>,
    message: impl Into<String>,
) -> CommandExecuteResult {
    command_result(false, title, message)
}
...
pub(crate) fn run_status_result_and_events(
    response: AppResponse,
) -> Result<(RunStatusResult, Vec<AppServerEvent>), RuntimeError> {
    result_and_events(response)
}

fn result_and_events<T: serde::de::DeserializeOwned>(
    response: AppResponse,
) -> Result<(T, Vec<AppServerEvent>), RuntimeError> {
    match response {
...
pub(crate) fn auth_token_is_expired(token: &str) -> bool {
    auth_token_is_expired_at(token, SystemTime::now())
}

fn auth_token_is_expired_at(token: &str, now: SystemTime) -> bool {
    ...
    let Ok(claims) = serde_json::from_slice::<TokenExpiryClaims>(&decoded) else {
        return false;
    };
    let Some(expires_at) = claims.exp else {
        return false;
    };
    let now = now
...
mod auth_token_tests {
    ...
    #[test]
    fn token_expiry_only_rejects_parseable_expired_jwts() {
        let now = UNIX_EPOCH + std::time::Duration::from_secs(100);
        assert!(auth_token_is_expired_at(&token_with_exp(100), now));
        assert!(auth_token_is_expired_at(&token_with_exp(99), now));
        assert!(!auth_token_is_expired_at(&token_with_exp(101), now));
        assert!(!auth_token_is_expired_at("opaque-token", now));
    }
}
