...
use crate::protocol::{RunRecord, RunStatus};
...
pub(crate) fn apply_stream_event_to_run(mut run: RunRecord, event: ApiStreamEvent) -> RunRecord {
    ...
    if let Some(pending_approval) = event.pending_approval {
        run.pending_approval = (!pending_approval.is_null()).then_some(pending_approval);
    }
