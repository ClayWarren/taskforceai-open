...
#[tokio::test]
async fn session_remote_approval_is_forwarded_and_remembered_for_the_thread() {
    ...
    assert!(
        broker
            .resolve(crate::protocol::JsonRpcResponse {
                ...
                id: Some(request.id),
                result: Some(json!({"decision": "acceptForSession", "result": {"remember": true}})),
                error: None,
            })
            .await
    );
    ...
    let grants = tokio::time::timeout(std::time::Duration::from_secs(2), async {
        loop {
            let AppResponse::Value(grants) =
                runtime.permission_grant_list(crate::protocol::PermissionGrantListParams {
                    thread_id: Some("approval-thread".to_string()),
                })
            else {
                panic!("permission grant list must return a value");
            };
            if grants["grants"].as_array().map(Vec::len) == Some(1) {
                break grants;
            }
    ...
    .await
    .expect("session approval grant should be saved");
