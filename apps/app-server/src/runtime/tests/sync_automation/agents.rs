...
use taskforceai_app_protocol::{
    TaskMode, ThreadIDParams, ThreadImportParams, ThreadItemRecord, ThreadItemStatus,
    ThreadItemType, ThreadRecord, ThreadStartParams, ThreadState, TurnInterruptParams, TurnRecord,
    TurnStartParams, TurnStatus, TurnSteerParams,
};
...
fn agent_sessions_round_trip_through_local_metadata() {
    ...
    assert_eq!(forked["session"]["taskMode"], "work");
...
#[test]
fn imported_threads_preserve_mode_and_history_without_reviving_active_work() {
    let store_path = test_store_path("thread-import");
    let config = RuntimeConfig::default().with_run_store_path(&store_path);
    let mut runtime = AppRuntime::try_new(config.clone()).expect("runtime should start");
    let thread = ThreadRecord {
        id: "handoff-work-thread".to_string(),
        title: "Remote work".to_string(),
        objective: "Continue on another desktop".to_string(),
        state: ThreadState::Active,
        archived: false,
        source: "desktop-local".to_string(),
        task_mode: TaskMode::Work,
        parent_thread_id: Some("parent-thread".to_string()),
        turns: vec![TurnRecord {
            id: "turn-active".to_string(),
            thread_id: "handoff-work-thread".to_string(),
            run_id: "run-active".to_string(),
            status: TurnStatus::InProgress,
            items: vec![ThreadItemRecord {
                id: "turn-active:agent-message".to_string(),
                turn_id: "turn-active".to_string(),
                item_type: ThreadItemType::AgentMessage,
                status: ThreadItemStatus::InProgress,
                content: serde_json::json!({"text": "partial response"}),
                created_at: 10,
                updated_at: 10,
            }],
            created_at: 10,
    ...
    let imported = result_value(
        runtime
            .thread_import(ThreadImportParams {
                thread: thread.clone(),
                overwrite: false,
            })
            .expect("thread import should work"),
    );
    assert_eq!(imported["thread"]["taskMode"], "work");
    assert_eq!(imported["thread"]["turns"][0]["status"], "interrupted");
    assert_eq!(
        imported["thread"]["turns"][0]["items"][0]["status"],
        "failed"
    );
    ...
    let sessions = result_value(
        runtime
            .agent_session_list()
            .expect("imported session should list"),
    );
    assert_eq!(sessions["sessions"][0]["taskMode"], "work");
    assert_eq!(
        sessions["sessions"][0]["activeRunId"],
        serde_json::Value::Null
    );
    assert_eq!(sessions["sessions"][0]["runIds"][0], "run-active");
    ...
    assert_eq!(
        runtime
            ...
            .expect_err("duplicate import should require overwrite")
            .message,
        "thread id already exists"
    );
    let overwritten = result_value(
        runtime
            .thread_import(ThreadImportParams {
                thread,
                overwrite: true,
    ...
    assert_eq!(overwritten["thread"]["taskMode"], "work");

    drop(runtime);
    let restarted = AppRuntime::try_new(config).expect("runtime should restart");
    let persisted = result_value(
        restarted
            .thread_read(ThreadIDParams {
                thread_id: "handoff-work-thread".to_string(),
            })
            .expect("imported thread should persist"),
    );
    assert_eq!(persisted["thread"]["taskMode"], "work");
    assert_eq!(persisted["thread"]["turns"][0]["status"], "interrupted");

    let _ = std::fs::remove_file(store_path);
}

#[tokio::test]
async fn typed_threads_persist_history_fork_archive_and_cancel_independently() {
    let store_path = test_store_path("typed-thread-lifecycle");
    let config = RuntimeConfig::default().with_run_store_path(&store_path);
    let mut runtime = AppRuntime::try_new(config.clone()).expect("runtime should start");
    ...
    result_value(
        runtime
            .thread_start(ThreadStartParams {
                objective: "Preserve authoritative history".to_string(),
                thread_id: Some("typed-thread".to_string()),
                title: Some("Typed lifecycle".to_string()),
    ...
    let started = result_value(
        runtime
            .turn_start(TurnStartParams {
                thread_id: "typed-thread".to_string(),
                input: "Inspect protocol history".to_string(),
                display_input: None,
                model_id: None,
    ...
    let parent_turn_id = started["turn"]["id"].as_str().expect("turn id").to_string();
    let steered = result_value(
        runtime
            .turn_steer(TurnSteerParams {
                thread_id: "typed-thread".to_string(),
                input: "Include the interaction broker".to_string(),
                display_input: None,
            })
            .await
            .expect("active turn should accept steering"),
    );
    assert_eq!(steered["turn"]["items"].as_array().expect("items").len(), 3);

    let forked = result_value(
        runtime
            .thread_fork(ThreadIDParams {
                thread_id: "typed-thread".to_string(),
            })
    ...
    let fork_id = forked["thread"]["id"]
        ...
        .expect("fork id")
        .to_string();
    assert_eq!(forked["thread"]["parentThreadId"], "typed-thread");
    assert_eq!(forked["thread"]["taskMode"], "code");
    assert_ne!(forked["thread"]["turns"][0]["id"], parent_turn_id);
    assert_eq!(
        forked["thread"]["turns"][0]["items"]
            .as_array()
            .expect("items")
            .len(),
        3
    ...
    assert_eq!(archived["thread"]["archived"], true);
    assert_eq!(archived["thread"]["state"], "active");
    let unarchived = result_value(
        runtime
            .thread_unarchive(ThreadIDParams {
                thread_id: "typed-thread".to_string(),
            })
            .expect("thread should unarchive"),
    );
    assert_eq!(unarchived["thread"]["archived"], false);
    ...
    let canceled = result_value(
        runtime
            ...
            .expect("fork should cancel"),
    );
    assert_eq!(canceled["thread"]["state"], "canceled");
    assert_eq!(canceled["thread"]["archived"], false);

    drop(runtime);
    let restarted = AppRuntime::try_new(config).expect("runtime should restart");
    let parent = result_value(
        restarted
            .thread_read(ThreadIDParams {
                thread_id: "typed-thread".to_string(),
            })
    ...
    assert_eq!(parent["thread"]["turns"][0]["id"], parent_turn_id);
    assert_eq!(parent["thread"]["taskMode"], "code");
    assert_eq!(
        parent["thread"]["turns"][0]["items"]
            .as_array()
            .expect("items")
            .len(),
        3
    );
    let fork = result_value(
        restarted
            .thread_read(ThreadIDParams { thread_id: fork_id })
            .expect("fork history should persist"),
    );
    assert_eq!(fork["thread"]["state"], "canceled");
    assert_eq!(fork["thread"]["taskMode"], "code");
...
fn agent_and_thread_methods_reject_invalid_inputs_and_report_authenticated_diagnostics() {
    ...
    let AppResponse::WithEvents {
        result: blank_thread,
        ..
    } = runtime
        .thread_start(ThreadStartParams {
            objective: "   ".to_string(),
            thread_id: Some("blank-thread".to_string()),
            title: None,
            source: None,
            task_mode: Default::default(),
            settings: None,
        })
        .expect("blank thread should be created without an initial turn")
    else {
        panic!("thread start should return its lifecycle events");
    };
    assert_eq!(blank_thread["thread"]["id"], "blank-thread");
    assert_eq!(blank_thread["thread"]["objective"], "");
