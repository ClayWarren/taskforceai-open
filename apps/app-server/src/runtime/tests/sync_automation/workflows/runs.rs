...
async fn workflow_run_tracks_linked_app_server_run_completion() {
    ...
    runtime
        ...
        .expect("single phase workflow should pause");
    let mut workflow_runs = runtime.workflow_runs().expect("workflow runs");
    workflow_runs[0]
        .agent_run_ids
        .push("missing-child-run".to_string());
    runtime
        .save_workflow_runs(&workflow_runs)
        .expect("save missing child fixture");
    ...
    runtime
        .workflow_run_resume(WorkflowRunIDParams {
            run_id: workflow_run_id.to_string(),
        })
        .await
        .expect("completed child should reconcile on resume");
    ...
    runtime
        .workflow_run_cancel(WorkflowRunIDParams {
            run_id: workflow_run_id.to_string(),
        })
        .expect("cancel should skip terminal child runs");
}

#[tokio::test]
async fn paused_workflow_ignores_child_events_and_resume_dispatches_ready_phase() {
    let mut runtime = AppRuntime::new(RuntimeConfig::default());
    runtime
        .workflow_save(WorkflowSaveParams {
            workflow: WorkflowDefinitionRecord {
                workflow_id: "pause-resume".to_string(),
                name: "Pause and resume".to_string(),
                description: None,
                version: "1.0.0".to_string(),
                visibility: WorkflowVisibility::Personal,
                args_schema: None,
                ...
                phases: vec![
                    ...
                    WorkflowPhaseDefinition {
                        ...
                        kind: WorkflowPhaseKind::Prompt,
                        prompt: Some("Do the second thing.".to_string()),
    ...
    let workflow_run_id = started["run"]["runId"]
        .as_str()
        .expect("workflow run id")
        .to_string();
    let first_run_id = started["run"]["phaseRuns"][0]["agentRunIds"][0]
        .as_str()
        .expect("first run id")
        .to_string();
    runtime
        ...
        .expect("workflow pause should work");

    let mut child = runtime
        .get_run(&first_run_id)
        .expect("first child should exist");
    child.status = RunStatus::Completed;
    child.output = Some("first done".to_string());
    runtime
        .apply_event(AppServerEvent::RunUpdated {
            run: Box::new(child),
        })
        .expect("paused child completion should be retained");
    let paused = result_value(
        runtime
            .workflow_run_get(WorkflowRunIDParams {
                run_id: workflow_run_id.clone(),
            })
            .expect("paused workflow should be readable"),
    );
    assert_eq!(paused["run"]["state"], "paused");
    assert!(paused["run"]["phaseRuns"][1]["agentRunIds"]
        .as_array()
        .expect("second phase ids")
        .is_empty());
    ...
    let resumed = result_value(
        runtime
            ...
            .await
            .expect("workflow resume should work"),
    );
    assert_eq!(resumed["run"]["state"], "running");
    assert_eq!(resumed["run"]["phaseRuns"][0]["state"], "completed");
    assert_eq!(
        resumed["run"]["phaseRuns"][1]["agentRunIds"]
            .as_array()
            .expect("second phase ids")
            .len(),
        1
