...
fn stream_events_merge_sources_tools_statuses_and_approvals() {
    ...
    let run = apply_stream_event_to_run(
        ...
        ApiStreamEvent {
            ...
            pending_approval: Some(Value::Null),
        },
    );
    ...
    assert_eq!(run.pending_approval, None);
}
...
fn new_mcp_approval_matches_go_tui_metadata_shape() {
    ...
    assert!(new_mcp_approval(Some(&approval), Some(&changed_arguments)).is_some());
    ...
    assert!(new_mcp_approval(Some(&malformed), Some(&malformed)).is_some());
    ...
    assert!(new_mcp_approval(Some(&missing_tool), Some(&missing_tool)).is_some());

    let mut first_identified = approval.clone();
    first_identified["approvalId"] = json!("approval-1");
    let mut second_identified = approval.clone();
    second_identified["approvalId"] = json!("approval-2");
    assert!(new_mcp_approval(Some(&first_identified), Some(&first_identified)).is_none());
    assert!(new_mcp_approval(Some(&first_identified), Some(&second_identified)).is_some());
