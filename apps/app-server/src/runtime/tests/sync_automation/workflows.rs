...
use taskforceai_app_protocol::{
    WorkflowIDParams, WorkflowPhaseRunRecord, WorkflowRunRecord, WorkflowRunState,
};
...
async fn workflows_round_trip_through_local_metadata() {
    ...
    for child_run_id in cancelled["run"]["agentRunIds"]
        .as_array()
        .expect("agent run ids")
    {
        assert_eq!(
            restarted
                .get_run(child_run_id.as_str().expect("agent run id"))
                .expect("child run should remain inspectable")
                .status,
            RunStatus::Canceled
        );
    }
...
async fn schedule_methods_reject_invalid_inputs_and_skip_disabled_ticks() {
    ...
    let malformed = runtime
        .schedule_add(ScheduleAddParams {
            name: "bad cadence".to_string(),
            cadence: "not-a-cadence".to_string(),
            prompt: "summarize".to_string(),
            target_session_id: None,
            enabled: true,
        })
        .expect_err("malformed cadence should fail at creation");
    assert_eq!(malformed.code, -32602);
    assert!(
        result_value(runtime.schedule_list().expect("schedule list should work"))["schedules"]
            .as_array()
...
fn workflow_run_state_updates_cover_failed_cancelled_running_and_missing_links() {
    ...
    assert_eq!(updated[1].state, WorkflowRunState::Cancelled);
    assert_eq!(
        updated[1].phase_runs[0].error.as_deref(),
        Some("workflow child run is missing")
    );
