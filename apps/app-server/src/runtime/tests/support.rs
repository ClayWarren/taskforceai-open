...
pub(super) struct MockStatusHttpResponse {
...
pub(super) fn start_status_response_sequence_server(
    responses: Vec<MockStatusHttpResponse>,
) -> (
    String,
    thread::JoinHandle<()>,
    Arc<Mutex<Vec<RecordedHttpRequest>>>,
    ...
    let handle = thread::spawn(move || {
        for mock_response in responses {
            ...
            recorded_requests
                ...
                .expect("recorded requests lock should not poison")
                .push(request);
            let reason = match mock_response.status {
                200 => "OK",
                403 => "Forbidden",
                _ => "Test Response",
            };
