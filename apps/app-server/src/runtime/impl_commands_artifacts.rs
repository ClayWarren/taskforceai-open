...
impl super::AppRuntime {
    ...
    async fn handle_artifact_list_command(&mut self) -> Result<CommandExecuteResult, RuntimeError> {
        ...
        Ok(CommandExecuteResult {
            handled: true,
            title: "Artifacts".to_string(),
            message: format_artifacts(&artifacts),
        })
    }
    ...
    async fn handle_artifact_detail_command(
        ...
        Ok(CommandExecuteResult {
            ...
            message: format_artifact_detail(&artifact, &versions),
        })
    }
    ...
    async fn handle_artifact_delete_command(
        ...
        Ok(CommandExecuteResult {
            handled: true,
            title: "Artifact Deleted".to_string(),
            message: format!("Deleted artifact {artifact_id}."),
        })
    }
    ...
    async fn handle_artifact_download_command(
        ...
        let Some(file_id) = version.file_id.as_deref().filter(|value| !value.is_empty()) else {
            return Ok(CommandExecuteResult {
                handled: false,
                title: "Artifact Download".to_string(),
                message: "Artifact has no downloadable file version.".to_string(),
            });
        };
        let bytes = self
            .api_client
        ...
        Ok(CommandExecuteResult {
            handled: true,
            title: "Artifact Downloaded".to_string(),
            message: format!("Saved {} bytes to {}.", bytes.len(), path.display()),
        })
    }
