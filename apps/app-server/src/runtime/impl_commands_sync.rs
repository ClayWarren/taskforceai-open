...
impl super::AppRuntime {
    pub(crate) async fn handle_sync_command(
        ...
        match action.as_str() {
            "status" => Ok(command_message(
                "Sync",
                format_sync_status(&self.sync_status_result()?),
            )),
            "ensure" | "device" => {
                ...
                Ok(command_message("Sync", format!("Device ID: {device_id}")))
            }
            "pull" => {
            ...
            _ => Ok(command_unhandled(
                "Sync",
                "Usage: /sync [status|ensure|pull|push|poll]",
            )),
        }
    }
}
