...
impl super::AppRuntime {
    pub(crate) async fn handle_settings_command(
        ...
        match area.as_str() {
            "summary" | "local" => Ok(CommandExecuteResult {
                handled: true,
                title: "Settings".to_string(),
                message: format_local_settings(&self.local_settings()?),
            }),
            ...
            _ => Ok(CommandExecuteResult {
                handled: false,
                title: "Settings".to_string(),
                message: "Usage: /settings [theme|telemetry|logging|model|mcp]".to_string(),
            }),
        }
    }
    ...
    pub(crate) fn handle_theme_settings(
        ...
        if args.is_empty() {
            return Ok(CommandExecuteResult {
                handled: true,
                title: "Settings".to_string(),
                message: format!("Theme: {}", self.local_settings()?.theme),
            });
        }
        ...
        let AppResponse::Value(value) = settings else {
            unreachable!("local_settings_update returns a value response");
        };
    ...
    pub(crate) fn handle_telemetry_settings(
        ...
        let update = match action.as_str() {
            ...
            _ => {
                return Ok(CommandExecuteResult {
                    handled: false,
                    title: "Settings".to_string(),
                    message: "Usage: /settings telemetry <on|off|dsn|env>".to_string(),
                })
            }
        };
        ...
        Ok(CommandExecuteResult {
            handled: true,
            title: "Settings".to_string(),
            message: "Telemetry settings updated.".to_string(),
        })
    }
}
