...
pub(super) async fn subscribe_http_events(
    ...
    broadcast::Receiver<SequencedHttpEvent>,
    VecDeque<OutgoingMessage>,
) {
    let (receiver, backlog, _) = subscribe_http_events_with_cursor(state).await;
    (receiver, backlog)
}

async fn subscribe_http_events_with_cursor(
    state: &HttpServerState,
) -> (
    broadcast::Receiver<SequencedHttpEvent>,
    VecDeque<OutgoingMessage>,
    ...
    (receiver, backlog.clone(), cursor)
}

pub(super) fn request_event_cursor(request: &HttpRequest) -> Option<u64> {
    request
        .headers
        .get("last-event-id")
        .or_else(|| request.headers.get("x-taskforce-event-cursor"))
...
pub(super) async fn handle_event_stream(
    ...
    let session_id = request_session_token(&request)
        .expect("authorized HTTP event request should include a session token")
        .to_string();
    ...
    let (mut receiver, backlog, cursor) = subscribe_http_events_with_cursor(&state).await;
    let oldest_cursor = cursor.saturating_sub(backlog.len() as u64);
    for (index, message) in backlog.into_iter().enumerate() {
        let event_id = oldest_cursor + index as u64 + 1;
        if after.is_some_and(|after| event_id <= after) {
            continue;
        }
        let allowed = {
            let guard = connection.lock().await;
            message_allowed_for_connection(&message, &guard)
        };
        if allowed {
            write_event_line(&mut stream, event_id, &message).await?;
        }
    }
    ...
    loop {
        match receiver.recv().await {
            Ok(event) => {
                let allowed = {
                    let guard = connection.lock().await;
                    message_allowed_for_connection(&event.message, &guard)
                };
                if allowed {
                    write_event_line(&mut stream, event.id, &event.message).await?;
                }
            }
...
pub(super) fn message_allowed_for_connection(
    message: &OutgoingMessage,
...
pub(super) async fn write_event_line(
    ...
    let mut value = serde_json::to_value(message).expect("event message should serialize");
    if let Some(object) = value.as_object_mut() {
        object.insert("eventId".to_string(), json!(event_id));
    }
    let mut encoded = serde_json::to_vec(&value).expect("event message should serialize");
    encoded.push(b'\n');
