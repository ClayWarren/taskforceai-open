...
use crate::{
    protocol::{ConversationRecord, MessageRecord},
...
impl SqliteRunStore {
    pub fn list_conversations(
        &self,
        limit: usize,
    ) -> Result<Vec<ConversationRecord>, RuntimeError> {
    ...
    pub fn list_messages(&self, conversation_id: &str) -> Result<Vec<MessageRecord>, RuntimeError> {
        let connection = self.open()?;
        let mut statement = connection
            .prepare(
    ...
    pub fn get_conversation(
        &self,
        conversation_id: &str,
    ) -> Result<Option<ConversationRecord>, RuntimeError> {
        let connection = self.open()?;
        let mut statement = connection
            .prepare(
                "select rowid, conversation_id, title, created_at, updated_at, last_message_preview, \
    ...
    pub fn delete_conversation(&self, conversation_id: &str) -> Result<(), RuntimeError> {
    ...
    pub fn delete_all_conversations(&self) -> Result<(), RuntimeError> {
        let mut connection = self.open()?;
        let transaction = connection.transaction().map_err(storage_error)?;
        transaction
    ...
    pub fn get_message(&self, message_id: &str) -> Result<Option<MessageRecord>, RuntimeError> {
        let connection = self.open()?;
        let mut statement = connection
            .prepare(
                "select rowid, message_id, conversation_id, role, content, is_streaming, is_agent_status, is_local_command_output, \
    ...
    pub fn delete_message(&self, message_id: &str) -> Result<(), RuntimeError> {
        let connection = self.open()?;
        connection
            .execute(
    ...
    pub fn upsert_message(&self, message: &MessageRecord) -> Result<(), RuntimeError> {
        let connection = self.open()?;
        let sources = serde_json::to_string(&message.sources).map_err(storage_error)?;
        let tool_events = serde_json::to_string(&message.tool_events).map_err(storage_error)?;
        let agent_statuses =
            serde_json::to_string(&message.agent_statuses).map_err(storage_error)?;
        connection
            .execute(
