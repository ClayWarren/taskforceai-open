...
use crate::protocol::OllamaPullEventRecord;

pub const DEFAULT_OLLAMA_BASE_URL: &str = "http://localhost:11434/v1";
pub const DEFAULT_OLLAMA_MODEL: &str = "gemma4:e2b";
pub const OLLAMA_PROVIDER_ID: &str = "ollama";
const MIN_RESPONSES_VERSION: Version = Version::new(0, 13, 4);
const OLLAMA_REQUEST_TIMEOUT: Duration = Duration::from_secs(10);
#[cfg(not(test))]
const OLLAMA_PULL_IDLE_TIMEOUT: Duration = Duration::from_secs(5 * 60);
#[cfg(test)]
const OLLAMA_PULL_IDLE_TIMEOUT: Duration = Duration::from_millis(50);
...
pub type OllamaPullEvent = OllamaPullEventRecord;
...
impl OllamaClient {
    ...
    async fn pull_model(&self, model: &str) -> Result<Vec<OllamaPullEvent>, OllamaError> {
        ...
        .await
        .map_err(|_| OllamaError::Pull("pull timed out waiting for response".to_string()))??;
        if !response.status().is_success() {
            return Err(OllamaError::Status {
                status: response.status().as_u16(),
            });
        ...
        loop {
            let Some(chunk) = tokio::time::timeout(OLLAMA_PULL_IDLE_TIMEOUT, stream.next())
                .await
                .map_err(|_| {
                    OllamaError::Pull("pull timed out waiting for progress".to_string())
                })?
            else {
                break;
            };
...
mod tests {
    ...
    #[tokio::test]
    async fn ensure_ready_reports_pull_progress_timeout() {
        let base_url = spawn_ollama_stalled_pull_server();
        let client = OllamaClient::new(format!("{base_url}/v1"));

        let err = client
            .ensure_ready(&format!("{base_url}/v1"), Some("missing:model"))
            .await
            .expect_err("stalled pull stream should time out");

        assert!(err
            .to_string()
            .contains("pull timed out waiting for progress"));
    }
...
"#
    ...
    fn spawn_ollama_stalled_pull_server() -> String {
        let listener = TcpListener::bind("127.0.0.1:0").expect("bind mock stalled ollama");
        let addr = listener.local_addr().expect("mock stalled address");
        thread::spawn(move || {
            for _ in 0..4 {
                let Ok((mut stream, _)) = listener.accept() else {
                    continue;
                ...
                if path == "/api/pull" {
                    stream
                        ...
                        .expect("write stalled pull headers");
                    stream.flush().expect("flush stalled pull headers");
                ...
                stream
                    .write_all(response.as_bytes())
                    .expect("write mock stalled response");
            }
        });
        format!("http://{addr}")
    }
