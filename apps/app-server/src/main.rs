...
use taskforceai_app_server::{protocol_schema, run_http, run_stdio, HttpServerConfig};
...
fn main() {
    ...
    if matches!(
        ...
        Some("generate-json-schema" | "--schema")
    ) {
        print!("{}", protocol_schema());
        return;
    }
...
mod tests {
    ...
    #[test]
    fn embedded_schema_describes_the_running_protocol() {
        let schema: serde_json::Value =
            serde_json::from_str(protocol_schema()).expect("embedded protocol schema");
        let methods = schema["$defs"]["AppServerRpcMethod"]["enum"]
            .as_array()
            .expect("method catalog");
        assert!(methods.iter().any(|method| method == "server/describe"));
        assert!(methods.iter().any(|method| method == "serverRequest/list"));
        assert!(methods.iter().any(|method| method == "fs/readDirectory"));
    }
}
