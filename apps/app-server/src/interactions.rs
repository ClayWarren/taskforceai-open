...
use taskforceai_app_protocol::{
    ...
    PendingServerRequest, ServerRequestPayload, ServerRequestResolvedParams, JSONRPC_VERSION,
};
use thiserror::Error;
...
impl InteractionBroker {
    ...
    pub(crate) async fn pending_requests(
        ...
        let mut requests = pending
            ...
            .filter(|(_, interaction)| {
                thread_id.is_none_or(|thread_id| interaction.thread_id == thread_id)
            })
            .map(|(request_id, interaction)| {
                (
                    *request_id,
                    PendingServerRequest {
                        request: interaction.request.clone(),
                        thread_id: interaction.thread_id.clone(),
    ...
    pub(crate) async fn replay_messages(&self, thread_id: &str) -> Vec<OutgoingMessage> {
        self.pending_requests(Some(thread_id))
            .await
            .into_iter()
            .map(|pending| OutgoingMessage::Request(pending.request))
            .collect()
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn pending_requests_are_ordered_filterable_and_replayable() {
        let (output, mut messages) = mpsc::channel(8);
        let broker = InteractionBroker::new(output);
        let first = {
            let broker = broker.clone();
        ...
        let second = {
            ...
            tokio::spawn(async move {
                broker
                    .request(tool_request("thread-2", "turn-2"), Duration::from_secs(5))
                    .await
            })
        };
        let _ = messages.recv().await;
        ...
        let listed = broker.pending_requests(Some("thread-1")).await;
        assert_eq!(listed.len(), 1);
        assert_eq!(listed[0].thread_id, "thread-1");
        assert_eq!(listed[0].request.params["turnId"], "turn-1");
        let replay = broker.replay_messages("thread-1").await;
        assert!(matches!(replay.as_slice(), [OutgoingMessage::Request(_)]));
