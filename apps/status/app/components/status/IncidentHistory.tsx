...
function isIncidentResolved(incident: Incident): boolean {
  ...
  const resolvedAt = parseStatusDate(incident.resolvedAt);
  return resolvedAt !== null && resolvedAt.getTime() <= Date.now();
}
                ...
                return (
                  <div key={incident.id} className="ml-2">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <Badge
                        variant="outline"
                        className={getStatusColor(resolved ? 'operational' : incident.status)}
                      >
                        {resolved ? 'RESOLVED' : incident.status.toUpperCase()}
                      </Badge>
                    </div>
