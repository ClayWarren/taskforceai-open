...
func TestResetFallbackRunRateLimiter(t *testing.T) {
	fallbackRunLimiter.allow("used", 1, time.Hour)
	ResetFallbackRunRateLimiter()
	assert.True(t, fallbackRunLimiter.allow("used", 1, time.Hour))
}

func TestEnforceRunRateLimit_CombinedUserExceeded(t *testing.T) {
	t.Run("fallback", func(t *testing.T) {
		swap(t, &runp.RedisClientGetter, func() (redispkg.Cmdable, error) { return nil, errors.New("offline") })
		swap(t, &fallbackRunLimiter, newInMemoryWindowCounter())
		t.Setenv("NODE_ENV", "development")
		for range 10 {
			require.True(t, fallbackRunLimiter.allow("user:combined-fallback@example.com", 10, time.Hour))
		}
		err := enforceRunRateLimit(context.Background(), "combined-fallback@example.com", 1, 42)
		require.ErrorContains(t, err, "User rate limit exceeded")
	})

	t.Run("redis", func(t *testing.T) {
		client := redispkg.NewMockClient()
		for range 10 {
			allowed, _, _, err := client.CheckRateLimit(context.Background(), "rl:run:u:combined-redis@example.com", 10, time.Hour)
			require.NoError(t, err)
			require.True(t, allowed)
		}
		swap(t, &runp.RedisClientGetter, func() (redispkg.Cmdable, error) { return client, nil })
		err := enforceRunRateLimit(context.Background(), "combined-redis@example.com", 1, 42)
		require.ErrorContains(t, err, "User rate limit exceeded")
	})
}
