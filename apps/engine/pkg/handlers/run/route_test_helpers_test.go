...
func postAttachment(t *testing.T, router http.Handler, filename string, content []byte) *httptest.ResponseRecorder {
	t.Helper()
	return postAttachmentReader(t, router, filename, bytes.NewReader(content))
}

func postAttachmentReader(t *testing.T, router http.Handler, filename string, content io.Reader) *httptest.ResponseRecorder {
	t.Helper()
	body := &bytes.Buffer{}
...
func postTaskSteer(router http.Handler, taskID, body string) *httptest.ResponseRecorder {
	req := httptest.NewRequest(http.MethodPost, "/api/v1/tasks/"+taskID+"/steer", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	return serve(router, req)
}
