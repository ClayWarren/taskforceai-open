...
func enforceTaskRateLimit(ctx context.Context, input taskRateLimitInput) error {
	...
	if err != nil {
		...
		if input.OrgID != 0 {
			if fallbackErr := checkFallback(input.FallbackOrgPrefix+strconv.Itoa(input.OrgID), orgLimit, orgWindow, input.OrgErrorMessage); fallbackErr != nil {
				return fallbackErr
			}
		}
	...
	if err := checkOrgTaskRateLimit(limitCtx, limiter, input, orgLimit, orgWindow, checkFallback); err != nil {
		return err
	}
...
func checkOrgTaskRateLimit(
	...
	checkFallback func(string, int, time.Duration, string) error,
) error {
	if input.OrgID == 0 {
		return nil
	}
	...
	if orgErr != nil {
		...
		return checkFallback(input.FallbackOrgPrefix+strconv.Itoa(input.OrgID), orgLimit, window, input.OrgErrorMessage)
	}
