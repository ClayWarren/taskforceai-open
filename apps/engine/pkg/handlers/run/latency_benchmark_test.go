...
func BenchmarkRunRouteLatencyProfile(b *testing.B) {
	...
	user := &auth.AuthenticatedUser{ID: 44, Email: "run-benchmark@example.com"}
...
func reportRunRouteLatencyProfile(b *testing.B, samples []time.Duration) {
	...
	b.ReportMetric(runRouteDurationMicroseconds(runRoutePercentileDuration(ordered, 0.50)), "p50_us")
	b.ReportMetric(runRouteDurationMicroseconds(runRoutePercentileDuration(ordered, 0.95)), "p95_us")
	b.ReportMetric(runRouteDurationMicroseconds(runRoutePercentileDuration(ordered, 0.99)), "p99_us")
}

func runRoutePercentileDuration(ordered []time.Duration, percentile float64) time.Duration {
	if len(ordered) == 0 {
		return 0
	}
