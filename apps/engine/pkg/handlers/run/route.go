...
func registerActiveTasksHandler(api huma.API) {
	huma.Register(api, handlercommon.Operation("Tasks", "list-active-tasks", http.MethodGet, "/api/v1/tasks/active", "List active tasks for the current user"), func(ctx context.Context, input *struct {
...
func registerAttachmentUploadHandler(api huma.API) {
	huma.Register(api, handlercommon.Operation("Tasks", "upload-attachment", http.MethodPost, "/api/v1/attachments/upload", "Upload a binary attachment"), func(ctx context.Context, input *struct {
...
func registerRunTaskHandler(api huma.API, q RunQueries, inngest run.InngestSender) {
	huma.Register(api, handlercommon.Operation("Tasks", "run-task", http.MethodPost, "/api/v1/run", "Run a task"), func(ctx context.Context, input *struct {
		Body      RunRequest
		...
		// 1. Rate Limiting
		if rateLimitErr := enforceRunRateLimit(ctx, input.User.Email, input.User.ID, input.OrgID, taskruntime.ResolveRateLimitPlan(input.User.Plan)); rateLimitErr != nil {
			return nil, rateLimitErr
		}
		...
		if ids.OrgID32 != nil {
			...
			if err != nil {
				return nil, huma.Error500InternalServerError("Failed to load organization policy")
			}
...
func registerPulseHandler(api huma.API, q RunQueries, inngest run.InngestSender) {
	huma.Register(api, handlercommon.Operation("Tasks", "run-pulse", http.MethodPost, "/api/v1/run/pulse", "Trigger a pulse turn for an agent"), func(ctx context.Context, input *struct {
		Body PulseRequest
		handler.AuthContext
		...
		if rateLimitErr := enforcePulseRateLimit(ctx, input.User.Email, input.User.ID, input.OrgID, taskruntime.ResolveRateLimitPlan(input.User.Plan)); rateLimitErr != nil {
			return nil, rateLimitErr
		}
...
func registerTraceHandler(api huma.API, q RunQueries) {
	huma.Register(api, handlercommon.Operation("Tasks", "get-execution-trace", http.MethodGet, "/api/v1/tasks/{taskId}/trace", "Get execution trace for a task"), func(ctx context.Context, input *struct {
...
func registerApproveHandler(api huma.API) {
	huma.Register(api, handlercommon.Operation("Tasks", "approve-task-action", http.MethodPost, "/api/v1/tasks/{taskId}/approve", "Approve or deny a pending task action"), func(ctx context.Context, input *struct {
		TaskId string `path:"taskId"`
...
func registerSteerHandler(api huma.API) {
	huma.Register(api, handlercommon.Operation("Tasks", "steer-task", http.MethodPost, "/api/v1/tasks/{taskId}/steer", "Add guidance to a running task"), func(ctx context.Context, input *struct {
		TaskId string `path:"taskId"`
		Body   SteerTaskRequest
		handler.SessionAuthContext
	}) (*struct{ Body string }, error) {
		task := run.GetRegistry().Get(input.TaskId)
		if task == nil {
		...
		if task.Status != run.StatusProcessing && task.Status != run.StatusAwaiting {
			return nil, huma.Error409Conflict("Task is not active")
		}
		if err := run.SendTaskSteering(ctx, input.TaskId, input.Body.Input); err != nil {
			if errors.Is(err, run.ErrTaskSteeringEmpty) || errors.Is(err, run.ErrTaskSteeringTooLarge) {
				return nil, huma.Error400BadRequest(err.Error())
			}
			if errors.Is(err, run.ErrTaskSteeringQuota) {
				return nil, huma.Error429TooManyRequests(err.Error())
			}
			return nil, huma.Error500InternalServerError("Failed to steer task")
		}
		return &struct{ Body string }{Body: "Steering accepted"}, nil
	})
}

func registerCancelHandler(api huma.API) {
	huma.Register(api, handlercommon.Operation("Tasks", "cancel-task", http.MethodPost, "/api/v1/tasks/{taskId}/cancel", "Cancel a running task"), func(ctx context.Context, input *struct {
		TaskId string `path:"taskId"`
		handler.AuthContext
	}) (*struct{ Body *RunResponse }, error) {
		task := run.GetRegistry().Get(input.TaskId)
