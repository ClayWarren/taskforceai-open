...
func TestRunThread_Success(t *testing.T) {
	...
	reg.On("Register", mock.Anything, 3, "hi", "zai/glm-5.2", mock.MatchedBy(func(opts runp.OrchestrateTaskOptions) bool {
		return opts.ConversationID != nil && *opts.ConversationID == 5 && strings.Contains(opts.ThreadContext, "assistant: prior answer")
	})).Return(nil)
	...
	q := new(mockDeveloperQueries)
	q.On("GetMessagesByConversation", mock.Anything, int32(5)).Return([]ThreadMessage{{Role: "assistant", Content: "prior answer"}}, nil)
	...
	assert.Equal(t, "5", response["threadId"])
}
...
func TestRunThread_MessageAndSubmissionErrors(t *testing.T) {
	conv := fakeConversationService{getFn: func(context.Context, string, *int, int) (*conversations.ConversationApiView, error) {
		return &conversations.ConversationApiView{ID: 5}, nil
	}}
	user := &auth.AuthenticatedUser{ID: 3, Email: "thread-errors@example.com"}

	t.Run("message query", func(t *testing.T) {
		q := new(mockDeveloperQueries)
		q.On("GetMessagesByConversation", mock.Anything, int32(5)).Return(nil, errors.New("messages failed"))
		router := setupDeveloperRouter(q, conv, new(mockInngest), user, 0)
		req := httptest.NewRequest(http.MethodPost, "/api/v1/developer/threads/5/runs", strings.NewReader(`{"prompt":"hi"}`))
		req.Header.Set("Content-Type", "application/json")
		resp := httptest.NewRecorder()
		router.ServeHTTP(resp, req)
		assert.Equal(t, http.StatusInternalServerError, resp.Code)
	...
	t.Run("submission", func(t *testing.T) {
		reg := new(mockTaskRegistry)
		reg.On("Register", mock.Anything, 3, "hi", "gpt", mock.Anything).Return(errors.New("register failed"))
		origReg := registryGetter
		registryGetter = func() TaskRegistry { return reg }
		t.Cleanup(func() { registryGetter = origReg })
		q := new(mockDeveloperQueries)
		q.On("GetMessagesByConversation", mock.Anything, int32(5)).Return([]ThreadMessage{}, nil)
		router := setupDeveloperRouter(q, conv, new(mockInngest), user, 0)
		req := httptest.NewRequest(http.MethodPost, "/api/v1/developer/threads/5/runs", strings.NewReader(`{"prompt":"hi","modelId":"gpt"}`))
		req.Header.Set("Content-Type", "application/json")
		resp := httptest.NewRecorder()
		router.ServeHTTP(resp, req)
		assert.Equal(t, http.StatusInternalServerError, resp.Code)
...
func TestBuildThreadContextTruncatesOversizedTurn(t *testing.T) {
	got := buildThreadContext(&conversations.ConversationApiView{UserInput: strings.Repeat("x", 70*1024)}, nil)
