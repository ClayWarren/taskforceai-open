...
func TestDeleteFile_RemovesLocalGeneratedBlobWithoutBlobToken(t *testing.T) {
	...
	assert.Equal(t, http.StatusOK, resp.Code)
	assert.Equal(t, int64(15), released)
	localGeneratedBlobs.RLock()
...
func TestFileContentHeaderHelpers(t *testing.T) {
	...
	assert.Equal(t, `attachment; filename=weird_name_.txt`, fileContentDisposition(" attachment ", "weird name?.txt"))
	assert.Equal(t, "DENY", fileFrameOptions("attachment"))
	assert.False(t, isInlineDisposition(" attachment "))
	assert.False(t, isActiveFileContent("application/octet-stream"))
	assert.False(t, isActiveFileContent("bad mime ;"))
	assert.Contains(t, fileContentSecurityPolicy("text/plain", "inline"), "img-src")
	assert.NotContains(t, fileContentSecurityPolicy("text/plain", "inline"), "sandbox")
	assert.Contains(t, fileContentSecurityPolicy("text/html", "attachment"), "frame-ancestors 'none'")
}
