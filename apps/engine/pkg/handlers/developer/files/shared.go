...
func releaseExpiredUploadReservations(ctx context.Context, q FilesQueries, userID int32, logUserID int) {
	expiredReservations, err := q.ReleaseExpiredDeveloperFileUploadReservationsForUser(ctx, userID)
	if err != nil {
		slog.Error("Failed to release expired upload reservations", "userId", logUserID, "error", err)
		return
	}
	for _, reservedBytes := range expiredReservations {
		releaseUserStorage(ctx, q, userID, logUserID, reservedBytes)
	}
}
