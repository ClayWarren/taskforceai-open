	...
	huma.Register(api, huma.Operation{
		...
		if blob, ok := loadLocalGeneratedBlob(fileRow.BlobURL, userID); ok {
			...
			return &fileContentResponse{
				...
				ContentDisposition: fileContentDisposition(input.Disposition, blob.Filename),
				ContentSecurity:    fileContentSecurityPolicy(contentType, input.Disposition),
				ContentTypeOptions: "nosniff",
				FrameOptions:       fileFrameOptions(input.Disposition),
				Body:               blob.Content,
			}, nil
		}
		...
		return &fileContentResponse{
			...
			ContentDisposition: fileContentDisposition(input.Disposition, fileRow.Filename),
			ContentSecurity:    fileContentSecurityPolicy(contentType, input.Disposition),
			ContentTypeOptions: "nosniff",
			FrameOptions:       fileFrameOptions(input.Disposition),
			Body:               content,
...
func fileContentDisposition(disposition string, filename string) string {
	mode := "attachment"
...
func fileContentSecurityPolicy(contentType string, disposition string) string {
	if !isInlineDisposition(disposition) {
		return "default-src 'none'; frame-ancestors 'none'; base-uri 'none'; form-action 'none'"
	}
	if isActiveFileContent(contentType) {
...
func fileFrameOptions(disposition string) string {
	if isInlineDisposition(disposition) {
		return "SAMEORIGIN"
	}
	return "DENY"
...
func isActiveFileContent(contentType string) bool {
