...
// ResolveRateLimitPlan preserves the legacy limiter only for unhydrated users.
// Authenticated production users always carry their persisted subscription plan.
func ResolveRateLimitPlan(userPlan *string) string {
	if userPlan == nil {
		return ""
	}
	return *userPlan
...
func BuildOrchestrateTaskOptions(params BuildOptionsParams) run.OrchestrateTaskOptions {
	...
	return run.OrchestrateTaskOptions{
		...
		Mode:                taskcontract.NormalizeTaskMode(ReadStringOption(params.Options, "mode")),
