...
func (i *FilesystemTeamInbox) getFilePath(teamName, agentName string) (string, error) {
	if err := coreteam.ValidateInboxName(teamName, "team"); err != nil {
		return "", err
	}
	if err := coreteam.ValidateInboxName(agentName, "agent"); err != nil {
		return "", err
	}
...
func (i *FilesystemTeamInbox) Write(teamName, to string, msg agent.InboxMessage) error {
	...
	if lineCount >= coreteam.MaxInboxMessages {
		return fmt.Errorf("inbox for %q in team %q has reached the maximum of %d messages", to, teamName, coreteam.MaxInboxMessages)
	}
...
var _ coreteam.InboxStore = (*FilesystemTeamInbox)(nil)
