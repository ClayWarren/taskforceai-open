...
func TestGetApprovalClient_ReturnsClientWithRedisURL(t *testing.T) {
	...
	if redisClient, ok := client.(redisApprovalClient); ok {
		t.Cleanup(func() {
			assert.NoError(t, redisClient.client.Close())
		})
	}
}
...
func TestPersistApprovalDecisionStoresPayload(t *testing.T) {
	...
	require.NoError(t, persistApprovalDecision(ctx, taskID, ApprovalDecision{Approved: true}))
...
func TestPersistApprovalDecisionRedisUnavailable(t *testing.T) {
	...
	err := persistApprovalDecision(context.Background(), "task-persist-fail", ApprovalDecision{Approved: true})
	assert.Error(t, err)
}
...
func TestPersistApprovalDecisionNilRedisClient(t *testing.T) {
	...
	err := persistApprovalDecision(context.Background(), "task-persist-nil", ApprovalDecision{Approved: true})
...
func TestPersistApprovalDecisionSetFailure(t *testing.T) {
	...
	err := persistApprovalDecision(context.Background(), "task-set-fail", ApprovalDecision{Approved: true})
	assert.Error(t, err)
}
