...
func supportsRedisEval(client redis.Cmdable) bool {
	evalSupport, ok := client.(interface{ SupportsEval() bool })
	return ok && evalSupport.SupportsEval()
}
...
func isStreamUnavailableError(err error) bool {
	...
	message := err.Error()
	return strings.Contains(message, "stream operations require REDIS_URL")
}
