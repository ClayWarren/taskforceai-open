...
var (
	enginecoreCSVWriterMu        sync.Mutex
	enginecoreCSVWriterInstalled bool
...
func installEnginecoreCSVWriter() {
	enginecoreCSVWriterMu.Lock()
	defer enginecoreCSVWriterMu.Unlock()
	if enginecoreCSVWriterInstalled {
		return
	}
	enginecoretools.SetCSVWriter(enginecoreFileCSVWriter{})
	enginecoreCSVWriterInstalled = true
}
