...
var (
	enginecoreDocumentWriterMu        sync.Mutex
	enginecoreDocumentWriterInstalled bool

	saveEnginecoreDocumentToFile = func(doc *document.Document, path string) error {
		return doc.SaveToFile(path)
	}
)

func installEnginecoreDocumentWriter() {
	enginecoreDocumentWriterMu.Lock()
	defer enginecoreDocumentWriterMu.Unlock()
	if enginecoreDocumentWriterInstalled {
		return
	}
	enginecoretools.SetDocumentWriter(enginecoreFileDocumentWriter{})
	enginecoreDocumentWriterInstalled = true
}
