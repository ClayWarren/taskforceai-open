...
func (r *orchestrateTaskRunner) setTaskStatus(ctx context.Context, status TaskStatus, result, errStr string) {
	updateCtx := context.WithoutCancel(ctx)
	if updateErr := updateTaskStatusWithLockRetry(updateCtx, r.registry, r.taskID, status, result, errStr); updateErr != nil {
...
func (r *orchestrateTaskRunner) prepare(ctx context.Context) (*orchestrationPreparation, bool) {
	...
	cacheInstance, requiresCurrentData, ok := r.prepareCache(ctx, hasAttachments)
	if !ok {
		return nil, false
	}
...
func (r *orchestrateTaskRunner) prepareCache(ctx context.Context, hasAttachments bool) (corecache.ICache, bool, bool) {
	if hasAttachments {
	...
	if result != "" {
		r.setTaskStatus(ctx, StatusCompleted, result, "")
