...
func TestRecordTaskUsageHandlesNilAndRecorderErrors(t *testing.T) {
	...
	recordTaskUsage(context.Background(), nil, "task-1", &conversationID, 7, "model", "prompt", OrchestrateTaskOptions{}, &orchestrator.OrchestrationTrace{})
	recordTaskUsage(context.Background(), &stubTaskPersistenceStore{}, "task-1", &conversationID, 7, "model", "prompt", OrchestrateTaskOptions{}, nil)
	...
	recordTaskUsage(context.Background(), store, "task-1", &conversationID, 7, "model", "prompt", OrchestrateTaskOptions{}, &orchestrator.OrchestrationTrace{
...
func TestPersistFinalTaskAppendsToExistingConversation(t *testing.T) {
	restore(t, &RunTaskPersistenceTx)
	conversationID := int32(55)
	createConversationCalled := false
	messages := make([]taskMessageCreateInput, 0, 2)
	RunTaskPersistenceTx = func(ctx context.Context, fn func(store taskPersistenceStore) error) error {
		return fn(&stubTaskPersistenceStore{
			...
			createMessageFunc: func(_ context.Context, input taskMessageCreateInput) error {
	...
	gotID, err := persistFinalTask(context.Background(), 7, "new question", "model", "new answer", nil, OrchestrateTaskOptions{ConversationID: &conversationID}, nil, nil, nil)
	require.NoError(t, err)
	assert.Equal(t, conversationID, gotID)
	assert.False(t, createConversationCalled)
	require.Len(t, messages, 2)
	assert.Equal(t, "user", messages[0].Role)
	assert.Equal(t, "new question", messages[0].Content)
	assert.Equal(t, conversationID, messages[0].ConversationID)
	assert.Equal(t, "assistant", messages[1].Role)
	assert.Equal(t, "new answer", messages[1].Content)
}

func TestPersistFinalTaskExistingConversationMessageFailure(t *testing.T) {
	restore(t, &RunTaskPersistenceTx)
	conversationID := int32(55)
	RunTaskPersistenceTx = func(ctx context.Context, fn func(store taskPersistenceStore) error) error {
		return fn(&stubTaskPersistenceStore{createMessageFunc: func(context.Context, taskMessageCreateInput) error {
			return errors.New("message failed")
		}})
	}

	_, err := persistFinalTask(context.Background(), 7, "question", "model", "answer", nil, OrchestrateTaskOptions{ConversationID: &conversationID}, nil, nil, nil)
	require.ErrorContains(t, err, "append user message")
}

type failingUsageEventStore struct{ stubTaskPersistenceStore }

func (*failingUsageEventStore) CreateUsageEvents(context.Context, []sharedusage.EventRow) error {
	return errors.New("media usage failed")
}

func TestRecordTaskUsageHandlesMediaEventError(t *testing.T) {
	recordTaskUsage(context.Background(), &failingUsageEventStore{}, "task-media", nil, 7,
		"google/gemini-2.5-flash-image", "prompt", OrchestrateTaskOptions{}, &orchestrator.OrchestrationTrace{})
}
...
func TestRecordCanceledTaskUsageHandlesMissingTraceAndTransactionFailure(t *testing.T) {
	...
	recordCanceledTaskUsage(context.Background(), "task-canceled", 1, "model", "prompt", OrchestrateTaskOptions{}, nil)
	assert.Zero(t, transactionCalls)

	recordCanceledTaskUsage(context.Background(), "task-canceled", 1, "model", "prompt", OrchestrateTaskOptions{}, &orchestrator.OrchestrationTrace{})
