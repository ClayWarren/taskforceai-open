...
var (
	enginecoreArchiveWriterMu        sync.Mutex
	enginecoreArchiveWriterInstalled bool
...
func installEnginecoreArchiveWriter() {
	enginecoreArchiveWriterMu.Lock()
	defer enginecoreArchiveWriterMu.Unlock()
	if enginecoreArchiveWriterInstalled {
		return
	}
	enginecoretools.SetArchiveWriter(enginecoreFileArchiveWriter{})
	enginecoreArchiveWriterInstalled = true
}
