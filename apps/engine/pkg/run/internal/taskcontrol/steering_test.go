...
func TestTaskSteeringPersistsAndProviderDrainsInOrder(t *testing.T) {
	...
	if ttl := mr.TTL(taskSteeringStream("task-1")); ttl != taskcontract.TTL {
		t.Fatalf("steering stream TTL = %s, want %s", ttl, taskcontract.TTL)
	}
	provider := NewTaskSteeringProviderWithClient("task-1", RedisClientGetter)
...
func TestTaskSteeringStoreFailures(t *testing.T) {
	t.Run("nil getter uses default", func(t *testing.T) {
		...
		if err := SendTaskSteeringWithClient(context.Background(), "task", "guidance", nil); err == nil {
			t.Fatal("expected default steering store error")
		}
	})
...
func TestTaskSteeringProviderStoreFailures(t *testing.T) {
	t.Run("nil getter uses default", func(t *testing.T) {
		...
		_, err := NewTaskSteeringProviderWithClient("task", nil)(context.Background())
		if err == nil {
			t.Fatal("expected default provider store error")
		}
	})
	t.Run("getter error", func(t *testing.T) {
		setRedisClientGetterForTest(t, func() (redis.Cmdable, error) {
		...
		_, err := NewTaskSteeringProviderWithClient("task", RedisClientGetter)(context.Background())
		if err == nil {
