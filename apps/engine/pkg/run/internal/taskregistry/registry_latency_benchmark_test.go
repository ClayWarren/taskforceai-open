...
func reportNamedLatencyProfile(b *testing.B, name string, samples []time.Duration) {
	...
	b.ReportMetric(float64(benchtest.PercentileDuration(ordered, 0.50).Microseconds()), name+"_p50_us")
	b.ReportMetric(float64(benchtest.PercentileDuration(ordered, 0.95).Microseconds()), name+"_p95_us")
	b.ReportMetric(float64(benchtest.PercentileDuration(ordered, 0.99).Microseconds()), name+"_p99_us")
}
