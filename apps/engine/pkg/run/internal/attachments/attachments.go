...
const AttachmentKeyPrefix = attachmentcontract.CollectionKeyPrefix
const AttachmentMetaKeyPrefix = attachmentcontract.BlobKeyPrefix

func Fetch(ctx context.Context, taskID string, deps Dependencies) (Attachments, error) {
	redisClient, redisErr := deps.RedisClient()
...
func ContentParts(attachments Attachments) []agent.ContentPart {
...
func ContentPart(file FileAttachment) (agent.ContentPart, bool) {
	switch {
