...
func TestDrainTaskSubmissionDeadLetterBranchCoverage(t *testing.T) {
	...
	err = drainTaskSubmissionDLQStreamRecords(ctx, &captureInngest{}, &setErrorRedisClient{MockClient: redis.NewMockClient()}, []goredis.XMessage{{
		ID: "32-0", Values: map[string]any{"payload": "{"},
	}})
