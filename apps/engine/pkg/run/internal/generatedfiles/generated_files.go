...
type ObservationFinisher func(startedAt time.Time, bytes int64, status string, err error)
type ObservationStarter func(context.Context, string, string, string) (context.Context, ObservationFinisher)
...
func defaultObservation(ctx context.Context, _, _, _ string) (context.Context, ObservationFinisher) {
	return ctx, func(time.Time, int64, string, error) {}
}

func Persist(ctx context.Context, input GeneratedFilePersistenceInput, deps Dependencies) ([]agent.ToolEvent, error) {
	if !hasUnpersistedGeneratedFile(input.Events) {
		return input.Events, nil
	}
	if deps.DBQueriesGetter == nil {
		deps.DBQueriesGetter = appdatabase.GetQueries
	}
	if deps.StartObservation == nil {
		deps.StartObservation = defaultObservation
	}
	q, err := deps.DBQueriesGetter(ctx)
	if err != nil {
	...
	for i := range events {
		...
		fileCtx, finishObservation := deps.StartObservation(ctx, toolName, mimeType, artifactType)
		startedAt := time.Now()
		if err := validateGeneratedFileLocalPath(file); err != nil {
			...
			finishObservation(startedAt, 0, "invalid_local_path", err)
			continue
		}
		...
		if err != nil {
			...
			finishObservation(startedAt, 0, "read_failed", err)
			continue
		}
		...
		if err != nil {
			...
			finishObservation(startedAt, bytes, "file_persist_failed", err)
			continue
		}
		...
		if err != nil {
			...
			finishObservation(startedAt, record.Bytes, "artifact_record_failed", err)
		} else {
			file.ArtifactID = artifactID
			finishObservation(startedAt, record.Bytes, "persisted", nil)
		}
