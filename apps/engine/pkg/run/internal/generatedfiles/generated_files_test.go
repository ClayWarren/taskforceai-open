...
func persistGeneratedFileArtifacts(ctx context.Context, input GeneratedFilePersistenceInput) ([]agent.ToolEvent, error) {
	return Persist(ctx, input, Dependencies{DBQueriesGetter: DBQueriesGetter})
}
...
func TestPersistAppliesDefaultDependencies(t *testing.T) {
	events := []agent.ToolEvent{{
		ToolName: "create_chart",
		GeneratedFile: &agent.GeneratedFile{
			Filename:  "chart.png",
	...
	_, err := Persist(context.Background(), GeneratedFilePersistenceInput{Events: events}, Dependencies{})
	require.Error(t, err)
}

func TestPersistGeneratedFileArtifactsUsesConfiguredGetter(t *testing.T) {
	restore(t, &DBQueriesGetter)
	DBQueriesGetter = func(context.Context) (*db.Queries, error) { return nil, errors.New("database unavailable") }
	events := []agent.ToolEvent{{
		ToolName: "create_chart",
		GeneratedFile: &agent.GeneratedFile{
			Filename:  "chart.png",
	...
	_, err := persistGeneratedFileArtifacts(context.Background(), GeneratedFilePersistenceInput{Events: events})
...
func TestCreateGeneratedFileArtifactErrorBranches(t *testing.T) {
	...
	dbMock := dbtest.NewMockPoolRegexp(t)
	q := db.New(dbMock)
	store := generatedFileSQLStore{SQLStore: adapterartifacts.NewSQLStore(q), DeveloperFilesStore: appdatabase.NewDeveloperFilesStore(q)}
	dbMock.ExpectBegin()
...
func TestGeneratedFileSQLStoreStorageAndReservationAdapters(t *testing.T) {
	now := pgtype.Timestamp{Time: time.Now(), Valid: true}
	dbMock := dbtest.NewMockPoolRegexp(t)
	store := generatedFileSQLStore{DeveloperFilesStore: appdatabase.NewDeveloperFilesStore(db.New(dbMock))}
