...
func loadRunUserContext(ctx context.Context, input UserContextLoadInput) (RunUserContext, error) {
	...
	userSettings, err := loadCachedUserSettings(ctx, store, input.UserID)
	if err != nil {
		return RunUserContext{}, fmt.Errorf("load user settings: %w", err)
	}
	applyUserSettings(&userContext, userSettings)
...
func loadCachedUserSettings(ctx context.Context, store userContextStore, userID int32) (userContextUserRow, error) {
	// Privacy and execution controls must reflect the committed database value
	// immediately. A stale distributed cache must never re-enable memory, web,
	// code execution, or trust-layer settings after the user disables them.
	return store.GetUserSettings(ctx, userID)
}
