...
var marshalTaskSubmissionIdempotency = json.Marshal

func handleTaskSubmissionIdempotency(
	ctx context.Context,
	userID int,
	idempotencyKey string,
	taskID string,
	registry SubmissionTaskRegistrar,
) (*TaskSubmissionResult, bool) {
	existingTaskID, reserved, reserveErr := reserveTaskSubmissionIdempotency(ctx, userID, idempotencyKey, taskID)
	if reserveErr != nil {
		slog.Warn("[RunSubmission] Failed to reserve idempotency key; continuing without dedupe", "error", reserveErr, "userId", userID)
		return nil, false
	}
	if reserved {
		return nil, true
	...
	if task := registry.Get(existingTaskID); task != nil {
		return &TaskSubmissionResult{TaskID: existingTaskID, Status: submissionResultStatus(task)}, false
	}
	if taskSubmissionIdempotencyReservationPending(ctx, userID, idempotencyKey, existingTaskID) {
		return &TaskSubmissionResult{TaskID: existingTaskID, Status: StatusProcessing}, false
	}
	...
	if releaseErr := releaseTaskSubmissionIdempotency(ctx, userID, idempotencyKey, existingTaskID); releaseErr != nil {
		slog.Warn("[RunSubmission] Failed to clear stale idempotency key; continuing without dedupe", "error", releaseErr, "userId", userID)
		return nil, false
	}

	recoveredTaskID, recovered, recoverErr := reserveTaskSubmissionIdempotency(ctx, userID, idempotencyKey, taskID)
	if recoverErr != nil {
		slog.Warn("[RunSubmission] Failed to re-reserve stale idempotency key; continuing without dedupe", "error", recoverErr, "userId", userID)
		return nil, false
	}
	if recovered {
		return nil, true
	...
	if task := registry.Get(recoveredTaskID); task != nil {
		return &TaskSubmissionResult{TaskID: recoveredTaskID, Status: submissionResultStatus(task)}, false
	}

	slog.Warn("[RunSubmission] Idempotency key still points to stale task; continuing without dedupe", "userId", userID)
	return nil, false
}
...
func reserveTaskSubmissionIdempotency(
	ctx context.Context,
	userID int,
	key string,
	taskID string,
	...
	encoded, err := marshalTaskSubmissionIdempotency(taskSubmissionIdempotencyReservation{TaskID: taskID, CreatedAt: time.Now().UnixMilli()})
	if err != nil {
...
func releaseTaskSubmissionIdempotency(ctx context.Context, userID int, key string, expectedTaskIDs ...string) error {
	...
	if supportsRedisEval(redisClient) {
...
func taskSubmissionIdempotencyReservationPending(ctx context.Context, userID int, key, expectedTaskID string) bool {
	redisClient, err := RedisClientGetter()
