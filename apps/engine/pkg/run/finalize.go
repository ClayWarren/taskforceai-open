...
func finalizeTask(ctx context.Context, taskID string, userID int, prompt, modelID, result string, trace *orchestrator.OrchestrationTrace, cfg coreconfig.Config, cacheInstance corecache.ICache, skipCacheSet, memoryEnabled bool, opts OrchestrateTaskOptions, traceID string) {
	...
	if state := registry.Get(taskID); state != nil && state.Status == StatusCanceled {
		recordCanceledTaskUsage(ctx, taskID, userID, modelID, prompt, opts, trace)
...
func recordCanceledTaskUsage(ctx context.Context, taskID string, userID int, modelID, prompt string, opts OrchestrateTaskOptions, trace *orchestrator.OrchestrationTrace) {
	if trace == nil {
		return
	}
	usageCtx := context.WithoutCancel(ctx)
	if err := RunTaskPersistenceTx(usageCtx, func(store taskPersistenceStore) error {
		recordTaskUsage(usageCtx, store, taskID, nil, userID, modelID, prompt, opts, trace)
		return nil
	}); err != nil {
...
func finalizeActiveTask(ctx context.Context, registry TaskRegistrar, taskID string, userID int, prompt, modelID, result string, trace *orchestrator.OrchestrationTrace, cfg coreconfig.Config, cacheInstance corecache.ICache, skipCacheSet, memoryEnabled bool, opts OrchestrateTaskOptions, traceID string) {
	...
	if noRetention {
		...
		if err := RunTaskPersistenceTx(usageCtx, func(store taskPersistenceStore) error {
			...
			slog.Error("[FinalizeTask] No-retention task usage transaction failed", "taskId", taskID, "error", err)
		}
	...
	conversationID, txErr := persistFinalTask(ctx, userID, prompt, modelID, result, trace, opts, sourcesData, toolEventsData, agentStatusesData)
	if txErr != nil {
		conversationID = 0
		completionErr = txErr.Error()
		slog.Error("[FinalizeTask] Database transaction failed", "taskId", taskID, "error", txErr)
	} else {
		if persistenceErr := finishPersistedTask(ctx, taskID, conversationID, userID, modelID, prompt, result, trace, cfg, memoryEnabled, isMediaGenerationModel, opts); persistenceErr != "" {
			completionErr = persistenceErr
		}
	}
...
func persistFinalTask(ctx context.Context, userID int, prompt, modelID, result string, trace *orchestrator.OrchestrationTrace, opts OrchestrateTaskOptions, sourcesData, toolEventsData, agentStatusesData []byte) (int32, error) {
	...
	err := RunTaskPersistenceTx(ctx, func(store taskPersistenceStore) error {
		...
		if opts.ConversationID != nil {
			...
			if err := store.CreateMessage(ctx, taskMessageCreateInput{
		...
		messageErr := store.CreateMessage(ctx, taskMessageCreateInput{
			MessageID:      "msg_" + uuid.New().String(),
			ConversationID: conversationID,
			Role:           "assistant",
			Content:        result,
		...
		if messageErr != nil {
			return fmt.Errorf("create message: %w", messageErr)
		}
...
func finishPersistedTask(ctx context.Context, taskID string, conversationID int32, userID int, modelID, prompt, result string, trace *orchestrator.OrchestrationTrace, cfg coreconfig.Config, memoryEnabled, mediaModel bool, opts OrchestrateTaskOptions) string {
	recordPersistedTaskUsage(ctx, taskID, conversationID, userID, modelID, prompt, opts, trace)
	if !memoryEnabled || opts.IsEval {
		return ""
	}
	if mediaModel {
	...
	return extractFinalTaskMemories(ctx, taskID, cfg, userID, opts.OrgID, conversationID, prompt, result)
}

func recordPersistedTaskUsage(ctx context.Context, taskID string, conversationID int32, userID int, modelID, prompt string, opts OrchestrateTaskOptions, trace *orchestrator.OrchestrationTrace) {
	usageCtx := context.WithoutCancel(ctx)
	if err := RunTaskPersistenceTx(usageCtx, func(store taskPersistenceStore) error {
		recordTaskUsage(usageCtx, store, taskID, &conversationID, userID, modelID, prompt, opts, trace)
		return nil
	}); err != nil {
		slog.Warn("[FinalizeTask] Usage transaction failed", "taskId", taskID, "conversationId", conversationID, "error", err)
	}
}
...
func recordTaskUsage(ctx context.Context, store taskPersistenceStore, taskID string, conversationID *int32, userID int, modelID, prompt string, opts OrchestrateTaskOptions, trace *orchestrator.OrchestrationTrace) {
	if store == nil {
		return
	}
	if trace == nil {
		trace = &orchestrator.OrchestrationTrace{}
	}
	...
	if eventStore, ok := store.(sharedusage.EventRepository); ok {
		if err := sharedusage.RecordMediaUsage(ctx, eventStore, sharedusage.MediaEventParams{
			TaskID: taskID, ConversationID: conversationIDInt, UserID: userIDPtr,
			OrganizationID: orgIDPtr, Plan: planPtr, Source: opts.Source,
			Model: modelID, Prompt: prompt,
		}); err != nil {
			slog.Warn("[FinalizeTask] Failed to record media usage", "taskId", taskID, "modelId", modelID, "error", err)
		}
	}
...
func extractSourcesFromToolEventsData(data []byte) []messageSource {
	...
	sources := make([]messageSource, 0, len(events))
