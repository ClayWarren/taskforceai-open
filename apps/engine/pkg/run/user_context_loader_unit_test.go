...
func TestCacheAndApplyUserSettingsEarlyReturns(t *testing.T) {
	setRedisClientGetterForTest(t, func() (redis.Cmdable, error) { return nil, errors.New("redis unavailable") })
	cacheUserSettings(context.Background(), "user:error", userContextUserRow{ID: 7})
	setRedisClientGetterForTest(t, func() (redis.Cmdable, error) { return nil, nil })
	cacheUserSettings(context.Background(), "user:nil", userContextUserRow{ID: 7})

	ctx := RunUserContext{UserPlan: "original"}
	applyUserSettings(&ctx, userContextUserRow{})
	assert.Equal(t, "original", ctx.UserPlan)
}
