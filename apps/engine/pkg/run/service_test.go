...
func TestOrchestrateTaskRunnerExecuteRegistersCallbacks(t *testing.T) {
	...
	runner := &orchestrateTaskRunner{
		...
		opts:     OrchestrateTaskOptions{ThreadContext: "assistant: earlier answer"},
	}
	...
	assert.Contains(t, executionPrompt, "Continue the existing conversation")
	assert.Nil(t, trace)
}

func TestOrchestrateTaskRunnerPrepareCacheExistingConversation(t *testing.T) {
	conversationID := int32(9)
	runner := &orchestrateTaskRunner{taskID: "existing-conversation", opts: OrchestrateTaskOptions{ConversationID: &conversationID}}
	_, requiresCurrentData, shouldContinue := runner.prepareCache(context.Background(), false, coreconfig.Config{})
	assert.True(t, requiresCurrentData)
