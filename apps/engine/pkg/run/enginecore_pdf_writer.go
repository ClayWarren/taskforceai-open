...
var (
	enginecorePDFWriterMu        sync.Mutex
	enginecorePDFWriterInstalled bool
)

func installEnginecorePDFWriter() {
	enginecorePDFWriterMu.Lock()
	defer enginecorePDFWriterMu.Unlock()
	if enginecorePDFWriterInstalled {
		return
	}
	enginecoretools.SetPDFWriter(enginecoreFilePDFWriter{})
	enginecorePDFWriterInstalled = true
}
