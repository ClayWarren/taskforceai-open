...
func prepareConfig(taskID string, modelID string, opts OrchestrateTaskOptions) (coreconfig.Config, error) {
	...
	if plan := strings.TrimSpace(opts.UserPlan); plan != "" {
		if err := validateModelEntitlements(plan, modelID, opts.RoleModels); err != nil {
			slog.Warn("[OrchestrateTask] Model entitlement validation failed", "taskId", taskID, "error", err)
			return coreconfig.Config{}, err
		}
	}
