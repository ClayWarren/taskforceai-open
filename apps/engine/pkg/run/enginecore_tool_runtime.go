...
var (
	enginecoreToolRuntimeMu        sync.Mutex
	enginecoreToolRuntimeInstalled bool
...
func installEnginecoreToolRuntime() {
	enginecoreToolRuntimeMu.Lock()
	defer enginecoreToolRuntimeMu.Unlock()
	if enginecoreToolRuntimeInstalled {
		return
	}
	coretools.SetEngineCoreToolRuntime(enginecoreFileToolRuntime{})
	enginecoreToolRuntimeInstalled = true
}
