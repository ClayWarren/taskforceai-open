...
func TestFetchUserContext_UserLookupErrorStillUsesDefaultMemorySetting(t *testing.T) {
	mock := dbtest.NewMockPool(t)
	...
	mock.ExpectQuery(`SELECT .* FROM memories`).
		WithArgs(userID).
		WillReturnRows(pgxmock.NewRows(memColumns).AddRow(int32(1), userID, nil, "memory from fallback", "fact", []byte("{}"), ts, ts))
	...
	assert.True(t, memoryEnabled)
	assert.False(t, trustLayerEnabled)
	assert.Equal(t, []string{"memory from fallback"}, memories)
	assert.Nil(t, driveClient)
...
func TestFinalizeTask_MemoryExtractionFailureAppendsError(t *testing.T) {
	...
	LoadMemoryStore = func(ctx context.Context) (memories.MemoryStore, error) {
		return memoryStoreAdapter{q: q}, nil
	}
...
func TestFinalizeTask_NoTrainingSkipsRetention(t *testing.T) {
	...
	assert.False(t, txCalled, "conversation persistence should be skipped when no-training is enabled")
