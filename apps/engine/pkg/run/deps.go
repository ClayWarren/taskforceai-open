...
func defaultPromptProvider() CorePromptProvider {
	return configadapter.NewPromptProviderFromEnv()
}
...
func loadMemoryStore(ctx context.Context) (memories.MemoryStore, error) {
	return loadFromQueries(ctx, func(q *db.Queries) memories.MemoryStore { return memoryadapters.NewStore(q) })
}
...
func (s sqlcTaskPersistenceStore) CreateUsageEvents(ctx context.Context, rows []sharedusage.EventRow) error {
	return s.q.CreateUsageEvents(ctx, rows)
}
