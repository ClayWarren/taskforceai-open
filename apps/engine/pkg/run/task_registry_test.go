...
func TestIsUpdateProgressValidationError_JSONDecodeDetails(t *testing.T) {
	if !isUpdateProgressValidationError("invalid budgetUsage json: Expected value but found T_END at character 1") {
		t.Fatal("expected detailed invalid budgetUsage json error to be treated as validation error")
	}

	if !isUpdateProgressValidationError("invalid agentStatuses json: malformed payload") {
		t.Fatal("expected detailed invalid agentStatuses json error to be treated as validation error")
	}

	if !isUpdateProgressValidationError("invalid toolEvents json: malformed payload") {
		t.Fatal("expected detailed invalid toolEvents json error to be treated as validation error")
	}

	if isUpdateProgressValidationError("something else failed") {
		t.Fatal("did not expect unrelated error to be treated as validation error")
	}
}

func TestIsWatchUnavailableError(t *testing.T) {
	if !isWatchUnavailableError(errors.New("redis watch operations require REDIS_URL")) {
		t.Fatal("expected watch unavailable error to be recognized")
	}
	if !isWatchUnavailableError(errors.New("mock does not support watch")) {
		t.Fatal("expected mock watch unsupported error to be recognized")
	}
	if isWatchUnavailableError(errors.New("some other error")) {
		t.Fatal("did not expect generic error to be recognized as watch unavailable")
	}
	if isWatchUnavailableError(nil) {
		t.Fatal("did not expect nil error to be recognized as watch unavailable")
	}
}
...
func TestMarkStartedWithError_CompletesWithinTimeout(t *testing.T) {
	redis.SetClient(redis.NewMockClient())
	t.Cleanup(func() { redis.SetClient(redis.NewMockClient()) })
	registry := requireTaskRegistry(t)

	taskID := "timeout-regression-mse"
	...
	go func() {
		_, _ = registry.MarkStartedWithError(taskID)
		close(done)
	}()
	...
	case <-time.After(3 * persistenceTimeout):
		t.Fatal("MarkStartedWithError did not return within 3×persistenceTimeout — context is not being applied")
	}
}

func TestMarkStartedWithSetNXLock_NilRedisClient(t *testing.T) {
	registry, client, cleanup := setupMiniredisRegistry(t)
	defer cleanup()

	taskID := "nil-redis-fallback"
	...
	started, err := registry.markStartedWithSetNXLock(context.Background(), taskID)
	require.Error(t, err)
...
func TestPersistenceTimeout_Defined(t *testing.T) {
	if persistenceTimeout <= 0 {
		t.Fatalf("persistenceTimeout must be positive, got %v", persistenceTimeout)
	}
	// A ceiling of 60 s prevents an accidentally huge value that would
	// still allow goroutines to block "indefinitely" in practice.
	if persistenceTimeout > 60*time.Second {
		t.Fatalf("persistenceTimeout is unreasonably large (%v), update the regression test if intentional", persistenceTimeout)
	}
}
