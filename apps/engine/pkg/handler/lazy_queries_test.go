...
func TestMapDeveloperFileRecord(t *testing.T) {
	createdAt := pgtype.Timestamp{Time: time.Unix(10, 0), Valid: true}
	updatedAt := pgtype.Timestamp{Time: time.Unix(20, 0), Valid: true}

	record := mapDeveloperFileRecord(db.DeveloperFile{
		ID:        "file-1",
		UserID:    42,
		Filename:  "notes.txt",
		Purpose:   "assistants",
	...
	assert.Equal(t, developerfiles.DeveloperFileRecord{
		ID:        "file-1",
		UserID:    42,
		Filename:  "notes.txt",
		Purpose:   "assistants",
