...
func TestStreamHandler_RecoversStaleUnstartedTask(t *testing.T) {
	redis.SetClient(redis.NewMockClient())
	originalRecoveryLease := recoveryLease
	recoveryLease = newRecoveryLeaseMap(recoveryLeaseInterval)
	t.Cleanup(func() { recoveryLease = originalRecoveryLease })
	...
	require.NoError(t, redisClient.Set(context.Background(), "task:"+taskID, serialized, run.TaskTTL))
