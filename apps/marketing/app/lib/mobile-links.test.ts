...
describe('resolveMobileIosUrl', () => {
  it('returns canonical TestFlight URL when value is missing', () => {
    expect(resolveMobileIosUrl(undefined)).toBe(MOBILE_IOS_TESTFLIGHT_URL);
  });

  it('returns canonical TestFlight URL when value contains placeholder token', () => {
    expect(resolveMobileIosUrl('https://testflight.apple.com/join/REPLACE_WITH_CODE')).toBe(
      MOBILE_IOS_TESTFLIGHT_URL
    );
  });

  it('preserves configured URL when value is valid', () => {
    expect(resolveMobileIosUrl('https://testflight.apple.com/join/CustomCode')).toBe(
      'https://testflight.apple.com/join/CustomCode'
    );
  });

  it('rejects unsafe configured URLs', () => {
    expect(resolveMobileIosUrl('javascript:alert(1)')).toBe(MOBILE_IOS_TESTFLIGHT_URL);
    expect(resolveMobileIosUrl('/mobile#ios-install')).toBe(MOBILE_IOS_TESTFLIGHT_URL);
  });
});
