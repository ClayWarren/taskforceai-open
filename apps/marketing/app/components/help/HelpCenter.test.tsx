...
describe('HelpCenter', () => {
  ...
  it('labels search and filters articles by title, description, and content', async () => {
    const user = userEvent.setup({ pointerEventsCheck: 0 });
    render(<HelpCenter />);

    const searchInput = screen.getByRole('textbox', { name: 'Search help articles' });
    ...
    await user.clear(searchInput);
    await user.type(searchInput, 'API key');

    expect(screen.getByText('1 result for "API key"')).toBeTruthy();
    expect(screen.getByRole('link', { name: /API authentication/i })).toBeTruthy();
  });
