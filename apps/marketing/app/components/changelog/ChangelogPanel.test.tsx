...
describe('ChangelogPanel', () => {
  ...
  it('uses the weekly fallback label for invalid update timestamps', () => {
    const view = renderPanel({ lastUpdated: 'not-a-date' });

    expect(view.getByText('Fresh updates arrive weekly')).toBeInTheDocument();
  });
});
