...
let LandingPage: typeof import('./LandingPage').default;
let CTAButton: typeof import('./CTAButton').CTAButton;
let Hero: typeof import('./Hero').Hero;
...
beforeAll(async () => {
  ...
  ({ CTAButton } = await import('./CTAButton'));
...
describe('Landing Components', () => {
  ...
  describe('LandingPage', () => {
    it('renders full landing page structure', () => {
      ...
      const gradientLayers = document.querySelectorAll<HTMLElement>(
        '#platforms article > div.pointer-events-none'
      );
      expect(gradientLayers).toHaveLength(3);
      expect([...gradientLayers].every((layer) => layer.style.backgroundImage !== '')).toBe(true);
    });
