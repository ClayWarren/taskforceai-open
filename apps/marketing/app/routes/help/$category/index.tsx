...
function isCategoryLoaderData(loaderData: unknown): loaderData is CategoryLoaderData {
  if (!loaderData || typeof loaderData !== 'object') {
    return false;
  }

  const maybeLoaderData = loaderData as Partial<CategoryLoaderData>;
  return Boolean(
    maybeLoaderData.category?.id &&
    maybeLoaderData.category.title &&
    maybeLoaderData.category.description &&
    maybeLoaderData.categoryId
  );
}
...
export const Route = createFileRoute('/help/$category/')({
  ...
  head: ({ loaderData }) => {
    if (!isCategoryLoaderData(loaderData)) {
      return pageHead({
        title: 'Help Category - Help Center',
        description: 'Browse TaskForceAI help categories and support articles.',
        path: '/help',
      });
    }
