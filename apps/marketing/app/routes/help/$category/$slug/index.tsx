...
type HelpCategory = (typeof helpCategories)[number];
type ArticleLoaderData = {
  article: HelpArticle;
  category: HelpCategory;
};

function isArticleLoaderData(loaderData: unknown): loaderData is ArticleLoaderData {
  if (!loaderData || typeof loaderData !== 'object') {
    return false;
  }

  const maybeLoaderData = loaderData as Partial<ArticleLoaderData>;
  return Boolean(
    maybeLoaderData.article?.title &&
    maybeLoaderData.article.description &&
    maybeLoaderData.article.categoryId &&
    maybeLoaderData.article.slug &&
    maybeLoaderData.category?.id
  );
}
...
export const Route = createFileRoute('/help/$category/$slug/')({
  ...
  head: ({ loaderData }) => {
    if (!isArticleLoaderData(loaderData)) {
      return pageHead({
        title: 'Help Article - Help Center',
        description: 'Get help with TaskForceAI support articles and guides.',
        path: '/help',
      });
    }
