...
describe('Help routes', () => {
  ...
  it('falls back to generic help metadata when loader data is missing', () => {
    const categoryHead = CategoryRoute.options.head({ loaderData: undefined });
    const articleHead = ArticleRoute.options.head({ loaderData: undefined });

    expect(categoryHead.meta).toContainEqual({ title: 'Help Category - Help Center' });
    expect(categoryHead.links).toContainEqual({
      rel: 'canonical',
      href: 'https://www.taskforceai.chat/help',
    });
    expect(articleHead.meta).toContainEqual({ title: 'Help Article - Help Center' });
    expect(articleHead.links).toContainEqual({
      rel: 'canonical',
      href: 'https://www.taskforceai.chat/help',
    });
