...
vi.mock('@/env', () => ({
  env: {
    NEXT_PUBLIC_MOBILE_IOS_APP_URL: 'https://testflight.apple.com/join/taskforce',
    NEXT_PUBLIC_MOBILE_ANDROID_APP_URL: '/mobile/android-beta',
  },
}));

const TESTFLIGHT_FALLBACK_URL = 'https://testflight.apple.com/join/NamSXhUZ';
const TESTFLIGHT_PLACEHOLDER_TOKEN = 'REPLACE_WITH_CODE';

vi.mock('@/lib/mobile-links', () => ({
  resolveMobileIosUrl: (candidateUrl?: string) => {
  ...
  resolveMobileAndroidUrl: (candidateUrl?: string) => {
    const normalizedUrl = candidateUrl?.trim();
  ...
  isExternalHref: (href: unknown) =>
    typeof href === 'string' && (href.startsWith('https://') || href.startsWith('http://')),
}));
...
beforeAll(async () => {
  ...
  ({ Route: MobileRoute } = await import('./mobile/index'));
...
describe('Marketing routes: about/mobile/sdk/company/enterprise/blog', () => {
  ...
  it('renders mobile route download links with external/internal behavior', () => {
    ...
    expect(iosLink.getAttribute('href')).toBe('https://testflight.apple.com/join/taskforce');
    expect(iosLink.getAttribute('target')).toBe('_blank');
    expect(iosLink.getAttribute('rel')).toBe('noopener noreferrer');
    expect(screen.getByText('Opens TestFlight')).toBeTruthy();
  ...
  it('renders SDK route installation, examples, and docs CTAs', () => {
    ...
    expect(screen.getByText('npm install taskforceai-sdk')).toBeTruthy();
  ...
  it('renders enterprise route features and compliance badges', () => {
    ...
    expect(screen.getByText('Enterprise SSO')).toBeTruthy();
    expect(screen.getByText('Directory Sync (SCIM)')).toBeTruthy();
    expect(screen.getByText('Dedicated Support')).toBeTruthy();

    expect(screen.getByText('SOC2 Type II Ready')).toBeTruthy();
    expect(screen.getByText('GDPR Compliant')).toBeTruthy();
    expect(screen.getByText('AES-256 Encryption')).toBeTruthy();
