...
\`\`\`python
    ...
    client = TaskForceAIClient(api_key=os.environ["TASKFORCEAI_API_KEY"])
...
\`\`\`python
    ...
    async def main():
        async with AsyncTaskForceAIClient(api_key=os.environ["TASKFORCEAI_API_KEY"]) as client:
...
use taskforceai_sdk::{TaskForceAI, TaskForceAIOptions};
    ...
    #[tokio::main]
    async fn main() {
        let api_key = std::env::var("TASKFORCEAI_API_KEY")
            .expect("TASKFORCEAI_API_KEY is required");
...
import (
    ...
    func main() {
        apiKey := os.Getenv("TASKFORCEAI_API_KEY")
        if apiKey == "" {
            log.Fatal("TASKFORCEAI_API_KEY is required")
        }
