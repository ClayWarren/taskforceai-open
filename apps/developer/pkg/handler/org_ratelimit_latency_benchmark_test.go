...
func reportOrgRateLimitLatencyProfile(b *testing.B, samples []time.Duration) {
	...
	b.ReportMetric(durationMicroseconds(orgRateLimitPercentileDuration(ordered, 0.50)), "p50_us")
	b.ReportMetric(durationMicroseconds(orgRateLimitPercentileDuration(ordered, 0.95)), "p95_us")
	b.ReportMetric(durationMicroseconds(orgRateLimitPercentileDuration(ordered, 0.99)), "p99_us")
}

func orgRateLimitPercentileDuration(ordered []time.Duration, percentile float64) time.Duration {
	if len(ordered) == 0 {
		return 0
	}
