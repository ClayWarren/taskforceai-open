...
func TestPgDeveloperRepository_GetUsageTotalsForKeys_RowsError(t *testing.T) {
	...
	mock.ExpectQuery("SELECT key_ids.api_key_id").
		WithArgs(pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg()).
		WillReturnRows(pgxmock.NewRows([]string{"api_key_id", "hourly_count", "daily_count", "weekly_count", "monthly_count"}).
			AddRow(int32(5), int32(0), int32(0), int32(0), int32(0)).
			AddRow(int32(6), int32(0), int32(0), int32(0), int32(0)).
			RowError(1, errors.New("rows error")))

	_, err := repo.GetUsageTotalsForKeys(context.Background(), []int{5, 6}, now, now, now, now)
