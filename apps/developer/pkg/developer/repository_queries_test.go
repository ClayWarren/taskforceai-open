...
func TestPgDeveloperRepository_GetUsageTotalsForKeys(t *testing.T) {
	mock := dbtest.NewMockPool(t)
	repo := NewDeveloperRepositoryFromSource(db.New(mock))
	...
	mock.ExpectQuery("SELECT key_ids.api_key_id").
		...
		WillReturnRows(pgxmock.NewRows([]string{"api_key_id", "hourly_count", "daily_count", "weekly_count", "monthly_count"}).
			AddRow(int32(5), int32(1), int32(2), int32(3), int32(4)).
			AddRow(int32(6), int32(5), int32(6), int32(7), int32(8)))

	totals, err := repo.GetUsageTotalsForKeys(context.Background(), []int{5, 6}, startOfHour, startOfDay, startOfWeek, startOfMonth)

	require.NoError(t, err)
	assert.Equal(t, UsageTotals{Hourly: 1, Daily: 2, Weekly: 3, Monthly: 4}, totals[5])
	assert.Equal(t, UsageTotals{Hourly: 5, Daily: 6, Weekly: 7, Monthly: 8}, totals[6])
	assert.NoError(t, mock.ExpectationsWereMet())
}

func TestPgDeveloperRepository_GetUsageTotalsForKeys_Empty(t *testing.T) {
	mock := dbtest.NewMockPool(t)
	repo := NewDeveloperRepositoryFromSource(db.New(mock))

	totals, err := repo.GetUsageTotalsForKeys(context.Background(), nil, time.Now(), time.Now(), time.Now(), time.Now())

	require.NoError(t, err)
	assert.Empty(t, totals)
}

func TestPgDeveloperRepository_GetUsageTotalsForKeys_QueryError(t *testing.T) {
	mock := dbtest.NewMockPool(t)
	repo := NewDeveloperRepositoryFromSource(db.New(mock))
	now := time.Now()

	mock.ExpectQuery("SELECT key_ids.api_key_id").
		WithArgs([]int32{5}, pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg()).
		WillReturnError(errors.New("batch usage totals query error"))

	_, err := repo.GetUsageTotalsForKeys(context.Background(), []int{5}, now, now, now, now)

	require.Error(t, err)
	assert.Contains(t, err.Error(), "batch usage totals query error")
}

func TestPgDeveloperRepository_GetUsageTotalsForKeys_ScanError(t *testing.T) {
	mock := dbtest.NewMockPool(t)
	repo := NewDeveloperRepositoryFromSource(db.New(mock))
	now := time.Now()
	...
	mock.ExpectQuery("SELECT key_ids.api_key_id").
		...
		WillReturnRows(pgxmock.NewRows([]string{"api_key_id", "hourly_count", "daily_count", "weekly_count", "monthly_count"}).
			AddRow("invalid", int32(0), int32(0), int32(0), int32(0)))
...
func TestPgDeveloperRepository_GetUsageTotalsForKeys_RowsError(t *testing.T) {
	rowsErr := errors.New("rows error")
	store := sqlcDeveloperStore{q: rowsErrorDeveloperSource{
		DeveloperQuerySource: db.New(nil),
		db:                   rowsErrorDB{rowsErr: rowsErr},
	}}
	now := time.Now()

	_, err := store.GetAPIUsageTotalsForKeys(context.Background(), usageTotalsForKeysInput{
		APIKeyIDs:   []int32{5, 6},
		StartOfHour: pgtype.Timestamp{Time: now, Valid: true},
	})
...
type rowsErrorDeveloperSource struct {
	DeveloperQuerySource
	db db.DBTX
}

func (s rowsErrorDeveloperSource) GetDB() db.DBTX { return s.db }

type rowsErrorDB struct {
	db.DBTX
	rowsErr error
}

func (d rowsErrorDB) Query(context.Context, string, ...interface{}) (pgx.Rows, error) {
	return &terminalRowsError{err: d.rowsErr}, nil
}

type terminalRowsError struct {
	pgx.Rows
	err error
}

func (*terminalRowsError) Close()     {}
func (*terminalRowsError) Next() bool { return false }
func (r *terminalRowsError) Err() error {
	return r.err
}

func TestPgDeveloperRepository_GetUsageTotalsForKeys_Overflow(t *testing.T) {
	mock := dbtest.NewMockPool(t)
	repo := NewDeveloperRepositoryFromSource(db.New(mock))

	_, err := repo.GetUsageTotalsForKeys(context.Background(), []int{math.MaxInt32 + 1}, time.Now(), time.Now(), time.Now(), time.Now())
...
func TestSqlcDeveloperStore_GetAPIUsageTotalsForKeysFallback(t *testing.T) {
	callCount := 0
	store := sqlcDeveloperStore{q: &mockDeveloperQuerySource{
		GetAPIUsageInWindowFunc: func(context.Context, db.GetAPIUsageInWindowParams) (int32, error) {
			callCount++
			return int32(callCount), nil
		},
	}}

	totals, err := store.GetAPIUsageTotalsForKeys(context.Background(), usageTotalsForKeysInput{APIKeyIDs: []int32{5, 6}})

	require.NoError(t, err)
	assert.Equal(t, UsageTotals{Monthly: 1, Weekly: 2, Daily: 3, Hourly: 4}, totals[5])
	assert.Equal(t, UsageTotals{Monthly: 5, Weekly: 6, Daily: 7, Hourly: 8}, totals[6])
	assert.Equal(t, 8, callCount)
}

func TestSqlcDeveloperStore_GetAPIUsageTotalsForKeysFallbackError(t *testing.T) {
	store := sqlcDeveloperStore{q: &mockDeveloperQuerySource{
		GetAPIUsageInWindowFunc: func(context.Context, db.GetAPIUsageInWindowParams) (int32, error) {
			return 0, errors.New("window error")
		},
	...
	_, err := store.GetAPIUsageTotalsForKeys(context.Background(), usageTotalsForKeysInput{APIKeyIDs: []int32{5}})
