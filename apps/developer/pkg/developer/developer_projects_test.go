...
func TestUsageService_GetUsageStats(t *testing.T) {
	...
	mockRepo.On("GetUsageTotalsForKeys", ctx, []int{10, 11}, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(map[int]UsageTotals{
		10: {Hourly: 10, Daily: 100, Weekly: 500, Monthly: 2000},
		11: {Hourly: 5, Daily: 50, Weekly: 250, Monthly: 1000},
	}, nil).Once()
...
func TestUsageService_GetUsageStats_UsesUTCWindows(t *testing.T) {
	...
	mockRepo.On("GetUsageTotalsForKeys", ctx, []int{10}, isUTC, isUTC, isUTC, isUTC).Return(map[int]UsageTotals{10: {}}, nil).Once()
...
func TestUsageService_GetUsageStats_HistoryError(t *testing.T) {
	...
	mockRepo.On("GetUsageTotalsForKeys", ctx, []int{10}, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(map[int]UsageTotals{10: {}}, nil)
...
func TestUsageService_GetUsageStats_SortsAggregatedHistory(t *testing.T) {
	...
	mockRepo.On("GetUsageTotalsForKeys", ctx, []int{10}, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(map[int]UsageTotals{10: {}}, nil).Once()
...
func TestUsageService_GetUsageStats_LastUsedAndRevoked(t *testing.T) {
	...
	mockRepo.On("ListKeysForUser", ctx, 99).Return([]DeveloperApiKeyRecord{k1}, nil).Once()
	mockRepo.On("GetUsageTotalsForKeys", ctx, []int{10}, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(map[int]UsageTotals{
		10: {Hourly: 10, Daily: 100, Weekly: 500, Monthly: 2000},
	}, nil).Once()
...
func TestUsageService_GetUsageStats_MissingBatchRowDefaultsToZero(t *testing.T) {
	mockRepo := new(MockDeveloperRepository)
	s := NewDeveloperUsageService(mockRepo)
	ctx := context.Background()
	...
	mockRepo.On("GetUsageTotalsForKeys", ctx, []int{10, 11}, mock.Anything, mock.Anything, mock.Anything, mock.Anything).
		Return(map[int]UsageTotals{11: {Hourly: 1, Daily: 1, Weekly: 1, Monthly: 1}}, nil).
		Once()
	mockRepo.On("ListUsageHistory", ctx, []int{10, 11}, mock.Anything).Return([]UsageHistoryRecord{}, nil).Once()

	stats, err := s.GetUsageStats(ctx, UsageUser{ID: 99})
	require.NoError(t, err)
	require.Len(t, stats.APIKeys, 2)
	assert.Zero(t, stats.APIKeys[0].MonthlyUsage)
	assert.Equal(t, 1, stats.APIKeys[1].MonthlyUsage)
}
...
func TestUsageService_GetUsageStats_UsageTotalsError(t *testing.T) {
	...
	mockRepo.On("ListKeysForUser", ctx, 99).Return([]DeveloperApiKeyRecord{k1}, nil)
	mockRepo.On("GetUsageTotalsForKeys", ctx, []int{10}, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(map[int]UsageTotals(nil), errors.New("db error"))
