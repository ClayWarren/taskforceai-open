...
func (m *MockDeveloperRepository) GetUsageTotalsForKeys(ctx context.Context, keyIDs []int, startOfHour, startOfDay, startOfWeek, startOfMonth time.Time) (map[int]UsageTotals, error) {
	args := m.Called(ctx, keyIDs, startOfHour, startOfDay, startOfWeek, startOfMonth)
	val, _ := args.Get(0).(map[int]UsageTotals)
	return val, args.Error(1)
}
