...
func TestCreateKeyInput_Struct(t *testing.T) {
	tier := TierPro
	...
	assert.Equal(t, 99, input.UserID)
	assert.Equal(t, TierPro, *input.Tier)
	assert.Equal(t, TierStarter, *input.UserTier)
}

func TestDeveloperApiKeyRecord_Struct(t *testing.T) {
	now := time.Now()
	record := DeveloperApiKeyRecord{
		ID:           1,
		DisplayKey:   "tfai_...abc",
		Tier:         TierPro,
		RateLimit:    5000,
		MonthlyQuota: 100000,
		CreatedAt:    now,
	...
	assert.Equal(t, "tfai_...abc", record.DisplayKey)
	assert.Equal(t, TierPro, record.Tier)
	assert.Equal(t, 5000, record.RateLimit)
	assert.Equal(t, 100000, record.MonthlyQuota)
	assert.NotNil(t, record.LastUsedAt)
	assert.Nil(t, record.RevokedAt)
}

func TestDeveloperApiTierConstants(t *testing.T) {
	assert.Equal(t, "STARTER", string(TierStarter))
	assert.Equal(t, "PRO", string(TierPro))
	assert.Equal(t, "ENTERPRISE", string(TierEnterprise))
}

func TestTierRank_Invalid(t *testing.T) {
	assert.Equal(t, -1, tierRank("INVALID"))
}
