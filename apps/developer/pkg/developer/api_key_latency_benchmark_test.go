...
func (r *latencyDeveloperRepository) GetUsageTotalsForKey(_ context.Context, keyID int, _, _, _, _ time.Time) (UsageTotals, error) {
	return UsageTotals{
		Hourly:  keyID,
		Daily:   keyID * 10,
		Weekly:  keyID * 100,
...
func reportDeveloperAPIKeyLookupLatencyProfile(b *testing.B, samples []time.Duration) {
	...
	b.ReportMetric(developerDurationMicroseconds(developerPercentileDuration(ordered, 0.50)), "p50_us")
	b.ReportMetric(developerDurationMicroseconds(developerPercentileDuration(ordered, 0.95)), "p95_us")
	b.ReportMetric(developerDurationMicroseconds(developerPercentileDuration(ordered, 0.99)), "p99_us")
}
