...
func (s sqlcDeveloperStore) getAPIUsageTotalsForKeyFallback(ctx context.Context, input usageTotalsInput) (UsageTotals, error) {
	monthlyCount, err := s.GetAPIUsageInWindow(ctx, usageWindowInput{
		APIKeyID:    input.APIKeyID,
		WindowStart: input.StartOfMonth,
		WindowEnd:   input.EndOfMonth,
	})
	if err != nil {
		return UsageTotals{}, err
	}

	weeklyCount, err := s.GetAPIUsageInWindow(ctx, usageWindowInput{
		APIKeyID:    input.APIKeyID,
		WindowStart: input.StartOfWeek,
		WindowEnd:   input.EndOfWeek,
	})
	if err != nil {
		return UsageTotals{}, err
	}

	dailyCount, err := s.GetAPIUsageInWindow(ctx, usageWindowInput{
		APIKeyID:    input.APIKeyID,
		WindowStart: input.StartOfDay,
		WindowEnd:   input.EndOfDay,
	})
	if err != nil {
		return UsageTotals{}, err
	}

	hourlyCount, err := s.GetAPIUsageInWindow(ctx, usageWindowInput{
		APIKeyID:    input.APIKeyID,
		WindowStart: input.StartOfHour,
		WindowEnd:   input.EndOfHour,
	})
	if err != nil {
		return UsageTotals{}, err
	}
