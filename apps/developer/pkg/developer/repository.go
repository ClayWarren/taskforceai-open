...
type developerStore interface {
	...
	GetAPIUsageTotalsForKeys(ctx context.Context, input usageTotalsForKeysInput) (map[int32]UsageTotals, error)
...
type usageTotalsForKeysInput struct {
	APIKeyIDs    []int32
	StartOfHour  pgtype.Timestamp
	EndOfHour    pgtype.Timestamp
	StartOfDay   pgtype.Timestamp
...
const getAPIUsageTotalsForKeysQuery = `
SELECT
	key_ids.api_key_id,
	COALESCE(SUM(u.count) FILTER (WHERE u.window_start >= $2 AND u.window_start < $3), 0)::int AS hourly_count,
	COALESCE(SUM(u.count) FILTER (WHERE u.window_start >= $4 AND u.window_start < $5), 0)::int AS daily_count,
	COALESCE(SUM(u.count) FILTER (WHERE u.window_start >= $6 AND u.window_start < $7), 0)::int AS weekly_count,
	COALESCE(SUM(u.count) FILTER (WHERE u.window_start >= $8 AND u.window_start < $9), 0)::int AS monthly_count
FROM unnest($1::int[]) AS key_ids(api_key_id)
LEFT JOIN developer_api_usage AS u
	ON u.api_key_id = key_ids.api_key_id
	AND u.window_start >= LEAST($2, $4, $6, $8)
	AND u.window_start < GREATEST($3, $5, $7, $9)
GROUP BY key_ids.api_key_id
`
...
func (s sqlcDeveloperStore) GetAPIUsageTotalsForKeys(ctx context.Context, input usageTotalsForKeysInput) (map[int32]UsageTotals, error) {
	totalsByKey := make(map[int32]UsageTotals, len(input.APIKeyIDs))
	if len(input.APIKeyIDs) == 0 {
		return totalsByKey, nil
	}

	provider, ok := s.q.(developerDBProvider)
	if !ok || provider.GetDB() == nil {
		for _, keyID := range input.APIKeyIDs {
			totals, err := s.GetAPIUsageTotalsForKey(ctx, usageTotalsInput{
				APIKeyID:     keyID,
				StartOfHour:  input.StartOfHour,
				EndOfHour:    input.EndOfHour,
				StartOfDay:   input.StartOfDay,
				EndOfDay:     input.EndOfDay,
				StartOfWeek:  input.StartOfWeek,
				EndOfWeek:    input.EndOfWeek,
				StartOfMonth: input.StartOfMonth,
				EndOfMonth:   input.EndOfMonth,
			})
			if err != nil {
				return nil, err
			}
			totalsByKey[keyID] = totals
		}
		return totalsByKey, nil
	}

	rows, err := provider.GetDB().Query(
		ctx,
		getAPIUsageTotalsForKeysQuery,
		input.APIKeyIDs,
		input.StartOfHour,
		input.EndOfHour,
		input.StartOfDay,
	...
	for rows.Next() {
		var keyID, hourlyCount, dailyCount, weeklyCount, monthlyCount int32
		if err := rows.Scan(&keyID, &hourlyCount, &dailyCount, &weeklyCount, &monthlyCount); err != nil {
			return nil, err
		}
		totalsByKey[keyID] = UsageTotals{
			Hourly:  int(hourlyCount),
			Daily:   int(dailyCount),
			Weekly:  int(weeklyCount),
			Monthly: int(monthlyCount),
...
func (r *PgDeveloperRepository) GetUsageTotalsForKeys(ctx context.Context, keyIDs []int, startOfHour, startOfDay, startOfWeek, startOfMonth time.Time) (map[int]UsageTotals, error) {
	dbKeyIDs, err := convert.Int32Slice(keyIDs, "key_id")
	if err != nil {
		return nil, err
	}

	totalsByDBKey, err := r.store.GetAPIUsageTotalsForKeys(ctx, usageTotalsForKeysInput{
		APIKeyIDs:    dbKeyIDs,
		StartOfHour:  pgtype.Timestamp{Time: startOfHour, Valid: true},
		EndOfHour:    pgtype.Timestamp{Time: startOfHour.Add(time.Hour), Valid: true},
		StartOfDay:   pgtype.Timestamp{Time: startOfDay, Valid: true},
	...
	totalsByKey := make(map[int]UsageTotals, len(totalsByDBKey))
	for keyID, totals := range totalsByDBKey {
		totalsByKey[int(keyID)] = totals
	}
	return totalsByKey, nil
}
