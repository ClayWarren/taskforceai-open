...
func TestObservabilityInit(t *testing.T) {
	_, _ = observability.InitTracer("test-service")
	_, _ = observability.InitMeter("test-service")
}
...
func TestRateLimiterAdapter_WithMockRedis(t *testing.T) {
...
func TestNewRouter_ProxiesDeveloperStorageFallbackRoute(t *testing.T) {
	restore(t, &adapterhandler.ProxyEngineHandler)
	adapterhandler.ProxyEngineHandler = func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("X-Proxied-To", "engine")
		w.WriteHeader(http.StatusAccepted)
	...
	req := httptest.NewRequestWithContext(context.Background(), http.MethodGet, "/api/v1/developer/storage", nil)
	user := &auth.AuthenticatedUser{ID: 123}
	req = req.WithContext(context.WithValue(req.Context(), adapterhandler.UserContextKey, user))
	w := httptest.NewRecorder()
