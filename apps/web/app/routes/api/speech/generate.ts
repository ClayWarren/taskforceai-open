...
const handleSpeechGenerate = async ({ request }: { request: Request }) => {
  ...
  const rateLimit = await consumeVoiceRequestLimit('speech', request);
  ...
  try {
    ...
    if (
      !(await recordCompletedVoiceUsage('speech', request, {
        model: SPEECH_MODEL_ID,
        quantity: text.length,
        unit: 'characters',
      }))
    ) {
      logger.warn('Speech usage tracking failed', { model: SPEECH_MODEL_ID });
    }
