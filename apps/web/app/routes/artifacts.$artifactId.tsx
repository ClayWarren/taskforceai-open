  ...
  <section className="rounded-md border border-slate-800 bg-slate-950/70 p-4">
    ...
    <dl className="mt-3 space-y-3 text-sm">
      {[
        ...
        ['Created', formatArtifactDate(artifact.createdAt)],
      ].map(([label, value]) => (
      ...
      {currentVersion?.mimeType ? (
        <div className="flex justify-between gap-3">
          ...
          <dd className="truncate text-right text-slate-200">{currentVersion.mimeType}</dd>
        </div>
      ) : null}
      {artifact.taskId ? (
        <div className="flex justify-between gap-3">
      ...
      {artifact.conversationId ? (
        <div className="flex justify-between gap-3">
          ...
          <dd className="text-right font-mono text-xs text-slate-200">{artifact.conversationId}</dd>
        </div>
      ) : null}
    </dl>
  </section>
...
export function ArtifactPage() {
  ...
  const setVisibility = async (visibility: 'PRIVATE' | 'ORGANIZATION') => {
    ...
    if (artifact.visibility === 'PUBLIC_LINK') {
      const revokeResult = await revokeArtifactPublicLinks(artifact.id);
      if (!revokeResult.ok) {
        setErrorMessage(revokeResult.error.message);
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-5xl flex-col px-5 py-6 text-slate-100 sm:px-8">
      <ArtifactPageHeader artifact={artifact} downloadUrl={downloadUrl} />
      ...
      {loading ? (
        ...
        <section className="grid gap-5 py-5 lg:grid-cols-[1fr_280px]">
          {errorMessage ? (
            <div className="rounded-md border border-red-900/60 bg-red-950/40 px-4 py-3 text-sm text-red-200 lg:col-span-2">
              {errorMessage}
            </div>
          ) : null}
          <div className="min-w-0">
          ...
          <aside className="space-y-4">
            ...
            <ArtifactDetails artifact={artifact} currentVersion={currentVersion} />
