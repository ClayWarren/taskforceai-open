...
describe('ArtifactPage', () => {
  ...
  it('revokes public links before switching a public artifact to workspace visibility', async () => {
    const actionOrder: string[] = [];
    fetchArtifactMock.mockResolvedValue({
      ok: true,
      value: artifact({ visibility: 'PUBLIC_LINK' }),
    });
    revokeArtifactPublicLinksMock.mockImplementation(async () => {
      actionOrder.push('revoke');
      return { ok: true };
    });
    updateArtifactVisibilityMock.mockImplementation(async () => {
      actionOrder.push('update');
      return { ok: true, value: artifact({ visibility: 'ORGANIZATION' }) };
    });
    ...
    expect(revokeArtifactPublicLinksMock).toHaveBeenCalledWith('artifact-1');
    expect(updateArtifactVisibilityMock).toHaveBeenCalledWith('artifact-1', 'ORGANIZATION');
    expect(actionOrder).toEqual(['revoke', 'update']);
  });

  it('keeps public visibility when revoking the old link fails', async () => {
    fetchArtifactMock.mockResolvedValue({
      ok: true,
      value: artifact({ visibility: 'PUBLIC_LINK' }),
    });
    revokeArtifactPublicLinksMock.mockResolvedValue({
      ok: false,
    ...
    expect(updateArtifactVisibilityMock).not.toHaveBeenCalled();
    expect(screen.getByText('PUBLIC_LINK')).toBeInTheDocument();
  });
