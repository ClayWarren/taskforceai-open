...
void vi.mock('@taskforceai/contracts/api/tasks', () => ({
  runTask: runTaskMock,
}));
  ...
  it('queues prompt when streaming bootstrap fails with explicit streaming failure', async () => {
    ...
    const params = createParams({
      ensureConversationId: vi
        ...
        .mockResolvedValueOnce('remote-99'),
      startStreaming: vi.fn(async () => {
    ...
    expect(params.ensureConversationId).toHaveBeenCalledTimes(2);
    expect(params.enqueuePrompt).toHaveBeenCalledWith('remote-99', 'Ship this', {
      prompt: 'Ship this',
      demo: false,
      options: {
        agentCount: 1,
