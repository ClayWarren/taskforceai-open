...
describe('AuthProvider web profile storage', () => {
  ...
  it('logs out desktop app-server auth from the shared logout hook', async () => {
    ...
    localStorage.setItem('taskforceai:pinned-conversations', '["desktop-conversation"]');
    await renderAuthProvider();
    ...
    expect(localStorage.getItem('taskforceai:pinned-conversations')).toBeNull();
  });

  it('clears browser conversation storage from the shared logout hook', async () => {
    localStorage.setItem('taskforceai:pinned-conversations', '["browser-conversation"]');
    mockStorageAdapterClearAll.mockRejectedValueOnce(new Error('storage cleanup failed'));
    await renderAuthProvider();
