...
const mockGetDesktopAppServerAuthStatus = vi.fn(
  async (): Promise<import('@taskforceai/contracts/app-server').AppServerAuthStatus> => ({
    authenticated: false,
  })
);
...
import { DESKTOP_APP_SERVER_AUTH_CHANGED_EVENT } from '../platform/desktop-api';
...
void vi.mock('@taskforceai/api-client/auth/auth-service', () => ({
...
describe('AuthProvider', () => {
  ...
  it('refreshes desktop auth when the native auth status changes', async () => {
    mockPlatformRuntime = 'desktop';
    mockGetDesktopAppServerAuthStatus
    ...
    expect(mockGetDesktopAppServerAuthStatus).toHaveBeenCalledTimes(2);
  });
  ...
  it('handles forced signout failure', async () => {
    ...
    await waitFor(() => {
      expect(mockLoggerError).toHaveBeenCalledWith('Forced sign-out failed', {
