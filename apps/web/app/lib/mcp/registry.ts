import { SharedMcpToolRegistry, formatMcpInventorySummary } from '@taskforceai/react-core';
...
import { WEB_MCP_SERVERS_CHANGED_EVENT, WEB_MCP_SERVERS_STORAGE_KEY } from './store';
...
export class WebMcpToolRegistry extends SharedMcpToolRegistry {
  ...
  bindWindowEvents(): (() => void) | null {
    ...
    const handleStorage = (event: StorageEvent) => {
      if (event.key === null || event.key === WEB_MCP_SERVERS_STORAGE_KEY) {
        handleRefresh();
      }
    };
