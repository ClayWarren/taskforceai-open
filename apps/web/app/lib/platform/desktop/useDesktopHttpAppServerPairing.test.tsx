...
describe('useDesktopHttpAppServerPairing', () => {
  ...
  it('ignores stale pairing results after a newer connect request wins', async () => {
    ...
    expect(pairMock).not.toHaveBeenCalledWith(oldInfo);
    expect(result.current.session).toEqual(freshSession);
  });

  it('stops an in-flight pairing request after unmount', async () => {
    const infoDeferred = deferred<typeof freshInfo>();
    getPairingInfoMock.mockReset();
    getPairingInfoMock.mockReturnValueOnce(infoDeferred.promise);

    const { unmount } = renderHook(() => useDesktopHttpAppServerPairing());
    await waitFor(() => expect(getPairingInfoMock).toHaveBeenCalledTimes(1));
    unmount();
    infoDeferred.resolve(freshInfo);
    await new Promise((resolve) => setTimeout(resolve, 0));
