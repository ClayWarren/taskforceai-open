...
describe('desktop app-server bridge', () => {
  ...
  it('passes history limit through the shared app-server command', async () => {
    ...
    expect(mockInvokeTauri).toHaveBeenCalledWith('app_server_history_list', {
      limit: 25,
    });
  });
  ...
  it('bridges environment, thread, replay, sync, and Ollama commands', async () => {
    const bridge = await loadModule();

    await bridge.useLocalDesktopAppServerEnvironment();
    await bridge.handoffDesktopAppServerThread({ threadId: 'thread-1' } as never);
    await bridge.listDesktopAppServerThreads();
    await bridge.startDesktopAppServerThread({ title: 'Investigate' } as never);
    await bridge.resumeDesktopAppServerThread('thread-1');
    await bridge.archiveDesktopAppServerThread('thread-1');
    await bridge.forkDesktopAppServerThread('thread-1');
    await bridge.startDesktopAppServerTurn({ threadId: 'thread-1', prompt: 'Continue' } as never);
    await bridge.steerDesktopAppServerTurn({ threadId: 'thread-1', prompt: 'Focus' } as never);
    await bridge.interruptDesktopAppServerTurn({ threadId: 'thread-1' } as never);
    await bridge.createDesktopRecordReplaySkill({ name: 'Replay' } as never);
    await bridge.getDesktopAppServerRunStatus('run-1');
    await bridge.configureDesktopAppServerSync({ deviceId: 'device-1', lastSyncVersion: 3 });
    await bridge.ensureDesktopAppServerOllama({
      baseUrl: 'http://localhost:11434',
      modelId: 'qwen',
    });

    expect(mockInvokeTauri).toHaveBeenCalledTimes(14);
  });
  ...
  it('manages sync metadata through the shared app-server command', async () => {
    ...
    await configureDesktopAppServerSync({
      deviceId: 'device-1',
      lastSyncVersion: 42,
    });
