...
export const createDesktopSyncClient = (
  ...
  options: Pick<HttpSyncClientOptions, 'onUnauthorized' | 'getCsrfToken'> = {}
): SyncClient => {
  ...
  return {
    pull(lastSyncVersion: number, deviceId: string): Promise<SyncPullResponse> {
      return invokeWithFallback('app_server_desktop_sync_pull', { lastSyncVersion, deviceId }, () =>
        httpClient.pull(lastSyncVersion, deviceId)
      );
    },
    ...
    push(
      ...
      return invokeWithFallback(
        ...
        () => httpClient.push(conversations, messages, deletions, deviceId)
      );
    },
