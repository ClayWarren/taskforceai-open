...
describe('confirmDialog', () => {
  ...
  it('uses custom title and warning kind', async () => {
    ...
    expect(confirmButton?.style.background).toBe('#dc2626');
  });
  ...
  it('uses custom button labels', async () => {
    confirmDialog('Install update?', { confirmLabel: 'Install', cancelLabel: 'Later' });
    ...
    expect(confirmButton?.style.background).toBe('#2563eb');
  });
