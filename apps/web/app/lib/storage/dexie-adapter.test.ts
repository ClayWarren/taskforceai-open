...
describe('DexieStorageAdapter', () => {
  ...
  describe('Conversations', () => {
    ...
    it('replaces a conversation id and updates related messages', async () => {
      await adapter.upsertConversation({
      ...
      await adapter.replaceConversationId('old-c', 'new-c');

      expect((await adapter.getConversation('old-c')).ok).toBe(false);
      const replaced = await adapter.getConversation('new-c');
      expect(replaced.ok).toBe(true);
      if (replaced.ok) {
        expect(replaced.value.conversationId).toBe('new-c');
        expect(replaced.value.title).toBe('Replace');
      }
      expect(await adapter.getMessages('old-c')).toHaveLength(0);
      expect((await adapter.getMessages('new-c')).map((message) => message.messageId)).toEqual([
        'm1',
      ]);
    });
  });
