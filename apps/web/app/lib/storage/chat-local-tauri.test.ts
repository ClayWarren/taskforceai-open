...
describe('chat-local-tauri', () => {
  ...
  it('delegates restoreConversation and handles error', async () => {
    await chatTauri.restoreConversation('id');
    expect(mocks.restoreConversation).toHaveBeenCalledWith('id');

    mocks.restoreConversation.mockRejectedValue(new Error('fail'));
    await chatTauri.restoreConversation('id');
    expect(logger.error).toHaveBeenCalledWith(
      '[chat-local-tauri] restoreConversation failed',
      expect.any(Error)
    );
  });
  ...
  it('delegates listArchivedConversations and handles error', async () => {
    mocks.listArchivedConversations.mockResolvedValue([baseConversation]);
    const result = await chatTauri.listArchivedConversations(10);
    expect(result).toEqual([baseConversation]);
    expect(mocks.listArchivedConversations).toHaveBeenCalledWith(10);

    mocks.listArchivedConversations.mockRejectedValue(new Error('fail'));
    const failed = await chatTauri.listArchivedConversations();
    expect(failed).toEqual([]);
    expect(logger.error).toHaveBeenCalledWith(
      '[chat-local-tauri] listArchivedConversations failed',
      expect.any(Error)
    );
  });
  ...
  it('delegates archiveAllConversations and handles error', async () => {
    await chatTauri.archiveAllConversations();
    expect(mocks.archiveAllConversations).toHaveBeenCalled();

    mocks.archiveAllConversations.mockRejectedValue(new Error('fail'));
    await chatTauri.archiveAllConversations();
    expect(logger.error).toHaveBeenCalledWith(
      '[chat-local-tauri] archiveAllConversations failed',
      expect.any(Error)
    );
  });

  it('delegates deleteAllConversations and handles error', async () => {
    await chatTauri.deleteAllConversations();
    expect(mocks.deleteAllConversations).toHaveBeenCalled();

    mocks.deleteAllConversations.mockRejectedValue(new Error('fail'));
    await chatTauri.deleteAllConversations();
    expect(logger.error).toHaveBeenCalledWith(
      '[chat-local-tauri] deleteAllConversations failed',
      expect.any(Error)
    );
  });
