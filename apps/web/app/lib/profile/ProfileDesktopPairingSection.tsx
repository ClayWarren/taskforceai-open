...
export function PairingSections() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setControllers([]);
        setError(caught instanceof Error ? caught.message : 'Remote devices are unavailable.');
      }
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'Remote settings are unavailable.');
    }
  }, []);
  ...
  const update = async (patch: { allowConnections?: boolean; keepAwake?: boolean }) => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : 'Remote settings could not be updated.');
    } finally {
      setBusy(false);
    }
  };
  ...
  const addConnection = async () => {
    ...
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : 'A connection code could not be created.'
      );
    } finally {
      setBusy(false);
    }
