...
export function UsageLimitsSection(props: {
  ...
  return (
    <div className="space-y-7">
      ...
      <section aria-labelledby="model-cost-tiers-title" className="space-y-3">
        <div>
          <h5 id="model-cost-tiers-title" className="text-base font-semibold">
            Model cost tiers
          </h5>
          <p className="mt-1 text-sm text-muted-foreground">
            Dollar signs show relative usage cost, from $ for low cost to $$$+ for very high cost.
          </p>
        </div>
        <div className="max-h-72 divide-y divide-border overflow-y-auto rounded-lg border border-border">
          {modelRows.map((model) => (
            ...
            >
              ...
              <span className="shrink-0 px-2 py-1 text-xs font-bold tracking-wide text-emerald-600 dark:text-emerald-300">
                {model.usageLabel}
              </span>
            </div>
          ))}
