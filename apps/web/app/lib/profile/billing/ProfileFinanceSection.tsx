...
const financeDashboardStatus = (dashboard: FinanceDashboardResponse | null) => {
  const providerStatus = dashboard?.provider_status ?? 'not_connected';
  const connectedAccounts = dashboard?.connected_accounts === true;
  const connectionCount = dashboard?.connections.length ?? 0;
  return {
    providerStatus,
    connectedAccounts,
    hasConnections: connectionCount > 0,
    isProviderReady: providerStatus === 'provider_configured' || providerStatus === 'connected',
    connectionLabel: connectedAccounts
...
export type FinanceSectionView = 'all' | 'dashboard' | 'accounts';

export function ProfileFinanceSection({ view = 'all' }: { view?: FinanceSectionView }) {
  ...
  const { providerStatus, connectedAccounts, hasConnections, isProviderReady, connectionLabel } =
    financeDashboardStatus(dashboard);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 rounded-lg border border-border p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="text-sm font-medium">{formatProviderStatus(providerStatus)}</div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            ...
            disabled={!connectedAccounts || busyAction !== null}
          >
      ...
      {view !== 'dashboard' && hasConnections && dashboard ? (
        <div className="space-y-3">
      ...
      {view !== 'accounts' && dashboard ? <RecurringStreams dashboard={dashboard} /> : null}
    </div>
  );
}
