...
const mockUpdateUserProject = vi.fn();
...
vi.mock('./project-service', () => ({
  ...
  updateUserProject: (...args: unknown[]) => mockUpdateUserProject(...args),
}));
...
const TestComponent = () => {
  ...
  return (
    <div>
      ...
      <span data-testid="project-names">{projects.map((project) => project.name).join(',')}</span>
      ...
      <button onClick={() => void renameProject(7, 'Renamed')}>Rename Project</button>
    </div>
  );
};
...
describe('ProjectsContext', () => {
  ...
  it('inserts projects created through a native client immediately', async () => {
    render(
      <ProjectsProvider>
        <TestComponent />
      </ProjectsProvider>
    ...
    await waitFor(() => expect(screen.getByTestId('loading')).toHaveTextContent('not-loading'));
    fireEvent.click(screen.getByText('Upsert Project'));

    expect(screen.getByTestId('project-names')).toHaveTextContent('Native project');
  });

  it('keeps a native project when an older project refresh finishes later', async () => {
    const pendingProjects = createDeferred<{ ok: true; value: never[] }>();
    mockFetchProjects.mockReturnValueOnce(pendingProjects.promise);
    ...
    pendingProjects.resolve({ ok: true, value: [] });
    await waitFor(() => expect(screen.getByTestId('loading')).toHaveTextContent('not-loading'));
    expect(screen.getByTestId('project-names')).toHaveTextContent('Native project');
  });
  ...
  it('renames a project in place after a successful update', async () => {
    mockFetchProjects.mockResolvedValue({
      ok: true,
      value: [{ id: 7, name: 'Original' }],
    });
    mockUpdateUserProject.mockResolvedValue({
      ok: true,
      value: { id: 7, name: 'Renamed' },
    });
    ...
    await waitFor(() => expect(screen.getByTestId('project-names')).toHaveTextContent('Original'));
    fireEvent.click(screen.getByText('Rename Project'));

    await waitFor(() => {
      expect(mockUpdateUserProject).toHaveBeenCalledWith(7, { name: 'Renamed' });
      expect(screen.getByTestId('project-names')).toHaveTextContent('Renamed');
    });
  });

  it('replaces an existing project when it is upserted', async () => {
    mockFetchProjects.mockResolvedValue({
      ok: true,
      value: [{ id: 7, name: 'Original' }],
    });
    ...
    expect(screen.getByTestId('project-names')).toHaveTextContent('Upserted');
  });

  it('keeps the existing project when a rename fails', async () => {
    mockFetchProjects.mockResolvedValue({
      ok: true,
      value: [{ id: 7, name: 'Original' }],
    });
    mockUpdateUserProject.mockResolvedValue({ ok: false, error: new Error('nope') });
    ...
    await waitFor(() => expect(mockUpdateUserProject).toHaveBeenCalledWith(7, { name: 'Renamed' }));
    expect(screen.getByTestId('project-names')).toHaveTextContent('Original');
  });
  ...
  it('rejects useProjects outside a provider', () => {
