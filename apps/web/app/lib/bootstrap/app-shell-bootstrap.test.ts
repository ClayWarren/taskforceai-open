...
describe('web app shell bootstrap', () => {
  ...
  it('defers to client model loading when the authenticated model endpoint is unavailable', async () => {
  ...
  it('does not self-fetch models for no-cookie public home bootstrap', async () => {
    ...
    if (!result.modelSelector) {
      throw new Error('Expected the public model selector catalog');
    }
    expect(
      result.modelSelector.options.find((option) => option.id === 'xai/grok-4.5')
    ).toMatchObject({
