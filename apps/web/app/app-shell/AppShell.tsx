...
export const AppShell: React.FC<AppShellProps> = ({ modelSelectorBootstrap }) => {
  ...
  const { availableUpdate, desktopUpdateAction, desktopUpdateMessage, handleCheckForUpdates } =
    useDesktopShellActions(platformRuntime === 'desktop' ? 'desktop' : 'browser');
  ...
  const handleOpenBrowserPreviewUrl = useCallback(
    (url: string) => {
      if (platformRuntime !== 'desktop') {
        return;
      }
      ...
      void openDesktopBrowserPreview({ url }).catch((error: unknown) => {
        logger.warn('Failed to open URL in desktop browser preview', {
          error,
          url,
        });
      });
  ...
  useEffect(() => {
    ...
    const handleDocumentClick = (event: MouseEvent) => {
      ...
      if (previewUrl) {
        handleOpenBrowserPreviewUrl(previewUrl);
      }
    };
  ...
  }, [handleOpenBrowserPreviewUrl, platformRuntime]);
    ...
    const registerMenuListeners = async () => {
      try {
        const [unlistenSettings, unlistenUpdates, unlistenBrowserPreview] = await Promise.all([
          ...
          listenTauriEvent('desktop-menu:browser-preview', () => {
  ...
  }, [handleCheckForUpdates, handleOpenBrowserPreview, openProfileModal, platformRuntime]);
  ...
  return (
    <>
      ...
      <AppShellOverlays
        ...
        desktopRuntime={platformRuntime === 'desktop'}
      ...
      <AppShellFrame
        ...
        enableWorkspaceFileTree={platformRuntime === 'desktop' && desktopTaskMode === 'code'}
        ...
        onOpenBrowserPreview={platformRuntime === 'desktop' ? handleOpenBrowserPreview : undefined}
        onCloseBrowserPreview={() => setIsBrowserPreviewOpen(false)}
