...
import { restoreDesktopWorkspaceCheckpoint } from '../lib/platform/desktop-api';
...
export const AppShell: React.FC<AppShellProps> = ({ modelSelectorBootstrap }) => {
  ...
  const loadPaletteTasks = useCallback(
    () => conversationStore.listConversations(50),
    [conversationStore]
  );
  ...
  const handleRestoreMessage = useCallback(
    async (message: Message) => {
      ...
      if (!conversationId || !conversationStore.truncateConversation) return;
      const confirmed = window.confirm(
        'Restore the entire Git workspace to before this message? This removes this turn and later messages, discards newer tracked changes, and deletes newer untracked files.'
      );
      if (!confirmed) return;
      try {
        await restoreDesktopWorkspaceCheckpoint({
          conversationId,
          beforeTimestamp: message.createdAt ?? Date.now(),
        });
        await conversationStore.truncateConversation(conversationId, message.id);
        setPromptDraft(message.content);
      } catch (error) {
        window.alert(error instanceof Error ? error.message : 'Restore failed.');
      }
    },
    [conversation.conversationId, conversationStore, setPromptDraft]
  );
  ...
  return (
    <>
      ...
      <AppShellFrame
        ...
        onPaletteTaskSelect={handleQuickSearchSelect}
        ...
        onRestoreMessage={(message) => void handleRestoreMessage(message)}
