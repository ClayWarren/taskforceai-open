...
import { persistDesktopTaskMode } from '../lib/desktop/task-mode';
import {
  AppShell,
...
describe('AppShell', () => {
  ...
  it('shows desktop quick actions and scopes the terminal to Code mode', async () => {
    const browserShell = await renderAppShell();
    ...
    browserShell.unmount();
    appShellTestState.platformRuntime = 'desktop';
    persistDesktopTaskMode('code');
    await renderAppShell();
    expect(screen.getByTestId('open-browser-preview-btn')).toBeTruthy();
    expect(screen.getByTestId('show-terminal-btn')).toBeTruthy();
