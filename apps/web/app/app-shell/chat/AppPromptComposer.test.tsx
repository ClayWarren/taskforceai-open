...
describe('AppPromptComposer', () => {
  ...
  it('marks a pinned-summary inset so narrow layouts can release the reserved space', () => {
    const { container } = render(
      <AppPromptComposer
        {...defaultProps}
        desktopPinnedSummaryInset
        desktopRightInset="24rem"
        promptVariant="bottom"
        variant="bottom"
      />
    );
    ...
    const composer = container.querySelector('.prompt-form-container') as HTMLElement | null;
    expect(composer?.className).toContain('prompt-form-container--desktop-browser-inset');
    expect(composer?.className).toContain('prompt-form-container--pinned-summary-inset');
  });
