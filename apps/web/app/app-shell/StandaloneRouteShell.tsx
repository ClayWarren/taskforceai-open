...
import { writeStorageItem } from "@taskforceai/browser-runtime/browser-storage";
...
export function StandaloneRouteShell({ children }: StandaloneRouteShellProps) {
  ...
  const [desktopTaskMode, setDesktopTaskMode] =
    useState<DesktopTaskMode>(readDesktopTaskMode);
  ...
  useEffect(() => {
    if (platformRuntime === "desktop") setIsSidebarOpen(true);
  }, [platformRuntime]);

  const handleDesktopTaskModeChange = useCallback((mode: DesktopTaskMode) => {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(1200px at 50% 72%, rgba(30,64,175,0.24), transparent 62%)",
        }}
      />
      ...
      {hasMounted ? (
        <>
          ...
          <Sidebar
            ...
            desktopTaskMode={
              platformRuntime === "desktop" ? desktopTaskMode : undefined
            }
