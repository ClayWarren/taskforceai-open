...
export function DesktopBrowserPanel({
      ...
      animationFrame = window.requestAnimationFrame(() => {
        ...
        mountIntoSlot()
          ...
          .catch((caught: unknown) => {
            if (!cancelled) {
              logger.warn('Failed to mount desktop browser preview', { error: caught });
  ...
  useEffect(() => {
    ...
    void closeDesktopBrowserPreview().catch((caught: unknown) => {
      logger.debug('Failed to close hidden desktop browser preview', { error: caught });
    });
  }, [open]);
  ...
  const runNavigationAction = useCallback(
    async (callback: () => Promise<void>) => {
      ...
      try {
        ...
        logger.warn('Desktop browser preview navigation failed', { error: caught });
    ...
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      if (!url) {
        await mountIntoSlot().catch((caught: unknown) => {
          logger.warn('Failed to show desktop browser start page', { error: caught });
        });
        return;
      }
      ...
      try {
        ...
        logger.warn('Failed to open desktop browser preview URL', { error: caught, url });
