...
const DESKTOP_AUTH_EVENT = 'taskforceai:desktop-auth-changed';
...
mock.module('../../lib/platform/desktop-api', () => ({
  DESKTOP_APP_SERVER_AUTH_CHANGED_EVENT: DESKTOP_AUTH_EVENT,
  dispatchDesktopAppServerAuthChanged: () => window.dispatchEvent(new Event(DESKTOP_AUTH_EVENT)),
...
import { DESKTOP_APP_SERVER_AUTH_CHANGED_EVENT } from '../../lib/platform/desktop-api';
...
describe('useAppShellNavigationActions', () => {
  ...
  it('opens desktop device login and notifies the app after auth confirmation', async () => {
    const authChanged = mock();
    window.addEventListener(DESKTOP_APP_SERVER_AUTH_CHANGED_EVENT, authChanged);
    ...
    await waitFor(() => expect(authChanged).toHaveBeenCalledTimes(1));
    ...
    window.removeEventListener(DESKTOP_APP_SERVER_AUTH_CHANGED_EVENT, authChanged);
  });
  ...
  it('adds desktop hints to malformed device login URLs and handles slow-down polling', async () => {
    ...
    expect(pollDesktopAppServerDeviceLogin).toHaveBeenCalledTimes(1);
