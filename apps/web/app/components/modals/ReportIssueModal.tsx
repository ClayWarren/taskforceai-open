...
import type { ReportIssueCategory } from '@taskforceai/shared/support/reportIssues';
import {
  REPORT_ISSUE_CATEGORIES,
  REPORT_ISSUE_MAX_LENGTH,
  REPORT_ISSUE_MIN_LENGTH,
} from '@taskforceai/shared/support/reportIssues';
...
const ReportIssueModal: React.FC<ReportIssueModalProps> = ({ open, onOpenChange, context }) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const result = await submitIssueReportWithVersion(payload);
      if (!result.ok) {
