...
import { submitIssueReport } from '@taskforceai/api-client/services/issue-report';
...
vi.mock('@taskforceai/api-client/services/issue-report', () => ({
...
describe('ReportIssueModal', () => {
  beforeEach(() => {
    ...
    (submitIssueReport as any).mockReset();
  });
  ...
  test('submits form data successfully', async () => {
    ...
    (submitIssueReport as any).mockResolvedValue({ ok: true, value: undefined });
    render(
      <ReportIssueModal
        open={true}
    ...
    await waitFor(() => {
      expect(submitIssueReport).toHaveBeenCalledTimes(1);
    });
  ...
  test('handles submission error', async () => {
    const user = userEvent.setup();
    (submitIssueReport as any).mockResolvedValue({
      ok: false,
