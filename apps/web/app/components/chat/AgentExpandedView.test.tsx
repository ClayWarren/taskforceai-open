...
describe('AgentExpandedView', () => {
  ...
  it('renders failed and model-labelled agents with plural active summary', () => {
    render(
      <AgentExpandedView
        agents={[
          createAgent({ id: 3, label: 'Alpha', state: 'running', model: 'openai/gpt-5.6-sol' }),
          createAgent({
            id: 4,
            label: 'Beta',
            state: 'running',
    ...
    clickAgentRow('GPT 5.6 Sol');
    expect(screen.getByLabelText('GPT 5.6 Sol detail')).toBeDefined();
    expect(screen.getByRole('progressbar', { name: 'GPT 5.6 Sol progress' })).toBeDefined();
  ...
  it('matches live tool events by ordinal labels and model labels', () => {
    render(
      <AgentExpandedView
        agents={[
          createAgent({ id: 0, label: 'AGENT 1', model: 'openai/gpt-5.6-sol' }),
          createAgent({ id: 1, label: 'AGENT 2', model: 'xai/grok-4.5' }),
        ]}
        elapsedSeconds={12}
        modelLabel="HEAVY"
        indicatorState="running"
        toolEvents={
          [
            ...
            createEvent({ agentLabel: 'openai/gpt-5.6-sol', toolName: 'code_interpreter' }),
          ] as any
        }
        onCollapse={vi.fn()}
      />
  ...
  it('maps internal Sentinel backing model ids to the public label in expanded views', () => {
    render(
      <AgentExpandedView
        agents={[createAgent({ id: 0, label: 'AGENT 1', model: 'zai/glm-5.2' })]}
        elapsedSeconds={12}
        modelLabel="HEAVY"
        indicatorState="running"
        toolEvents={[]}
    ...
    expect(screen.queryByText(/glm|zai/i)).toBeNull();
