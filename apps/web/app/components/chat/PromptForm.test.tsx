...
void vi.mock('../../lib/platform/desktop/bridge', () => ({
  invokeTauri: vi.fn(),
...
void vi.mock('./prompt-form/AutonomousPanel', () => ({
...
void vi.mock('./prompt-form/AutoResizingTextarea', () => ({
...
void vi.mock('./prompt-form/OrchestrationModal', () => ({
...
describe('PromptForm', () => {
  ...
  it('renders input area', () => {
    ...
    expect(screen.getByRole('button', { name: /send|voice|dictate/i })).toBeTruthy();
  });
  ...
  it('submits with computer use enabled after toggling the mode menu', async () => {
    render(<PromptForm {...defaultProps} />);

    fireEvent.click(screen.getByTitle('Mode Options'));
    fireEvent.click(await screen.findByRole('button', { name: /Computer Use/i, pressed: false }));

    const input = screen.getByRole('textbox');
    fireEvent.input(input, { target: { value: 'Use computer use to take a screenshot.' } });
  ...
  it('adds dictated voice text to the prompt input through the shared voice hook', async () => {
    ...
    fireEvent.click(screen.getByTitle('Dictate with your microphone'));

    await waitFor(() => expect(screen.getByRole('textbox')).toHaveValue('hello from voice'));
    expect(mockVoice.manager.record).not.toHaveBeenCalled();
  });
