...
vi.mock('@taskforceai/contracts/api/tasks', () => ({
  fetchExecutionTrace: vi.fn(),
}));
...
describe('ExecutionReplay', () => {
  ...
  it('returns null when loading', () => {
    (fetchExecutionTrace as Mock<any>).mockImplementation(
  ...
  it('returns null when trace has no steps', async () => {
    (fetchExecutionTrace as Mock<any>).mockResolvedValue({
      ok: true,
      value: { steps: [] },
    });
    ...
    await waitFor(() => {
      expect(fetchExecutionTrace).toHaveBeenCalled();
    });
