...
export function MessageActionBar(props: MessageActionBarProps) {
  ...
  const actionButtonClass = props.compact ? iconButtonClass : buttonClass;
  ...
  if (props.compact) {
    return (
      <div className="chat-aligned chat-edge-left mt-2 flex items-center gap-1">
        ...
        <button
          type="button"
          className={`${actionButtonClass} ${props.rating === 1 ? 'text-emerald-500' : ''}`}
        ...
        <button
          type="button"
          className={`${actionButtonClass} ${props.rating === -1 ? 'text-rose-500' : ''}`}
          onClick={() => props.onRate(-1)}
          title="Not helpful"
