...
describe('ConversationItem', () => {
  ...
  it('shows unread state and handles dropdown pin/rename/archive/delete actions', () => {
    ...
    expect(screen.getByLabelText('Task completed with new activity')).toBeTruthy();
    fireEvent.click(screen.getByRole('button', { name: 'Pin Chat' }));
    fireEvent.click(screen.getByRole('button', { name: 'Rename' }));
    fireEvent.click(screen.getByRole('button', { name: 'Archive' }));
    fireEvent.click(screen.getByRole('button', { name: 'Delete' }));

    expect(onPinToggle).toHaveBeenCalledWith(123);
    expect(onRenameRequest).toHaveBeenCalledWith(123);
    expect(onArchive).toHaveBeenCalledWith(123);
    expect(onDelete).toHaveBeenCalledWith(123);
  });

  it('shows live task activity ahead of unread completion state', () => {
    render(
      <ConversationItem
        conversation={mockConversation}
        onClick={vi.fn()}
        onDelete={vi.fn()}
        activeActivity="working"
        hasUnread={true}
        isActive={true}
      />
    );

    expect(screen.getByLabelText('Task is working')).toBeTruthy();
    expect(screen.queryByLabelText('Task completed with new activity')).toBeNull();
  });

  it('offers to unpin a pinned conversation', () => {
    render(
      <ConversationItem
        conversation={mockConversation}
        onClick={vi.fn()}
