...
describe('AgentProgress', () => {
  ...
  it('uses configured custom model labels when streaming agent statuses are generic', () => {
    (useStreaming as any).mockReturnValue({
      isStreaming: true,
      agentLabels: ['gpt-5.6-sol', 'claude-fable-5'],
      agentStatuses: [
    ...
    expect(screen.getByRole('progressbar', { name: 'gpt-5.6-sol progress' })).toBeTruthy();
  });

  it('prefers backend model labels over configured labels', () => {
    (useStreaming as any).mockReturnValue({
      isStreaming: true,
      agentLabels: ['gpt-5.6-sol'],
  ...
  it('maps Sentinel backing ids to the public Sentinel label', () => {
    (useStreaming as any).mockReturnValue({
      isStreaming: true,
      agentLabels: [],
      agentStatuses: [{ status: 'PROCESSING...', agent_id: 0, model: 'zai/glm-5.2' }],
    });
