...
describe('MessageBubble', () => {
  ...
  beforeEach(() => {
    ...
    globalThis.fetch = vi.fn(
      async () => ({ ok: true, statusText: 'OK' }) as Response
  ...
  it('does not speak blank content or while voice is unavailable', () => {
    useVoice.mockReturnValue({
      manager: mockVoiceManager,
      status: 'error',
    });
    renderMessage({ content: 'Cannot speak', id: '16' });
  ...
  it('stops active speech when the listen button is clicked during playback', async () => {
    const speakDeferred = { finish: null as (() => void) | null };
    mockVoiceManager.speak.mockImplementation(
      () =>
        new Promise<void>((resolve) => {
          speakDeferred.finish = () => resolve();
        })
    );
    ...
    await waitFor(() => expect(mockVoiceManager.cancel).toHaveBeenCalled());

    await act(async () => {
      speakDeferred.finish?.();
    });
  });

  it('logs and resets state when speech fails', async () => {
    mockVoiceManager.init.mockRejectedValueOnce(new Error('voice unavailable'));
  ...
  it('logs when stopping speech fails', async () => {
    ...
    mockVoiceManager.cancel.mockRejectedValueOnce(new Error('stop failed'));
  ...
  it('cleans up voice playback on unmount', async () => {
    mockVoiceManager.cancel.mockReturnValueOnce(
      Promise.reject(new Error('cancel on unmount failed'))
    );
