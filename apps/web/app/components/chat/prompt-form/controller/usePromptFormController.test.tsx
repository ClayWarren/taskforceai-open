...
vi.mock('../../../../lib/platform/desktop-api', () => ({
  createVoiceGatewayRequestOptions: async () => ({
    fetchImpl: async (_input: RequestInfo | URL, init?: RequestInit) => {
      const formData = init?.body instanceof FormData ? init.body : null;
      const audio = formData?.get('audio');
      const file = audio instanceof File ? audio : null;
      const result = await tauriInvokeMock('app_server_voice_transcribe', {
        params: {
          audioBase64: file ? btoa(await file.text()) : '',
...
vi.mock('../orchestration/usePromptFormPreferences', () => ({
...
vi.mock('../orchestration/usePromptModelSelector', () => ({
...
vi.mock('../submission/useWebPromptSubmission', () => ({
...
describe('usePromptFormController', () => {
  beforeEach(() => {
    ...
    useProjectsMock.mockReturnValue({ activeProjectId: 11 });
  ...
  it('assigns a new persisted conversation to the active project', async () => {
    window.localStorage.setItem(
      'taskforceai.desktop.projects.roots.v1',
      JSON.stringify({ '11': '/Users/test/project' })
    );
    const setConversationProjectId = vi.fn(async () => undefined);
    useConversationStoreMock.mockReturnValue({
      enqueuePrompt: vi.fn(),
      setConversationProjectId,
    });
    const ensureConversationId = vi.fn(async () => 'conversation-11');
    ...
    const submissionOptions = useWebPromptSubmissionMock.mock.calls.at(-1)?.[0];
    await submissionOptions.ensureConversationId();

    expect(ensureConversationId).toHaveBeenCalledTimes(1);
    expect(setConversationProjectId).toHaveBeenCalledWith('conversation-11', 11);
    expect(submissionOptions.desktopProjectWorkspace).toBe('/Users/test/project');
  });
