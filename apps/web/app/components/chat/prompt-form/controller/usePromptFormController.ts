...
import {
  DESKTOP_PROJECT_WORKSPACE_CHANGED_EVENT,
...
export function usePromptFormController({
  ...
  const [desktopProjectWorkspace, setDesktopProjectWorkspace] = useState<string | null>(() =>
    desktopTaskMode === 'code' || desktopTaskMode === 'work'
      ? (readDesktopProjectWorkspace(activeProjectId) ?? readDesktopCodeWorkspace())
      : null
  );

  useEffect(() => {
    const refreshWorkspace = () => {
      setDesktopProjectWorkspace(
        desktopTaskMode === 'code' || desktopTaskMode === 'work'
          ? (readDesktopProjectWorkspace(activeProjectId) ?? readDesktopCodeWorkspace())
          : null
      );
    ...
    refreshWorkspace();
    window.addEventListener(DESKTOP_PROJECT_WORKSPACE_CHANGED_EVENT, refreshWorkspace);
    return () =>
      window.removeEventListener(DESKTOP_PROJECT_WORKSPACE_CHANGED_EVENT, refreshWorkspace);
  }, [activeProjectId, desktopTaskMode]);
