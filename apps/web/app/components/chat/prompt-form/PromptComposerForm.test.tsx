  ...
  PromptAttachments: ({
    ...
    <div>
      {files.map((file, index) => (
        <button key={`${file.name}-${index}`} type="button" onClick={() => onRemove(index)}>
          {file.name}
        </button>
      ))}
    </div>
...
const baseProps = (): PromptComposerFormProps => ({
  ...
  lockedComputerUseLabel: 'Computer Use',
  minPromptHeight: 44,
  modelOptions: [],
  modelSelectorDisabled: false,
  modelSelectorEnabled: true,
  modelSelectorLoading: false,
  onAutonomyToggle: vi.fn(),
  onClearCustomModels: vi.fn(),
  onComputerUseToggle: vi.fn(),
  onCustomizeOrchestration: vi.fn(),
  onFileButtonClick: vi.fn(),
  onKeyDown: vi.fn(),
  onLockedComputerUseToggle: vi.fn(),
  onModelSelect: vi.fn(),
  onOpenAutonomousPanel: vi.fn(),
  ...
  onUseLoggedInServicesToggle: vi.fn(),
