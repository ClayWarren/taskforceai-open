...
import type { PromptPrimaryActionMode } from '@taskforceai/shared';
import React, { useEffect, useMemo, useState } from 'react';

import { AttachFileIcon } from '../../../lib/prompt/prompt-icons';
import {
    ...
    >
      ...
      <div className={`flex w-full gap-2 ${isExpandedPrompt ? 'items-end' : 'items-center'}`}>
        ...
        >
          <button
            type="button"
            className={`${iconButtonBaseClass} text-white`}
            onClick={onFileButtonClick}
            disabled={controlsDisabled}
            aria-disabled={controlsDisabled}
            title={!isAuthenticated ? 'Login required to attach files' : 'Attach file'}
          >
            <AttachFileIcon />
          </button>
          <PromptTemplateMenu
