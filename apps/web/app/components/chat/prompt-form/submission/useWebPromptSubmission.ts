...
} from '../../../../lib/prompt/prompt-submission';
...
import type { AppServerSubmitRunParams } from '@taskforceai/contracts/app-server';
...
> & {
  ...
  desktopProjectWorkspace?: string | null;
};
...
export const useWebPromptSubmission = ({
  ...
  const desktopRunTask: SubmitPromptParams['runTask'] = async (payload) => {
    try {
      if (desktopProjectWorkspace) {
        await enableDesktopLocalCoding({ workspace: desktopProjectWorkspace });
      }
      const mode = payloadOption(payload, 'mode');
      if (
        desktopProjectWorkspace &&
        payload.conversation_id &&
        (mode === 'work' || mode === 'code')
      ) {
        await captureDesktopWorkspaceCheckpoint({
          conversationId: payload.conversation_id,
          root: desktopProjectWorkspace,
        });
      }
