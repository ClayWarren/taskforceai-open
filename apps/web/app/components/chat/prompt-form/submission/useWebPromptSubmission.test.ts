...
const enableDesktopLocalCodingMock = vi.fn();
const getDesktopAppServerComputerUseModeMock = vi.fn();
const parseDesktopBrowserOpenTargetMock = vi.fn((prompt: string) =>
  prompt.includes('threejs.com') ? 'threejs.com' : null
);
...
mock.module('../../../../lib/platform/desktop-api', () => ({
  enableDesktopLocalCoding: enableDesktopLocalCodingMock,
...
mock.module('../../../../lib/mcp/local-command', () => ({
  parseDesktopBrowserOpenTarget: parseDesktopBrowserOpenTargetMock,
}));
...
describe('useWebPromptSubmission', () => {
  beforeEach(() => {
    ...
    enableDesktopLocalCodingMock.mockResolvedValue(undefined);
    getDesktopAppServerComputerUseModeMock.mockRejectedValue(new Error('not desktop'));
    parseDesktopBrowserOpenTargetMock.mockImplementation((prompt: string) =>
      prompt.includes('threejs.com') ? 'threejs.com' : null
    );
  });
  ...
  it('forwards private chat mode to prompt submission', async () => {
    useWebPromptSubmission({} as any);
    const hookConfig = getHookConfig();

    await hookConfig.submitPrompt({
      prompt: 'private hello',
      privateChat: true,
    });
  ...
  it('short-circuits desktop in-app browser open prompts through local command handling', async () => {
    platformRuntime = 'desktop';
    const onLocalCommand = vi.fn(async () => true);
    const onSendMessage = vi.fn();
    const resetFormState = vi.fn();
    const ensureConversationId = vi.fn(async () => 'local-1');
    ...
    expect(parseDesktopBrowserOpenTargetMock).toHaveBeenCalledWith(
      'can you pull up threejs.com in the in-app browser'
    );
    expect(onLocalCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        prompt: 'can you pull up threejs.com in the in-app browser',
    ...
    expect(onSendMessage).toHaveBeenCalledWith('can you pull up threejs.com in the in-app browser');
  ...
  it('uses the desktop app-server run task adapter in desktop runtime', async () => {
    ...
    useWebPromptSubmission({
      desktopProjectWorkspace: '/Users/test/project',
      mcpToolItems,
    } as any);
    const hookConfig = getHookConfig();
    await hookConfig.submitPrompt({
    ...
    expect(enableDesktopLocalCodingMock).toHaveBeenCalledWith({
