...
// eslint-disable-next-line complexity -- Composer branches represent independent input controls.
export function PromptComposerForm({
  ...
  const mentionMatch = enableWorkspaceTools
    ? prompt.slice(0, cursorPosition).match(/(?:^|\s)@([^\s@]*)$/)
    : null;
  const mentionQuery = mentionMatch?.[1] ?? null;
  const mentionStart = mentionMatch
    ? cursorPosition - mentionMatch[0].length + (mentionMatch[0].startsWith(' ') ? 1 : 0)
    : -1;
  ...
  const handleTextareaPaste = (event: React.ClipboardEvent<HTMLTextAreaElement>) => {
    const pastedImages = Array.from(event.clipboardData.items).flatMap((item) => {
  ...
  const insertWorkspaceMention = (path: string) => {
    if (mentionStart < 0) return;
    const nextCursor = mentionStart + path.length + 2;
    setPrompt(`${prompt.slice(0, mentionStart)}@${path} ${prompt.slice(cursorPosition)}`);
  ...
  return (
    <form
      ...
      className={`prompt-form relative ${variant === 'bottom' ? 'chat-aligned chat-edge-left' : ''} ${isExpandedPrompt || enableWorkspaceTools ? 'prompt-form--expanded !items-stretch !rounded-[24px]' : ''} ${files.length > 0 || enableWorkspaceTools ? '!flex-col !gap-1' : ''} ${isDraggingFiles ? 'ring-2 ring-blue-400/80' : ''}`}
    ...
    >
      ...
      {enableWorkspaceTools ? <DesktopWorkspaceTargetSelector projectId={activeProjectId} /> : null}

      {mentionQuery !== null && !shouldShowSlashSuggestions && !controlsDisabled ? (
        <DesktopWorkspaceMentionMenu query={mentionQuery} onSelect={insertWorkspaceMention} />
      ) : null}
