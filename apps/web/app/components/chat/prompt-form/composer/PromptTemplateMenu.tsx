...
export function PromptTemplateMenu({
  ...
  return (
    <DropdownMenu modal={false}>
      ...
      >
        <PromptTemplateMenuItems templates={templates} onInsertTemplate={onInsertTemplate} />
      </DropdownMenuContent>
    </DropdownMenu>
  );
}

export function PromptTemplateMenuItems({
  templates,
  onInsertTemplate,
}: Pick<PromptTemplateMenuProps, 'templates' | 'onInsertTemplate'>) {
  return (
    <>
            ...
            {categoryTemplates.map((template) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-xs leading-5 text-slate-400">{template.description}</p>
                </div>
              </DropdownMenuItem>
            ))}
          </div>
