...
vi.mock('../../../../lib/platform/desktop-ui', () => ({
  DesktopWorkspaceMentionMenu: ({
    ...
    <button type="button" onClick={() => onSelect('apps/web')}>
      Mention {query}
    </button>
  ),
  DesktopWorkspaceTargetSelector: ({ projectId }: { projectId: number | null }) => (
    <div>Worktree target {projectId ?? 'none'}</div>
  ),
}));
...
describe('PromptComposerForm', () => {
  ...
  it('shows worktree and mention controls only when Code workspace tools are enabled', async () => {
    const { promptSetter } = renderComposer({
      activeProjectId: 7,
      enableWorkspaceTools: true,
      prompt: 'Review @apps/',
    });

    expect(screen.getByText('Worktree target 7')).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: 'Mention apps/' }));
    await waitFor(() => expect(promptSetter).toHaveBeenCalledWith('Review @apps/web '));

    cleanup();
    renderComposer({ enableWorkspaceTools: false, prompt: 'Review @apps/' });
    expect(screen.queryByText(/Worktree target/)).toBeNull();
    expect(screen.queryByRole('button', { name: 'Mention apps/' })).toBeNull();
  });
  ...
  it('attaches pasted clipboard images instead of inserting them into the prompt', () => {
    const onPasteFiles = vi.fn();
    renderComposer({ onPasteFiles });
    const textbox = screen.getByRole('textbox');
    ...
    expect(event).toBe(false);
    expect(onPasteFiles).toHaveBeenCalledWith([screenshot]);
  });
