...
export const ApprovalCard: React.FC<ApprovalCardProps> = ({ taskId, approval, onDecision }) => {
  ...
  const sessionKey = `taskforceai.approval.v1:${taskId}:${approval.permission}:${approval.patterns.toSorted().join('\u001f')}`;

  const handleDecision = async (approved: boolean, rememberForSession = false) => {
    setLoading(true);
    if (rememberForSession) sessionStorage.setItem(sessionKey, 'approved');
    try {
      await submitTaskApprovalDecision(taskId, { approved });
      onDecision(approved);
    } catch (error) {
      if (rememberForSession) sessionStorage.removeItem(sessionKey);
  ...
  useEffect(() => {
    if (
      autoSubmittedKeyRef.current === sessionKey ||
      sessionStorage.getItem(sessionKey) !== 'approved'
    ) {
      return;
    }
    autoSubmittedKeyRef.current = sessionKey;
    void handleDecision(true);
    // The fingerprint changes only when a distinct approval is rendered.
      ...
      <div className="overflow-hidden rounded-2xl border border-amber-500/30 bg-amber-500/5 shadow-lg shadow-amber-500/10 backdrop-blur-sm">
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            ...
            <button
              onClick={() => {
                void handleDecision(true, true);
              }}
              disabled={loading}
              className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-2.5 text-sm font-bold text-emerald-300 transition hover:bg-emerald-500/20 active:scale-[0.98] disabled:opacity-50"
            >
              Approve for Session
            </button>
            <button
              ...
              disabled={loading}
              className="rounded-xl border border-rose-500/30 bg-rose-500/10 px-4 py-2.5 text-sm font-bold text-rose-400 transition hover:bg-rose-500/20 active:scale-[0.98] disabled:opacity-50"
            >
              Deny
            </button>
          </div>
