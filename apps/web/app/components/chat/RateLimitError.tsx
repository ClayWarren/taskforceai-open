...
import { navigateTo } from '@taskforceai/shared/utils/browser-actions';
import { authClient } from '@taskforceai/contracts/auth/auth-client';
import { formatRateLimitResetDate } from '@taskforceai/shared/errors/rate-limit-view';
import { useOptionalProfileModal } from '../../lib/profile/ProfileModalContext';
  ...
  const handleUpgrade = (targetPlan: 'pro' | 'super') => {
    ...
    const startUpgradeFlow = async () => {
      try {
        ...
        const checkoutResult = await startUpgradeCheckout({ targetPlan });
            ...
            <div className="mb-4 rounded-lg border border-orange-200 bg-white p-4 dark:border-orange-800 dark:bg-gray-800">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {availableUpgrades.map((planOption) => (
                  <button
                    key={planOption}
                    onClick={() => handleUpgrade(planOption as 'pro' | 'super')}
