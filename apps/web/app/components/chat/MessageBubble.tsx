...
import { formatMessageTime } from '@taskforceai/presenters/time/display-format';
import { mergeSources } from '@taskforceai/client-core/utils/source-extraction';
import { submitMessageFeedback } from '../../lib/api/messages';
...
import { CodeExecutionTimeline } from './CodeExecutionTimeline';
...
const getUserMaxWidthStyle = (isRealtimeVoiceMessage: boolean): React.CSSProperties =>
...
const getMessageScrollStyle = (
  isRealtimeVoiceMessage: boolean,
  isUser: boolean
): React.CSSProperties | undefined => {
  if (isRealtimeVoiceMessage) {
    return {
      ...(!isUser ? { scrollMarginBlockStart: '96px' } : {}),
      scrollMarginBlockEnd: 'var(--realtime-voice-transcript-bottom-offset, 0px)',
    };
  }
  return isUser ? undefined : { scrollMarginBlockStart: '96px' };
};

const computeBubbleVisibility = (
  message: Message,
  isUser: boolean,
  generatedFileCount: number
): { showAgentExecutionPanel: boolean; showFinalMessage: boolean } => {
  ...
  const showAgentExecutionPanel =
    !isUser && (message.isAgentStatus || (hasStoredExecutionMetadata && generatedFileCount === 0));

  // Show final message bubble for non-agent-status messages with content
  const showFinalMessage =
    !isUser &&
    !message.isAgentStatus &&
    (message.content.trim().length > 0 || generatedFileCount > 0);

  return { showAgentExecutionPanel, showFinalMessage };
};

const bubbleRowClassName = (isUser: boolean): string =>
  `message-bubble ${isUser ? 'user' : 'bot'} flex ${isUser ? 'justify-end' : 'justify-start'}`;
...
const bubbleInnerClassName = (
  ...
  const prefix = isUser
    ...
    : `chat-aligned chat-edge-left ${
        executionPresentation === 'code' || isRealtimeVoiceMessage
          ? ''
          : 'border-l-2 border-slate-800 pl-4'
      }`;
  const appearance = isUser
    ? isRealtimeVoiceMessage
      ? 'rounded-3xl bg-blue-950/80 px-5 py-3 text-white shadow-sm'
      : 'rounded-3xl bg-gray-800 p-4 text-white shadow-sm'
    : 'bg-transparent text-gray-100';
  return `${prefix} transition-all duration-200 ${appearance}`;
};

const isListenDisabled = (
  canListenToMessage: boolean,
  platformRuntime: string,
  voiceStatus: string
): boolean =>
  !canListenToMessage ||
  (platformRuntime !== 'browser' && platformRuntime !== 'desktop' && voiceStatus === 'error');

const UserMessageLines: React.FC<{ content: string }> = ({ content }) => {
...
interface FinalMessageFrameProps {
  message: Message;
  isUser: boolean;
  isRealtimeVoiceMessage: boolean;
...
const FinalMessageFrame: React.FC<FinalMessageFrameProps> = ({
  message,
  isUser,
  isRealtimeVoiceMessage,
  ...
  >
    <div
      ref={bubbleRef}
      className={bubbleRowClassName(isUser)}
      style={getMessageScrollStyle(isRealtimeVoiceMessage, isUser)}
    >
      <div
        className={bubbleInnerClassName(isUser, isRealtimeVoiceMessage, executionPresentation)}
        style={isUser ? getUserMaxWidthStyle(isRealtimeVoiceMessage) : undefined}
      >
        <div className="message-content prose prose-sm dark:prose-invert max-w-none">
          {isUser ? (
            <UserMessageLines content={message.content} />
          ) : (
        ...
        {isUser && timestampLabel && !isRealtimeVoiceMessage && (
          <div className="mt-3 text-xs text-blue-100">{timestampLabel}</div>
        )}
      </div>
    </div>
    {isUser && (
      <UserMessageActionBar copied={copied} compact={isRealtimeVoiceMessage} onCopy={onCopy} />
    )}
    {showAssistantActions && (
      <MessageActionBar
        copied={copied}
        compact={isRealtimeVoiceMessage}
        hideFeedback={isPrivateChat}
        hideListen={isRealtimeVoiceMessage || isPrivateChat}
        isSpeaking={isSpeaking}
        listenDisabled={listenDisabled}
        canShare={!isPrivateChat && canShare}
...
const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  const handleRating = async (value: number) => {
    ...
    try {
      const result = await submitMessageFeedback(message.id, newRating);
      if (result === 'not-rateable') {
  ...
  useEffect(() => {
    return () => {
      clearBrowserSpeechPlayback({ updateState: false });
      try {
        void voiceRef.current.cancel().catch((error: unknown) => {
  ...
  const { showAgentExecutionPanel, showFinalMessage } = computeBubbleVisibility(
    message,
    isUser,
    ...
    <>
        ...
        <div className="message-bubble bot mb-6 flex justify-start">
          <div className="chat-aligned chat-edge-left flex flex-col gap-2">
            {executionPresentation === 'code' ? (
              <CodeExecutionTimeline message={message} onShowSources={openSourcesPanel} />
            ) : (
      ...
      {(isUser || showFinalMessage) && (
        <FinalMessageFrame
          message={message}
          isUser={isUser}
          isRealtimeVoiceMessage={isRealtimeVoiceMessage}
          ...
          copied={copied}
          isSpeaking={isSpeaking}
          listenDisabled={isListenDisabled(canListenToMessage, platformRuntime, voiceStatus)}
          hasSources={hasSources}
          sourceCount={sourceCount}
          rating={rating}
          onShare={onShare}
