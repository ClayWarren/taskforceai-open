...
import type { AppServerPluginListResult } from '@taskforceai/contracts/app-server';
...
export function PluginsPage() {
  ...
  const [desktopPlugins, setDesktopPlugins] = useState<AppServerPluginListResult['plugins']>([]);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener(WEB_MCP_SERVERS_CHANGED_EVENT, refreshInstalled);
  }, [refreshInstalled]);
  ...
  const visibleInstalled = useMemo(
    () => installed.filter((plugin) => matchesQuery(normalizedQuery, plugin.name, plugin.endpoint)),
    [installed, normalizedQuery]
  );
  const visibleDesktopPlugins = useMemo(
    () =>
      desktopPlugins.filter((plugin) =>
        matchesQuery(normalizedQuery, plugin.name, plugin.description ?? '', plugin.source ?? '')
      ),
  ...
  const visibleCatalog = useMemo(
    () =>
      pluginCatalog.filter((plugin) =>
        matchesQuery(normalizedQuery, plugin.name, plugin.description, plugin.category)
      ),
    [normalizedQuery]
  );
  ...
  const savePlugin = () => {
    ...
    const next = persistWebMcpServers(upsertMcpServerByName(installed, result.value));
    setInstalled(next);
    setEditorOpen(false);
    setDraft(emptyDraft);
  };
  ...
  const removePlugin = (plugin: WebMcpServerConfig) => {
    const next = persistWebMcpServers(removeMcpServerByName(installed, plugin.name));
    setInstalled(next);
  };

  const toggleDesktopPlugin = async (plugin: AppServerPluginListResult['plugins'][number]) => {
    const result = await setDesktopAppServerPluginEnabled(plugin.id, !plugin.enabled);
    ...
    <section className="mx-auto min-h-screen w-full max-w-6xl px-4 pt-20 pb-16 sm:px-8 lg:pt-24">
      <div className="mx-auto w-full max-w-4xl">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight text-white">Plugins</h1>
            ...
            {visibleInstalled.map((plugin) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm font-medium text-slate-100">{plugin.name}</div>
        ...
        {(['Featured', 'Productivity'] as const).map((category) => (
          <PluginCatalogSection
            key={category}
            category={category}
            plugins={visibleCatalog.filter((plugin) => plugin.category === category)}
            installed={installed}
...
function PluginCatalogSection({
  ...
  return (
    <section className="mt-12" aria-labelledby={`plugins-${category.toLocaleLowerCase()}`}>
      <h2
      ...
      <div className="mt-3 grid gap-x-10 md:grid-cols-2">
        {plugins.map((plugin) => {
          ...
          const isInstalled = installed.some(
            (item) => item.name.toLocaleLowerCase() === plugin.name.toLocaleLowerCase()
          );
          const isDesktopInstalled = desktopPlugins.some(
            (item) => item.name.toLocaleLowerCase() === plugin.name.toLocaleLowerCase()
          );
          return (
            <article key={plugin.id} className="flex min-w-0 items-center gap-4 py-3.5">
              <span
              ...
              <div className="min-w-0 flex-1">
                <h3 className="truncate text-sm font-medium text-slate-100">{plugin.name}</h3>
                <p className="truncate text-sm text-slate-500">{plugin.description}</p>
              </div>
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 id="plugin-editor-title" className="text-xl font-semibold text-white">
              Add plugin
            </h2>
            <p className="mt-1 text-sm text-slate-400">
              Connect a remote MCP endpoint. Desktop also supports configured local endpoints.
            </p>
          </div>
          <button
            type="button"
        ...
        <label className="mt-4 block text-sm font-medium text-slate-200">
          ...
          <input
            ...
            onInput={(event) => onChange({ ...draft, endpoint: event.currentTarget.value })}
