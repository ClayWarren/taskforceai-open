...
describe("PluginsPage", () => {
  ...
  it("adds a catalog plugin with a real MCP endpoint", () => {
    ...
    expect(
      (screen.getByLabelText("Plugin name") as HTMLInputElement).value,
    ).toBe("GitHub");
  ...
  it("filters catalog entries and toggles installed plugins", () => {
    storedServers = [
      { name: "Docs", endpoint: "https://mcp.example.com/docs", enabled: true },
    ];
    render(<PluginsPage />);
  ...
  it("lists and toggles installed desktop plugin bundles", async () => {
    ...
    expect(setDesktopAppServerPluginEnabled).toHaveBeenCalledWith(
      "computer-use",
      false,
    );
  });
