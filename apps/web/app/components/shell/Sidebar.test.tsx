...
describe("Sidebar", () => {
  ...
  it("calls onClose when overlay clicked", () => {
    const { container } = render(
      <Sidebar isOpen={true} onClose={onClose} onNewChat={onNewChat} />,
    );
  ...
  it("does not render profile menu when user is null", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Open scheduled tasks" }),
    ).toBeNull();
  });
  ...
  it("shows desktop update action when provided", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Install TaskForceAI 0.4.9" }),
    );
  ...
  it("passes active conversation id to the conversation list", () => {
    ...
    expect(
      screen
        ...
        .getAttribute("data-active-conversation-id"),
    ).toBe("local-active");
    expect(mockConversationList).toHaveBeenCalledWith(
  ...
  it("shows Code-only repository navigation and optional projects in the desktop task shell", () => {
    mockProjects = [{ id: 1, name: "taskforceai" }];
    mockActiveProjectId = 1;
    ...
    expect(screen.getByText("TaskForceAI Code")).toBeTruthy();
    fireEvent.click(screen.getByRole("button", { name: "Pull requests" }));
    expect(onAgentManagerClick).toHaveBeenCalledTimes(1);
    fireEvent.click(
      screen.getByRole("button", { name: "Don't work in a project" }),
    );
  ...
  it("keeps repository navigation out of the desktop Work shell", () => {
    ...
    expect(screen.getByText("TaskForceAI Work")).toBeTruthy();
