...
const SIDECAR_SIGNING_IDENTIFIER = 'com.taskforceai.desktop.app-server';
...
export function parseCodeSigningIdentities(output: string): string[] {
  return output
    .split('\n')
...
export function developmentSigningIdentity({
  target,
  ...
  if (
    ...
    env['TASKFORCEAI_DESKTOP_SKIP_SIDECAR_SIGNING'] === '1'
  ) {
    return undefined;
  }

  const configured = env['TASKFORCEAI_DESKTOP_SIDECAR_SIGNING_IDENTITY']?.trim();
  if (configured) {
    return configured;
  }
...
async function main(): Promise<void> {
  ...
  if (process.platform === 'darwin' && target.endsWith('-apple-darwin')) {
    const identityOutput = await $`security find-identity -v -p codesigning`.quiet().nothrow();
    if (identityOutput.exitCode === 0) {
      identities = parseCodeSigningIdentities(identityOutput.text());
    }
  }
  const signingIdentity = developmentSigningIdentity({ target, identities });
  if (signingIdentity) {
    await $`codesign --force --options runtime --sign ${signingIdentity} --identifier ${SIDECAR_SIGNING_IDENTIFIER} ${destination}`;
    await $`codesign --verify --strict ${destination}`;
    console.log(`Signed ${basename(destination)} for stable local Keychain access.`);
  } else if (process.platform === 'darwin' && target.endsWith('-apple-darwin')) {
    console.warn(
      'No Apple Development identity found; the local sidecar may prompt for Keychain access after rebuilds.'
    );
  }
